...
public class ChartStyleApplier {
    ...
    public void applyAxisStyles(JFreeChart chart, ChartModel chartModel, ICellRefResolver resolver) {
        ...
        if (chartModel.getPlotArea() != null) {
            // 应用X轴样式 - 查找分类轴
            ChartAxisModel categoryAxis = chartModel.getPlotArea().getAxisByType(ChartAxisType.CATEGORY);
            if (categoryAxis != null) {
                axisRenderer.renderAxis(plot, categoryAxis, resolver, true);
            }

            // 应用Y轴样式 - 查找数值轴
            ChartAxisModel valueAxis = chartModel.getPlotArea().getAxisByType(ChartAxisType.VALUE);
            if (valueAxis != null) {
