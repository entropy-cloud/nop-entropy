...
public class DefaultChartStyleProvider implements IChartStyleProvider {
    ...
    public String getSeriesColor(int seriesIndex) {
        if (seriesIndex < 0) {
            return getThemeColor("accent1");
        }

        // 使用accent1-accent6颜色序列，循环使用
        int colorIndex = seriesIndex % 6 + 1;
        return getThemeColor("accent" + colorIndex);
    }

    @Override
    public List<String> getColorSequence() {
        // 返回accent1-accent6颜色序列
        return Arrays.asList(
                getThemeColor("accent1"),
                getThemeColor("accent2"),
                getThemeColor("accent3"),
                getThemeColor("accent4"),
                getThemeColor("accent5"),
                getThemeColor("accent6"));
    }
    ...
    public void applyVaryColors(ChartSeriesModel series, boolean varyColors, ChartModel chartModel) {
        ...
        if (varyColors) {
            ...
            // 优先从图表模型的style.colors中获取颜色
            if (chartModel != null && chartModel.getStyle() != null) {
                ChartStyleModel chartStyle = chartModel.getStyle();
                if (chartStyle.hasCustomColors()) {
                    color = chartStyle.getColorByIndex(series.getIndex());
                    LOG.debug("Using custom color from chart style for series {}: {}", series.getIndex(), color);
                }
            }

            // 如果没有自定义颜色，则使用默认的主题颜色
            if (color == null) {
                color = getSeriesColor(series.getIndex());
                LOG.debug("Using theme color for series {}: {}", series.getIndex(), color);
            }

            // 确保shapeStyle存在
            ChartShapeStyleModel shapeStyle = series.getShapeStyle();
            if (shapeStyle == null) {
                shapeStyle = getDefaultStyle("series");
            ...
            LOG.debug("Applied vary colors for series {}: {}", series.getIndex(), color);
        }
    }
