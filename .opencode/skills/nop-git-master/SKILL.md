...
**❌ 错误做法：先检查是否有更新**
```bash
# 错误：git fetch 后用 git log 检查
# 问题：refs 可能未正确更新，导致误判"已是最新"
git fetch origin
git log HEAD..origin/master --oneline  # 可能为空，但实际有更新
```

**✅ 正确做法：直接 pull**
```bash
# 正确：直接 pull，让 git 告诉你结果
git pull --ff-only origin master
# 如果已是最新：输出 "Already up to date"
# 如果有更新：自动合并并显示变更
```
...
```bash
# 1. 直接执行 pull（不要先 fetch 检查）
git pull --ff-only origin master

# 2. 如果有冲突（在 master 上极少见）
#    按提示解决后继续
...
1. **refs 缓存问题**：`git fetch origin` 可能只更新 FETCH_HEAD，而不更新 refs/remotes/origin/master
2. **不必要的复杂度**：`git pull` 本身会 fetch + merge，多此一举
3. **浪费时间**：先检查再 pull 是两次网络请求，pull 只需一次

## 特殊情况

**如果必须先查看远程有什么更新（仅供参考，不立即合并）：**
```bash
# 强制更新 refs
...
**但在 SYNC 模式下，用户意图是"更新"，应该直接 pull，不要先检查。**
...
7. **SYNC时从不先fetch检查再pull** - 直接pull，不要多此一举
