...
// src/constants.ts
var HOOK_NAME = "nop-ralph-loop";
var DEFAULT_STATE_DIR = ".nop";
var DEFAULT_STATE_FILE = "ralph-loop.local.md";
var DEFAULT_MAX_ITERATIONS = 100;
var DEFAULT_COMPLETION_PROMISE = "DONE";
var ULTRAWORK_VERIFICATION_PROMISE = "VERIFIED";
var DEFAULT_API_TIMEOUT = 5000;
var SYSTEM_DIRECTIVE_PREFIX = "[NOP-SYSTEM: RALPH-LOOP";
...
// src/storage.ts
function getStateFilePath(directory, stateDir) {
...
var logFile = path.join(os.tmpdir(), "nop-ralph-loop.log");
  ...
  return {
    startLoop(sessionID, prompt, loopOptions) {
      ...
      const state = {
        active: true,
        iteration: 1,
        max_iterations: loopOptions?.ultrawork ? undefined : loopOptions?.maxIterations ?? config?.default_max_iterations ?? DEFAULT_MAX_ITERATIONS,
...
function getMaxIterationsLabel(state) {
...
function buildContinuationPrompt(state) {
  ...
  const continuationPrompt = template.replace("{{ITERATION}}", String(state.iteration)).replace("{{MAX}}", getMaxIterationsLabel(state)).replace("{{INITIAL_PROMISE}}", state.initial_completion_promise ?? state.completion_promise).replace("{{PROMISE}}", state.completion_promise).replace("{{PROMPT}}", state.prompt);
  return state.ultrawork ? `ultrawork ${continuationPrompt}` : continuationPrompt;
}
function buildVerificationFailurePrompt(state) {
  const continuationPrompt = ULTRAWORK_VERIFICATION_FAILED_PROMPT.replace("{{ITERATION}}", String(state.iteration)).replace("{{MAX}}", getMaxIterationsLabel(state)).replace("{{PROMISE}}", state.completion_promise).replace("{{PROMPT}}", state.prompt);
  return state.ultrawork ? `ultrawork ${continuationPrompt}` : continuationPrompt;
}
...
// src/continuation-injector.ts
var INTERNAL_INITIATOR_MARKER = "<!-- NOP_RALPH_LOOP_CONTINUE -->";
...
function detectCompletionInTranscript(transcriptPath, promise, startedAt) {
  if (!transcriptPath)
    return false;
  try {
...
async function selectSessionInTui(client, sessionID) {
  ...
  try {
    await selectSessionValue.bind(tuiValue)({ body: { sessionID } });
    return true;
  } catch (error) {
...
// src/verification-failure-handler.ts
  ...
  return async ({ event }) => {
    ...
    if (event.type === "session.idle") {
      ...
      try {
        ...
        const verificationSessionID = state.verification_pending ? state.verification_session_id : undefined;
        ...
        const transcriptPath = completionSessionID && options.getTranscriptPath ? options.getTranscriptPath(completionSessionID) : undefined;
        const completionViaTranscript = completionSessionID && options.getTranscriptPath ? detectCompletionInTranscript(transcriptPath, state.completion_promise, state.started_at) : false;
        ...
        }) : state.verification_pending ? false : await detectCompletionInSessionMessages(ctx, {
          sessionID,
          promise: state.completion_promise,
          apiTimeoutMs: options.apiTimeoutMs,
          directory: options.directory,
...
function parseLoopArguments(rawArgs) {
  ...
  let prompt = args.replace(/--max-iterations=\d+/gi, "").replace(/--completion-promise=\S+/gi, "").replace(/--strategy=(reset|continue)/gi, "").replace(/^-{1,2}\S+\s*/g, "").trim();
...
function createRalphLoopHook(ctx, config) {
...
function createToolExecuteBeforeHandler(ctx, ralphLoopHook) {
...
function createChatMessageHandler(ctx, ralphLoopHook) {
  return async (input, output) => {
    const sessionID = input.sessionID || getMainSessionID();
    if (!sessionID)
      return;
...
`).trim() || "";
    const isRalphLoopTemplate = promptText.includes("You are starting a NOP Ralph Loop") && promptText.includes("<nop-user-task>");
    const isUlwLoopTemplate = promptText.includes("You are starting a NOP ULTRAWORK Loop") && promptText.includes("<nop-user-task>");
...
var ralphLoopHook = null;
var DEFAULT_CONFIG = {
  enabled: true,
  default_max_iterations: 100,
  default_strategy: "continue"
};
var NopRalphLoopPlugin = async (ctx) => {
...
var src_default = NopRalphLoopPlugin;
function getRalphLoopHook() {
