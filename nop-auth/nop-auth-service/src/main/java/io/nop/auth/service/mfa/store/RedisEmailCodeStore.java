...
/**
 * {@link EmailCodeStore} 的 Redis 实现（设计 §5.3.3，W15-impl；{@code RedisSmsCodeStore} 平行类）。
 * <p>
 * 写路径约束同 {@link RedisSmsCodeStore}：写一律 {@code putExAsync}（psetex），读用不刷新 TTL 的
 * {@code get}，成功原子消费 {@code removeIfMatch}(Lua CAS)，失败计数用
 * {@link INosqlCounter#increment}(INCRBY，独立键 {@code email:fail:{key}})。
 * key 前缀（{@code email:code:}/{@code email:fail:}）与 sms 侧（{@code sms:*}）通道隔离。
 */
public class RedisEmailCodeStore implements EmailCodeStore {

    private static final String CODE_PREFIX = "email:code:";
    private static final String FAIL_PREFIX = "email:fail:";

    private final INosqlService nosql;
    private final EmailCodeStoreConfig config;

    @Inject
    public RedisEmailCodeStore(INosqlService nosql) {
        this(nosql, new EmailCodeStoreConfig());
    }

    public RedisEmailCodeStore(INosqlService nosql, EmailCodeStoreConfig config) {
        this.nosql = nosql;
        this.config = config;
    }
    ...
    public CodeVerifyResult verify(String key, String code) {
        ...
        if (!(obj instanceof EmailCodeEntry))
        ...
        if (entry.getCode().equals(code)) {
            ...
            // （与 Db 实现的 affected==0 → EXPIRED 语义对齐，双花防御；RedisSmsCodeStore 同步修复）
