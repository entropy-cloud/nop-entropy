...
public final class PackagingURIHelper {
    ...
    // This class should heavily leverage Java library code to reduce the number of lines of code that POI has to
    // maintain and test
    private static final Logger LOG = LoggerFactory.getLogger(PackagingURIHelper.class);
     ...
     * @param msCompatible if true then remove leading slash from the relativized URI. This flag violates [M1.4]: A part name
     *                     shall start with a forward slash ('/') character, but allows generating URIs compatible with MS Office
     *                     and OpenOffice.
     * @return A fully relativize part name URI ('word/media/image1.gif', '/word/document.xml' =&gt; 'media/image1.gif')
     * else {@code null}.
     */
    public static URI relativizeURI(URI sourceURI, URI targetURI, boolean msCompatible) {
        ...
        if (sourceURI.toString().equals("/")) {
            ...
            if (msCompatible && path.length() > 0 && path.charAt(0) == '/') {
                try {
                    ...
                    LOG.warn("Failed to relativize", e);
                    return null;
                }
            }
            return targetURI;
        ...
        if ((segmentsTheSame == 0 || segmentsTheSame == 1) && segmentsSource[0].isEmpty()
                && segmentsTarget[0].isEmpty()) {
     ...
     * @throws NopException Throws if the specified URI is not OPC compliant.
     */
     ...
     * @throws NopException Throws if the specified part name is not OPC compliant.
     ...
     * @throws IllegalArgumentException Throws if the specified URI is a relationshp part.
     */
    public static PackagePartName getRelationshipPartName(PackagePartName partName) {
        ...
        if (partName.isRelationshipPartURI())
            throw new InvalidFormatException("Can't be a relationship part");
    ...
    public static String getRelationshipPartName(String partName) {
        ...
        if (PackagingURIHelper.PACKAGE_ROOT_URI.getPath().equals(partName))
            return PackagingURIHelper.PACKAGE_RELATIONSHIPS_ROOT_PART_NAME.getName();

        String fullPath = partName;
        String filename = StringHelper.fileFullName(partName);
        fullPath = fullPath.substring(0, fullPath.length() - filename.length());
        fullPath = combine(fullPath, PackagingURIHelper.RELATIONSHIP_PART_SEGMENT_NAME);
        fullPath = combine(fullPath, filename);
        fullPath = fullPath + PackagingURIHelper.RELATIONSHIP_PART_EXTENSION_NAME;
     ...
     * <p>
     * 1. Percent-encode each open bracket ([) and close bracket (]). 2. Percent-encode each percent (%) character that
     * is not followed by a hexadecimal notation of an octet value. 3. Un-percent-encode each percent-encoded unreserved
     * character. 4. Un-percent-encode each forward slash (/) and back slash (\). 5. Convert all back slashes to forward
     * slashes. 6. If present in a segment containing non-dot (?.?) characters, remove trailing dot (?.?) characters
     * from each segment. 7. Replace each occurrence of multiple consecutive forward slashes (/) with a single forward
     * slash. 8. If a single trailing forward slash (/) is present, remove that trailing forward slash. 9. Remove
     * complete segments that consist of three or more dots. 10. Resolve the relative reference against the base URI of
     * the part holding the Unicode string, as it is defined in ?5.2 of RFC 3986. The path component of the resulting
     * absolute URI is the part name.
     * </p>
     *
