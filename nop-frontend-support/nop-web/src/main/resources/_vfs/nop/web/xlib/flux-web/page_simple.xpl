<c:unit xmlns:c="c" xmlns:thisLib="thisLib" xmlns:xpl="xpl" xmlns:j="j">
    <c:script><![CDATA[
        ...
        if(api != null){
            ...
            if(_n != null){
                // data 语义：withFormData 时显式映射表单字段（genScope.formData）；
                // 未指定 data 时走 includeScope 机制（运行时从 scope 收集数据）。
                // $form 等运行时内部状态由 host 侧 removeSpecialKeys 顶层过滤。
        ...
        if(initApi != null){
            ...
            if(_n != null){
                ...
                // 与 submitAction 相同：data/includeScope 语义由 NormalizeApi 决定，
                // $form 等运行时状态由 host 侧 removeSpecialKeys 顶层过滤
