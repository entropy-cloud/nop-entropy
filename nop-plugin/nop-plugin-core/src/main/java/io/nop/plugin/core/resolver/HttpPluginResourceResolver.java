package io.nop.plugin.core.resolver;
...
import io.nop.plugin.core.PluginCoreConstants;
...
import static io.nop.plugin.core.PluginCoreErrors.ARG_FILE_NAME;
import static io.nop.plugin.core.PluginCoreErrors.ARG_PARAM_NAME;
import static io.nop.plugin.core.PluginCoreErrors.ERR_PLUGIN_DOWNLOAD_RENAME_FILE_FAIL;
import static io.nop.plugin.core.PluginCoreErrors.ERR_PLUGIN_INVALID_PARAM_NAME;
...
public class HttpPluginResourceResolver implements IPluginResourceResolver {
    ...
    private String buildServiceUrl(ArtifactCoordinates coordinates) {
        return StringHelper.renderTemplate(pluginServiceUrl, name -> {
            if (name.equals(PluginCoreConstants.VAR_PLUGIN_GROUP_ID)) {
                return StringHelper.encodeUriPath(coordinates.getGroupId());
            } else if (name.equals(PluginCoreConstants.VAR_PLUGIN_ARTIFACT_ID)) {
                return StringHelper.encodeUriPath(coordinates.getArtifactId());
            } else if (name.equals(PluginCoreConstants.VAR_PLUGIN_VERSION)) {
