...
public class ListDirectoryExecutorTest {
    private ListDirectoryExecutor executor;
    ...
    @Test
    void testToolName() {
        assertEquals("list-dir", executor.getToolName());
    }
    ...
    void testListEmptyDirectory() {
        AiToolCall call = createCall("/empty");
        AiToolCallResult result = executor.executeAsync(call, new MockContext(mockFs)).toCompletableFuture().join();
        assertEquals("success", result.getStatus());
    }
