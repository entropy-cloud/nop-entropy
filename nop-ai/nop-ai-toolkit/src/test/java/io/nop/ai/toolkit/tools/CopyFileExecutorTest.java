...
public class CopyFileExecutorTest {
    private CopyFileExecutor executor;
    private MockFileSystem mockFs;

    @BeforeEach
    void setUp() {
        executor = new CopyFileExecutor();
        mockFs = new MockFileSystem();
    }

    @Test
    void testToolName() {
        assertEquals("copy-file", executor.getToolName());
    }

    @Test
    void testExecuteWithoutFileSystem() {
        AiToolCall call = createCall("/src.txt", "/dst.txt");
    ...
    @Test
    void testExecuteWithEmptyPaths() {
        ...
        assertEquals("failure", result.getStatus());
        assertTrue(result.getError().getBody().contains("Source and target paths are required"));
    }

    @Test
    void testCopyFile() {
        mockFs.setContent("/src.txt", "test content");
        AiToolCall call = createCall("/src.txt", "/dst.txt");
        AiToolCallResult result = executor.executeAsync(call, new MockContext(mockFs)).toCompletableFuture().join();
        assertEquals("success", result.getStatus());
        assertEquals("test content", mockFs.getContent("/dst.txt"));
    }

    private AiToolCall createCall(String source, String target) {
        XNode node = XNode.make("copy-file");
        node.setAttr("id", "1");
        node.setAttr("source", source);
    ...
    static class MockFileSystem implements IToolFileSystem {
        ...
        @Override public void copy(String fromPath, String toPath, boolean recursive, boolean overwrite) { files.put(toPath, files.get(fromPath)); }
