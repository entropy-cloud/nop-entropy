...
public class ApplyDeltaExecutorTest {
    private ApplyDeltaExecutor executor;
    private MockFileSystem mockFs;

    @BeforeEach
    void setUp() {
        executor = new ApplyDeltaExecutor();
        mockFs = new MockFileSystem();
    }

    @Test
    void testToolName() {
        assertEquals("apply-delta", executor.getToolName());
    }

    @Test
    void testExecuteWithoutFileSystem() {
        AiToolCall call = createCall("/test.xml");
    ...
    void testFileNotExists() {
        AiToolCall call = createCall("/nonexistent.xml");
        AiToolCallResult result = executor.executeAsync(call, new MockContext(mockFs)).toCompletableFuture().join();
        assertEquals("failure", result.getStatus());
    ...
    @Test
    void testExecuteWithoutDelta() {
        mockFs.setContent("/test.xml", "<root/>");
        XNode node = XNode.make("apply-delta");
        node.setAttr("id", "1");
        node.setAttr("path", "/test.xml");
        AiToolCall call = AiToolCall.fromNode(node);
        AiToolCallResult result = executor.executeAsync(call, new MockContext(mockFs)).toCompletableFuture().join();
        assertEquals("failure", result.getStatus());
        assertTrue(result.getError().getBody().contains("deltaContent or deltaPath is required"));
    }

    @Test
    void testApplyDeltaWithContent() {
        mockFs.setContent("/test.xml", "<root><item>old</item></root>");
        XNode node = XNode.make("apply-delta");
        node.setAttr("id", "1");
        node.setAttr("path", "/test.xml");
