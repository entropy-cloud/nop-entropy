...
public class PatchFileExecutorTest {
    private PatchFileExecutor executor;
    private MockFileSystem mockFs;

    @BeforeEach
    void setUp() {
        executor = new PatchFileExecutor();
        mockFs = new MockFileSystem();
    }

    @Test
    void testToolName() {
        assertEquals("patch-file", executor.getToolName());
    }

    @Test
    void testExecuteWithoutFileSystem() {
        AiToolCall call = createCall("/test.txt", "--- a/test.txt\n+++ b/test.txt\n@@ -1 +1 @@\n-old\n+new");
    ...
    void testExecuteWithEmptyPath() {
        AiToolCall call = createCall("", "diff content");
    ...
    @Test
    void testExecuteWithEmptyDiff() {
        mockFs.setContent("/test.txt", "old content");
        ...
        AiToolCallResult result = executor.executeAsync(call, new MockContext(mockFs)).toCompletableFuture().join();
        assertEquals("failure", result.getStatus());
        assertTrue(result.getError().getBody().contains("Diff content"));
    }
    ...
    void testFileNotExists() {
        XNode node = XNode.make("patch-file");
        node.setAttr("id", "1");
        node.setAttr("path", "/nonexistent.txt");
        node.makeChild("input").setContentValue("diff");
        AiToolCall call = AiToolCall.fromNode(node);
        AiToolCallResult result = executor.executeAsync(call, new MockContext(mockFs)).toCompletableFuture().join();
        assertEquals("failure", result.getStatus());
        assertTrue(result.getError().getBody().contains("does not exist"));
    }
    ...
    void testDryRun() {
        ...
        node.setAttr("id", "1");
        node.setAttr("path", "/test.txt");
        node.setAttr("dryRun", "true");
        node.makeChild("input").setContentValue(diff);
        AiToolCall call = AiToolCall.fromNode(node);
        AiToolCallResult result = executor.executeAsync(call, new MockContext(mockFs)).toCompletableFuture().join();
        assertEquals("success", result.getStatus());
        assertTrue(result.getOutput().getBody().contains("Dry run"));
    }

    private AiToolCall createCall(String path, String diff) {
        XNode node = XNode.make("patch-file");
        node.setAttr("id", "1");
