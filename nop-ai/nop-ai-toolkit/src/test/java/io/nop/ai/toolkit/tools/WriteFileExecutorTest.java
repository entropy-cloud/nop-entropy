...
public class WriteFileExecutorTest {
    private WriteFileExecutor executor;
    ...
    @Test
    void testToolName() {
        assertEquals("write-file", executor.getToolName());
    }
    ...
    void testWriteFile() {
        ...
        node.setAttr("id", "1");
        node.setAttr("path", "/test.txt");
        node.makeChild("input").setContentValue("Hello World");
        AiToolCall call = AiToolCall.fromNode(node);
        AiToolCallResult result = executor.executeAsync(call, new MockContext(mockFs)).toCompletableFuture().join();
        assertEquals("success", result.getStatus());
        assertEquals("Hello World", mockFs.getContent("/test.txt"));
    }
