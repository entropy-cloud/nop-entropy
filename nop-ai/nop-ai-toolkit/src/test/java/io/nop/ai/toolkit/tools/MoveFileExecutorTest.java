...
public class MoveFileExecutorTest {
    private MoveFileExecutor executor;
    private MockFileSystem mockFs;

    @BeforeEach
    void setUp() {
        executor = new MoveFileExecutor();
        mockFs = new MockFileSystem();
    }

    @Test
    void testToolName() {
        assertEquals("move-file", executor.getToolName());
    }
    ...
    void testMoveFile() {
        ...
        assertNull(mockFs.getContent("/src.txt"));
    }

    private AiToolCall createCall(String source, String target) {
        XNode node = XNode.make("move-file");
        node.setAttr("id", "1");
    ...
    static class MockFileSystem implements IToolFileSystem {
        ...
        @Override public void move(String fromPath, String toPath, boolean overwrite) {
            files.put(toPath, files.get(fromPath));
