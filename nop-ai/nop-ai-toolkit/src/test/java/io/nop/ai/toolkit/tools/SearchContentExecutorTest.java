...
public class SearchContentExecutorTest {
    private SearchContentExecutor executor;
    private MockFileSystem mockFs;

    @BeforeEach
    void setUp() {
        executor = new SearchContentExecutor();
        mockFs = new MockFileSystem();
    }

    @Test
    void testToolName() {
        assertEquals("grep", executor.getToolName());
    }

    @Test
    void testExecuteWithoutFileSystem() {
        AiToolCall call = createCall("pattern", ".");
    ...
    @Test
    void testExecuteWithEmptyPattern() {
        ...
        assertEquals("failure", result.getStatus());
        assertTrue(result.getError().getBody().contains("Pattern is required"));
    }

    @Test
    void testSearchContent() {
        mockFs.addMatch(new SearchMatch("/test.txt", 10, "line with pattern match", "pattern", false));
        AiToolCall call = createCall("pattern", ".");
        AiToolCallResult result = executor.executeAsync(call, new MockContext(mockFs)).toCompletableFuture().join();
        assertEquals("success", result.getStatus());
        assertTrue(result.getOutput().getBody().contains("pattern"));
    }

    private AiToolCall createCall(String pattern, String path) {
        XNode node = XNode.make("grep");
        node.setAttr("id", "1");
    ...
    static class MockFileSystem implements IToolFileSystem {
        ...
        void addMatch(SearchMatch match) { matches.add(match); }
        ...
        @Override public List<SearchMatch> grep(String pattern, String searchDir, boolean recursive, boolean ignoreCase, int maxMatchesPerFile, int maxFiles, int maxDepth) { return matches; }
    }
}
