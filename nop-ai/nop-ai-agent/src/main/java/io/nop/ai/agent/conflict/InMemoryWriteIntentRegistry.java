...
public final class InMemoryWriteIntentRegistry implements IWriteIntentRegistry {
    ...
    public Set<WriteIntent> registerAndGetConflicting(WriteIntent intent) {
        if (intent == null) {
            throw new NopAiAgentException(NopAiCoreErrors.ERR_AI_AGENT_INVALID_ARG).param(NopAiCoreErrors.ARG_MSG, "intent must not be null");
        }
        String path = intent.getFilePath();
        if (path == null) {
            throw new NopAiAgentException(NopAiCoreErrors.ERR_AI_AGENT_INVALID_ARG)
                    .param(NopAiCoreErrors.ARG_MSG, "intent.filePath must not be null");
        }
