...
public class CorpusAttackPlugin implements AttackPlugin {
    ...
    public CorpusAttackPlugin(String name, String corpusDir) {
        if (name == null || name.isEmpty()) {
            throw new NopAiAgentException(ERR_AGENT_INTERNAL_DETAIL).param(ARG_DETAIL, "CorpusAttackPlugin: name must not be empty");
        }
        if (corpusDir == null || corpusDir.isEmpty()) {
            throw new NopAiAgentException(ERR_AGENT_INTERNAL_DETAIL).param(ARG_DETAIL, "CorpusAttackPlugin: corpusDir must not be empty");
        }
        this.name = name;
