...
public class ExcelDateHelper {
    ...
    public static double getExcelDate(Date date, boolean use1904windowing) {
        Calendar calStart = LocaleHelper.getLocaleCalendar();
     ...
     * representing its internal Excel representation, which is the number of
     * days since 1/1/1900. Fractional days represent hours, minutes, and
     * seconds.
     *
    ...
    private static double internalGetExcelDate(Calendar date, boolean use1904windowing) {
        ...
        double fraction = (((date.get(Calendar.HOUR_OF_DAY) * 60 + date.get(Calendar.MINUTE)) * 60
                + date.get(Calendar.SECOND)) * 1000 + date.get(Calendar.MILLISECOND)) / (double) DAY_MILLISECONDS;
    ...
    /**
     * Given an Excel date with using 1900 date windowing, and converts it to a
     * java.util.Date.
     * <p>
     * Excel Dates and Times are stored without any timezone information. If you
     * know (through other means) that your file uses a different TimeZone to
     * the system default, you can use this version of the getJavaDate() method
     * to handle it.
     *
     ...
     * <code>Europe/Copenhagen</code>, on 2004-03-28 the minute after 01:59 CET
     * is 03:00 CEST, if the excel date represents a time between 02:00 and
     ...
     * @param date             The Excel date.
     * @param tz               The TimeZone to evaluate the date in
     * @param use1904windowing true if date uses 1904 windowing, or false if using 1900 date
     *                         windowing.
     * @return Java representation of the date, or null if date is not a valid
     * Excel date
     */
    ...
    /**
     * Get EXCEL date as Java Calendar (with default time zone). This is like
     * {@link #getJavaDate(double)} but returns a Calendar object.
     *
     * @param date The Excel date.
     * @return Java representation of the date, or null if date is not a valid
     * Excel date
    ...
    /**
     * Get EXCEL date as Java Calendar (with default time zone). This is like
     * {@link #getJavaDate(double, boolean)} but returns a Calendar object.
     *
     * @param date             The Excel date.
     * @param use1904windowing true if date uses 1904 windowing, or false if using 1900 date
     *                         windowing.
    ...
    /**
     * Get EXCEL date as Java Calendar with UTC time zone. This is similar to
     * {@link #getJavaDate(double, boolean)} but returns a Calendar object that
     * has UTC as time zone, so no daylight saving hassle.
     *
     * @param date             The Excel date.
     * @param use1904windowing true if date uses 1904 windowing, or false if using 1900 date
     *                         windowing.
     * @return Java representation of the date in UTC, or null if date is not a
     * valid Excel date
     */
    public static Calendar getJavaCalendarUTC(double date, boolean use1904windowing) {
        return getJavaCalendar(date, use1904windowing, LocaleHelper.TIMEZONE_UTC, false);
    }
    ...
    public static Calendar getJavaCalendar(double date, boolean use1904windowing, TimeZone timeZone,
        ...
        Calendar calendar;
        if (timeZone != null) {
            calendar = LocaleHelper.getLocaleCalendar(timeZone);
        } else {
            calendar = LocaleHelper.getLocaleCalendar(); // using default
            // time-zone
        }
     ...
     * represents a date format or not. Firstly, it will check to see if the
     * format ID corresponds to an internal excel date format (eg most US date
     * formats) If not, it will check to see if the format string only contains
     * date formatting characters (ymd-/), which covers most non US date
     * formats.
     *
     * @param formatIndex  The index of the format, eg from
     ...
     * internal excel date format or not.
     *
    ...
    /**
     * Converts a string of format "HH:MM" or "HH:MM:SS" to its (Excel) numeric
     * equivalent
     *
    ...
    /**
     * Converts a string of format "YYYY/MM/DD" to its (Excel) numeric
     * equivalent
     *
     * @return a double representing the (integer) number of days since the
     * start of the Excel epoch
     */
    ...
    private static int parseInt(String strVal, String fieldName, int lowerLimit, int upperLimit)
        ...
        if (result < lowerLimit || result > upperLimit) {
            throw new FormatException(
                    fieldName + " value (" + result + ") is outside the allowable range(0.." + upperLimit + ")");
        }
        return result;
    }
}
