...
import io.nop.commons.bytes.LittleEndian;
...
public enum FileMagic {
    ...
    JPEG(new byte[]{(byte) 0xFF, (byte) 0xD8, (byte) 0xFF, (byte) 0xDB},
            ...
            new byte[]{(byte) 0xFF, (byte) 0xD8, (byte) 0xFF, (byte) 0xE1, '?', '?', 'E', 'x', 'i', 'f', 0x00,
                    0x00}),
    /**
     * GIF image
     */
    ...
    /**
     * Get the file magic of the supplied {@link File}
     * <p>
     * <p>
     * Even if this method returns {@link FileMagic#UNKNOWN} it could potentially mean, that the ZIP stream has leading
     * junk bytes
     *
    ...
    public static FileMagic valueOf(final File inp) throws IOException {
        try (FileInputStream fis = new FileInputStream(inp)) {
            ...
            int read = IoHelper.read(fis, data, 0, MAX_PATTERN_LENGTH);
            if (read != MAX_PATTERN_LENGTH) {
     ...
     * <p>
     * <p>
     * If unsure if your InputStream does support mark / reset, use {@link #prepareToCheckMagic(InputStream)} to wrap it
    ...
    public static FileMagic valueOf(InputStream inp) throws IOException {
        ...
        byte[] data = IoHelper.peekFirstNBytes(inp, MAX_PATTERN_LENGTH);
