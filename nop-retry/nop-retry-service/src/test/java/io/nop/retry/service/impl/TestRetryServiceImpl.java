...
import io.nop.retry.service.store.IRetryRecordStore;
...
/**
 * RetryServiceImpl 核心业务逻辑单元测试
 */
@NopTestConfig(
        localDb = true,
        initDatabaseSchema = OptionalBoolean.TRUE
)
public class TestRetryServiceImpl extends JunitAutoTestCase {
    ...
    void testNewRetryTask_shouldCreateTaskWithCorrectServiceInfo() {
        IRetryTask task = retryService.newRetryTask("test-service", "test-method");
    ...
    void testNewRetryTask_shouldSupportFluentConfiguration() {
        IRetryTask task = retryService.newRetryTask("svc", "method")
    ...
    void testRetryFromDeadLetter_shouldInvokeRpcWhenValid() throws Exception {
        ...
        CompletableFuture<ApiResponse<?>> future = retryService.retryFromDeadLetter("dl-1", null)
                .toCompletableFuture();
    ...
    void testExecuteTask_shouldSucceedOnFirstTry() throws Exception {
        ...
        CompletableFuture<ApiResponse<?>> future = retryService.executeTask(task, request, null)
                .toCompletableFuture();
