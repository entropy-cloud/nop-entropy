...
CREATE TABLE nop_retry_template(
  ...
  BACKOFF_STRATEGY INTEGER NULL    COMMENT '退避策略',
  BACKOFF_CONFIG VARCHAR(4000) NULL    COMMENT '退避配置',
  ...
  CALLBACK_MAX_ATTEMPTS INTEGER NULL    COMMENT '回调最大次数',
  CALLBACK_INTERVAL_SECONDS INTEGER NULL    COMMENT '回调间隔(秒)',
...
CREATE TABLE nop_retry_record(
  ...
  SERIALIZER_NAME VARCHAR(32) NULL    COMMENT '序列化器名称',
...
CREATE TABLE nop_retry_dead_letter(
  ...
  constraint PK_nop_retry_dead_letter primary key (SID)
   ...
   ALTER TABLE nop_retry_template COMMENT '重试模板';
