...
- [ ] `git clone --depth 1 https://github.com/apache/seatunnel ~/sources/seatunnel`
- [ ] 记录所分析源码的 commit SHA（报告 metadata 一节）
- [ ] 按 plan 1 矩阵逐维度分析（必答：连接器生态组织方式、CDC 产品化、多引擎适配层 Source/Sink API 抽象、部署形态本地/集群/K8s、监控与运维、配置 DSL 与向导）
- [ ] 架构层结论引用 `2026-05-19a-seatunnel-vs-nop-stream-comparison.md`，不重做；两报告结论冲突时显式记录
- [ ] 产出 `ST-n` 编号 P-REQ 候选条目（含验收标准 + 源码证据路径 + 建议归属工作项）
- [ ] 报告遵循 analysis guide（metadata + 必需章节），Status: resolved（后续工作指向 item 5）

Exit Criteria:

- [ ] `~/sources/seatunnel` 存在且为有效 git 仓库（`git -C ~/sources/seatunnel rev-parse HEAD` 成功）
- [ ] 报告存在且 7 个评估维度全部有评分（每维度附置信度标注，按 plan 1 分级）+ 源码证据引用（引用落到竞品仓库内具体文件路径），并记录 commit SHA
- [ ] plan 1 校准结论中该竞品的**必答维度逐项有结论**（以 plan 1 校准结论为准且必须 ⊇ roadmap item 2 列出的 6 项，逐一可对应到报告章节/条目）
- [ ] P-REQ 候选章节存在，条目均有 `ST-` 编号、要求陈述、可判定验收标准、证据指针、建议归属（item 6—18 编号或 Follow-up）
- [ ] 报告显式引用 2026-05-19a 报告并声明架构对比不重做
- [ ] No owner-doc update required（纯 analysis 产出）
...
- [ ] Decision: clone 策略裁定——完整 `--depth 1` vs partial clone（`--filter=blob:none` + sparse-checkout 限 streaming 相关子模块），以磁盘占用与可分析性为准，裁定结论写入报告附录（roadmap item 3 允许「仅 streaming 相关子集」）。sparse 候选目录起始集：`sql/core`（streaming 执行主力）、`sql/api`、`sql/catalyst`、`core`（调度）、`streaming`（旧 DStream 仅参照）、`docs/`（文档维度证据）、`external/`（连接器生态证据）；7 个评估维度的证据路径（含 AQE 跨 `sql/core` 执行与 `core` 调度）所需目录必须在裁定中逐项确认覆盖，避免过度剪枝丢失证据路径
- [ ] 执行选定策略 clone 到 `~/sources/spark`，记录 commit SHA
- [ ] 按 plan 1 矩阵逐维度分析（必答：micro-batch vs continuous 双模式取舍、adaptive query execution、状态存储与 checkpoint 产品化、Structured Streaming API 设计、运维/监控集成）
- [ ] 产出 `SPS-n` 编号 P-REQ 候选条目
- [ ] 报告遵循 analysis guide，Status: resolved

Exit Criteria:

- [ ] `~/sources/spark` 存在且为有效 git 仓库，streaming 相关子模块（`sql/core` streaming 源码等）内容可读
- [ ] clone 策略 Decision 已记录（含理由 + 必答维度目录覆盖确认）
- [ ] 报告存在且 7 维度全部有评分（每维度附置信度标注，按 plan 1 分级）+ 源码证据引用，并记录 commit SHA
- [ ] plan 1 校准结论中该竞品的必答维度逐项有结论（以 plan 1 校准结论为准且必须 ⊇ roadmap item 3 列出的 5 项，逐一可对应到报告章节/条目）
- [ ] P-REQ 候选章节存在，条目均有 `SPS-` 编号、要求陈述、可判定验收标准、证据指针、建议归属（item 6—18 编号或 Follow-up）
- [ ] No owner-doc update required（纯 analysis 产出）
- [ ] `ai-dev/logs/` 对应日期条目已更新
...
- [ ] `git clone --depth 1 https://github.com/apache/kafka ~/sources/kafka`（分析范围以 `streams/` 子模块为主），记录 commit SHA
- [ ] 按 plan 1 矩阵逐维度分析（必答：库形态 vs 引擎形态的运维差异、事务性 exactly-once 集成、状态存储 RocksDB 内嵌、interactive query、liveness/健康暴露）
- [ ] 产出 `KS-n` 编号 P-REQ 候选条目
- [ ] 报告遵循 analysis guide，Status: resolved

Exit Criteria:

- [ ] `~/sources/kafka` 存在且为有效 git 仓库，`streams/` 子模块内容可读
- [ ] 报告存在且 7 维度全部有评分（每维度附置信度标注，按 plan 1 分级）+ 源码证据引用，并记录 commit SHA
- [ ] plan 1 校准结论中该竞品的必答维度逐项有结论（以 plan 1 校准结论为准且必须 ⊇ roadmap item 4 列出的 5 项，逐一可对应到报告章节/条目）
- [ ] P-REQ 候选章节存在，条目均有 `KS-` 编号、要求陈述、可判定验收标准、证据指针、建议归属（item 6—18 编号或 Follow-up）
- [ ] No owner-doc update required（纯 analysis 产出）
- [ ] `ai-dev/logs/` 对应日期条目已更新
...
- [ ] 三个竞品 clone 均存在且有效（三个 `git rev-parse HEAD` 均成功）
- [ ] 三份报告均存在、均按 7 维度矩阵评分、均有编号 P-REQ 候选章节、均记录 commit SHA
- [ ] 每份报告的 P-REQ 候选可被 item 5 直接汇编（编号无冲突、字段齐全、验收标准可判定）
- [ ] 每竞品必答维度（roadmap items 2/3/4 stage details 所列）逐项有结论
- [ ] `node ai-dev/tools/check-doc-links.mjs --strict` 退出码 0
- [ ] `node ai-dev/tools/check-plan-checklist.mjs <plan-file> --strict` 退出码 0（closure 前置，guide Minimum Rule #26）
- [ ] 独立子 agent closure-audit 已完成并记录证据
- [ ] roadmap items 2/3/4 状态写回（closure audit 通过后）
