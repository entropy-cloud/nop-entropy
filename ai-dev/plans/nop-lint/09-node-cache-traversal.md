...
- [ ] 缓存落地且行为零差异（全部既有测试绿 + 新焦点测试背书）
- [ ] JMH/JFR 双口径验证完成，无回归（perf-baseline.md 增注为证）
- [ ] 候选 2 已裁定（启动→另立 successor 或本轮继续；不启动→Deferred 附理由）
- [ ] owner docs（design 03 增注 + perf-baseline.md）已同步
- [ ] 独立子 agent closure-audit 已完成并记录证据
- [ ] Anti-Hollow Check：缓存确实被热路径消费（findMatches/scanTree/NodeIterator → children() → 缓存命中，代码追踪 + JFR 证据双重确认）；无空方法体/静默跳过
