...
- [ ] R2-01-03: nop-bom/pom.xml 新增 `nop-stream-connector` 和 `nop-stream-runtime` 版本管理
- [ ] R1-01-04: nop-stream-fraud-example/pom.xml 移除 `<version>${project.version}</version>`
- [ ] R1-01-05: nop-stream-runtime/pom.xml 删除冗余 maven.compiler 属性（3 行）
- [ ] R1-02-02: 删除 nop-stream-{checkpoint,flink,flow} 下的 `.classpath`、`.project`、`.settings/` 目录；在 `nop-stream/.gitignore` 中添加 `.classpath`、`.project`、`.settings/` 模式
- [ ] R1-01-03: 在 nop-stream-fraud-example/pom.xml 中添加注释说明实际仅依赖 cep（与 spec 中"runtime+cep"不同）

Exit Criteria:

- [ ] `grep -rn "nop-stream-connector\|nop-stream-runtime" nop-bom/pom.xml` 返回非空结果
- [ ] fraud-example pom.xml 中无 `${project.version}`
- [ ] runtime pom.xml 中无 `maven.compiler.source/target/encoding` 属性
- [ ] checkpoint/flink/flow 下无 `.classpath`/`.project`/`.settings` 文件
...
- [ ] R1-09-04: CheckpointCoordinator 添加连续触发失败计数器（consecutiveTriggerFailures），超过阈值（默认 3）时 LOG.error 告警；成功触发后重置计数器
- [ ] R1-09-08: PendingCheckpoint.acknowledgePrecedingCheckpoint 判断是否有调用方——若无则移除该 public 方法（死代码）；若有则补全实现或标注 @Deprecated

Exit Criteria:

- [ ] CheckpointCoordinator 拥有 consecutiveTriggerFailures 计数字段；连续 N 次失败后日志包含连续失败次数（如 "Checkpoint trigger failed 3 consecutive times"）；成功触发后计数器重置为 0
- [ ] PendingCheckpoint 中无未实现的 public 方法（要么已移除，要么已实现，要么已 @Deprecated）
- [ ] 新增测试：连续失败告警阈值、PendingCheckpoint API 状态
...
- [ ] R1-16-01: TestWindowedStreamAggregation 中 3 处 assertDoesNotThrow 替换为验证 transformation 名称/DAG 结构的强断言
- [ ] R1-16-02: TestCheckpointIntegration 中 Thread.sleep 替换为 CountDownLatch.await(timeout, TimeUnit) 或 CompletableFuture.get(timeout)（项目无 Awaitility 依赖，不引入新依赖）
- [ ] R1-16-05: 新增 TestProcessingTimeoutTrigger 测试覆盖超时触发、清空窗口、状态恢复
- [ ] R1-16-09: TestCheckpointParticipant（nop-stream-runtime 的 TestCheckpointParticipantIntegration）增加 IKeyedStateBackend 集成验证（至少验证状态恢复后的值正确性）

Exit Criteria:

- [ ] TestWindowedStreamAggregation 中无 assertDoesNotThrow
- [ ] TestCheckpointIntegration 中无 Thread.sleep（用 CountDownLatch 或 CompletableFuture.get 替代）
- [ ] ProcessingTimeoutTrigger 有 >= 3 个测试方法覆盖核心行为
- [ ] TestCheckpointParticipant 验证状态后端恢复后的值
- [ ] `./mvnw test -pl nop-stream -am` 全部通过
- [ ] No owner-doc update required
- [ ] `ai-dev/logs/` 对应日期条目已更新
...
- [ ] R1-09-09: ChainingOutput 异常包装附带算子名称上下文——给 ChainingOutput 添加 String operatorName 构造器参数，在 catch 块中 `.param(ARG_OPERATOR_NAME, operatorName)`；同步修改所有 ChainingOutput 实例化点（搜索 `new ChainingOutput` 或 `new ChainingOutputWrapper`）传入算子名称
- [ ] R1-15-06: StreamRecord.replace() rogue pattern 添加 Javadoc 警告不得保留旧引用
- [ ] R1-15-07: Output.collectElement() 添加 else 分支处理未覆盖的 StreamElement 类型——因 Output 是接口 default 方法不能直接用 Logger，用 `System.getLogger()` 或在 else 中抛 `StreamException(ERR_STREAM_UNSUPPORTED)` 快速失败
- [ ] R1-15-08: KeyedStreamImpl sum/min/max 方法添加 @SuppressWarnings("unchecked") + 注释说明 Comparable 假设限制（类级泛型 T 无法添加 Comparable 约束，运行时 instanceof 检查已存在）
- [ ] R1-15-09: CheckpointSerDe.java（反序列化逻辑已从 LocalFileCheckpointStorage 提取到此类）中 unchecked cast 添加 instanceof Map 检查

Exit Criteria:

- [ ] ChainingOutput 异常 param 包含算子名称（operatorName）
- [ ] StreamRecord.replace() 有 Javadoc 警告
- [ ] Output.collectElement 有 else 分支处理未覆盖类型
- [ ] KeyedStreamImpl sum/min/max 有 @SuppressWarnings + 注释说明
- [ ] CheckpointSerDe.java 中反序列化有 instanceof Map 类型检查
- [ ] `./mvnw test -pl nop-stream -am` 全部通过
- [ ] No owner-doc update required
- [ ] `ai-dev/logs/` 对应日期条目已更新
...
- [ ] R1-02-05: NFA.EventWrapper 提取为独立顶层类 NFAEventWrapper（实际约 38 行内部类，NFA 总行数缩减）
- [ ] R1-17-06: SharedBuffer 删除注释掉的代码残留（~30 行）
- [ ] R1-17-07: StreamRecord 删除注释掉的异常抛出逻辑
- [ ] R2-02-05: GraphModelCheckpointExecutor 提取公共 `restoreFromCompletedCheckpoint()` 方法，消除 restoreFromCheckpoint 和 restoreFromSavepointPath 的 ~100 行重复
- [ ] R2-02-04: WindowOperator.processElement 提取 `processElementForMergingWindow` 和 `processElementForRegularWindow` 子方法

Exit Criteria:

- [ ] NFA.java 中无 EventWrapper 内部类（已提取为独立文件）
- [ ] SharedBuffer.java 中无注释掉的代码块
- [ ] StreamRecord.java 中无注释掉的异常逻辑
- [ ] GraphModelCheckpointExecutor 中 restore 重复代码已消除
- [ ] WindowOperator.processElement < 80 行
- [ ] `./mvnw test -pl nop-stream -am` 全部通过
- [ ] No owner-doc update required
- [ ] `ai-dev/logs/` 对应日期条目已更新
...
- [ ] R1-16-06: CoMapFunction/CoFlatMapFunction 如不使用则标注 @Deprecated，否则新增基本测试
- [ ] R1-16-07: PrintSinkFunction 新增基本测试（输出到 ByteArrayOutputStream 验证）
- [ ] R1-16-08: FunctionUtils 新增基本测试
- [ ] R1-16-04: CEP operator 测试中反射注入 processingTimeService 提取为共享工具方法 `CepTestUtils.injectProcessingTimeService()`
- [ ] R1-02-10: CepOperator 添加 Javadoc 说明当前硬编码 MemoryKeyedStateBackend 的设计原因和改进方向
- [ ] R1-01-02: nop-stream-runtime pom.xml 添加注释说明 cep/connector 集成规划（spec 漂移治理）

Exit Criteria:

- [ ] CoMapFunction/CoFlatMapFunction 有 @Deprecated 或基本测试
- [ ] PrintSinkFunction 有 >= 1 个测试
- [ ] FunctionUtils 有 >= 1 个测试
- [ ] CEP operator 测试中使用共享工具方法而非各自反射注入
- [ ] CepOperator 有状态后端设计说明 Javadoc
- [ ] runtime pom.xml 有 cep/connector 集成注释
- [ ] `./mvnw test -pl nop-stream -am` 全部通过
- [ ] No owner-doc update required
- [ ] `ai-dev/logs/` 对应日期条目已更新
...
- [ ] 全部 8 项未覆盖 P2 发现已修复，有对应测试或代码审查证明
- [ ] 可执行的 P3 发现已修复
- [ ] 不存在被静默降级到 deferred / follow-up 的 in-scope P2 缺陷
- [ ] No owner-doc update required（全部为代码/测试/pom 修复，不涉及平台约定变更）
