...
- [x] D1 dashboardName/screenName 唯一作用域裁定（multi-audit P2-14，P0-03 前置）：全表唯一 vs `(tenantId,createdBy,name)` 组合键 vs 维持声明但暂缓物化（须给出理由）——裁定必须评估：(a) 用户可见行为变更（同名看板创建被拒）；(b) ChatBI 生成路径兼容性——重名兜底现状不对称（Screen executor 有 `existsScreenName` 预检查 + rename-retry，Dashboard executor 无任何重名处理，见 Baseline），若裁定物化 dashboardName UK，**dashboard 侧镜像 Screen 预检查/rename-retry 是否随本 plan 实施**必须显式裁定（入 scope 或显式移出并记录理由——禁止 scope 悬空）；(c) 组合键选项的落地约束——`tenantId` 不在基础 orm 模型列内（租户列仅由 `_add_tenant_nop-datav.sql` 部署脚本按 `getColumnModelsWithTenant` 织入），组合键能否在 `<unique-key columns>` 中表达需先核实平台机制，不可表达时该选项降为不可选
- [x] D2 物化路径裁定：确认三条建表路径（deploy SQL 三方言、`init-database-schema` auto-DDL、测试基类 createAllTables）在补 `constraint` 属性后全部生效（同一 `ddl.xlib` 模板）；存量迁移脚本形态（单一 `_add_unique` + `_add_index` vs 合并）与 nop-auth `_add_ext_login_unique.sql` 先例对齐
- [x] D3 索引物化范围裁定：22 个 `<index>` 全量物化 vs 裁剪低价值索引（评估依据落档）；AlertState 既有冗余（UK 物化后 `(alertRuleId)` 唯一约束与 `IX_NOP_DATAV_ALERT_STATE_RULE` 普通索引物理重复）的处理结论——P2-11 一并收敛（UQ_ 前缀统一为 UK_ 或反之，选定单一惯例）
- [x] 裁定结论落档 `model-design.md`（唯一键矩阵：键/列组合/作用域/物化形态；索引矩阵与迁移策略）

Exit Criteria:

- [x] D1-D3 均有明确裁定并落档 `model-design.md`，每条裁定引用 live 代码核实事实
- [x] `node ai-dev/tools/check-doc-links.mjs --strict` 退出码 0
- [x] `ai-dev/logs/` 对应日期条目已更新
- [x] No new test required: 纯 Decision/文档 Phase，行为验证落在 Phase 2-3
...
- [x] Fix P0-03：按 D1 裁定为 8 个 `<unique-key>` 补/改 `constraint="UK_..."` 属性（名字键按 D1 作用域调整列组合）；按 D3 收敛 UQ_ 前缀与 AlertState 冗余索引声明
- [x] 存量重复数据策略（UK 零强制期间完全可能已有重复 dashboardName/screenName 等）：迁移脚本执行前审计存量重复（`GROUP BY ... HAVING count>1`），裁定去重/失败策略（预检脚本拒绝执行并报告重复行 vs 自动去重后缀）并写入迁移脚本或 model-design.md；同步排查测试 fixture（既有测试若创建同名实体将批量变红，须逐套件排查修正）
- [x] 重建三方言 `_create_nop-datav.sql`（经 `mvn install` 生成管线再生，禁止手改）；新增存量迁移脚本（`_add_unique_nop-datav.sql` + `_add_index_nop-datav.sql` 或 D2 裁定形态）三方言齐全，语法对照 nop-auth 先例
- [x] Fix P1-08：22（或 D3 裁定子集）个索引经迁移脚本物化
- [x] 验证：`./mvnw clean install -pl nop-datav -am -DskipTests` 重建生成物后，测试基类建表（同一 DdlSqlCreator）产出的表含全部 UK——至少一条测试断言 UK 存在（如直接对测试库 `INSERT` 重复 shareToken/(sessionId,seq)/(dashboardId,snapshotVersion) 断言约束拒绝，锚定 P0-03 五类并发兜底中的可测面）；迁移脚本经测试库执行验证语法（或裁定人工验证并记录）
- [x] `model-design.md` 物化结果与 D1-D3 裁定一致（无漂移）

Exit Criteria:

- [x] 三方言 `_create_nop-datav.sql` 各含全部 UK 约束；迁移脚本三方言齐全且覆盖 8 UK + 裁定索引集；存量重复数据策略已裁定并落地（脚本预检/去重），测试 fixture 已排查修正
- [x] 至少一条测试证明数据库层唯一性强制生效（重复插入被拒，非应用层检查；测试库为 H2，补 constraint 后建表即含约束，机制成立）
- [x] auto-DDL/测试建表路径与 deploy SQL 产出一致（同一模板机制核实落档）
- [x] 若 D1 裁定物化 dashboardName UK：dashboard 侧重名处理裁定已执行（入 scope 实施完成，或显式移出并记录 successor 归属——无 scope 悬空）
- [x] `ai-dev/backlog/nop-datav-audit-followups.md` 已标注 P2-11/P2-14 由本 plan 消化（P2-11 随 Phase 2 收敛、P2-14 随 D1 裁定）
- [x] `./mvnw test -pl nop-datav -am` 与 `./mvnw clean install -pl nop-datav -am -DskipTests` 通过；生成物无手改
- [x] `ai-dev/logs/` 对应日期条目已更新
...
- [x] Fix：`NopDatavReportTaskBizModel.doDeleteEntity` 在既有 `unregisterTask` 基础上级联删除 ReportDelivery 子行（先注销调度再删子行，与 Dashboard/Screen 级联模式一致）
- [x] Fix：`NopDatavAlertRuleBizModel.doDeleteEntity` 级联删除 AlertState 子行（`unregisterRule` 保持）
- [x] Fix：`NopDatavChatSessionBizModel` 覆写 `doDeleteEntity` 级联删除 ChatMessage——标准 `__delete` 与自定义 `deleteChatSession` 语义收敛为单一路径；注意 `ChatBiSessionManager.deleteSession` 含 owner 校验（`requireSession`）与 session 行自删，**不可在 doDeleteEntity 内整体调用**（会双重删除 + 把 owner-scope 校验注入 admin 标准路径），只复用其消息级联逻辑片段
- [x] Fix：`NopDatavDashboardTabBizModel` 覆写 `doDeleteEntity`：删除前解绑引用该 tab 的 `Panel.tabId`（置 null）或按仓库惯例拒绝删除非空 tab（裁定后落档，禁止悬挂）
- [x] 回归 ×4：各实体标准 delete 后无孤儿/悬挂（新 session 断言子表行数/tabId 引用），调度注销副作用保持；既有 Dashboard/Screen/Panel 级联测试不回归

Exit Criteria:

- [x] 四处级联/解绑全部有代码路径 + 回归测试（子表/引用断言，非仅无异常）
- [x] 标准路径与自定义路径删除语义一致（ChatSession）；调度注销（unregisterTask/unregisterRule）不回归
- [x] `./mvnw test -pl nop-datav -am` 通过
- [x] 级联裁定（Tab 处理方式等）落档 `model-design.md` 或对应 owner doc
- [x] `ai-dev/logs/` 对应日期条目已更新
...
- [x] Fix：80+（实测 96 个 define）错误码描述全部改为中文（对齐 `error-handling.md` 契约与 nop-auth 惯例——nop-auth 先例仅覆盖 zh 描述半边）；同步补 en i18n 键：机制为 `I18nMessageManager` 按错误码字符串查 `_vfs/i18n/{locale}/*.i18n.yaml`，en 键缺失时回退 define() 默认描述（改中文后 en 用户将看到中文，故 en 键是 load-bearing）——**目标文件与键格式**：键=错误码全串（如 `nop.err.datav.dashboard-not-found`），全仓唯一先例 `nop-cli-core` 的 `nop-cli-errors.i18n.yaml`；nop-datav 侧 i18n 脚手架在 nop-datav-meta（`nop-datav/nop-datav-meta/src/main/resources/_vfs/i18n/en/nop-datav.i18n.yaml` retention 文件），Phase 内核实后落位（新增 en 错误码 i18n 文件 nop-datav-errors.i18n.yaml，或并入既有 retention 文件）；ARG 声明与错误码字符串格式占位符保持不变（不破坏既有 `.param(...)` 调用）
- [x] 若执行中发现个别错误码属「转换类、逐字保留历史消息」的分界情形（error-handling.md 来源驱动分界），按契约逐条裁定并记录，不得整体沿用英文
- [x] 回归：既有测试中锚定错误码 message 文本的断言全部同步更新并通过；无错误码字符串键（`nop.err.datav.*`）变更（保持 wire 兼容）

Exit Criteria:

- [x] `NopDatavErrors.java` 无英文业务错误码描述残留（转换类裁定例外逐条记录）；en i18n 键齐备
- [x] 错误码字符串键与 ARG 集合零变更（wire 兼容，测试锚定）
- [x] `./mvnw test -pl nop-datav -am` 通过
- [x] No owner-doc update required: `error-handling.md` 白名单无需变更（nop-datav 走默认中文规则）
- [x] `ai-dev/logs/` 对应日期条目已更新
...
- [x] Fix P1-11：新增 IM 部分成功分支测试——`MockChannelMessageService.setNoBindingUsers(Set.of("u1"))` + recipients `["u1","u2"]` → 断言 `deliveredChannels` 含 im（anySent 聚合语义被测试锚定；mock 能力已存在，`mock/MockChannelMessageService.java:65-67`）
- [x] Fix P1-12：新增 `TestPanelSqlBuilder` 纯 JUnit 单测（无容器依赖）：空 SQL 分支 A、未声明占位符分支 B（均断言 NopException + panelId param）、多占位符顺序绑定、恶意参数值（`north' OR '1'='1`）进 `getParams()` 且不出现在 SQL 文本（注入回归锚点）
- [x] 两测试入位后运行 mutate 自检（可选但建议）：临时反转 anySent 语义/改拼接实现确认测试能红（验证测试保护力），随后恢复

Exit Criteria:

- [x] 两项新测试存在、命名可检索、断言覆盖上述分支与注入面，全绿
- [x] `./mvnw test -pl nop-datav -am` 通过
- [x] No owner-doc update required: 纯测试补充
- [x] `ai-dev/logs/` 对应日期条目已更新

## Closure Gates

- [x] P0-03/P1-07/P1-08/P1-09/P1-11/P1-12 六项缺陷全部修复且各有回归锚定（UK 物化有数据库层强制证明、级联有子表断言、错误码有 wire 兼容锚定、两测试缺口有新测试）
- [x] dashboardName/screenName 作用域经显式裁定后才物化（无未裁定的用户可见行为变更）
- [x] 三方言 DDL 与迁移脚本一致完备；`_gen`/`_*.xml` 生成物仅经构建重建，无手改
- [x] 既有测试套件全绿（错误码描述变更的断言同步完成）
- [x] 不存在被降级为 follow-up 的 in-scope 缺陷
- [x] 独立子 agent closure audit 已完成并写入 Closure 段落（含 Anti-Hollow 检查：迁移脚本真实可执行、级联真实删除子行）
- [x] `./mvnw test -pl nop-datav -am` 通过
...
- closure audit Minor 观察（2026-08-16）：plan Phase 4 文本「实测 96 个 define」为立项时旧数（live 100 = 99 既有 + 1 D1(b) 新增），语义「80+/全部」不受影响，不改历史 plan 文本。

## Closure

Status Note: 六项缺陷（P0-03/P1-07/P1-08/P1-09/P1-11/P1-12）全部修复且各有回归锚定；前置决策（D1 作用域/D2 物化路径/D3 索引范围 + dashboard 侧重名兜底入 scope）显式裁定并落档；三方言 DDL/迁移脚本完备、生成物零手改；5 个 Phase 全部 completed 且 Exit Criteria 全勾选；独立子 agent closure audit 19/19 PASS，verdict CLOSABLE。

Completed: 2026-08-16

Closure Audit Evidence:

- Reviewer / Agent: 独立子 agent closure audit（fresh session `ses_ff65c1b45ffeanpNMCICDA9tR1`，research-only）
- Evidence:
  - Phase 3（P1-09）5/5 PASS：ReportTaskBizModel.java:96-102（unregisterTask→ReportDelivery deleteByQuery，异常传播）、AlertRuleBizModel.java:102-110（AlertState 级联 + unregisterRule 保持）、ChatSessionBizModel.java:40-44（只镜像消息级联片段 = ChatBiSessionManager.java:109-111，无 requireSession/无双重删除）、DashboardTabBizModel.java:40-41（解绑先于删除，面板保留）；TestNopDatavSubEntityDeleteCascade 5 用例全部经 biz 标准 delete（:118/:152/:178/:223）+ jdbcTemplate 行数断言（:245-249）+ 注销断言（:126-128/:158-160）+ ③a/③b 语义收敛锚定（:170/:192）；audit 独立复跑 5/0/0。
  - Phase 4（P1-07）3/3 PASS：NopDatavErrors 100 define 零纯 ASCII 描述（程序化核验）；git diff HEAD 仅描述行 + javadoc + 1 个 D1(b) 新增键（既有 99 键字节不变、零 ARG 行变更）；en i18n 100 键与 Java 键集/占位符 100/100 一致；TestNopDatavErrorsI18n 覆盖中文契约/en 解析/键唯一性，复跑 2/0/0。
  - Phase 5（P1-11/P1-12）2/2 PASS：TestNopDatavReportE2E.java:397-419（setNoBindingUsers + u1/u2 → SUCCEEDED + deliveredChannels=im + 双 userId 均尝试）；TestPanelSqlBuilder 纯 JUnit 7 用例（空 SQL×2/未声明占位符/null 绑定/多占位符顺序含重复/恶意值注入锚点 north' OR '1'='1），复跑 7/0/0。
  - Phase 1-2 抽查 6/6 PASS：orm.xml 8 UK 全 constraint=UK_（:172/:423/:485/:558/:696/:834/:1184/:1306）+ UK_ 前缀收敛 + 冗余索引删除；三方言 _create 各 8 UK、_add_unique（8 ALTER + 审计 SQL 头注释）/_add_index（21 CREATE）齐全；model-design.md 裁定章节（:55-96）+ 级联契约章节（:98-109 含 Tab 解绑裁定 :107）；audit-followups.md:69/:72 P2-11/P2-14 已标消化；TestNopDatavUniqueKeyEnforcement 复跑 6/0/0；DatavGenerateDashboardExecutor 重名预检查 :160-163 + UK 冲突映射 :175-186。
  - Anti-Hollow：CrudBizModel.java:1045-1066 delete(id)→doDelete→虚分派 doDeleteEntity（级联非死代码，标准路径实证）；四个覆写均有真实方法体、零 try/catch 吞异常/空方法体；生成物 diff 均机械再生（+8 UK 行/_add_tenant 24 行 NOP_TENANT_ID 前缀/xmeta 1 行 UQ_→UK_），无手改签名。
  - `node ai-dev/tools/check-doc-links.mjs --strict` 退出码 0（6 条 warning 均为无关 plan 338 既有项）；`scan-hollow-implementations.mjs --module nop-datav --severity high` 0 findings；`./mvnw test -pl nop-datav -am -T 1C` BUILD SUCCESS（datav-service 621/0/0）；`./mvnw clean install -pl nop-datav -am -T 1C -DskipTests` BUILD SUCCESS。
  - Deferred 项分类检查：无 in-scope live defect 被降级（Non-Blocking Follow-ups 仅 P2 细项/多方言人工验证裁定/文本观察）。
  - `node ai-dev/tools/check-plan-checklist.mjs <plan-file> --strict` 退出码 0。

Follow-up:

- P2 ORM 细项（P2-10/P2-12/P2-13/P2-39）与文档漂移组（P2-45~48）→ `ai-dev/backlog/nop-datav-audit-followups.md`（successor：backlog 轮次）。
- Oracle/PostgreSQL 迁移脚本语法以 Phase 2 裁定的人工核对方式验证（H2/MySQL 已由测试建表路径覆盖），不阻塞本关闭。
- 无其余 plan-owned 遗留工作。
