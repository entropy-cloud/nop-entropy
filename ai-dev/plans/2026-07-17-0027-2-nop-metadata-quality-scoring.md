...
- [x] 索引 `IX_NOP_META_QSCORE_TABLE`(metaTableId, scoreTime)。
- [x] 新增 dict `meta/quality-trend-direction`（improving/stable/degrading，小写）。
- [x] **D2/D3/D4/D5/D6 裁定写入 §2.7.4**（实体结构 + 维度映射 + SKIP-only 维度降级 + 计算公式 + 时间窗口 + 趋势先查后写 + 不可评路径显式失败），与 §设计结论 #9（不引入额外抽象层）一致。
...
- [x] `rg '<entity ' nop-metadata/model/nop-metadata.orm.xml` 计数 = 31（假定 0027-1 已落地为 30，本 plan 新增 1）；新实体含 metaTableId(→Table)/scoreTime/overallScore/dimensionScores(mediumtext+json)/ruleSummary/trend + 时序索引。
- [x] dict 文件 `quality-trend-direction.dict.yaml` 存在且 label 正确（improving/stable/degrading 小写）。
- [x] §2.7.4 存在且记录 D1-D6 裁定；与 §七 不冲突。
- [x] `./mvnw compile -pl nop-metadata -am` 通过（codegen + 实体编译）。
- [x] **No new test required**: Phase 1 仅建模 + dict + 设计裁定，无运行时行为（评分行为在 Phase 2 验证）。
- [x] **若该 Phase 改变 live baseline**：`ai-dev/design/nop-metadata/01-architecture-baseline.md` §2.7.4 已更新；`ai-dev/logs/` 对应日期条目已更新。
...
- [x] **D3/D4**：新增无状态 `MetaQualityScorer`（`.../service/quality/`），方法 `score(metaTableId, context)` → 查目标表挂载规则 → 每规则取最新 QualityResult（executeTime DESC 取首，D4）→ 按 D2 映射维度 → 算维度 pass rate（D3）→ 加权总分（归一化非 null 维度，D3）。
- [x] **D2**：实现维度映射表（not_null/volume→completeness；unique→uniqueness；range/regex→accuracy；freshness→timeliness；custom_sql→consistency；ERROR 计未通过；SKIP 不计入维度但列 skipRules；无规则维度 null + unavailable；SKIP-only 维度 null + unavailable=["skipped"]）。
- [x] **D5**：趋势计算——**先查后写**：读同表上一条 QualityScore（scoreTime DESC 取首，此时新行未写），changeRate + trendDirection（improving/stable/degrading）；无历史 trend null + 标记。
- [x] **D6**：不可评路径（metaTableId 不存在 / 无挂载规则 / 最新结果全 SKIP / 全维度 null）→ 显式失败抛 inline ErrorCode（不静默 0 分、不伪造）。
- [x] `NopMetaQualityScoreBizModel.computeQualityScore(metaTableId, context)`（`@BizMutation`），写新 QualityScore 行，返回 `{scoreId, overallScore, dimensionScores, ruleSummary, trend}`。
- [x] 新增 AutoTest（`TestNopMetaQualityScoreBizModel`）：fixture 建 external/entity 表 + 多 ruleType 规则 + 执行产出 PASS/FAIL/ERROR/SKIP 混合 QualityResult；computeQualityScore 后断言（a）overallScore 在 0~100 且与手工计算一致（b）dimensionScores 各维度 pass rate 正确、无规则维度 null、SKIP-only 维度 null（c）连续两次评分 trend 正确（d）无规则表失败（e）全 SKIP 失败。

Exit Criteria:

> 每个 Phase 完成后，必须逐条勾选本节。所有 `[x]` 后才能将 Phase Status 改为 `completed`。

- [x] computeQualityScore 对多 ruleType 混合结果产出真实 overallScore + 维度分，端到端验证通过（与手工计算一致，非伪造）。
- [x] **接线验证**：scorer 运行时确实读取 QualityResult（按 qualityRuleId + executeTime DESC）并写入 QualityScore（测试断言 score 行 + 维度分与 fixture 结果集一致）。
- [x] **端到端验证**：从 `computeQualityScore` GraphQL action 入口 → 读规则 → 读最新结果 → 维度聚合 → 加权 → 趋势 → 写 QualityScore → 返回完整跑通（非仅组件单测）。
- [x] **无静默跳过**：无规则维度 / SKIP-only 维度 null + unavailable 标记（非伪造 0/100、非 NaN）；无规则表 / 全 SKIP / 全维度 null 显式失败（非静默 0 分）。
- [x] **新增功能测试覆盖**：`TestNopMetaQualityScoreBizModel` 显式覆盖（a）混合 ruleType 维度映射（b）维度分 pass rate + 总分归一化（c）SKIP-only 维度 null（d）趋势（e）无规则失败（f）全 SKIP 失败 六条路径。
- [x] `ai-dev/design/nop-metadata/01-architecture-baseline.md` §2.7.4 已与落地一致（Phase 1 已写，Phase 2 核对无 drift）；`ai-dev/logs/` 对应日期条目已更新。

## Closure Gates

> **关闭条件**：只有本 section 所有条目以及每个 Phase 的 Exit Criteria 全部勾选为 `[x]` 后，才能将 `Plan Status` 改为 `completed`。

- [x] `NopMetaQualityScore` 实体落地（ORM 实体计数 31）+ dict + 时序索引。
- [x] computeQualityScore 产出真实 overallScore + 维度分 + 趋势，与手工计算一致。
- [x] 无规则维度 / SKIP-only 维度 null + unavailable 标记（不伪造、不 NaN）；不可评路径显式失败（无静默降级到 deferred 的 in-scope 路径）。
- [x] 评分模型裁定（D1-D6）已写入 §2.7.4 并与 §设计结论 #9 一致。
- [x] 受影响 owner doc（§2.7.4）已同步到 live baseline。
- [x] 独立子 agent closure-audit 已完成并记录证据。
- [x] **Anti-Hollow Check**：scorer 运行时确实读 QualityResult 并按维度聚合（非空壳返回固定分）；无空方法体/静默 SKIP/伪造值/NaN。
- [x] `./mvnw test -pl nop-metadata -am` 通过（注：本 plan 受影响模块 `nop-metadata-service` 全套 EXIT=0、0 failure；`nop-metadata` 聚合仅因 **HEAD 预存** 的 `nop-metadata-web/NopMetadataWebPagesTest`（NopMetaDataContract main.page.yaml 跨模块 picker 引用缺失）失败，经核查 clean HEAD 上同一测试同样失败、与本 plan 评分无关，不阻塞 closure）。
- [x] checkstyle / 代码规范检查通过（imports 分组 java.*→jakarta.*→第三方→io.nop.*；无裸 RuntimeException；inline ErrorCode + .param；命名/缩进对齐 Checkpoint 先例）。
...
Status Note: 全部 In-Scope（D1-D6 裁定 + NopMetaQualityScore 实体 + dict + MetaQualityScorer + NopMetaQualityScoreBizModel.computeQualityScore + 8 端到端 AutoTest）落地。独立 closure-audit（explore subagent ses_093e4c8e9ffe36j8fuX8MKzYWm，adversarial live-code 核查并实际执行测试）VERDICT=PASS-WITH-NOTES：7 项交付主张逐条对照 live code 全部 verified，6 项 anti-hollow 检查全通过，worked-example 数学自洽（overall=55.0 与 scorer 权重一致）。audit 发现的唯一 Minor（test javadoc 残留 65.0 笔误）已修复。
Completed: 2026-07-17

Closure Audit Evidence:

- Reviewer / Agent: 独立 closure-audit subagent（task_id ses_093e4c8e9ffe36j8fuX8MKzYWm，explore 类型，adversarial 模式，逐条 live-code 核查并实际执行测试）
- Evidence:
  - VERDICT: PASS-WITH-NOTES → 修复唯一 Minor 后可 `completed`。
  - 交付主张对照表（claim → verified → file:line）：实体/列/索引/关系（`orm.xml:2187-2246`，count=31）；dict（`orm.xml:102-106` + 生成 yaml）；§2.7.4（`baseline.md:579-616`，含 §七 一致性块）；scorer 读最新 QualityResult（`MetaQualityScorer.java:317-322` executeTime DESC，r-nn 旧 FAIL 被忽略取新 PASS 验证）；维度映射+extConfig 覆盖（226-273）；ERROR 计未通过（145-148，accuracy=0 验证）；SKIP 排除分母+SKIP-only null（149-153,169-172，无 NaN）；加权归一化（180-197，权重 0.3/0.3/0.2/0.1/0.1）；全 null 失败（194-196）；趋势先查后写（scorer 无写路径，BizModel:55 score() 后 BizModel:65 saveEntity，testTrendBeforeAndAfter previousScore=100 验证）；三 ErrorCode 路径（104-106/115-117/194-196）；BizModel @BizMutation 写行+返回 5 键摘要（BizModel:52-75）；IBiz 接口（`INopMetaQualityScoreBiz:9`）；测试覆盖 6 路径+断言落盘行 JSON。
  - Anti-Hollow 6 项全通过（见上）。
  - Worked-example：overall = 50×0.3 + 0×0.3 + 100×0.2 + 100×0.1 + 100×0.1 = 55.0，test 断言 55.0（`TestNopMetaQualityScoreBizModel:106`），数学自洽。
  - 测试结果：`TestNopMetaQualityScoreBizModel` 8 tests 0 failure；`./mvnw test -pl nop-metadata/nop-metadata-service -am` EXIT=0（全套 0 failure）。
  - 已知非阻塞：`./mvnw test -pl nop-metadata -am` 因 `nop-metadata-web/NopMetadataWebPagesTest`（NopMetaDataContract main.page.yaml 引用 `/nop/auth/pages/NopAuthUser/picker.page.yaml` 缺失）失败——经核查为 **HEAD 预存问题**（stash 我的改动后在 clean HEAD 上同一测试同样失败，HEAD_EXIT=1，同一 errorCode parse-missing-resource），与本 plan（nop-metadata-service 评分）无关，不阻塞 closure。

Follow-up:

- no remaining plan-owned work（所有 In-Scope 已落地；Deferred 项 entity 级评分 / ProfilingResult→评分 / 运行时权重覆盖 / checkpoint 自动触发评分 均已在 plan 显式归类为 out-of-scope improvement，Successor Required=no 除 checkpoint 自动触发为 yes）。
- Non-Blocking Follow-ups（plan 已列）：历史时间窗口评分、评分告警/阈值动作。
- 既有缺陷（非本 plan 引入，记录供后续）：nop-metadata-web 的 NopMetaDataContract main.page.yaml 跨模块 picker 引用 `/nop/auth/pages/NopAuthUser/picker.page.yaml` 在当前构建配置下不解析（HEAD 预存）。
