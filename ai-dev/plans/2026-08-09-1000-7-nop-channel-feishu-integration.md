...
- [ ] **Decision：出站数据流路径裁定**。经代码核实，事件 payload 不含响应文本（`EXECUTION_COMPLETED` payload 只有 metrics，`ReActAgentExecutor.java:1015-1027`）。裁定 connector 出站路径：
...
- [ ] 裁定记录写入设计文档 `nop-ai-agent-channel-connector.md` §7.2（更新伪代码为 execute() 路径，注明事件 payload 不含文本的事实 + 拒绝事件订阅方案的理由）

Exit Criteria:

- [ ] 出站路径已裁定（`execute()` + future callback），经代码验证的事实（事件 payload 无文本、execute 返回 messages）记录在设计文档 §7.2
- [ ] 设计文档 §7.2 原伪代码（`event instanceof AgentResult`）更新为实际可行的 execute() 路径
- [ ] **No new test required**: Phase 0 是设计裁定，事实经代码核实（`ReActAgentExecutor.java:1015-1027` + `AgentExecutionResult.java:60-73`），非 throwaway
...
Status: planned
Targets: `nop-integration-feishu/src/main/java/io/nop/integration/feishu/bind/FeishuBindProvider.java` · `nop-integration-feishu/src/test/java/.../bind/TestFeishuBindProvider.java`

- Item Types: `Decision | Fix`

- [ ] **Decision：QR payload 选型**（设计 §六 Open Question）。评估：
  - 选项 A：飞书扫码登录二维码（构造 OAuth 授权 URL，不需 bot 推送）
  - 选项 B：自建券 + 机器人推送（需 FeishuClient.sendMessage）
  - 选定后记录到设计文档 §六收口
- [ ] 实现 `FeishuBindProvider`（`IChannelBindProvider` 实现，落 `nop-integration-feishu/.../bind/`）：
...
- [ ] **`FeishuBindProvider` 不依赖 `nop-ai-*`**——只实现 `nop-integration-api` 接口 + 同模块 FeishuClient

Exit Criteria:

- [ ] QR payload 选型已做出，记录在设计文档 §六收口
- [ ] `FeishuBindProvider` 存在于 `nop-integration-feishu/.../bind/`，实现 3 个接口方法
- [ ] **新增功能测试覆盖**（Test-Mandated Feature Rule，用 stub FeishuClient）：
  - [ ] `createBindTicketReturnsPendingTicketWithQrPayload`：BindTicket status=PENDING、qrPayload 非空非占位
  - [ ] `onCallbackReturnsCompletedWithOpenId`：模拟飞书回调 → ChannelBindResult status=BINDING_COMPLETED、extId 非空
  - [ ] `onCallbackWithUnknownTicketFailsExplicitly`：未知 ticketId → 显式失败
  - [ ] `onCallbackWithMissingPayloadFailsExplicitly`：rawPayload 缺关键字段 → 显式抛异常（No Silent No-Op）
- [ ] **无静默跳过**：缺字段 / 未知 ticket / 过期 ticket 均显式失败
- [ ] `grep nop-ai nop-integration-feishu/pom.xml` 为空（依赖纯净）
- [ ] `./mvnw test -pl nop-integration-feishu -am` 通过
- [ ] `ai-dev/logs/` 对应日期条目已更新
...
Status: planned
Targets: `nop-ai-gateway/pom.xml` · `nop-ai-gateway/src/main/java/io/nop/ai/gateway/channel/feishu/FeishuConnector.java` · `nop-ai-gateway/src/test/java/.../channel/feishu/TestFeishuConnector.java`

- Item Types: `Fix`

- [ ] `nop-ai-gateway/pom.xml` 新增 `nop-integration-feishu` 依赖（取 FeishuClient/FeishuCredentials/FeishuBindProvider，无环）
- [ ] 实现 `FeishuConnector`（`IChannelConnector` 实现，落 `nop-ai-gateway/.../channel/feishu/`）：
  - `getChannelType()` → `"feishu"`
...
- [ ] **单体部署约束声明**：execute() 的 future 在同 JVM 完成。多实例部署需 future 跨进程传递（Deferred）

Exit Criteria:

- [ ] `FeishuConnector` 存在于 `nop-ai-gateway/.../channel/feishu/`，实现 `IChannelConnector` 全部方法
- [ ] `nop-ai-gateway/pom.xml` 含 `nop-integration-feishu` 依赖且无环
- [ ] **新增功能测试覆盖**（Test-Mandated Feature Rule，用 stub FeishuClient + stub IAgentEngine）：
  - [ ] `inboundMessageTriggersEngineExecuteAndSessionMapping`：模拟飞书消息 → `IAgentEngine.execute` 被调用（callCount > 0）→ 新 session 写入 IChannelSessionStore
  - [ ] `executeResultTriggersFeishuReply`：stub execute future 完成 → FeishuClient.sendMessage 被调用（callCount > 0），回复内容含 result.messages 中的文本
  - [ ] `executeFailureTriggersErrorReply`：stub execute future 异常完成 → FeishuClient.sendMessage 被调用，内容含 error
  - [ ] `groupMessageWithoutBotMentionIsIgnored`：群聊无 @bot → execute callCount == 0
  - [ ] `dmMessageIsProcessed`：私聊 → execute callCount > 0
  - [ ] `sendOutboundWithAttachmentDegradesToTextLink`：附件 + supportsFileUpload=false → 降级
- [ ] **端到端验证**（Anti-Hollow Rule #22）：`fullInboundToOutboundRoundTrip`——模拟飞书消息到达 → engine.execute → future 完成 → FeishuClient.sendMessage 回复。**从用户入口（飞书消息）到最终输出（飞书回复）完整跑通**（经 stub，调用链真实连通）
- [ ] **接线验证**（Wiring Verification Rule #23）：start 后 IMessageHandler 注册到 FeishuClient（handler 注册计数 > 0）；inbound 真实调 engine.execute（callCount 断言）；future callback 真实调 FeishuClient.sendMessage（callCount 断言）
- [ ] **无静默跳过**（Rule #24）：stop 后 sendOutbound 显式失败；附件降级有明确行为；future 异常不静默吞掉
- [ ] `./mvnw test -pl nop-ai-gateway -am` 通过
- [ ] No owner-doc update required（设计文档 §7.2 更新已在 Phase 0 完成）
- [ ] `ai-dev/logs/` 对应日期条目已更新
...
- [ ] 创建 beans.xml 装配：FeishuConnector bean（`@Inject` FeishuClient + IChannelSessionStore）+ FeishuClient bean（凭证 `@InjectValue`）+ FeishuBindProvider bean（`IChannelBindProvider` SPI 经 collect-beans 自动收集）+ FeishuCredentials bean。`@Inject` 字段用 `protected`（NopIoC 约束）
- [ ] **运行时装配验证**（Anti-Hollow）：容器启动 → ChannelConnectorManager.getConnector("feishu") 解析成功 + FeishuBindProvider 经 collect-beans 被扫码登录端点发现
- [ ] **W7-1 设计文档同步**：
  - [ ] `nop-ai-channel-integration-design.md` §3.5 表（`:238`）`IChannelConnector` 行由 `nop-ai-agent` 改为 `nop-ai-gateway`
  - [ ] `nop-ai-channel-integration-design.md:257` 散文中 `FeishuConnector` 实现归属由 `nop-integration-feishu` 改为 `nop-ai-gateway`（connector 依赖 `IAgentEngine`/`IChannelSessionStore`，不能下沉到不依赖 AI 的厂商模块）
  - [ ] 设计文档 + roadmap 中 `AgentEventPublisher` 统一为 `IAgentEventPublisher`
  - [ ] `nop-ai-agent-channel-connector.md` §7.2 已在 Phase 0 更新（execute() 出站路径）；§7.3 中间事件策略（delta 合并/文本增量）因改用 execute() 而不再适用于飞书，标注为 SSE/WebSocket 专用策略
  - [ ] 运行 `node ai-dev/tools/check-doc-links.mjs --strict`（修改设计文档后必须）

Exit Criteria:

- [ ] beans.xml 注册 FeishuConnector + FeishuClient + FeishuBindProvider + FeishuCredentials
- [ ] **新增功能测试覆盖**：IoC 装配测试——容器启动后 getConnector("feishu") 返回非 null + FeishuBindProvider 经 collect-beans 注册
- [ ] **接线验证**：beans.xml bean 经容器解析成功（getBean 非 null），FeishuConnector 真实注入 FeishuClient + IChannelSessionStore（字段非 null）
- [ ] W7-1 设计文档同步完成：§3.5 表 + IAgentEventPublisher 命名 + §7.2
- [ ] `node ai-dev/tools/check-doc-links.mjs --strict` 退出码 0
- [ ] `./mvnw test -pl nop-integration-feishu,nop-ai-gateway -am` 通过
- [ ] `ai-dev/logs/` 对应日期条目已更新
...
- [ ] Phase 0 出站路径裁定（execute() + future callback），设计文档 §7.2 已更新
- [ ] W5-2 FeishuBindProvider 实现 + 测试（QR 决策 + 扫码回调解析 open_id + 单测）
- [ ] W5-3 FeishuConnector 实现 + 测试（入站→execute() + future→飞书回复 + @bot 过滤 + 附件降级 + 端到端 round-trip）
- [ ] W5-4 IoC 装配（beans.xml + 容器解析验证）
- [ ] W7-1 设计文档同步（§3.5 表 + IAgentEventPublisher 命名 + §7.2）
- [ ] `nop-ai-gateway` → `nop-integration-feishu` 依赖无环
- [ ] `nop-integration-feishu` 仍不依赖 `nop-ai-*`
- [ ] **Anti-Hollow Check**：FeishuConnector 非空壳（inbound 真实调 execute、future callback 真实调 FeishuClient.sendMessage），FeishuBindProvider 非空壳（回调真实解析 rawPayload）
- [ ] 不存在被静默降级的 in-scope live defect
- [ ] `./mvnw test -pl nop-integration-feishu,nop-ai-gateway -am` 全绿
- [ ] `node ai-dev/tools/check-doc-links.mjs --strict` 退出码 0
