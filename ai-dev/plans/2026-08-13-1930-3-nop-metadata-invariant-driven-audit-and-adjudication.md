...
- [ ] 复跑 4 条门禁，逐条记录命中（`文件:行` + 不变式编号 + 门禁名）
- [ ] 将每条命中回溯到历史 audit-finding-ID（如属已知族）或标记"新发现"
- [ ] 与 I1 初始 red list 快照比对：命中集一致则注明；不一致则按上述优先级归因（漂移→记录 commit；非漂移→深查门禁缺陷）
- [ ] 按"族"分组 red list（4 族 + 可能的新族），便于 Phase 3 裁决与 I4 类别清扫

Exit Criteria:

- [ ] `formal-red-list.md` 存在，含每条门禁的命中分节，每条命中可被独立复跑定位
- [ ] 命中集与 I1 初始快照一致性已核对（一致则注明；不一致则查明并记录）
- [ ] 每条命中标注族归属与"已知/新发现"
- [ ] **无静默跳过**：red list 不得人为剔除命中；全部原样记录，裁决归 Phase 2
...
- [ ] 比对 I0 目标集与 live code：是否有 I0 漏扫的 processor/bizmodel 方法？（漏扫本身就是 I0 gap，记录并补入目标集）
- [ ] 对每个族的"门禁未覆盖子模式"做手动核查（如 silent-swallow 门禁若只查 `getMessage()` 模式，手动核查"catch 后 log.warn 后继续"是否也属吞异常）
- [ ] 跨模块调用链：追踪 service→core→dao 路径上是否有"上层 catch 后未传播 ErrorCode 到边界"的实例
- [ ] 新发现并入 `formal-red-list.md`（标注"对抗探查新增"）

Exit Criteria:

- [ ] `adversarial-probing-notes.md` 存在，记录每个盲区探查方向、方法、结论（命中 or 无命中）
- [ ] 探查新增项已并入 `formal-red-list.md` 并标注来源
- [ ] I0 目标集漏扫（如有）已记录，并产出"补扫目标集"的明确清单（反馈给 I0 或在 I4 前补齐）
- [ ] **无静默跳过**：探查方向不得因"看起来没问题"就跳过；每个方向必须有明确结论（命中/无命中 + 依据）
- [ ] No owner-doc update required
- [ ] `ai-dev/logs/` 对应日期条目已更新
...
- [ ] 逐条裁决 formal red list 每条命中：`P0` / `P1` / `新族→Cycle2` / `deferred(non-blocking理由)`，禁止"待定"
- [ ] 已确认的 live defect / contract drift 一律归 P0/P1，**不得**降级为 deferred/follow-up（Anti-Slacking + Non-Degradable Items）
- [ ] 对每族产出 I4 工作项描述：实例清单 + **具体可执行的类别清扫 grep 命令**（如 `rg 'catch\s*\(' nop-metadata/nop-metadata-service/src/main/java --type java`，而非"grep 所有 processor"式泛指）+ test-first 要求 + 门禁复跑零命中验收
- [ ] 对新族（若有）登记触发证据（先例链 PD-n 编号），派发 Cycle 2 / I1（Loop Rule 预授权，不需步骤 4 审查，但登记留痕）
- [ ] 裁决表零悬挂自检：每条 red-list 条目都有非"待定"的归属

Exit Criteria:

- [ ] `adjudication-table.md` 存在，formal red list 的每条命中都有明确裁决（零"待定"）
- [ ] 每族 I4 工作项描述含：实例清单 + 类别清扫 grep 范围 + test-first 要求 + 门禁复跑零命中验收点
- [ ] 无已确认 live defect 被降级为 deferred/follow-up（逐条可核）
- [ ] 新族（若有）已登记触发证据并派发 Cycle 2/I1
- [ ] **端到端验证**：从"门禁命中"到"裁决归属"到"I4 工作项可执行描述"链条完整，无断点
- [ ] **无静默跳过**：裁决不得用"暂不处理"等模糊词；每条必须落到 P0/P1/新族/deferred-with-reason 之一
- [ ] No owner-doc update required
- [ ] `ai-dev/logs/` 对应日期条目已更新
...
- [ ] formal red list（含对抗探查新增）完整、可复跑、与 I1 快照一致性已核对
- [ ] 裁决表零悬挂（每条命中有非"待定"归属）
- [ ] 每族 I4 工作项描述含：实例清单 + **具体可执行的类别清扫 grep 命令**（非泛指）+ test-first 要求 + 门禁零命中验收点
- [ ] 无已确认 live defect / contract drift 被降级为 deferred/follow-up
- [ ] 新族（若有）已按 Loop Rule 派发并留痕
- [ ] 受影响 owner docs：No owner-doc update required（裁决为过程产物；owner-doc 同步留待 I4 修复落地后）
- [ ] 独立子 agent / 独立审阅者 closure-audit 已完成并记录证据
- [ ] **Anti-Hollow Check**：closure audit 验证（a）red list 每条命中在 live code 可定位；（b）裁决非"全部 P1 了事"式敷衍（抽查 ≥3 条裁决理由可核）；（c）对抗探查每个方向有实质结论而非"未发现问题"空话
- [ ] `node ai-dev/tools/check-doc-links.mjs --strict` 退出码 0（产出含链接的 .md）
