...
- [x] nop-biz.md（P1×2 P2×6 P3×7）— 2026-08-22 完成：14 修复 + 1 部分修复暂缓（P2-6 截断语义决策，warn 日志已落地）。P1: CacheActionDecorator 命中返回缓存值+回源写缓存（含异步 thenApply/null 不入缓存）/copyForNew 逻辑删除实体误抛 ALREADY_EXISTS（buildEntityDataForSave 增 recoverDeleted 重载，copyForNew 传 false）。P2: doCopyForNew 改带 context/delayedActions 的 copier+executeDelayedRelationActions（BIZ 关联不再静默丢弃）/OrmEntityCopier objMeta null 租户防护/doFindTreeEntityList 数据权限 action 归位 findTreeList/batchUpdate null 元素防御（skip 或 UnknownEntityException）/多对三 mutation 补 checkDataAuth(METHOD_UPDATE)/updateByQuery·deleteByQuery·asDict 截断 WARN（语义决策暂缓）。P3: DevStat.clearStats 改 @BizMutation/validator 双执行改 else if/method_invoke 裸 IAE 改 ERR_BIZ_ACTION_NO_SVC_CONTEXT（两参场景语义修正为缺上下文）/isAllowGetDeleted 判空/batchDelete 缺失 id 回填原始入参（返回集无 null）/DownloadHelper os 提升方法级 catch 关闭（抽包内重载供注入）/resolveQuery 死代码删除。新增 5 测试类 + 3 既有类扩展共 32 用例，红验证 17 处失败形态与审计一致（含 P2-1 分阶段红：仅 P1-2 补丁下单参 copier 仍 0 invocations）；48 tests 绿 + 回归 nop-sys-service 7 / nop-auth-service 393 / nop-wf 8+3 绿；报告 15 条全部标注（grep 15=15）
- [x] nop-graphql.md（P1×3 P2×5 P3×7；统计表 P3 初记 6 系笔误已按发现列表修正）— 2026-08-23 完成：12 修复 + 2 暂缓 + 1 部分修复（P1-3 两子项修复+autoconfig 注册暂缓）。P1: BatchFetcher 非 source 参数纳入 loader 注册名（无参保持原名兼容）/WebSocket 订阅 onComplete/onError 补双参 remove+sendStreamingMessage put 先于 subscribe（红测试发现审计建议的单点 remove 不足：同步 publisher 清理先于注册空跑）/subscription beans 删不存在 property+start() @PostConstruct+装配守卫测试（autoconfig 注册暂缓：GraphQLSubscriptionManager 生产零调用方属未接线原型，激活无收益需先设计接线）。P2: fragment 深度绕过改引用嵌套层数≤maxDepth 语义（初版展开总深度语义被标准 IntrospectionQuery 夹具证伪会拒绝所有标准客户端）+循环引用解析期拒绝/JSON-RPC batch 单 entry 失败隔离/notification WARN/mutation 串行链/两处 DataLoader check-then-act 与 dispatchAll 同锁。P3: bare IAE/ISE 6 处错误码化（7 新错误码 i18n 同步）/批量上限 off-by-one/空批 List 类型+wire format 单对象/gRPC 反射配置化默认关+迁移 V1（非默认构建模块单独 compile 验证）/日志 logIfNotTraced 对齐/inline fragment 显式错误码；P3-1 all-or-nothing 错误语义暂缓（平台级契约变更需设计决策）。新增 5 测试类+3 既有类扩展；七轮单文件 stash 红验证失败形态与审计一致（参数丢弃 2≠1/4409 复用断连/链式静默放行/ClassCastException/批量 400/events=[start1,start2]/duplicated-loader/inline 无异常）；core 101 tests 绿+orm/message 构建绿+grpc 编译绿+回归 nop-auth-service 393 绿；报告 15 条全部标注（15=15）
- [x] nop-auth.md（P1×1 P2×3 P3×9）— 2026-08-23 完成：12 修复 + 1 部分修复（P3-3 Map 无界增长已修 Caffeine asMap 硬上限+2天过期，XFF 伪造裁定不修复——需部署代理链知识无法平台内置，伪造键内存面已封顶且仅影响 IP 维度）。P1: NopAuthRoleDataAuthBizModel.afterEntityChange 清 DataAuthModel 缓存+clearCache null 防御（check-changed/timeout 不做：即时失效语义更强）。P2: updateRoleResources+NopAuthRoleResourceBizModel 双路径刷 sitemap 缓存/敏感令牌列 tagSet（session 两列+oauth 六列 masked,not-pub、clientSecret masked,var,not-pub）经 codegen 再生成 xmeta（published=false）与 OutputBean（字段移除，grep 无消费方）/resetUserPassword·enableUser·disableUser 补 requireAdmin。P3: 审计 requestData 合并（红验证 {"failCount":3} 逐字吻合）/四处限流间隔检查+lastSendMs 占用原子化（竞态窗口窄无法稳定红复现，如实注明）/ILoginService.revokeUserSessionsAsync+改密吊销其他会话·重置吊销全部/bindSms 前置 checkProofRateLimit/SSO refreshToken 日志摘要化/JWK kty 判空+四处 bare RuntimeException 转错误码（红验证 NPE/RuntimeException 逐字吻合）/MfaFactorVerifier jdbcTemplate null 实体写退化/匿名 getSiteMap 按空角色集过滤。新增 6 测试类 20 用例；auth-service 406 tests 绿+biz-auth-core/meta×2/sso/oauth-service 回归绿+下游 4 模块编译绿；报告 13 条全部标注（13=13，P0 既有标注保留）
