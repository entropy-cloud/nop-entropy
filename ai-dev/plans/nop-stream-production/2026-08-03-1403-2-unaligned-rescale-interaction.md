...
- [ ] **(D1) rescale + channel state 首版策略**，裁定并记录到 `checkpoint-design.md` §2.11 新增子章节：
...
- [ ] **(D3) `InflightDataRescalingDescriptor` future-work 设计探索**（非阻塞，记录 open questions）：
...
- [ ] `checkpoint-design.md` §2.11 新增 rescale 子章节含 D1-D2 全部裁定 + D3 设计探索 + 拒绝的替代方案及原因。
- [ ] D1 裁定明确首版策略（预期 fail-fast 拒绝）。
- [ ] D2 裁定明确 live defect 修复位置（不依赖 instanceof 静默跳过）。
- [ ] D3 记录了设计方向 + open questions（明确标注为 future-work 探索，非可消费契约）。
- [ ] `ai-dev/logs/` 对应日期条目已更新。
- [ ] No new test required: 纯决策 Phase，无代码变更（guide #25）。
...
- [ ] **新增 error code**：在 `NopStreamErrors.java` 新增 `ERR_STREAM_CHANNEL_STATE_RESCALE_UNSUPPORTED`（描述：rescale 恢复路径检测到 channel state 存在，无法跨并行度重映射 in-flight data）。
- [ ] **修复静默丢弃 live defect**：依 Phase 1 D2 裁定的修复位置，使 rescale 恢复路径在源 checkpoint 含 channel state 时 fail-fast 抛出 `ERR_STREAM_CHANNEL_STATE_RESCALE_UNSUPPORTED`，不依赖 `instanceof TaskEpochSnapshot` 静默跳过。具体行为：rescale 检测（`GraphModelCheckpointExecutor.java:975` `boolean rescale`）为 true 时，检查源 `TaskEpochSnapshot.getChannelState()` 非空 → 抛异常。
- [ ] **非 rescale 路径无回归**：同并行度恢复路径（`stateLookup.lookup()` 返回原始 `TaskEpochSnapshot`）行为不变，channel state 正常恢复。

Exit Criteria:

- [ ] **行为验证**：存在测试证明 rescale 恢复路径在 channel state 存在时**不再静默丢弃**——fail-fast 抛 `ERR_STREAM_CHANNEL_STATE_RESCALE_UNSUPPORTED`。
- [ ] **无静默跳过**（#24）：修复后的路径在 channel state rescale 不支持时显式失败（抛异常 with error code），不返回 null/空/静默 continue。
- [ ] **新功能测试**（#25）：新增覆盖以下场景的 focused tests：
...
- [ ] **Anti-Hollow**：fail-fast 路径在运行时确实被触发（非仅类型存在）。
- [ ] `./mvnw test -pl nop-stream/nop-stream-runtime -am` 通过。
- [ ] owner-doc：`checkpoint-design.md` §2.11 rescale 子节已更新（Phase 1 裁定 + Phase 2 实现）。
- [ ] `ai-dev/logs/` 对应日期条目已更新。
...
- [ ] **D4 fail-fast guard focused test**：为 `InputGate.java:464-471`（Stage 45 留下的 unaligned + multi-in-flight guard）补充直接断言测试——unaligned enabled + 2+ in-flight barriers → 抛 `ERR_STREAM_INVALID_STATE`（非静默行为）。
- [ ] **端到端测试：unaligned checkpoint + 同并行度恢复**：unaligned checkpoint 后同并行度恢复，channel state 正确 replay，exactly-once 保持（无丢数/无重复）。此测试验证现有功能无回归。
- [ ] **端到端测试：unaligned checkpoint + rescale 恢复**：unaligned checkpoint 后 rescale 恢复，验证 fail-fast 正确触发（抛 `ERR_STREAM_CHANNEL_STATE_RESCALE_UNSUPPORTED`），而非静默丢弃 channel state。
- [ ] **端到端测试：aligned checkpoint + rescale 恢复**（无回归基线）：aligned checkpoint（无 channel state）后 rescale 恢复正常工作（Stage 35 keyed state rescale 不受影响）。

Exit Criteria:

- [ ] **端到端验证**（#22）：从 `StreamExecutionEnvironment` 到 sink 的 unaligned checkpoint + rescale 恢复完整路径已验证（fail-fast 正确触发，exactly-once 不被静默破坏）。
- [ ] **接线验证**（#23）：`buildRescaledTaskState` / rescale 检测点修复路径在运行时确实被调用（E2E 或 mock verify）。
- [ ] **新功能测试**（#25）：显式列出每个新增 E2E / focused test 验证的行为。
- [ ] D4 guard 有直接断言测试（Stage 45 Follow-up 收口）。
- [ ] `./mvnw test -pl nop-stream -am -T 1C` 通过。
- [ ] `checkpoint-design.md` §2.11 unaligned + rescale 交互小节为最终状态。
- [ ] `ai-dev/logs/` 对应日期条目已更新。

## Closure Gates

- [ ] Channel state rescale 静默丢弃 live defect 已修复（显式 fail-fast，非 instanceof 静默跳过）。
- [ ] `ERR_STREAM_CHANNEL_STATE_RESCALE_UNSUPPORTED` error code 已新增并在 fail-fast 路径使用。
- [ ] 首版 rescale + unaligned 策略已裁定并实现（fail-fast 拒绝）。
- [ ] D4 fail-fast guard 有直接断言测试（Stage 45 Follow-up 收口）。
- [ ] unaligned checkpoint + rescale 恢复端到端验证通过（fail-fast 正确触发，exactly-once 不被静默破坏）。
- [ ] 不存在被静默降级到 deferred 的 in-scope live defect（channel state 静默丢弃已修复，非降级）。
- [ ] 受影响 owner docs 已同步（`checkpoint-design.md` §2.11 rescale 子节）。
- [ ] 独立子 agent / 独立审阅者 closure-audit 已完成并记录证据。
- [ ] **Anti-Hollow Check**：closure audit 已验证（a）fail-fast 路径在运行时被调用（E2E 覆盖），（b）无 instanceof 静默跳过残留。
- [ ] `./mvnw test -pl nop-stream -am -T 1C` 通过。
- [ ] checkstyle / 代码规范检查通过。
