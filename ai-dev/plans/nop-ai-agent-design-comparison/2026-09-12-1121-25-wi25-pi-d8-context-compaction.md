...
# WI25 pi-D8 上下文工程与压缩 对比

## Current Baseline

- 前置依赖：deps = WI3；契约与材料齐备——00-dimension-matrix.md（D8 子机制拆解 + 锚点候选 + 6 节模板 + 5 值裁定）、02-terminology-map.md（T 族翻译口径 + 伪差异警示）、03-flow-agent-loop.md（§2.3 pi 调用链权威源）、04-extension-capability-matrix.md（③3.3 pi 能力）、05-extension-ordering.md（pi 加载序×注册序）、06-extension-composition.md（pi 槽位-事件桥接与恢复循环）
- pi 侧机制事实来自 WI3-WI7 当日调研（HEAD c49906ec7 实测）；执行时确认 pi HEAD 未漂移，若漂移则重钉
- 本报告为 pi-D8 维度对比：6 节模板，专项主题引用 03-06 只写对比增量；M2 同维报告（dsh-D8）已产出，本报告与其共享 ② 节 nop 侧事实基线
...
- 产出 Deliverable：ai-dev/analysis/compare-agent-design/pi-D8-context-compaction.md
...
- 不覆盖 dsh 侧（dsh-D8 已由 M2 产出）

## Phase 1 — pi-D8 维度报告产出

Status: completed

Targets: ai-dev/analysis/compare-agent-design/pi-D8-context-compaction.md（新建文件）

- Item Types: `Proof | Decision | Follow-up`

- [x] `Proof` 复核 pi 侧 D8 相关机制锚点（沿用 WI3-WI7 调研，抽查关键锚点行号）；核对 nop 侧锚点与 Owner doc 声明
- [x] `Decision` 按 6 节模板撰写：① 结论摘要 ≤10 行 ② nop 侧机制与锚点（与 dsh-D8 报告共享 nop 事实基线，按本维子机制裁剪）③ pi 侧机制与锚点（专项主题引专项结论+只写增量）④ 子机制逐项对照表（D8-1 token 预算与压力检测/D8-2 触发条件/D8-3 策略分层/D8-4 spill 与引用式保真/D8-5 产物形态 每行 5 值裁定+证据）⑤ 语义差异与取舍（按 02 术语表翻译，含"双方均无"裁定如适用）⑥ 维度总裁定 + 可吸收增量建议
...
- [x] ai-dev/analysis/compare-agent-design/pi-D8-context-compaction.md 存在，6 节模板无缺节（① ≤10 行），头部记录三方 HEAD 与分析日期
