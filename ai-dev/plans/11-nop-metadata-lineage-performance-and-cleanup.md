...
- [x] In each of the 3 upsert methods, replace per-candidate SELECT with a single batch query for all existing edges by the key 5-tuple `(sourceTableId, targetTableId, lineageSource)`
- [x] Batch-save new edges (one INSERT with multiple rows, or batched saves within the same transaction)
- [x] Ensure existing edges are updated in batch if possible, or at most 1 UPDATE per matched edge
- [x] Verify `extractColumnLineageFromSql`, `extractLineageFromSql`, `extractMeasureLineage` callers are consistent with new batch pattern

Exit Criteria:

> 每个 Phase 完成后，必须逐条勾选本节。所有 `[x]` 后才能将 Phase Status 改为 `completed`。

- [x] Each upsert method issues at most 1 SELECT batch query (not 1 per candidate)
- [x] New edges are batch-saved within the same transaction
- [x] Old per-candidate SELECT/INSERT/UPDATE loop pattern is removed
...
- [x] Delete `NopMetadataConstants.java`
- [x] Grep for any imports of `NopMetadataConstants` and remove them

Exit Criteria:

> 每个 Phase 完成后，必须逐条勾选本节。所有 `[x]` 后才能将 Phase Status 改为 `completed`。

- [x] `NopMetadataConstants.java` no longer exists
- [x] No remaining imports reference `io.nop.metadata.service.NopMetadataConstants`
...
- [x] Lineage edge extraction no longer produces N+1 SELECT/INSERT per candidate
- [x] Empty `NopMetadataConstants` interface removed with no remaining references
...
Status Note: All phases completed. N+1 upsert pattern replaced with batch operations. Dead NopMetadataConstants removed.
Completed: 2026-07-22

Closure Audit Evidence:

- Reviewer / Agent: opencode (mission-driver)
- Evidence: compile + test green; batchLoadExistingSqlParseSourceIds/batchLoadExistingColumnParseEdgeMap replace per-candidate SELECT; batchSaveEntities for new edges; deleteMeasureParseEdges + batchSaveEntities for measure lineage
