...
- [ ] Add `NopMetadataException(String message)` and `NopMetadataException(String message, Throwable cause)` constructors.
- [ ] Decide: rename all ~80 ErrorCodes to dot convention OR update `NopMetadataErrors.java` javadoc to document hyphen convention as accepted deviation.
- [ ] If rename: bulk-rename all `nop.err.metadata.XXX-YYY` to `nop.err.metadata.XXX.YYY` in all ErrorCode definition files.
- [ ] If rename: update all throw sites and tests referencing renamed ErrorCodes.
- [ ] Replace 6 bare `NopException` throws in `NopMetaDataContractBizModel` with `NopMetadataException`.
- [ ] Replace 3 bare `NopException` throws in `MetaTableFieldResolver` with `NopMetadataException`.
- [ ] Replace `MetaQualityRuleExecutor` bare `NopException` + FQN class paths with proper imports and `NopMetadataException`.
- [ ] Replace 1 bare `NopException` in `NopMetaTagLabelBizModel` with `NopMetadataException`.
- [ ] Replace 3 bare `NopException` throws in `LineageTagPropagationService`/`AutoClassificationService` with `NopMetadataException`.
- [ ] Make `NopMetadataArgs` public (currently package-private) or move ARG_* constants to `NopMetadataErrors` (public) to allow cross-package `.param(ARG_*, value)` usage. Replace inline `.param("key", value)` with `ARG_*` constants across 8+ files.
- [ ] Add `LOG.trace()` or `LOG.debug()` to the SQLException catch block in `MetaTableProfiler`.

Exit Criteria:

- [ ] `NopMetadataException` has working `(String)` and `(String, Throwable)` constructors — confirmed by compilation.
- [ ] All ErrorCodes follow consistent hyphen or dot pattern — confirmed by grep on `ErrorCode.define(`.
- [ ] No bare `NopException` throws in nop-metadata-service — confirmed by grep for `new NopException(` (excluding framework code).
- [ ] No inline `.param("literal", ...)` found in throw sites — confirmed by grep in changed files.
- [ ] `NopMetadataArgs` is accessible from sub-packages (either made public or constants moved to `NopMetadataErrors`).
- [ ] Empty catch of SQLException now has at least trace logging.
...
- [ ] Decide: uncomment `gen-crud-api.xgen` or add a build-time JUnit test that verifies every entity has a corresponding `*BizModel.java`.
- [ ] If decision is build-time test: implement `TestAllEntitiesHaveBizModels.java` that scans entities and BizModel classes.
- [ ] Add codegen plugin reference comment to `nop-metadata-dao/pom.xml` (not functional change, just documentation).
- [ ] For 38/42 empty retention xmeta files: identify entities with sensitive fields and add explicit `published`/`insertable`/`updatable` overrides. At minimum document the gap.
- [ ] Verify `nop-metadata-service` → `nop-metadata-core` dependency chain exists (needed for `_NopMetadataCoreConstants` import). Then replace `STATUS_MANUAL` duplicate with `_NopMetadataCoreConstants.RECONCILIATION_STATUS_MANUAL`.
- [ ] Remove redundant `@GraphQLReturn(bizObjName = "NopMetaGlossaryTerm")` from `update()` method.

Exit Criteria:

- [ ] CRUD API coverage is ensured (either codegen enabled or build-time test exists).
- [ ] `nop-metadata-dao/pom.xml` documents codegen expectations.
- [ ] Empty xmeta files have documented assessment or explicit overrides.
- [ ] No duplicate `STATUS_MANUAL` constant — confirmed by grep.
- [ ] No redundant `@GraphQLReturn` annotations on methods where return type is unambiguous.
- [ ] `./mvnw compile -pl nop-metadata-service -am` passes.
- [ ] `./mvnw test -pl nop-metadata-service -am` passes (or new build-time test passes).
- [ ] `No owner-doc update required`.
- [ ] `ai-dev/logs/` 对应日期条目已更新
...
- [ ] Add `JunitAutoTestCase` + `@EnableSnapshot` to 2-3 key BizModel integration test classes (e.g., DataContract, QualityScore, DataSource).
- [ ] Remove or consolidate repetitive CRUD tests across entities; keep 1-2 representative entity CRUD tests.
- [ ] Add focused tests for 4 uncovered BizModel methods: `judgeByRuleId`, `activateContract`, `deprecateContract`, `retireContract`.
- [ ] Restructure `TestCheckpointActionDispatcherConcurrency` to either share mutable state across threads or document the test as stateless-parallelism-only.
- [ ] Data auth test (`TestDataAuthRowLevelScoping`): class javadoc already documents cross-reference to framework-level nop-auth tests. Acknowledge as sufficient — no additional work unless runtime enforcement gap is identified in audit.
- [ ] Replace `Thread.sleep(1100)` with explicit timestamp manipulation (e.g., `Clock.fixed()` or settable time provider).
- [ ] Remove 4 pure getter/setter tests from `TestCheckpointExtConfigDataBean`.
- [ ] Remove `assertNotNull` precondition guards that only improve error messages.
- [ ] Evaluate snapshot feasibility for aggregation tests; add `@EnableSnapshot` if deterministic.
- [ ] Split the 4 largest test files (TestNopMetaLineageEdgeBizModel ~1300 lines, TestNopMetaQualityCheckpointBizModel ~795, TestNopMetaDataSourceBizModel ~843, TestNopMetaQualityRuleBizModel ~606) into domain-focused test classes.

Exit Criteria:

- [ ] At least 2 BizModel integration tests use AutoTest snapshots.
- [ ] Repetitive CRUD tests reduced — confirmed by test file review.
- [ ] All 4 previously uncovered BizModel methods have focused tests — confirmed by grep for test method names.
- [ ] Concurrent test either tests shared mutable state or documents its stateless scope.
- [ ] Data auth test gap is acknowledged as intentionally documented cross-reference — no change needed beyond confirmation.
- [ ] No `Thread.sleep(...)` in test code — confirmed by grep.
- [ ] No pure getter/setter round-trip tests on `@DataBean` — confirmed by file review.
- [ ] No `assertNotNull` precondition-only assertions.
- [ ] No test file exceeds 600 lines (after splitting) — confirmed by `wc -l` on test files.
- [ ] `./mvnw test -pl nop-metadata-service -am` passes (including new/modified tests).
- [ ] **无静默跳过**: No empty catch blocks in test code.
- [ ] `No owner-doc update required` — test-only changes.
- [ ] `ai-dev/logs/` 对应日期条目已更新
...
- [ ] `NopMetadataException` has String constructors.
- [ ] All ErrorCodes follow consistent naming (hyphen or dot).
- [ ] No bare `NopException` throws in nop-metadata-service.
- [ ] All `.param()` calls use `ARG_*` constants.
- [ ] No silent SQLException catches without logging.
- [ ] CRUD API coverage is ensured by codegen or build-time verification.
- [ ] 4 untested BizModel methods have regression tests.
- [ ] Test anti-patterns fixed (Thread.sleep, getter/setter tests, repetitive CRUD, precondition guards).
- [ ] `./mvnw compile -pl nop-metadata-service -am` passes.
- [ ] `./mvnw test -pl nop-metadata-service -am` passes.
- [ ] 独立子 agent closure-audit 已完成并记录证据。
- [ ] **Anti-Hollow Check**: AutoTest snapshots actually capture response data; new tests actually execute the target methods (not just method stubs).
- [ ] **No Silent No-Op**: No test marked `@Disabled` or empty test methods.
