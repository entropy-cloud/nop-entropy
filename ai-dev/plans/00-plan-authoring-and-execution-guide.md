...
### Before Drafting: Design Readiness Check

**写 plan 之前，必须先验证设计文档是否足够支撑实现。** 历史（nop-stream checkpoint）表明：设计文档描述了"要什么"但没有描述"怎么接"，导致实现者造出空壳——组件存在、单测通过，但系统级功能不可用。

对于涉及**新组件、新接口、或跨模块集成**的计划，写 plan 之前必须用独立子 agent 做一轮 **对抗性设计审查（Adversarial Design Review）**。

#### 核心方法：想象实现，而非枚举检查项

**不要用预设的检查清单逐条打勾。** 清单只能发现已知类别的问题。正确的方法是：让子 agent 扮演一个有经验的实现者，**想象自己正在按设计文档编码**，然后找出所有"读到这段设计时，我不知道该怎么写"的地方。

子 agent 的 prompt 应包含以下指令：

```
你是一个有经验的分布式系统工程师。现在给你一份设计文档和相关的现有代码。
你的任务是：想象你要按这份设计文档实现这个功能。

请逐段阅读设计文档，对每一段问自己：
1. 如果我现在就要为这段设计写代码，我知道具体写哪个类的哪个方法吗？
2. 我知道这个组件与已有代码的接线方式吗（构造器注入？方法回调？配置文件？）？
3. 如果出了问题（IO失败、超时、并发冲突），我知道该怎么处理吗？
4. 设计中有没有隐含假设（单线程、单实例、某种初始化顺序）没有显式声明？
5. 有没有描述了概念但缺少具体规格的地方（"某个机制"但没说哪个类、什么接口）？

输出格式：
- 对每个发现的问题，给出：具体位置（哪份文档的哪个章节）、问题描述、
  如果不补会导致什么错误的实现、严重程度（Blocker/Major/Minor）

最后给出总结：当前设计是否足够支撑实现？如果不够，哪些必须先补？
```

#### 操作方式

1. 用独立子 agent（不同 task_id）执行对抗性审查，输入设计文档 + 现有代码
2. 子 agent 输出的 Blocker 级问题：**必须在写 plan 之前补充设计文档**
3. Major 级问题：在 plan 的 `Current Baseline` 中记录为 **Design Gap**，可以安排在 plan 的 Phase 中补充
4. 补充设计后，重新执行对抗性审查，直到没有 Blocker

#### 已知的高频盲区（启发用，不是检查清单）

以下是历史审查中反复出现的设计盲区，供子 agent 参考但不限于这些：
- 端到端调用链的完整路径（不只是"A调用B"，还要具体方法签名）
- 线程模型和并发安全契约
- 新组件与已有代码的集成 API（构造器参数、注入方式、回调接口）
- 错误恢复策略（每个失败点的处理方式）
- 生命周期时序（创建、初始化、恢复、销毁的顺序）
- 未显式声明的隐含假设
