...
对于涉及**新组件、新接口、或跨模块集成**的计划，写 plan 之前必须用独立子 agent 做一轮 **Design Readiness Check**，回答以下问题。任何一项回答"否"的，必须先补充设计文档，再写 plan。

#### Design Readiness Checklist

1. **端到端可追踪性**：从用户入口点到最终输出，每一步的完整调用链是否在设计文档中明确指定？（不只是"组件 A 调用组件 B"——还要指定通过哪个方法、什么参数、返回什么。）

2. **线程模型**：每个组件在哪个线程上运行？访问哪些共享状态？需要什么同步措施？如果设计引入了异步或并发交互，是否指定了线程安全契约？

3. **集成 API**：新组件与已有组件之间的接口——构造器参数、方法签名、回调机制——是否具体到可以直接编码？还是只描述了概念？

4. **错误处理**：每个可能失败的步骤（IO 错误、超时、数据损坏），设计文档是否指定了恢复策略？（重试？降级？中止？回滚？）

5. **生命周期时序**：组件的创建、初始化、运行、销毁的顺序是否明确？特别是恢复场景——先恢复再初始化，还是先初始化再恢复？

6. **无隐含假设**：设计是否假设了某些前提条件（如"单线程"、"单链"、"parallelism=1"）但没有显式声明？这些假设是否在目标范围内成立？

#### 操作方式

- 用独立子 agent 读设计文档和现有代码，逐条检查上述 6 项
- 对每项给出：✅ 充分 / ⚠️ 部分缺失（列出具体缺口）/ ❌ 缺失
- ⚠️ 或 ❌ 的项：在 plan 的 `Current Baseline` 中记录为 **Design Gap**，并在写 plan 之前先补充设计文档
- 补充后，重新检查受影响的项，直到全部 ✅ 或 ⚠️（⚠️ 的项必须在 plan 的 Phase 中安排补充设计的工作）
