...
- [ ] `InvokeProcessor.invokeUrl()` 在收到 429 时解析 `Retry-After` + 退避重试（最多 3 次）
- [ ] 超过重试上限后抛出 `NopException` 携带原始 HTTP 状态码和响应体
- [ ] `AiBackendType` 包含 `DEEPSEEK` / `GEMINI`
- [ ] **无静默跳过**：5xx 错误不静默忽略，抛异常供上层捕获
- [ ] `./mvnw compile -pl nop-service-framework/nop-gateway -am` 通过
...
- [ ] 三个拦截器类存在并实现 `IGatewayInterceptor`
- [ ] **端到端验证**：
  - Failover：mock 首个 URL 返回 503，验证 fallback URL 被调用且返回正确结果
  - Auth：有效 key → 200，无效 key → 401
  - Rate-limit：超限请求 → 429
- [ ] **接线验证**：`gateway-defaults.beans.xml` 中注册三个 bean，测试中通过 IoC 加载并按序执行
- [ ] **无静默跳过**：流式 failover 抛 `UnsupportedOperationException`；认证失败不 fallthrough
- [ ] `./mvnw compile -pl nop-service-framework/nop-gateway -am` 通过
- [ ] `ai-dev/logs/` 对应日期条目已更新
...
- [ ] `DeepSeekMessageConverter` 实现 `IBackendMessageConverter`，通过 IoC 加载
- [ ] `GeminiMessageConverter` 实现 `IBackendMessageConverter`，通过 IoC 加载
- [ ] **端到端验证**：`AiBackendMessageConverter.toBackendRequest(req, DEEPSEEK)` 返回非 null 结果
- [ ] `./mvnw compile -pl nop-service-framework/nop-gateway -am` 通过
- [ ] `ai-dev/logs/` 对应日期条目已更新
...
- [ ] `AiFailoverGatewayInterceptorTest` — mock `IHttpClient` 验证 failover 链
- [ ] `AiRateLimitGatewayInterceptorTest` — 验证桶满后请求被限流、正常请求通过
- [ ] `AiAuthGatewayInterceptorTest` — 验证有效 key 通过、无效 key 拒绝
- [ ] `InvokeProcessorRetryTest` — 验证 429 Retry-After 退避
- [ ] `DeepSeekMessageConverterTest` — 验证 OpenAI ↔ DeepSeek 格式转换
- [ ] `GeminiMessageConverterTest` — 验证 Gemini ↔ OpenAI 格式转换
- [ ] `./mvnw clean test -pl nop-service-framework/nop-gateway -am` 全部通过
...
- [ ] 6 个测试类全部编写并提交
- [ ] 每个测试至少有一个断言验证正确的语义行为
- [ ] `./mvnw test -pl nop-service-framework/nop-gateway -am` 通过
- [ ] `ai-dev/logs/` 对应日期条目已更新
...
- [ ] Provider failover 在非流式场景下可用（primary 失败 → fallback 返回正确结果）
- [ ] API Key 认证 + 限流可组合使用
- [ ] Provider 429 退避生效
- [ ] DeepSeek + Gemini Provider 适配可用
- [ ] 不存在被静默降级到 deferred / follow-up 的 in-scope live defect
- [ ] 独立子 agent / 独立审阅者 closure-audit 已完成并记录证据
- [ ] **Anti-Hollow Check**：closure audit 已验证（a）组件间调用链在运行时确实连通（不只是类型系统），（b）无空方法体/静默跳过/no-op 作为正常实现
- [ ] `./mvnw compile -pl nop-service-framework/nop-gateway -am`
- [ ] `./mvnw test -pl nop-service-framework/nop-gateway -am`
...
- Evidence: <<task id / 每条 Exit Criterion 和 Closure Gate 验证结果>>

Follow-up:

- 本计划关闭后，后续 plan 应覆盖流式 failover + 分布式限流 + Token 计费
