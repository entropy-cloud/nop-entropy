...
- [ ] 在 `<entities>` 段新增 `NopMetaBusinessDomain` 实体：
...
- [ ] 在 `<dicts>` 段新增 `meta/business-domain-type` dict：`SourceAligned`/`ConsumerAligned`/`Aggregate`（使用 PascalCase 风格，与现有 dict option 值一致）
- [ ] 在 `<entities>` 段新增 `NopMetaDataProduct` 实体：
...
- [ ] 新增集成测试：BusinessDomain/DataProduct 的 GraphQL CRUD（增删改查，含 domain→product 嵌套创建）

Exit Criteria:

> 每个 Phase 完成后，必须逐条勾选本节。所有 `[x]` 后才能将 Phase Status 改为 `completed`。

- [ ] `NopMetaBusinessDomain` 实体在 ORM XML 中存在，含全部列（含 `i18n-en:displayName`）、UK（`(parentDomainId, name)`）、索引、relations
- [ ] `NopMetaDataProduct` 实体在 ORM XML 中存在，含全部列、UK、索引、relations（FK→NopMetaBusinessDomain）
- [ ] 5 个新增 dict 在 `<dicts>` 段中存在（1 个 business-domain-type + 4 个 data-product-*），option 值与设计文档一致
- [ ] `./mvnw compile -pl nop-metadata -am` 编译通过
- [ ] **端到端验证**：集成测试覆盖创建 BusinessDomain → 在其下创建 DataProduct 的完整路径
- [ ] **接线验证**：GraphQL 端点测试触发 CrudBizModel 返回正确结果
- [ ] **无静默跳过**：新增实体代码中无空方法体/continue/吞异常
- [ ] No owner-doc update required（设计变更已在 design doc 中规划）
...
- [ ] 在以下资产实体上追加 `businessDomainId` 字段（string32, FK→NopMetaBusinessDomain, nullable）：
...
- [ ] 为上述资产实体创建 `to-one` relation `businessDomain`（反向无 to-many，避免 N+1 查询）
- [ ] 为 `businessDomainId` 列创建普通索引（非 UK）：`IX_NOP_META_TABLE_DOMAIN` 等
- [ ] 运行 codegen 重新生成 DAO/BizModel
- [ ] 实现业务域继承机制（在资产 BizModel 的 pre-save hook 中）：
...
- [ ] 新增集成测试：验证业务域继承（EntityField 未设 businessDomainId 时继承自父 Entity，TableMeasure/TableDimension 未设时继承自父 Table；Table/Entity 自身显式赋值生效或 null）

Exit Criteria:

- [ ] 5 个资产实体均已追加 `businessDomainId` 列 + FK + 索引 + relation
- [ ] 业务域继承机制在 BizModel save hook 中实现（显式值 > 父层级继承 > null）
- [ ] **端到端验证**：从 GraphQL save 资产实体 → BizModel pre-save hook 触发业务域继承逻辑 → 持久化 businessDomainId 值的完整路径已验证
- [ ] 继承链路径完整（EntityField→Entity、Measure/Dimension→Table），代码链连通
- [ ] **接线验证**：BizModel.save 运行时确实调用了业务域继承逻辑
- [ ] **无静默跳过**：未提供 businessDomainId 且父层级也无值时静默写 null（非失败场景）；若 FK 指向不存在的 domainId 则 FK 约束自然报错
...
- [ ] 在 `NopMetaDataProductBizModel` 中新增 action `linkAsset(dataProductId, entityType, entityId)`：
...
- [ ] 新增 action `unlinkAsset(dataProductId, entityType, entityId)`：
  ...
  - 匹配数为 1 时删除并返回 true
- [ ] 新增 action `getLinkedAssets(dataProductId)`：
  - 查询 `(labelType=Automated, metadata="{\"dataProductId\":\"" + dataProductId + "\"}")` 的 TagLabel，返回资产列表
- [ ] 在 `NopMetadataErrors.java` 新增 linkAsset/DataProduct 专有 ErrorCode：
...
- [ ] `linkAsset` action 创建正确的 TagLabel（source=Classification, labelType=Automated, metadata 含 dataProductId）
- [ ] `unlinkAsset` action 正确删除匹配的 TagLabel
- [ ] `getLinkedAssets` action 正确按 dataProductId 查询
- [ ] 幂等键有效（重复 linkAsset 不创建重复 TagLabel）
- [ ] **端到端验证**：从 DataProduct 创建 → linkAsset → TagLabel 查询 → unlinkAsset → 清理 完整路径
- [ ] **接线验证**：linkAsset action 在运行时确实通过 CrudBizModel 调用 TagLabel DAO
- [ ] **无静默跳过**：linkAsset 失败路径（entityType 不存在、entityId 不存在）均显式抛异常
- [ ] `./mvnw test -pl nop-metadata -am` 通过
- [ ] `ai-dev/design/nop-metadata/11-enterprise-semantic-layer.md` 更新 Phase 3 为已实现，同步 BusinessDomain UK 改为 `(parentDomainId, name)`、`domainType` dict 值使用 PascalCase 等设计变更
- [ ] `ai-dev/design/nop-metadata/01-architecture-baseline.md` 更新核心对象清单（新增 2 实体）
- [ ] `ai-dev/design/nop-metadata/nop-metadata-roadmap.md` 将 S3 标记为 done
- [ ] `ai-dev/logs/` 对应日期条目已更新
...
- [ ] `NopMetaBusinessDomain` 实体在 ORM XML 中定义并 codegen 通过
- [ ] `NopMetaDataProduct` 实体在 ORM XML 中定义并 codegen 通过
- [ ] 5 个资产实体（Table/Entity/EntityField/TableMeasure/TableDimension）已追加 `businessDomainId` 列 + FK + 索引
- [ ] 业务域继承机制在 BizModel save hook 中实现
- [ ] DataProduct 资产关联（linkAsset/unlinkAsset/getLinkedAssets）已实现
- [ ] `./mvnw test -pl nop-metadata -am` 通过
- [ ] 不存在被静默降级到 deferred / follow-up 的 in-scope live defect
- [ ] `11-enterprise-semantic-layer.md` 更新 Phase 3 为已实现
- [ ] `01-architecture-baseline.md` 更新核心对象清单
- [ ] 独立子 agent closure-audit 已完成并记录证据
- [ ] **Anti-Hollow Check**：closure audit 已验证 linkAsset 在运行时实际调用 TagLabel DAO，业务域继承在 BizModel.save hook 中实际触发，无空壳组件
- [ ] `./mvnw compile -pl nop-metadata -am`
- [ ] `./mvnw test -pl nop-metadata -am`
- [ ] 代码规范检查通过（imports 分组：java.* → jakarta.* → third-party → io.nop.*；无 System.out/printStackTrace；无空方法体）
