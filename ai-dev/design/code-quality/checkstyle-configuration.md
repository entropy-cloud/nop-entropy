...
# PMD
# -Dpmd.failOnViolation=false: 独立 API 模块（无 parent 继承）使用默认 PMD 规则集，
#   会检出生成代码的 UnnecessaryImport 等 Code Style 问题。设为 false 避免阻断。
mvn pmd:check -Pqa -Dpmd.failOnViolation=false
...
# PMD（仅 nop-commons）
mvn pmd:check -pl nop-commons -Pqa -Dpmd.failOnViolation=false
