...
>
> **Phase 2 批次落地增注（2026-09-24，item 29 plan 2026-09-24-0330-1，live 以源码为准）**：首批 20 条的余量 9 条裁定为**落地 7 / 路由 2**——落地：quality 4（no-system-out＝any 接收方+方法名捕获形态；no-return-null＝裸 `return null` 语句；no-transactional-annotation＝独立 marker-annotation pattern（全方法形态 pattern 不可编译）；no-star-import＝双分支普通/static 星号导入）+ security 2（no-sensitive-literal＝关系式 string_literal+inside+full-match regex 约束（capture 含引号、sanitized 标记 negative lookahead；行级同现的已裁定 delta=AST 包含覆盖全参数位）+ xscript 通道门控日志接收方/param 方法名；no-hardcoded-crypto＝首参捕获 + full-match regex 钉字面量（变量 key 不命中））+ XNode 1（nop-orm-unique-key＝**xscript 通道**：pattern open-world 全命中 + attrValue 判 null/blank report——XML 路径 pattern/any 面无 attribute 缺失 spec，round-2 审查 N1 裁定）。路由：import-order→maintain-mjs（item 28 manifest 行 #8，序列属性非模式匹配）；xpl-escaping→deferred（XNode text 全等无 contains + XML 拒 constraints ⇒ faithful 不可落地，successor＝design 06 §3.3 xscript/AST 查询面）。全部带 RuleTester 套件；生产规则库 11→18。
...
>
> **Phase 3 批量落地增注（2026-09-24，item 35 plan 2026-09-24-1400-1，live 以源码为准）**：生产规则库 **18 → 48**（`TestProductionRuleCount` EXPECTED_IDS + size 双断言钉死），本节标题"首批 20 条"口径升级为：首批 20 中 17 条已进 48（16 条 item 11/23/29 落地 + query-limit-required 本批 INV-04 忠实移植——语句级锚三形态 any-of + `follows` + $Q 同节点再绑定 + $M inList 收窄；errorcode-param-consistency 维持 item 28 路由、import-order 维持 maintain-mjs 路由、xpl-escaping 维持 deferred），本批新增 30 条全带 RuleTester 套件（valid 1 + invalid 1–2 + `.expect` 钉消息/行号）：Nop 特有 2（query-limit-required、no-direct-datasource-inject）+ 反模式 8（double-brace-init/system-exit/print-stack-trace/empty-if-block/new-primitive-boxing/empty-sync-block/catch-npe/throw-in-finally）+ deep 档 7（metrics 3：cyclomatic>10/cognitive>15/npath>200；L3 dataflow 2：unused-local-variable/self-assigned-local；L4 semantic 2：no-proxy-hostile-method/no-nonslf4j-logger-call——suite 走 deep-suites/ + 显式 provider 构造，TestMetricsRuleSuites/TestL3RuleSuites/TestL4RuleSuites 三 family 类 + 发现数断言 11）+ 质量/安全迁移吸收 13（no-runtime-exec/no-class-forname/no-md5-digest/no-des-encryption/no-catch-throwable/negated-equals/no-finalize/loose-coupling-hashset/replace-hashtable/replace-vector/empty-while-body/for-loop-can-be-foreach/control-statement-braces）。执行期裁定回写规则头注：grammar 无 `empty_statement` 符号（no-empty-statement 候选池替换为 for-loop-can-be-foreach，for 的 body 为必选子节点故 pattern 带 `{ $$$BODY }`）；kind 名为 `object_creation_expression`；`inside` 关系算子 inner 仅接受 pattern（方法域收窄改 xscript `node.ancestor`）；negated-equals 与既有 throw-exception-base/no-raw-exception 字面子集去重后为独立形态。
> **item 36 批量落地增注（2026-09-24，plan 2026-09-24-2300-1，live 以源码为准）**：生产规则库 **48 → 62**（`TestProductionRuleCount` 双断言同步）——ESLint 移植 2（no-constant-condition=kind+xscript 字面量条件版，忠实 ESLint 默认语义、常量传播通道留 follow-up；no-self-compare=same-name 元变量）+ ErrorProne 正确性 6（equals-null［+PMD 双 credit］/self-comparison/self-equals/collection-size-nonnegative/throw-null/random-mod［接收者名 regex 收窄］）+ PMD 质量 6（use-collection-isempty/string-instantiation［xscript 面判定排除转换形态］/biginteger-instantiation/no-throw-npe/empty-finally-block/simplify-boolean-expression）。执行期裁定回写规则头注：if_statement 的 consequence 为必选字段（字面 pattern 靠 error-recovery 永不匹配——kind+condition 字段读形态）；any 双分支共享 top-level 约束在未绑定 capture 分支运行期抛错（改 $$$ 序列捕获 xscript 面）；finally_clause 无 body field（descendant('block') 定位）。同日 manifest 对账 tier 1: 4→33（design 06 §7.1 v2 增注）。
