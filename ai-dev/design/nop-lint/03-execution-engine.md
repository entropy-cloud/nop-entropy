...
> **预编译入口契约（2026-09-25，plan 10，live 以源码为准）**：`LintEngine.lintCompiled`（多种源形态重载：String/byte[]/LintTree × languageId/language）接受一次编译、多处复用的 `List<CompiledRule>`——每文件工作只剩 gate 重判 + 匹配 + 抑制尾。裁定：(a) **gate 逐文件重判**经 `gateForTokens` 与 per-model gate 共享同一 token 逻辑；`CompiledRule.rawRequires` 携带原始声明 tokens（含未知 token，XML precompiled 路径同）——未知 token 恒为 skipped-by-profile，绝不因 `resolveRequires` 丢弃而变 runnable；(b) **TypeQuerySupport 求值期注入**：per-file support 由 `RuleSetRunner.run` 9 参通道传入、经 `ConstraintContext` 到达 typeOf 约束——编译产物零 per-file 状态，guard-fail 语义逐字保留；(c) **坏规则时机前移**：预编译在 run 启动即发生，无法编译的规则在消费任何文件前失败（原形态在首个命中文件才抛）——显式 fail-closed 前移，TestCheckRunner 焦点测试钉死；(d) **单编译收敛**：复合 matcher 的 kind 观点在编译下降时结构化推导（`CompiledNode`），同一 pattern 不再编译两次；(e) legacy `lint(List<RuleDslModel>,…)` 入口保留为对照面（bench 与既有测试），其逐调用编译为已知记录形态。
