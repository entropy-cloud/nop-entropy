...
| 20 | check-orm-unique-key-constraint.mjs | 1 规则 / 2 条件（constraint=/columns= 非空） | XML 模型(orm)，**CI invariant-gate 接线** | **XNode 规则**（design 02 §3 明示"orm-unique-key 首批"） | item 21（done） | 规则 + fixtures + 全仓 orm 对照零 diff + run-nop-metadata-invariants.sh 步骤切换 | 脚本下线、sh 步骤改接 nop-lint | candidate |
| 21 | check-sensitive-literal-leak.mjs | 1 规则 / 2 模式（JDBC URL/内联 SQL） | Java 源码，**CI invariant-gate 接线** | 候选 pattern + regex constraint（item 22 已交付；`.param(`/logger 行级同现需 xscript 或 text 约束面裁定） | item 22（done） | 规则 + fixtures + 同语料命中集对照 + sh 步骤切换 | 脚本下线 | candidate |
...
| 24 | check-xpl-escaping.mjs | 1（8 种上下文判定） | VFS 前端资源(view/page.yaml/xpl/json) | 候选 XNode 规则（design 02 §7 已列入规则库 Phase 2 两条 Nop 特有之一）；当前恒 exit 0 无接线 | item 21（done） | 规则 + fixtures + 上下文判定面对照 | 脚本下线 | candidate |
