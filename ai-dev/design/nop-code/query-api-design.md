...
**状态**：**目标架构**（部分 API 已实现，xmeta 缺失导致 GraphQL schema 不完整）
...
| 符号查询（搜索/层级/注解/引用/死代码） | `NopCodeSymbol` | ✅ `nop_code_symbol` | 符号实体 |
| 文件查询（大纲/树/源码） | `NopCodeFile` | ✅ `nop_code_file` | 文件实体 |
| 继承关系查询 | `NopCodeInheritance` | ✅ `nop_code_inheritance` | 继承实体 |

> **注意**：依赖图查询（`getDeps` / `getReverseDeps` / `findCycles` / `getDepGraph`）虽然操作 `NopCodeFileDependency` 实体，但输入是 `indexId`，属于索引级操作，应归属 `NopCodeIndexBizModel`。`NopCodeFileDependencyBizModel` 只保留标准 CRUD。

**拒绝了什么**：
- 所有方法放 NopCodeIndex（之前的做法）→ 违反单一职责 → **已部分修正**：索引级操作归 NopCodeIndex，实体级查询归对应 BizModel
- 为调用关系单独建 `NopCodeCallBizModel` → 调用关系存储在 `nop_code_usage` 表（实体 `NopCodeUsage`），调用链查询已归入 `NopCodeSymbolBizModel.getCallHierarchy()`，无需额外聚合根
...
```
NopCodeFile__get(id) → Entity（通过 BizLoader 加载 outline 字段）
```

BizLoader `outline`：单次 SQL 查询该文件所有符号 → 内存按 parentId 组装嵌套树。`includePrivate=false` 时过滤 PRIVATE 符号。
...
依赖 `ISourceCodeProvider` 接口从文件系统按行号读取。主实现：文件系统读取。
...
```
NopCodeInheritance__findImplementations(qualifiedName, indexId, directOnly, maxDepth) → [NopCodeInheritance Entity]
```
...
```
NopCodeSymbol__findReferencedBy(indexId, qualifiedName, kind) → [ReferenceDTO]
```

### 2.6 类型层级（Type Hierarchy）
...
归属 `NopCodeIndexBizModel`（输入是 indexId，索引级操作）：
...
```
NopCodeSymbol__findByAnnotation(indexId, annotationName) → [NopCodeSymbol Entity]
```
...
集成 nop-search 后支持混合搜索。详见 `search-integration-design.md`。
...
查找两个符号之间的语义关联路径。详见 `semantic-edge-design.md` §七。

```
NopCodeSymbol__findSemanticPath(sourceSymbolId, targetSymbolId, maxDepth) → SemanticPathResult
```
...
NopCodeIndex__triggerIncrementalIndex(indexId, projectPath) → int
...
归属 `NopCodeIndexBizModel`（输入是 indexId，操作索引级分析）：
...
NopCodeIndex__detectFlows(indexId) → FlowDetectionResultDTO
...
NopCodeIndex__analyzeChanges(indexId, baseCommitish, targetCommitish) → ChangeAnalysisResultDTO
NopCodeIndex__getCriticalNodes(indexId, topN) → CriticalNodesDTO
NopCodeIndex__exportGraph(indexId, format, communityView) → String
NopCodeIndex__diffGraph(indexId, baselineCommitish, targetCommitish) → GraphDiffDTO
```
...
```
NopCodeSymbol__detectDeadCode(indexId, kinds) → DeadCodeReportDTO
```
...
| BizModel | 实体名 | xmeta 状态 |
|----------|--------|-----------|
| NopCodeIndexBizModel | NopCodeIndex | 需新建 |
| NopCodeSymbolBizModel | NopCodeSymbol | 需新建 |
| NopCodeFileBizModel | NopCodeFile | 需新建 |
| NopCodeUsageBizModel | NopCodeUsage | 需新建 |
| NopCodeAnnotationUsageBizModel | NopCodeAnnotationUsage | 需新建 |
| NopCodeInheritanceBizModel | NopCodeInheritance | 需新建 |
| NopCodeFileDependencyBizModel | NopCodeFileDependency | 需新建 |
