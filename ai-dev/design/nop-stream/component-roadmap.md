...
| abort 接线 | ✅ **已修复（local + distributed 均已落地）**：local 路径 `CheckpointCoordinator.setAbortHandler` + `GraphModelCheckpointExecutor.registerLocalAbortHandler` 实现 abort → task cancel 接线，abort 后 executor 侧 abort 标记使 job 明确失败（`checkAbortMarker` 抛异常），跳过 final checkpoint。**distributed 路径 Stage 39 Phase 3 已落地**：`JobCoordinator.registerDistributedAbortHandler()`（`JobCoordinator.java:1593`）在 `CheckpointCoordinator.setAbortHandler` 上注册 handler，checkpoint abort 时对所有已分配远程 task 经 `IStreamTaskRpcService.cancelTask` RPC 触发取消（远程 `TaskManager.cancelTask` → `RunningTask.cancel()` 解除阻塞对齐读；**2026-09-03 roadmap item 27 起 cancelTask 携带 fencing epoch，stale 协调器 cancel 被 TM typed 拒绝**）；`RpcDistributedExecutor.java:250` 注册 distributed handler，embedded 形态注册 local handler，二者共存（§8.7） | **P0** ✅ |
...
| abort 传播通道 | ✅ **已修复（Stage 39 Phase 3）**：abort 信号经独立于数据流的控制通道传播到所有 task——local 形态 mailbox + interrupt（`GraphModelCheckpointExecutor.registerLocalAbortHandler`），distributed 形态 `cancelTask` RPC（`JobCoordinator.registerDistributedAbortHandler`；携带 fencing epoch，控制面 mutating 入口 fencing 全覆盖）。无 `CancelCheckpointMarker`（§13.2.1 裁定 Decision-only，无消费方，不引入空壳） | P1 ✅ |
...
| 并发能力一致 | ✅ **已修复（Stage 45）**：Coordinator 层完整尊重 `maxConcurrentCheckpoints` 配置值——`effectiveMaxConcurrent = config.getMaxConcurrentCheckpoints()`（`CheckpointCoordinator.java:370`），**无 `Math.min(1, config)` clamp、无降级语义**；task 层 / 对齐器层多 in-flight 已满足（`CheckpointBarrierTracker` per-epoch ACK 追踪 + `InputGate` 多 in-flight barrier 对齐 + epoch 精准 abort）。aligned 多 in-flight 端到端成立；unaligned 保持 single-in-flight（§2.8.1 D4，successor Stage 47） | P1 ✅ |
| channel 心跳（distributed） | ✅ **已修复（Stage 43 Phase 1；AR-1 P1 修复后覆盖单通道 + 多通道）**：`RemoteResultPartition` 实现生产侧 idle-heartbeat 发射（`sendHeartbeatIfIdle()` + `startHeartbeat(sharedScheduler)`，`heartbeatIntervalMs` 可配，默认禁用）；`RemoteInputChannel` 实现消费侧超时检测（`channelTimeoutMs`，`lastReceivedTime` 在通过 fencing 过滤后刷新，`read()` 路径 piggyback 检查 → `ERR_STREAM_CHANNEL_TIMEOUT`）。fencing：错误 epoch 的 heartbeat 不刷新 liveness。EOS 与 timeout 区分（`finished` 抑制 timeout）。**单通道覆盖（AR-1 P1 修复，plan `2026-08-09-1253-2`）**：`InputGate.readSingleChannel()` 改为有界 poll 循环（`read(50, MILLISECONDS)`，镜像已正确的 `readMultiChannel`），使 `checkChannelTimeout()` 每 ~50ms 重新触发——旧的无界 `read()`→`queue.take()` 让单通道 consumer 永久 park、超时检查永不重新执行。回归测试 `TestInputGateSingleChannelRemoteLiveness`（liveness / 负对照 / EOS）。生产默认启用待 task runtime 共享 scheduler 接线（能力已验证） | P2 ✅ |
| 背压逃生（unaligned） | ✅ **已修复（Stage 43 Phase 2–3）**：`CheckpointConfig.unalignedCheckpointEnabled`/`unalignedThreshold`（`validateUnalignedConfig()` fail-fast）；`InputGate` aligned→unaligned 回退（`switchToUnalignedAndEmit` per-channel `captureInFlightData`）；`ChannelState` 持久化于 `TaskEpochSnapshot`（`CheckpointSerDe` round-trip）。端到端背压验证 ✅：`TestUnalignedCheckpointBackpressure`（Anti-Hollow E2E：capture → persist → restore → replay，exactly-once 无丢无重）+ `TestUnalignedCheckpointMultiInput` + `TestChannelStateRescaleE2E` | P3 ✅ |
...
**演进路径**：P0 补 abort 接线（`scheduleTimeout` 回调调 `SubtaskTask.cancel`）+ 对齐超时（已完成）→ P1 trigger synchronized + 失败计数 + abort 控制通道（synchronized/失败计数已完成；abort 控制通道 local 路径完成）→ P2 `RemoteInputChannel` 心跳 → P3 unaligned checkpoint。注：`BarrierAligner` 已于 Stage 23 删除，多输入对齐增强若需要应在 `InputGate` 上扩展。
...
| 触发竞态 / 失败无计数 / abort 无控制通道 / BarrierAligner 未启用 / 并发能力不一致 | ✅ 触发竞态 + 失败计数 + 并发能力一致 已修复（Plan 172）；abort 控制通道 local + distributed 均已落地（Stage 39 Phase 3：local mailbox + interrupt，distributed `cancelTask` RPC，见 §3 C5）；BarrierAligner 已于 Stage 23 删除（reference code，对齐走 `InputGate`） | P1 ✅ |
| RemoteInputChannel 无心跳 / unaligned checkpoint | ✅ 心跳已修复（Stage 43 Phase 1，见 §3 C5）；unaligned checkpoint 已实现（Stage 43 Phase 2–3 + 背压 E2E，见 §3 C5） | P2 ✅ / P3 ✅ |
