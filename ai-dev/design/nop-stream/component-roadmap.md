...
| abort 传播通道 | ✅ **已修复（Stage 39 Phase 3）**：abort 信号经独立于数据流的控制通道传播到所有 task——local 形态 mailbox + interrupt（`GraphModelCheckpointExecutor.registerLocalAbortHandler`），distributed 形态 `cancelTask` RPC（`JobCoordinator.registerDistributedAbortHandler`）。无 `CancelCheckpointMarker`（§13.2.1 裁定 Decision-only，无消费方，不引入空壳） | P1 ✅ |
