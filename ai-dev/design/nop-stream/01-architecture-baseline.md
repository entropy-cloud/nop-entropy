...
- 重启计数器 **仅 `globalRecovery()` 递增**；默认上限 `maxRestarts=3`（可配）。Stage 27 已正式裁定 targeted/scoped 重启在当前架构下 **NO-GO**（全 pipelined → 单 region，drain/reconnect 不可设计），详见 `failover-design.md`。per-region 计数器随 G57/G28 一起 deferred → Stage 44（需 blocking edge + supervision loop 前置）。Stage 44 go 裁定后：successor 1（物化点机制）+ successor 2（region 概念与识别，`io.nop.stream.core.jobgraph.region`）+ successor 3（supervision loop 执行模型，`io.nop.stream.runtime.execution.SupervisionLoop`）+ successor 4（drain/reconnect，consistent-cut epoch 对齐 + overflow-bypass + producer-region 重启 + consumer reconnect-to-live-queue）+ successor 5（per-region restart 上限可配置，`CheckpointConfig.maxRestartsPerRegion`，默认 3，经 `submitAndRun` 参数穿透到 `SupervisionLoop.run` 全参签名）均已落地。`GraphModelCheckpointExecutor.submitAndRun` 现委托 `SupervisionLoop.run`（传入 `CheckpointCoordinator` + `CheckpointPlan` 以支持 consistent-cut state restore，并传入 `checkpointConfig.getMaxRestartsPerRegion()` 以支持 per-region 上限可配置）：mid-execution 检测 FAILED task → drainable region（含 producer-region）经物化重放 + state restore scoped 重启 → budget 耗尽时 fallback `globalRecovery()`。`globalRecovery()` 仍为兜底恢复路径（未被删除）。region-based failover 端到端可用（drain → restart → reconnect → exactly-once，含 stateful operator）。**per-region counter 生命周期说明**：counter 为 in-memory scoped to one `SupervisionLoop.run()`（LOCAL 路径）；与 `globalRecovery()`（DISTRIBUTED 路径）完全分离，无"跨 cycle 重置"问题（两路径互不调用）。
