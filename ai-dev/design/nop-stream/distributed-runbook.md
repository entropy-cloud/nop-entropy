...
- **HA failover 新 leader 接管在 task_assignment 唯一键冲突处中止**（Follow-up item 34，**Phase 4 复验发现并确认 pre-existing**）：CHAOS-2 复验（2026-09-04，runId `1788457688118-1`）判定 = 租约翻转子判据 PASS（leaseEpoch 1→2→3 严格递增）但终态不收敛；coordinator 日志揭示机制：新 leader `activateAsLeader` 完成租约获取 + fencing 轮转（epoch=2000000）后，重发 assignment 的 `INSERT INTO nop_stream_task_assignment`（attempt_number=1）与旧 leader 的存留行唯一键冲突（`nop.err.dao.sql.duplicate-key`）→ become-leader listener 异常中止 → 接管半途而废（无重部署、无后续 checkpoint，输出冻结）。**同一签名存在于 item 15 演练产物**（`1788385016277-1` coordinator-2/3 日志）——当时报告归因「jam 抑制重部署触发」应修正为本缺陷（jam 掩盖了真实机制）。修复方向（successor plan）：接管时 attempt 计数从注册表 attempt history 种子化或 assignment 落库改 upsert。复现：§5 EX 行 `chaosJcHaFailover`。
- **EpochManifest 文件不随 checkpoint retention 清理**（Follow-up item 33，**Phase 4 复验发现**）：`cleanupOldCheckpoints` 只删 CompletedCheckpoint 状态，`.epoch` manifest 文件无清理路径——健康 SOAK-3（280s，interval 2s）留档 137 个 manifest 文件（maxRetained=5 不生效于 manifest 面）。item 15 报告「retained manifests 全程有界」的正观察系 jam 早停所致（epoch 冻结 ⇒ manifest 自然不再增长），非 retention 生效证据。影响 = 长运行作业 manifest 文件无界累积（小文件磁盘缓慢泄漏，非正确性/稳定性缺陷——bounded per run，不影响 exactly-once）。修复方向：retention 路径同步裁剪 epoch manifests（双存储）。
