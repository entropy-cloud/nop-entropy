...
4. **部署模式选择**：`DeploymentMode.LOCAL`（线程池）或 `DeploymentMode.DISTRIBUTED`（多 TaskManager 实例），通过 `IStreamExecutionDispatcher` SPI 分发
5. **分布式 exactly-once**：epoch checkpoint + barrier 对齐 + CheckpointParticipant + fencing
