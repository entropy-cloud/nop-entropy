...
> 更新（Cycle 2 / I4，2026-08-12）：侧输出已落地——`OutputTag` + 链式侧输出通道（RL-7 修复 `b20fcd0e1` 后 `ChainingOutput` 转发至注册消费者，无消费者 fail-fast）。6 个发射点：`ProcessOperator.java:111/:134`（ProcessFunction / OnTimer ctx.output）、`WindowOperator.java:1030/:1860`（late-data / ProcessWindowFunction ctx.output）、`CepOperator.java:483/:777`（late-data / PatternProcessFunction ctx.output）。跨 task 边界（`RecordWriterOutput` / `BroadcastingRecordWriterOutput`）`collect(OutputTag)` **已 fail-fast**（Cycle 2 / I4 interim fail-fast 落地：抛 `ERR_STREAM_SIDE_OUTPUT_NO_CONSUMER`，替代静默丢弃）。
>
> 更新（HG-01，2026-08-14，plan `2026-08-14-0900-1`，人工批准落地）：**跨 task 完整线协议支持已落地**——`RecordWriterOutput`/`BroadcastingRecordWriterOutput.collect(OutputTag, record)` 不再 fail-fast，改为构造 side-output 元素经 `RecordWriter.emitElement` 广播到全部下游 partition 送达消费 task；消费端 `processInputGate` 新增 side-output 元素分支路由到 task 级 `sideOutputConsumers`（按 tag id 遍历匹配），**无消费者 fail-fast 移至消费端路由点**（错误码复用 `ERR_STREAM_SIDE_OUTPUT_NO_CONSUMER`）。

### 6.1 不变式（Invariants）

> 交叉引用：`ai-dev/audits/nop-stream-invariants/invariant-catalog.md` §5 不变式 #6（PD-15 输出契约族，覆盖 RL-7 + 跨 task 缺口）与不变式 #7（生产 wiring 存在性，Cycle 3 / I1 沉淀，覆盖 open-audit P0-01/P1-02 运行时服务注入完整性族）。

- **输出契约族（side-output 转发契约）**：任何 `Output.collect(OutputTag, X)` 调用必须被转发到注册的 side-output 消费者，不得静默丢弃；无注册消费者 → fail-fast（`ERR_STREAM_SIDE_OUTPUT_NO_CONSUMER`）。跨 task 实例（`RecordWriterOutput` / `BroadcastingRecordWriterOutput`）**处置状态 = forward 已落地**（HG-01，2026-08-14，plan `2026-08-14-0900-1`：`collect(OutputTag)` 构造 side-output 元素 → 线协议送达消费 task → 消费端路由；无消费者在**消费端**路由点 fail-fast，生产端不再抛）。线协议设计裁决（Phase 1 落档）：
  - **envelope 形态**：`StreamMessageEnvelope` 新增 `TYPE_SIDE_OUTPUT_RECORD` 类型常量 + `outputTagId` 字段（默认 null，既有 5 类型零改动）；`StreamElement` 新增 side-output 子类型（携带 tag id + 内层 `StreamRecord`，`isRecord()` 等既有判定不受影响）。
  - **valueType 派生**：side-output 元素忽略 edge 级 valueType（`RemoteResultPartition.java:162-164` 的 `typeRegistry.getOutputTypeClassName(edgeId)` 只标主输出边）——一律从内层 record value 派生类名（`record.getValue().getClass().getName()`）。
  - **生产端通道**：经 `RecordWriter.emitElement(StreamElement)`（`RecordWriter.java:216-225`，广播全部 partition；`emit(StreamRecord)` 只收 StreamRecord 且按 partitioner 路由，不可用于 side-output）→ 语义 = 所有下游 subtask 都收到 side-output 元素，任一 subtask 未注册消费者即 fail-fast；拒绝新增 tag 路由 writer 方法（OutputTag 无 partitioner 概念，收窄广播面收益低、新增 API 面大）。
  - **消费端查找**：`sideOutputConsumers.keySet()` 遍历按 `getId()` 匹配（`OutputTag` 构造器 `Guard.notNull(typeInfo)`（`OutputTag.java:63`）——不能 `new OutputTag<>(id, null)` 做查找 key；tag id 等值语义）。
  - **record copy**：BRWO 扇出多 partition 时内层 record `copy(...)`（防别名化，`RecordWriterOutput.collect(StreamRecord)` :873 同模式）。
  - **materialization 双写路径 = 排除（non-blocking）**：`ResultPartition.java:198` `if (point != null && element.isRecord())` 只双写主记录流——side-output 子类型天然不入 materialization store。**显式裁决 = 不扩展双写门**：unaligned checkpoint 恢复期 in-flight side-output 元素静默丢失；理由 = (a) side-output 语义为尽力送达增强（消费端无「错过即账」强一致契约，与主数据流不等级）；(b) 扩展需要 materialization store 全链支持 tag 元数据（store 格式 + 恢复路径 + barrier 对齐交互），超出本 plan 批准面；(c) 恢复期 in-flight side-output 丢失不违反不变式 #6（无静默丢弃面 = 运行期转发契约，checkpoint 恢复面属独立语义）。此排除已在 Phase 3 测试/文档注明，留痕在案。若未来消费端出现强一致需求（side-output 必须跨 checkpoint 送达），另立 plan 扩展。
  - **无消费者 fail-fast 位置**：从生产端（I4 interim 位置）移至消费端路由点（`processInputGate` side-output 分支，未命中消费者抛 `ERR_STREAM_SIDE_OUTPUT_NO_CONSUMER`，参数含 tag id + 详情）——语义对齐「转发契约 + 无消费者 fail-fast」，与 in-task `ChainingOutput` 一致。
- **门禁（已入 CI）**：JUnit `TestOutputContractInvariant`（`nop-stream-core`，参数化穷举 4 个 main `Output` 实现类三态行为（分类以 `ai-dev/audits/nop-stream-invariants/output-contract-registry.json` 为准）+ `registerSideOutputConsumer` ↔ `ChainingOutput.sideOutputConsumers` 接线断言 + 反射实例化断言）；mjs `scan-output-contract`（V1 类级枚举 / V2 失效类 / V3 行为漂移 / V4 新增发射点 / V5 失效发射点，`check-nop-stream-invariants.mjs all` 纳入）；端到端证据 = runtime `TestSideOutputChainingE2E`（含跨 task 送达 + 无消费者 fail-fast 用例）。
- **历史证据**：R15-AR-4（`ChainingOutput` 侧输出静默丢弃，pre-fix `ChainingOutput.java:84-86`）、I2 探查 PR-1、RL-7 修复 `b20fcd0e1`、I6 裁定跨 task 缺口双层处置（interim fail-fast 自动信封 + `HG-01` 人工确认门，Cycle 2 / I4 修复落地，详见 catalog §5 不变式 #6）、HG-01 线协议落地（2026-08-14，plan `2026-08-14-0900-1`，注册表分类迁移 fail-fast → forward）。
- **生产 wiring 存在性（运行时服务注入完整性，Cycle 3 / I1 沉淀）**：任何被 main 代码（非测试）消费的运行时服务注入 API（`AbstractStreamOperator.setProcessingTimeService` / `setTimeServiceManager` / `setStateBackend` / `setKeyedStateBackend` / `setOperatorStateBackend` / `setOutput` / `setSnapshotCallback`）必须存在生产接线点（main 调用方 + 注入时机）；仅被测试注入（main 零调用）→ 违约。生产接线路径（live）：PTS/TSM 由 `StreamTaskInvokable.setupProcessingTimeServices` 在**构造函数**无条件注入（:462/:463，先于任何 `operatorChain.open()`）；`Output` 由 `wireOperators` / `wireTailToRecordWriter` **装配布线**（:230/:268/:287/:293/:426）；`IStateBackend` 由 `GraphModelCheckpointExecutor.wireTaskCheckpointPipeline` **checkpoint 装配**（:742）；`snapshotCallback` 由 `setupSnapshotCallbacks` **快照接线**（:435）。`setKeyedStateBackend` / `setOperatorStateBackend` = **`internal-creation` 裁定**：生产经 `stateBackend.createKeyedStateBackend()` 在 `open()` 内直建（`CepOperator.java:257` / `WindowOperator.java:421` / `AbstractStreamOperator.java:65`），setter 为测试注入后门——V1 carve-out 受棘轮约束（弱化/豁免需人工确认 + 留痕，理由登记于 `wiring-registry.json`）。
- **门禁（已入 CI，Cycle 3 / I1）**：JUnit `TestWiringExistenceInvariant`（`nop-stream-core`，10 用例——注册表驱动参数化 API 面完备性（反射枚举 `AbstractStreamOperator` 全部 `set*` 注入方法，参数类型 ∈ 已登记服务类型，新注入 API 不入表即红）+ 接线运行时连通断言（invokable 构造注入 ↔ 算子 getter 同一实例 + `numTimerServices()` 注册证据））；mjs `scan-wiring`（V1 仅测试注入检测（receiver 限定成员访问形态；production-wired main 零调用点 → 红，internal-creation carve-out）/ V2 消费方枚举完备性 / V3 失效接线点 / V4 新注入 API / V5 失效消费方/服务，`check-nop-stream-invariants.mjs all` 纳入）；接线点注册表 `ai-dev/audits/nop-stream-invariants/wiring-registry.json`（服务 × 接线点（文件:行 + 注入时机）× 消费方枚举 × disposition × test-only 豁免）。无服务 fail-fast 行为断言归属 cep/runtime 既有 E2E（`TestCepProductionExecutionE2E` PT 无服务 fail-fast + `TestProcessingTimeWindowProductionE2E` WARN 断言）。
- **历史证据**：open-audit P0-01（`CepOperator.java:347` open 路径 null NPE）、P1-02（PT 窗口永不触发，`WindowOperator.java:479`）；修复基线 = plan `2026-08-13-0132-1`（`StreamTaskInvokable.setupProcessingTimeServices` :456-477 生产接线）；13 个测试类 mock 注入规避机制（`CepTestUtils.injectProcessingTimeService` 等），详见 catalog §5 不变式 #7。
