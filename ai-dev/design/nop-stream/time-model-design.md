...
nop-stream 的时间模型定义了事件时间（event time）语义的基础：如何为记录分配时间戳、如何推进 watermark、以及 watermark 如何驱动窗口触发和定时器。这套子系统直接从 Flink 移植，接口完整，但当前未与快速执行路径集成。
...
**快速路径不使用 TimestampsAndWatermarksOperator**。`StreamExecutionEnvironment.execute()` 直接构建算子链，Source 结束后发送 `MAX_WATERMARK`。这意味着：

1. **没有持续 watermark 推进** — 只有一次性的 MAX_WATERMARK
2. **没有乱序处理** — 不需要，因为 MAX_WATERMARK 已经超过所有可能的时间戳
3. **事件时间窗口只在流结束时触发** — EventTimeTrigger 在 watermark >= window.maxTimestamp() 时触发，而 MAX_WATERMARK = Long.MAX_VALUE 一定满足
4. **ContinuousEventTimeTrigger 永远不会在运行中触发** — 没有 watermark 中间值来驱动持续触发

### 7.2 集成方式（设计目标）

要启用持续 watermark 推进，需要在 Source 和下游算子之间插入 `TimestampsAndWatermarksOperator`：

```
当前快速路径:
  Source → Map → Window → Sink
  Source 结束后发送 MAX_WATERMARK
...
**集成所需的工作**：
1. `execute()` 在 Source Transformation 后自动插入 TimestampsAndWatermarksOperator（如果用户配置了 WatermarkStrategy）
2. 设置 `watermarkInterval > 0`（当前硬编码为 0）
3. TimestampsAndWatermarksOperator 的 `processElement` 输出通过 ChainingOutput 传递到下游
