...
- **裁定**：ChatBI 全部数据集消费工具——`datav-list-datasets`（枚举）与 `datav-query-dataset`/`datav-describe-dataset`（执行/描述）以及 `datav-generate-dashboard`/`datav-generate-screen`（生成绑定，AR-1 修复 plan `2026-08-16-2137-1` 接线）——在接受任何 `datasetSid` 前接入 `NopReportDataset.createdBy`/admin 可见性：admin 全量；非 admin 仅 `createdBy == 当前用户`（list 侧 SQL 过滤 + describe/query/generate 侧显式拒绝 `ERR_DATAV_CHATBI_DATASET_NO_ACCESS`；generate 侧不存在/非活跃仍走 `ERR_DATAV_CHATBI_GENERATE_DATASET_NOT_FOUND` 分工）。`createdBy` 为空的历史数据集对非 admin 不可见（fail-closed；由 admin 重新保存认领）。身份经 `ChatBiToolExecuteContext`（operator + admin 标志）显式传递（镜像裁定 G 耦合契约），由 `NopDatavChatBiBizModel` 从 `IServiceContext` 解析。
...
- **`PanelDataBinder` 数据集可达性检查点：排除（definitive）**。论证（对照审计 P1-03「datav 全部查询路径」口径）：面板查询路径的授权经 **Dashboard RLS 传递成立**——Phase 2 修复后 `getPanelData/refreshPanel/resolveLinkage/resolveJump` 有 panel→Dashboard 归属校验（非 owner 且未发布即拒绝），`getDashboardData` 经 `requireEntity(Dashboard)` RLS；「发布看板」是领域显式的数据视图共享行为（`getPublishedDashboard` 同样把含数据的看板内容开放给全部登录用户），面板执行其 DatasetRef 绑定的数据集 SQL 属于该共享语义之内，不构成越权面。D2 收敛 + ChatBI generate 工具可见性接线（plan `2026-08-16-2137-1`，AR-1 修复）后，DatasetRef 的绑定与篡改面仅 admin 可达（generate 侧非 admin 仅可绑定对自己可见的数据集，不可见即拒绝 `ERR_DATAV_CHATBI_DATASET_NO_ACCESS`），非 admin 无法构造「引用他人数据集的看板」。**AR-1 教训（2026-08-16 落档）**：排除论证的承重前提必须随新消费工具出现而复核——generate-dashboard/generate-screen 上线时曾打破本前提（非 admin 经 ChatBI 构造引用任意数据集的看板，再经自有看板查询路径外泄），修复后恢复成立；后续新增任何可构造 `datasetSid` 绑定的消费面时，必须对照 `ai-design.md` §6「数据集 sid 全部消费面固定清单」补接线并复核本论证前提。
