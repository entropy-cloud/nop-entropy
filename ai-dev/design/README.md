...
| `nop-credential/` | 加密凭证库（Credential Vault） | [README](nop-credential/README.md) | active — 独立可复用模块（api/dao/service/web）；对称加密原语下沉 nop-security（AES-256-GCM + HKDF + 版本化密文）；类型注册表 + 实例加密存储；`ICredentialProvider` 服务端 SPI + `@credential:` resolver；一期不做 RBAC/OAuth/KMS |
