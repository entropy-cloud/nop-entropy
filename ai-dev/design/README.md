...
| `nop-credential/` | 加密凭证库（Credential Vault） | [README](nop-credential/README.md) | active — 独立可复用模块（api/dao/meta/service/web）；复用 `nop-commons` `AESTextCipher`（AES-256-GCM + PBKDF2 + 版本化密文 `v1:`）包装为 `cv1:{keyId}:{v1密文}` 多密钥格式；类型注册表（`*.credential-type.xml`）+ 实例加密存储；`ICredentialProvider` 服务端唯一解密点 SPI（明文边界结构性强制）；配置加密复用既有 `@sec:`（拒绝 `@credential:` resolver，见 baseline §4）。二期设计已定稿（`02-phase2-design.md`：出站 OAuth 客户端引擎 / KMS 材料交付模式 / 归属统一 scope=system\|user（不做租户隔离）/ 凭证级 RBAC 授权），待 W9-W11-impl 实施 |
