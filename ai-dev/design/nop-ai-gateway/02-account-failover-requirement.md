...
**日期**：2026-08-14（更新于 2026-08-14，R1-R6 审查通过后需求修订：模型分类路由组、动态选择机制、网关不依赖 nop-ai-agent）
**范围**：nop-ai-gateway（网关形态拦截器 + 本地 `IChatService` 适配器 + 流式 failover 缺口 + 账号并发限流 + 模型分类路由组 + 动态选择策略）；复用/下沉 nop-ai-core 的 LLM 可靠性子集；补全 nop-ai-core `ILlmDialect` 双向转换
**状态**：审查通过（R1-R6）→ 需求修订 → 待 R7 复审
...
8. **模型分类路由组（新增能力）**：模型按"级别/类"组织（如 deepseek-v4 级别），每类对应一组**候选（备选模型 + 账号组合）**，形成组内 fallback 链。请求携带的模型属于某类时，在该类的候选集内游走——不是全局一组账号（见 §3.3）。
...
| 熔断三态（CLOSED/OPEN/HALF_OPEN + 冷却 + 探活，粒度 `provider:model`） | nop-ai-agent `ThresholdBreaker` | 已实现 |
| 多通道失败分流：QUOTA/AUTH → 账号链；TRANSIENT → 模型 tier 回退（`IModelRouter.getFallback`）；账号链耗尽 → provider 链（`ProviderFailoverChain`，配置载体 `llm-failover.xdef` + `_default.llm-failover.xml`） | nop-ai-agent `LlmCallCoordinator` | 已实现（agent 引擎路径，强耦合 `AgentExecutionContext`，非流式） |
...
5. **`parseRequestBody` 双向转换不完整**：需补全 Anthropic/Gemini/Ollama 等全部 dialect 的前端解析（nop-ai-core 改动，见 §3.7/§4.3）。
...
- 账号链、熔断、冷却、探活**复用 nop-ai-agent reliability 既有机制**，不新建：
  - `ThresholdBreaker` 既有语义：CLOSED（连续失败达阈值 → OPEN）→ OPEN（拒绝调用，冷却期满 → HALF_OPEN）→ HALF_OPEN（放行一个探活调用，成功 → CLOSED 并清零失败计数，失败 → 回到 OPEN 并重启冷却）。
- **熔断粒度：`provider:model` 复合键**（既有实现，`buildModelKey`）。账号切换共用同一熔断键：同 provider:model 内多账号连续失败记账跨账号累计，全部账号失败 → 该键 OPEN → 冷却后整体恢复。此行为沿用 coordinator 语义，不扩展熔断键维度。
- 冷却参数沿用 `ThresholdBreaker` 构造器配置（provider:model 级），账号级不设冷却参数。
- **账号并发限流（新增机制）**：
  - 每个账号一个**进程内并发计数**（进行中请求数，流式请求在流结束/取消时释放）。
  - 账号配置 `concurrencyLimit`（新增字段，见 §3.4）；选账号时并发已达上限的账号跳过，视为"当前不可用"。
  - 并发超限**不记入熔断失败计数**（非失败事件），只影响本轮选择；**主动切换不消耗重试预算**（与不记熔断同族规则）。
  - **全池饱和语义**：所有账号均达并发上限 → **fail-loud**（与 fail-loud 哲学一致，不排队不无限等待），并产出饱和指标（§3.6 可观测性）。
  - **主动路径不走 provider 链**（决策）：主动路径是**预检语义**（未发生失败事件），跨 provider 决策留给被动失败路径——避免无失败证据的 provider 级乒乓；且并发饱和是**瞬态资源状态**（进行中请求会释放）。故主动路径仅在**账号链内**游走；被动失败路径（账号链耗尽）才走 provider 链（§4.3）。"所有账号"指同 provider 账号链内全部账号。
...
新能力落 **nop-ai-gateway**（已依赖 nop-gateway + nop-ai-api + nop-ai-core + **nop-ai-agent**，见 pom）。nop-gateway 保持零 nop-ai 依赖、nop-ai-core 不感知网关（沿用 `01-architecture.md` 决策）。

### 4.2 分层

```mermaid
flowchart LR
    subgraph reuse[复用层: nop-ai-core + nop-ai-agent]
        AC[账号链 AccountChain/LlmConfigHelper]
        CB[熔断 ThresholdBreaker provider:model]
        EC[错误分类 LlmErrorClassifier]
        CS[ChatServiceImpl: 主账号凭证链 + accountKey/accountBaseUrl 下沉]
    end
    subgraph gw[网关形态: nop-ai-gateway]
        GI[IGatewayInterceptor: 切换/缓冲/重试]
        CC[跨协议转换 AiDialectBackendMessageConverter 流式扩展]
        HTTP[IHttpClient 转发]
    end
...
### 4.3 复用 vs 新增边界（R1/R2 裁决）

| 机制 | 裁决 | 理由 |
|------|------|------|
| 账号链 `<accounts>` / `LlmConfigHelper` | **复用** | 已实现且被 agent 引擎使用 |
| 熔断 `ThresholdBreaker`（provider:model 粒度） | **复用** | 三态 + 冷却 + 探活齐全；粒度沿用 |
| 错误分类 `LlmErrorClassifier` | **复用** | 已落地 |
| 主账号凭证解析链（nop-credential） | **复用** | `ChatServiceImpl` 既有实现 |
| 备用账号 apiKey 下沉 | **复用** | coordinator 既有行为（`doAccountSwitch`） |
| `LlmCallCoordinator` 多通道语义 | **语义沿用 + 两项偏离** | 沿用 QUOTA/AUTH → 账号链、账号链耗尽 → provider 链；偏离 1：TRANSIENT/RATE_LIMITED 在网关语境扩展为账号链切换（§3.1 裁决，产品决策）；偏离 2：网关语境不引入 `IModelRouter`/模型 tier（provider 链经 `llm-failover.xdef` 声明 + `ProviderFailoverChain` 消费） |
| 跨 provider 切换（`ProviderFailoverChain` 第三通道 + `llm-failover.xdef`） | **复用两层链** | 游走顺序：账号链（同 provider）→ provider 链（llm-failover 声明，可含 model 覆盖）→ 新 provider 的账号链；不合并为单一账号池（保持既有两层结构，零新增配置面） |
| 网关格式转换 `AiDialectBackendMessageConverter` | **复用 + 扩展** | 需新增：① 流式支持（stream=true 请求体生成）；② **per-request 动态 dialect 解析**——账号池含不同 `apiStyle` 的账号，切换后 backendLlm 须按目标账号的 apiStyle 经 `LlmDialectFactory` 动态解析（当前 `frontendLlm/backendLlm` 为固定 bean 属性）；③ **目标 provider 的真实 `LlmModel` config**——apiStyle 取自 `{provider}.llm.xml` 根属性，切换后加载目标 provider 的完整配置（errorMappings 等），替换现 converter 的 `new LlmModel()` 空配置 |
| `ILlmDialect` 双向转换补全（parseRequestBody + buildResponse/buildStreamChunk） | **新增**（nop-ai-core 改动） | 仅 OpenAI 请求方向 + 恒 OpenAI 响应方向；双向闭环的前提（§3.7）；补充说明：parseRequestBody 是 dialect 自身反向转换能力，非网关概念，"nop-ai-core 不感知网关"决策不受影响 |
...
非流式调用（本地形态 callAsync，包装 ChatServiceImpl）:
  1. 经凭证解析链取主账号；RATE_LIMITED/AUTH_INVALID/TRANSIENT 失败时解析账号链
  2. 熔断器校验（provider:model OPEN 则 fail-loud）+ 并发计数检查（≥ concurrencyLimit 则跳过；全池饱和 → fail-loud）
  3. ChatOptions.accountKey/accountBaseUrl 下沉选中账号 → ChatServiceImpl.call
     （协议转换由 ChatServiceImpl 内部按 provider 选 dialect，converter 不参与本地形态）
  4. 成功 → 返回；失败 → 判定分类（错误响应读 response.errorClassification，由 parseErrorResponse 设定；
     传输异常经 LlmErrorClassifier）
  5. retryable 且账号链有下一个 → 熔断记账 + 切换，回到 2
  6. 账号链耗尽 → provider 链（llm-failover 声明）→ 新 provider 的账号链，回到 2

流式调用（本地形态 callStream，Flow.Publisher）:
  1. 选择账号 + 熔断/并发检查 + callStream 建立 chunk 流（计数 +1，流结束/取消 -1）
  2. 首段缓冲窗口内失败 → 熔断记账 + 切换账号，回到 1（从头重试 = 重订阅，消耗重试预算）
  3. 越过缓冲窗口 → 向订阅者转发；此后失败不再切换（断流报错）

网关形态（IGatewayInterceptor）:
  1. 按目标账号 apiStyle 动态取 dialect → AiDialectBackendMessageConverter 转换请求体（stream 按需）
  2. 非流式：invoke 内包装切换/重试（语义同上，含并发检查与两层链游走）
...
| # | 问题 | 影响 |
|---|------|------|
| 1 | 模块归属确认：全部新增能力放 nop-ai-gateway？（已依赖 nop-ai-agent，建议确认） | 依赖结构 |
...
| 6 | 账号并发上限默认值：provider 级缺省值 / 不限制时的行为 | 主动限流行为 |
...
- **新建第二套账号池/熔断/错误分类**：拒绝。nop-ai-agent reliability + nop-ai-core 已实现且被 agent 引擎使用，重复建设必然双轨漂移（R1 审查确认）。
- **本地形态重新实现 dialect 选择/请求构造/转发管线**：拒绝。`ChatServiceImpl` 已提供全链路 + `accountKey/accountBaseUrl` 下沉；适配器只做包装与流式重试（R2 审查确认）。
- **网关语境引入 `IModelRouter`/模型 tier 回退**：拒绝。TRANSIENT 切换账号即满足用户目标，且网关账号池天然含跨 provider 账号，无需 tier 概念。
...
- **路由核心下沉 nop-ai-core**：拒绝。延续 `01-architecture.md` 方案 C 决策——nop-ai-core 不感知网关/路由概念。
...
| `ai-dev/design/nop-ai-agent/nop-ai-agent-reliability.md` | 账号链/熔断/多通道分流/调用协调的既有语义来源，本项目复用的对象（§3.1 有两项显式偏离） |
...
| `nop-ai-agent` reliability 包 | `ThresholdBreaker` / `AccountChain` / `ProviderFailoverChain` / `LlmErrorClassifier` / `LlmCallCoordinator` 的复用对象（TRANSIENT 通道有一项偏离） |
