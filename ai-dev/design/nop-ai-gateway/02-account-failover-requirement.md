...
**LLM 可靠性子集下沉（新增工作项）**：`ThresholdBreaker` / `ICircuitBreaker` / `CircuitState` / `AccountChain` / `IAccountChainResolver` / `ProviderFailoverChain` / `ProviderFailoverQueue` / `LlmErrorClassifier` / `IRetryPolicy` / `StandardRetryPolicy` 等从 nop-ai-agent 移至 **nop-ai-core**，并同步迁移：异常 `NopAiAgentException` + 错误码 `NopAiAgentErrors` → nop-ai-core 等价物（`LlmErrorClassifier` 已用 `NopAiCoreErrors`，有先例）、熔断键构造 `buildModelKey`（现为 `LlmCallCoordinator` 的 static 方法，随下沉移入 nop-ai-core，nop-ai-agent 内既有调用方如 `ReActAgentExecutor` 保留在原处调用）。agent 引擎专用部分（`Checkpoint*` / `GoalTracker` / `Sustainer` / `WaitCoordinator` / `CompactionAwareTruncation`）留在 nop-ai-agent。`LlmCallCoordinator` 强耦合 agent 引擎，**不迁移**（语义沿用）。下沉后 nop-ai-agent 继续依赖 nop-ai-core（既有方向不变）。
