...
- `02-account-failover-requirement.md` — 账号级透明 failover + 账号并发限流 + 模型类路由组 + 动态选择策略的需求规格（状态：审查通过，R1-R8 共识达成）。注意：其 §3.7 要求双向协议转换补全，是对 `01-architecture.md`"前端恒 OpenAI"假设的显式偏离；§4.1 要求 LLM 可靠性子集下沉 nop-ai-core（网关能力级不依赖 nop-ai-agent）。
