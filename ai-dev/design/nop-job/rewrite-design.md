...
可选模块 `nop-job-retry-adapter` 提供基于 `nop-retry` 的实现：

1. 监听 `JobFireFailedEvent`
2. 把失败 fire 包装成 retry task
3. 写入 `nop_retry_record`
4. 把生成的 `retry_record_id` 回填到 `nop_job_fire`
...
2. 数据库批量获取候选记录。
3. 乐观锁批量 claim。
4. `INamingService + PartitionAssignHelper` 动态计算分区。

不再保留 `nop_job_assignment` 这类静态分配表。
