...
- **system-prompt 自动注入 ✅**（plan 192）：`DefaultAgentEngine.buildBaseExecutionContext`（`doExecute` + `resumeSession` 共用）在每轮执行启动时，经 `memoryStoreProvider.getOrCreate(sessionId)` → `store.readBudgeted(budget, context)` 将非空 budgeted memory 格式化为 `## Working Memory` 段落追加到 system prompt 尾部（base prompt 在前，memory 段落在后，合并为单条 `ChatSystemMessage`）。空 memory / `readBudgeted` 返回空列表时不注入（无 memory 段落、无多余 system 消息——向后兼容）。budget 在 engine 级配置（`memoryInjectionBudgetTokens` 字段 + setter，shipped 默认 1024，≤ 0 禁用注入 = 显式 opt-out）。null provider 跳过注入不抛异常（测试场景 opt-out）。注入的 memory 集合直接采用 `readBudgeted` 排序结果（pinned 始终包含 → 非 pinned 按 priority 降序），不重新排序或过滤
