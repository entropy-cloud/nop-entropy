...
> Related: `nop-ai-agent-multi-agent.md` §「Agent 作为 Nop Flow 节点类型」、`nop-ai-agent-actor-runtime-vision.md` §8.2 / §2.2（ITaskStep + Decorator）
...
### 决策 4：节点 → 成员 agent 解析 — 消费已绑定成员，不 spawn

编排器不自动 spawn 成员。每个任务节点执行时，经 `ITeamManager` 反查团队**已绑定**成员会话，将任务委派给对应成员 agent 执行。解析策略：优先 `TeamTask.claimedBy`；未 claim 时按团队成员花名册解析（优先 MEMBER 角色，回退任意已绑定成员）。**未绑定任何成员的团队执行编排 = 快速失败**（No Silent No-Op #24），而非静默跳过。auto-spawn 成员 agent 为独立 successor。
...
- `blockedBy` 自动调度守护进程（定时扫描就绪任务自动 claim/派发）— successor `L4-blockedBy-resolution-engine`。
- auto-spawn 成员 agent（声明式自动启动/调度成员执行）— successor `L4-auto-spawn-member-agent`。
- 异步/长时/跨进程流编排执行 — successor。
- LLM 直面编排工具（如 `team-execute-flow`）— successor（依赖 auto-spawn + 调度策略裁定）。
