...
4. **`AgentExecStatus.paused` 语义独立**：`paused` 与 `forced_stopped`/`cancelled`/`escalated`/`truncated` 语义不同——paused 是治理策略自动触发（denial threshold exceeded），非用户干预（cancelled）、非系统决策（forced_stopped）、非升级路径（escalated）、非预算截断（truncated）。post-loop bookkeeping 排除 `paused`（及 `truncated`，plan 277 AR-14-b），暂停/截断 session 不发布 `EXECUTION_COMPLETED` / 不执行 `POST_CALL` hooks。
...
- **(a) 当前来源**：截至本注记，`SandboxRequest` 在 main src 中**无任何生产构造点**（grep `SandboxRequest.builder()/SandboxRequest.of()/new SandboxRequest` 于 `nop-ai/nop-ai-agent/src/main/java/` 仅命中 `SandboxRequest.java` 自身的 `Builder.build()`）。`ISandboxBackend.execute(...)` 在 main src 中**从未被调用**（grep `sandboxBackend.execute` / `getSandboxBackend().execute` / `*.execute(...sandbox` 于 main src = 0 命中）。`DefaultAgentEngineConfig.sandboxBackend` 默认 `NoOpSandboxBackend.INSTANCE`（`DefaultAgentEngineConfig.java:142-143`），经 `DefaultAgentEngine.setSandboxBackend` setter（`DefaultAgentEngine.java:456`，接线点 `:257`）与 `ReActAgentExecutorBuilder.sandboxBackend`（`ReActAgentExecutorBuilder.java:523`，构造透传 `ReActAgentExecutor.java:219/:244-246`）进入 `ReActAgentExecutor.sandboxBackend` 字段（`ReActAgentExecutor.java:166`，getter `:341-342`）后**仅存储、从不调用**。因此 `environmentVariables` 当前仅来源于测试 fixture（`TestDockerSandboxBackend`/`TestNoOpSandboxBackend`/`TestSandboxWiring` 构造 `SandboxRequest` 时直接传入 env map）与 `Builder` 默认空 map。
