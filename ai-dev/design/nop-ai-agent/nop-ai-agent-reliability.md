...
- **可恢复状态策略（running/pending only）**：候选 = `running`（崩溃时正在执行）+ `pending`（崩溃前未开始）。`paused` 跳过（denial-ledger sticky-pause，governance 状态，需人类 `resumeSession` 恢复——plan 180；auto-restore `paused` 会绕过人类干预契约）。terminal（completed/failed/cancelled/forced_stopped/escalated/truncated）跳过（已完成）。**关键**: orchestrator 在调用 `restoreSession` **前** 自行过滤 `paused`（`restoreSession` 的 `isTerminalStatus` 检查不包含 `paused`，所以 `restoreSession` 会接受 `paused` session——由调用方决定是否调用，`restoreSession` 单 session 契约不变）。注：plan 277 AR-14-c 新增 `truncated` 到 `isTerminalStatus`，截断会话已达到终态不应被 restore。
...
- **跨进程接管锁基础层 + 定时扫描 daemon ✅ 已落地（plan 221 / plan 222 / L4-8-P4）**：`DBSessionStore` 使 session 可被发现和加载（任何共享 DB 的服务实例都能 `get(sessionId)`），"防止两个 JVM 实例同时恢复并执行同一 session" 的并发接管锁已由 plan 221 落地——`ISessionTakeoverLock` 契约（`tryAcquire`/`release`/`isHeld`/`tryRenew`，CAS 语义 + lease/TTL 被动过期 + conditional release）+ `NoOpSessionTakeoverLock` shipped 默认（单进程零回归）+ `DbSessionTakeoverLock` 功能实现（独立 `ai_agent_session_lock` 表，lease/TTL CAS acquire + stale-lock 抢占）+ `DefaultAgentEngine` 三入口点接线（`instanceId` + `sessionTakeoverLock` + `tryAcquire` before `putIfAbsent` + 全路径 `releaseLockQuietly`）+ `restorePendingSessions` `isHeld` 跳过增强。**定时扫描 daemon 已由 plan 222 落地**：`IRecoveryManager` 契约（`start`/`stop` 幂等 + `scanOnce → RecoveryScanResult`）+ `NoOpRecoveryManager` shipped 默认 + `ScheduledRecoveryManager` 功能实现（`IScheduledExecutor`（nop-commons）周期调度默认 60s，`scanOnce` = stale lock cleanup（幂等 DELETE）+ orphan session detection（LOG.warn））+ 引擎 `recoveryManager` 字段 + `setRecoveryManager` setter（部署层管理 start/stop）。RecoveryManager 其余能力（orphan 进程主动 liveness 检测 / 超时强制中止 / 归档清理 / 心跳自动续约）仍为独立 successor。**恢复模式策略已由 plan 226 落地**：`IOrphanRecoveryHandler` 可插拔策略 + `RecoveryMode`(RESUME/ABORT/SKIP) + `DefaultOrphanRecoveryHandler` 功能实现 + `ScheduledRecoveryManager` 集成（`setOrphanRecoveryHandler` setter + scanOnce 对每个 orphan 调 `handler.handleOrphan`，汇总到 `RecoveryScanResult.recoveryActions`）；RESUME 委托 `engine.restoreSession` fire-and-forget（takeover 锁防 double-execution），ABORT raw JDBC `UPDATE status='failed' WHERE STATUS IN ('running','pending')`（条件 WHERE），SKIP LOG.warn（shipped 默认 NoOpOrphanRecoveryHandler 零回归）；RETRY（从头重试）为独立 successor（需 `ISessionStore` reset 语义 + 工具副作用 idempotency 裁定）。
...
**实现状态（plan 207 ✅ 已落地 — 错误分类 + 重试；W2e-0..3 ✅ 已落地 — 错误规范化信号通路；W2e-4/W2e-5 ✅ 已落地 plan 2026-08-01-1505-1 — 账号回退链 + QUOTA/AUTH→FALLBACK；P2-REL ✅ FALLBACK 总步数上限）**：`LlmErrorClassifier`（按 `ARG_HTTP_STATUS` 把异常映射为 `ErrorClassification`：429→RATE_LIMITED、5xx/超时→TRANSIENT、4xx→NON_TRANSIENT、配额→QUOTA_EXCEEDED）+ `IRetryPolicy`（RETRY/STOP/FALLBACK 决策）+ `NoRetryPolicy`（shipped 默认，恒 STOP，零行为回归）+ `StandardRetryPolicy`（功能性 opt-in：最多 3 次指数退避 + **full jitter（MA6.3-AR-5 ✅，延迟在 `[0, min(baseDelay*2^attempt, maxDelay)]` 均匀随机）**，TRANSIENT/RATE_LIMITED 重试，QUOTA/AUTH→FALLBACK）。重试循环接线到 `ReActAgentExecutor` 的单次 LLM 调用点（`chatService.call`）。**W2e 更新**：`ChatServiceImpl` 非 2xx 不再抛异常，改为经 `dialect.parseErrorResponse`（消费 `<errorMappings>`，first-match-wins）规范化为携带 `errorClassification`/`retryAfterMs`/`httpStatus` 的错误 `ChatResponse`；`LlmCallCoordinator` 重试循环的 `!isSuccess()` 分支读 `errorClassification` 进入 RETRY 决策（不再一律终止）；`StandardRetryPolicy` 的 RATE_LIMITED 用 `retryAfterMs` 作 **floor**（`delay = retryAfterMs + uniform(0, jitterCap)`，永不低于 retryAfterMs，W2e-3 ✅），TRANSIENT 保持纯 full-jitter；`ErrorClassification` 已迁到 `io.nop.ai.api.chat`（nop-ai-api 最低层，信号通路全程同一类型）。Retry-After 多源（HTTP 头 + body 字段）经 `parseErrorResponse` 归一为 `retryAfterMs`。**W2e-4/W2e-5 ✅（plan 2026-08-01-1505-1）**：`StandardRetryPolicy` 对 `QUOTA_EXCEEDED`/`AUTH_INVALID` 返回 FALLBACK（design §6.8，从今日 STOP 改）；`LlmCallCoordinator` 的 FALLBACK 按 `errorClassification` 分流两通道——QUOTA/AUTH→**账号链**（provider 级 `<accounts>` 有序备用账号，经 `ChatOptions.accountKey`/`accountBaseUrl` 跨层下沉到 `ChatServiceImpl`，`AccountChain` 游走器推进，链耗尽 fail-loud design §6.9），TRANSIENT 等→`IModelRouter.getFallback`（模型 tier，行为不变）。两通道区分避免 QUOTA/AUTH 错误降级模型（design §4.4）。账号链是同 provider 内多账号；跨 provider 故障转移属 W2-4 successor。**P2-REL 防御性总步数上限**：每次 FALLBACK 切换（账号链 / 跨 provider failover / 模型 tier）都重置 attempt=0，因此单次 `doLlmCallWithRetry` 内的 FALLBACK 循环另设总步数上限 `MAX_FALLBACK_STEPS`（16，与 veto cap 3 / circuit scan 64 的显式 cap 风格一致）——自定义 `IModelRouter` A→B→A 循环或循环账号/故障转移链超限 fail-loud（抛 `NopAiAgentException`，不静默退出），LLM 调用次数 ≤ 上限 + 1。roadmap §4 L3-2 ✅、W2e-0..5 ✅。
...
**实现状态（plan 210 ✅ 断路器契约 + AlwaysClosed 默认 + ThresholdBreaker 功能实现；plan 213 ✅ 熔断感知路由解析；P2-REL ✅ HALF_OPEN 探针超时逃生；P2 round-4 ✅ 条目表收缩）**：`ICircuitBreaker`（`allowCall(modelKey)` / `getState(modelKey)` / `recordSuccess(modelKey)` / `recordFailure(modelKey)` 契约）+ `CircuitState`（CLOSED/OPEN/HALF_OPEN 三态枚举）+ `AlwaysClosed`（shipped 默认，恒 CLOSED、恒放行、record 为显式 no-op，零行为回归）+ `ThresholdBreaker`（功能性 opt-in：per-model-key 状态机——CLOSED 连续失败达 `failureThreshold`（默认 3）→ OPEN；OPEN 冷却 `cooldownMs`（默认 60s）后 lazy 转 HALF_OPEN；HALF_OPEN 单 probe 原子占位，成功→CLOSED 复位，失败→OPEN 重启冷却；**P2-REL 逃生门：探针在 `probeTimeoutMs`（默认 30s，构造器可配，0=禁用）内未回报（取消/hang/线程终止）时，下一次 `allowCall` 重新占位放行该 caller 为新探针（LOG.warn 记录，状态保持 HALF_OPEN，不产生双探针并发）**；**P2 round-4 条目表收缩：pristine 条目（CLOSED + 零失败计数 + 无 probe 在飞）与缺席等价，在归位同一次 per-key compute 临界区内移除，带失败记忆/OPEN/probe 在飞条目永不移除**；线程安全 via per-key `entries.compute` 临界区（同 `ConcurrencyRegistry` 模式）+ volatile state）。熔断检查接线到 `ReActAgentExecutor`（见下），失败记录接线到 retry 循环内 per-attempt（catch 入口处 `recordFailure`，在 FALLBACK 切换 model-key 之前）+ 非异常失败路径（`!response.isSuccess()`），成功记录在循环后（`recordSuccess` 复位）。`DefaultAgentEngine` 通过 field+setter+resolveExecutor 装配 circuitBreaker（默认 AlwaysClosed）。
...
**实现状态（plan 210 ✅ 已落地 — 三态状态机；W2 下沉 nop-ai-core，plan 2026-08-15-0604-2 ✅；P2-REL ✅ 探针超时逃生；P2 round-4 ✅ 条目表收缩）**：`ThresholdBreaker`（`io.nop.ai.core.reliability` 包，opt-in）实现 per-model-key（`provider:model` 复合键）三态状态机：CLOSED 连续失败计数达 `failureThreshold`（默认 3，构造器可配）→ OPEN；OPEN 经 `cooldownMs`（默认 60000ms，构造器可配）后 lazy 转 HALF_OPEN（无后台线程，下次 `allowCall` 时检查）；HALF_OPEN 单 probe 原子占位（首个 caller 放行作 probe，并发 caller 拒绝视为仍 OPEN），probe 成功→CLOSED 复位计数，probe 失败→OPEN 重启冷却；**P2-REL 逃生门：probe 在 `probeTimeoutMs`（默认 30000ms，构造器可配，0=禁用）内未回报（取消/hang/线程终止）时，下一次 `allowCall` 重占探针槽放行该 caller（LOG.warn 记录，状态保持 HALF_OPEN——探针永不回报不再永久卡死 HALF_OPEN）**。线程安全：全部状态迁移与条目移除在 per-key `entries.compute` 临界区内执行（与 `ConcurrencyRegistry` 同款 per-key 串行化模式，不再用 per-entry `synchronized`）+ `volatile state` 无锁诊断读。**P2 round-4 条目表收缩：pristine 条目（CLOSED + 零失败计数 + 无 probe 在飞）与缺席语义完全等价，在使其归位的同一次 compute 临界区内被移除——带失败记忆的条目（CLOSED 且 failures>0）、OPEN 条目、probe 在飞的 HALF_OPEN 条目永不移除；网关按任意 model 路由不再无界增长表**。状态 in-memory per-breaker-instance（跨 execute() 调用累积，持久化/跨进程共享是 Non-Goal successor）。circuit OPEN 时抛 `NopAiCoreException`（含 model-key + state + 指引，W2 前为 `NopAiAgentException`），不静默返回。动态阈值校准（滑动窗口失败率）是独立 successor（设计本节原注"阈值和冷却时间需要真实运行数据校准"——首版交付静态可配置阈值）。
...
**STUCK abort 语义**：`assessGoal` 返回 `STUCK` 时，设 `ctx.setStatus(escalated)` + `ctx.setLastError(...)` + `break reactLoop` + 发布 `SESSION_ESCALATED` 事件（plan 277 AR-07：不再复用 `SESSION_PAUSED`，escalated 是终态需人类重评估，paused 是可恢复的治理暂停），与 denial-ledger pause break 同级 governance-abort 模式。不静默继续、不吞异常信号。
