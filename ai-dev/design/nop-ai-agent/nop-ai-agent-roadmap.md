...
| xpl-bizfatal-wrap-preserve | **xpl 方法调用异常包装保留 bizFatal 标记（retry 分类 E2E 端到端修复，plan 249）**：闭合 plan 247 §Follow-up line 181 + §Closure line 162 已知限制「bizFatal 分类在 .task.xml E2E 路径不生效」——修复 `AbstractObjFunctionExecutable`（`nop-kernel/nop-xlang`）5 个 `doInvoke*` 方法（`doInvoke`/`doInvoke0~3`）catch 块无条件包装为 `NopEvalException(ERR_EXEC_INVOKE_METHOD_FAIL).forWrap()`、丢弃原 `NopException.bizFatal=true` 标记的缺陷。`RetryPolicy.isRecoverableException:152` 只检查传入异常自身的 `isBizFatal()`（不递归 `getCause()`），故包装后 bizFatal=false → 判定可恢复 → 退化为无条件重试至耗尽，而非 bizFatal 的立即 honest throw。修复 = 新增 private `wrapInvokeException(Exception e, Object obj)` helper：构造 `newError(ERR_EXEC_INVOKE_METHOD_FAIL, e).forWrap().param(...)`（保留 wrapException/cause/errorCode 全部既有语义，设计裁定 1 方式 A 保守拷贝标记），且当 `e instanceof NopException && isBizFatal()` 时 `err.bizFatal(true)`。5 个 `doInvoke*` catch 块统一委托 helper。非 bizFatal 异常包装行为零变更。blast-radius 评估：`isBizFatal` 消费方 = RetryPolicy（retry 分类，本计划结果面）/ RetryEngineImpl / NamedExceptionFilter / error 元数据传播（GraphQLResponseBean/ApiResponse/ErrorMessageManager/NopRebuildException）；`shouldRollback` 用 `isNotRollback()`（独立字段），**不查 `isBizFatal()`**，事务回滚不受影响。2 xlang focused 单元测试（`TestInvokeMethodBizFatalWrap`：bizFatal 5 重载穿透 isBizFatal()==true + 非 bizFatal 回归 isBizFatal()==false/isWrapException==true/cause 保留/errorCode 不变）+ 1 新增 E2E（`TestReliabilityDecorators.retry_bizFatalFailFastE2e`：`.task.xml` retry decorator → step 经 xpl 调用 `FailureSimulatorBean.throwBizFatal()` → doInvoke0 包装 bizFatal 保留 → TaskStepHelper.retry:138 → RetryPolicy.java:152 判定不可恢复 → delay<0 → 立即 honest throw 执行次数=1，修复前执行次数=1+maxRetryCount=3）+ nop-xlang 428 + nop-task-core 34 + nop-task-ext 25（含 18 decorator 测试）+ nop-ai-agent 2714 全量零回归。attr 读/写包装 bizFatal 穿透（ERR_EXEC_READ_ATTR_FAIL/ERR_EXEC_WRITE_ATTR_FAIL 同类缺陷独立代码路径）/ 其他 NopException 语义标记传播裁定 / error code 改变 / 跨重启 exception 持久化 均为显式 Non-Goals successor。 | L4-8 | ✅ |
| xpl-attr-bizfatal-wrap-preserve | **xpl 属性读/写异常包装保留 bizFatal 标记（retry 分类 attr/bracket 路径端到端修复，plan 250）**：闭合 plan 249 §Non-Goals:42 + §Deferred:146-151 + §Closure Follow-up:194「attr 读/写包装 bizFatal 穿透」——修复 `AbstractExecutable`（`nop-kernel/nop-xlang`，bracket 记法 `obj[attrExpr]` 路径）的 `readAttr`(`:146`)/`setAttr`(`:162`) catch 块无条件包装为 `NopEvalException(ERR_EXEC_READ_ATTR_FAIL/ERR_EXEC_WRITE_ATTR_FAIL).forWrap()`、丢弃原 `NopException.bizFatal=true` 标记的缺陷（plan 249 修复方法调用路径 doInvoke* 的 sibling，同类缺陷不同代码路径/不同 error code/不同 blast radius）。`GetAttrExecutable.execute:94`/`SetAttrExecutable.execute:80`（bracket 记法编译产物）调 readAttr/setAttr，dot 记法 `obj.prop` 走 readProp/setProp（独立 successor）。修复 = 新增 private `wrapAttrException(ErrorCode, Throwable, Object, Object)` helper（设计裁定 1 方式 B）：`.forWrap()` 包装后当 `e instanceof NopException && isBizFatal()` 时 `err.bizFatal(true)`（与 plan 249 `wrapInvokeException:78-85` 模式一致），保留 wrapException/cause/errorCode 全部既有语义；readAttr/setAttr 两处 catch 块统一委托 helper。非 bizFatal 异常包装行为零变更。blast-radius 评估（复用 plan 249 结论，同一标记同一传播机制）：`isBizFatal` 消费方 = RetryPolicy（retry 分类，本计划结果面）/ RetryEngineImpl / NamedExceptionFilter / error 元数据传播；`shouldRollback` 用 `isNotRollback()`（独立字段），**不查 `isBizFatal()`**，事务回滚不受影响。4 xlang focused 单元测试（`TestAttrBizFatalWrap`：readAttr bizFatal 穿透 isBizFatal()==true + readAttr 非 bizFatal 回归 isBizFatal()==false/isWrapException==true/cause 保留/errorCode=ERR_EXEC_READ_ATTR_FAIL + setAttr bizFatal 穿透 + setAttr 非 bizFatal 回归 errorCode=ERR_EXEC_WRITE_ATTR_FAIL；经 bracket 记法 `o['prop']`/`o['prop']=v` compileFullExpr 编译为 GetAttrExecutable/SetAttrExecutable）+ 1 新增 E2E（`TestReliabilityDecorators.retry_bizFatalAttrFailFastE2e`：`.task.xml` retry decorator → step 经 xpl **bracket 属性访问** `sim['bizFatalAttr']` → GetAttrExecutable → readAttr 包装 bizFatal 保留 → TaskStepHelper.retry:138 → RetryPolicy.java:152 判定不可恢复 → delay<0 → 立即 honest throw 执行次数=1，修复前执行次数=1+maxRetryCount=3；**Anti-Hollow 反向验证**：临时回退 bizFatal 拷贝后本测试 FAIL，恢复后 PASS）+ `FailureSimulatorBean.getBizFatalAttr`/`getRecoverableAttr` getter fixture + nop-xlang 432（plan 249 基线 428 → +4）+ nop-task-core 34 + nop-task-ext 26（TestReliabilityDecorators 19，plan 249 基线 18 → +1）+ nop-ai-agent 2714 全量零回归。dot/prop 路径 bizFatal 穿透（readProp/setProp 同类缺陷独立代码路径）/ 其他 NopException 语义标记传播裁定 / error code 改变 / 跨重启 exception 持久化 均为显式 Non-Goals successor。 | L4-8 | ✅ |
| xpl-prop-bizfatal-wrap-preserve | **xpl 属性读/写异常包装保留 bizFatal 标记（retry 分类 prop/dot 路径端到端修复，plan 251）**：闭合 plan 250 §Non-Goals:46 + §Follow-up:180「dot/prop 路径 bizFatal 穿透」——修复 `AbstractPropertyExecutable.readProp`/`setProp`（基类）+ `GetterGetPropertyExecutable.readProp` + `SetterSetPropertyExecutable.setProp` + `StaticGetterGetPropertyExecutable.execute` 共 **5 处 catch 块**无条件 `.forWrap()` 包装、丢弃原 `NopException.bizFatal=true` 标记的缺陷（plan 249/250 同类缺陷的 dot 记法 sibling，经 `IPropertyGetter`/`IPropertySetter` 反射调用，独立代码路径/不同 error code `ERR_EXEC_READ_PROP_FAIL`/`ERR_EXEC_WRITE_PROP_FAIL`）。dot 记法 `obj.prop` 编译为 `GetPropertyExecutable`（extends `AbstractPropertyExecutable`）→ 基类 `readProp`。修复 = 新增 protected `wrapPropException(ErrorCode, Throwable, String className, String propName)` helper（放 `AbstractExecutable`，因 `StaticGetterGetPropertyExecutable` 继承 `AbstractExecutable` 非 `AbstractPropertyExecutable`，设计裁定 1 方式 B 泛化）：`.forWrap()` 包装后当 `e instanceof NopException && isBizFatal()` 时 `err.bizFatal(true)`（与 plan 249 `wrapInvokeException` / plan 250 `wrapAttrException` 模式一致），保留 wrapException/cause/errorCode 全部既有语义；5 处 catch 块统一委托 helper。基类 setProp 重复 `.forWrap()` 收敛为单次。非 bizFatal 异常包装行为零变更。blast-radius 评估（复用 plan 249/250 结论，同一标记同一传播机制）：`isBizFatal` 消费方 = RetryPolicy（retry 分类，本计划结果面）/ NamedExceptionFilter / error 元数据传播；`shouldRollback` 用 `isNotRollback()`（独立字段），**不查 `isBizFatal()`**，事务回滚不受影响。4 xlang focused 单元测试（`TestPropBizFatalWrap`：readProp bizFatal 穿透 isBizFatal()==true + readProp 非 bizFatal 回归 isBizFatal()==false/isWrapException==true/cause 保留/errorCode=ERR_EXEC_READ_PROP_FAIL + setProp bizFatal 穿透 + setProp 非 bizFatal 回归 errorCode=ERR_EXEC_WRITE_PROP_FAIL；经 dot 记法 `o.prop`/`o.prop=v` compileFullExpr 编译为 GetPropertyExecutable/SetPropertyExecutable → 基类 readProp/setProp）+ 1 新增 E2E（`TestReliabilityDecorators.retry_bizFatalPropFailFastE2e`：`.task.xml` retry decorator → step 经 xpl **dot 属性访问** `sim.bizFatalProp` → GetPropertyExecutable → 基类 readProp 包装 bizFatal 保留 → TaskStepHelper.retry:138 → RetryPolicy.java:152 判定不可恢复 → delay<0 → 立即 honest throw 执行次数=1，修复前执行次数=1+maxRetryCount=3）+ `FailureSimulatorBean.getBizFatalProp`/`getRecoverableProp` getter fixture + nop-xlang 436（plan 250 基线 432 → +4）+ nop-task-core 34 + nop-task-ext 27（TestReliabilityDecorators 20，plan 250 基线 19 → +1）+ nop-ai-agent 2714 全量零回归。其他 NopException 语义标记传播裁定 / error code 改变 / 跨重启 exception 持久化 / bizFatal 文档化 均为显式 Non-Goals。**至此 doInvoke*（plan 249）+ readAttr/setAttr（plan 250）+ readProp/setProp（plan 251）三条 xpl 包装路径 bizFatal 穿透均已收敛。** | L4-8 | ✅ |

| task-state-lifecycle-noops | **nop-task `TaskStepStateBean` result/success 状态机实现 + retry 成功路径 succeed-driver wiring（plan 252）**：闭合 plan 248 §Non-Goals:41 + plan 247 §Non-Goals:42「succeed/isDone/isSuccess/result 完整生命周期 successor」——把 `TaskStepStateBean`（`nop-task-core`）5 个 no-op 生命周期方法（`succeed` / `isDone` / `isSuccess` / `result()` / `result(TaskStepReturn)`）从「空方法体 / 恒 false / 恒 null」（违反 Minimum Rules #24）收敛为「基于既有字段（`resultValue` 父类 / `stepStatus` 本类）的 runtime-driven 状态机」：`succeed(R,...)` = `setResultValue(R)` + `setStepStatus(COMPLETED=40)`；`isDone()` = null-safe 终态集 {COMPLETED=40, EXPIRED=50, FAILED=60, KILLED=70}；`isSuccess()` = `COMPLETED.equals(getStepStatus())`；`result()` = `resultValue==null ? null : RETURN_RESULT(resultValue)`（显式 null 返回避开 CONTINUE null 陷阱）；`result(T)` = `setResultValue(T.getResult())`。**Anti-Hollow 必需项**（独立审计 Round 1 Blocker 修复）：在 `TaskStepHelper.retry:175` 同步成功路径（plan 248 `return result;` 站点）接入 `state.succeed(result.getResult(), result.getNextStepName(), taskRt)` driver，与 `:177` fail-driver 对称。retry-wrapped step 同步成功后 stepStatus ACTIVE→COMPLETED 可观测（状态机在 retry 成功路径 runtime-driven 的 Proof）。设计裁定 7 项：基于既有字段不新增字段 / null-safe 派生 / succeed 设 resultValue+COMPLETED / result() null 陷阱 / fail 行为零变更（plan 247 一致）/ succeed-driver 接入 retry 同步成功路径 / how 属 execution。`fail()`/`exception()` 行为零回归（plan 247 裁定保持）。9 focused 单元测试（`TestTaskStepStateBeanLifecycle`：fresh null-safe / succeed 转移 / null-result null-trap / round-trip / fail 不终止 / 全终态覆盖）+ 1 新增 E2E（`TestReliabilityDecorators.retry_syncSuccessSucceedDriver_stateMachineObservable`：`.task.xml` retry decorator → sync 成功 step → succeed-driver 触发 → state.getStepStatus()==COMPLETED + isDone + isSuccess + result 非 null 可观测；`StateCapturingTaskStateStore extends DefaultTaskStateStore` 捕获 state 引用）+ nop-task-core 43 + nop-task-ext 28 + nop-ai-agent 2714 全量零回归。非 retry step succeed-driver / async 成功路径 driver / continuation-skip 消费侧（isDone/result 的 production reader，依赖 DB-backed state persistence successor）/ fail done 转移 / 终态失败路径 / nextStepId 持久化 / 跨重启持久化 / afterLoad-beforeSave / 完整 outputs 持久化 均为显式 Non-Goals successor。 | L4-8 | ✅ |

| succeed-driver-non-retry-and-async | **nop-task succeed-driver 对称扩展：retry async 成功路径 + 非 retry step 全部成功路径 wiring（plan 253）**：闭合 plan 252 §Non-Goals L52-53「非 retry step + retry async 成功路径 succeed-driver successor」——把 `TaskStepStateBean.succeed()` 状态机从「仅 retry 同步成功路径 runtime-driven」收敛为「retry-sync + retry-async + 非 retry step **全部**成功路径 runtime-driven」。Phase 1：在 `TaskStepHelper.doRetry` 公共 choke-point 成功分支注入 `state.succeed(...)`，覆盖全部 retry async 成功路径（primary thenCompose + scheduled-retry delay>0）；同时将 already-done async 快捷路径路由到 doRetry 使其成为唯一 async 成功 choke-point（DRY）。Phase 2（execution-time 裁定）：识别 `TaskStepExecution.executeWithParentRt` thenCompose 成功分支为所有 step 类型的公共完成点，在单一 wiring 点注入 `state.succeed(...)` DRY 覆盖全部目标 step 类型（Sequential/Selector/Loop composite + EvalTaskStep/BeanTaskStep simple）。6 focused E2E（retry async primary + retry delay>0 scheduled + sequential + selector + loop + xpl/simple）+ nop-task-core 43 + nop-task-ext 34 + nop-ai-agent 2714 全量零回归。 | L4-8 | ✅ |
