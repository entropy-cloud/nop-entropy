...
**状态**：active
**架构层**：Layer 2 (Execution Extensions)
...
本篇属于 Layer 2（Execution Extensions）。所有接口有 pass-through 默认实现。
...
Agent Engine Layer
   │
   │ 调用 IMessageFormat, IRouter, ITalent
   │
   ▼
...
`IRouter` 负责路由决策和回退判断。路由结果包含目标模型 ID、复杂度分级（simple/medium/complex）、按分级精简的工具集和调整后的 ChatOptions。回退判断根据错误类型决定是否切换到备选模型。
...
- `PassThroughRouter`（默认）——直连 AgentModel 配置的模型，无路由逻辑
- `SmartRouter`——可选的功能实现，使用便宜模型做 Judge 分类
...
## 八、Cache-First 三区域架构（Layer 2 辅助）
...
- `nop-ai-agent-react-engine.md` — ReAct 引擎（使用 IMessageFormat 和 IRouter）
- `nop-ai-agent-reliability.md` — 可靠性增强（IRetryPolicy 与断路器协同）
- `nop-ai-agent-roadmap.md` — 分层架构（本篇在 Layer 2 中的定位）
