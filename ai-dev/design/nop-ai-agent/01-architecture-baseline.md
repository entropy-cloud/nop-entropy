...
| 层级 | 内容 | 实现阶段 |
|------|------|---------|
| **短期记忆** | Context window 内的消息历史，含 5 层渐进 compaction（Layer 0-4） | Phase 1 |
| **Working Memory** | Per-session KV store，支持 JSON schema 验证或 Markdown 模板，session 启动时注入 system prompt，Agent 通过工具读写 | Phase 1-2 |
| **长期记忆** | IMessageService + 向量存储 + retain/recall/reflect 工具 + EdgeClaw 风格的 captureTurn/retrieve | Phase 3+ |
...
| 会话存储 | VFS `.nop/` 逻辑路径（Phase 1 文件后端，Phase 2+ DB 后端） | Event Sourcing + VFS，见 session-and-storage.md §4 §5 |
