...
- Plan 对外发布按 AGE 规范写入 `ai-dev/plans/`，与 VFS 内部存储分离
...
| 字段 | 用途 |
|------|------|
| `snapshotId` | 指向压缩前快照，审计时可确定性链接压缩前后状态 |
| `lastDiscardedEntryId` | 被丢弃的最后一条消息 ID，配合 `firstKeptEntryId` 界定被移除的范围 |
...
```
ai-dev/plans/{planId}-{title}.md
```

- 遵循 AGE（Attractor-Guided Engineering）目录约定
- 文件名：`{planId}-{title}.md`
- 格式为 Markdown，遵循 AGE Plan Template
