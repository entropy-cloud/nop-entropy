...
**日期**：2026-07-21（更新于 2026-07-21，Phase 1 + Phase 2 + Phase 3 定型）
**范围**：`nop-metadata` 模块 ORM 模型 + `nop-sys` 标签系统
**状态**：已实现（Phase 1 + Phase 2 + Phase 3），Phase 4 仍为草案
...
血缘传播是 Phase 4 增强特性。以下裁定基于 `NopSysEvent` 事件驱动 + `NopMetaLineageEdge` 血缘图。

> 裁定日期：2026-07-22

##### 3.3.3.1 触发机制

- **选型**：事件驱动（Event-driven）。监听 LineageEdge save/update 事件，通过 `NopSysEvent` 发布异步传播任务。
- **拒绝方案**：定时扫描 Job（轮询）。理由：轮询延迟不可控、产生不必要的数据库扫描、难以确定最优扫描频率。
- **补充用户触发路径**：提供 `propagateTags(entityType, entityId)` action，供用户显式调用传播。

##### 3.3.3.2 传播范围

- **首版范围**：仅沿 `transformType=DIRECT` 的 lineage edge 做 **一层（one-level）** 传播。方向从源端（source column）→ 目标端（target column）。
- **用户显式触发**：传播由用户调用 `propagateTags` action 显式触发，非 LineageEdge save 时自动执行。
- **拒绝方案**：递归多层传播。理由：首版不确定中间层标签的语义正确性，一层传播提供可预期的行为，用户确认后可逐层调用。

##### 3.3.3.3 防环策略

- **visited set**：每次传播任务的单次执行中使用内存 visited set 记录已处理的 `(entityType, entityId)` 对。
- **深度限制**：最大传播深度 ≤ 3 层。超过深度时记录 warn 日志，不报错中断。
- **已传播标签不重复传播**：目标端已存在相同 `(tagId, labelType=Propagated)` 的 TagLabel 时跳过；若已有 tag 的 `state=Confirmed` 且传播建议为 `Suggested`，不降级覆盖。

##### 3.3.3.4 传播出的 TagLabel 属性规格

| 属性 | 值 | 说明 |
|------|-----|------|
| `source` | `Classification` / `Glossary` | 继承自来源 TagLabel |
| `tagId` / `glossaryTermId` | 继承来源 | 指向同一 Tag 或 GlossaryTerm |
| `labelType` | `Propagated` | 标记为血缘传播 |
| `state` | `Suggested` | 建议状态，由 G2 审批流接管 |
| `reason` | `"propagated from {entityType}:{entityId} via edge {edgeId}"` | 记录来源和传播路径 |
| `appliedBy` | `"system"` | 系统自动传播 |
| `appliedAt` | 当前时间 | |
...
### Phase 2: Glossary — ✅ 已实现

新增实体：`NopMetaGlossary`、`NopMetaGlossaryTerm`
- GlossaryTerm → Classification Tag 自动传播（Path A: glossary term tags → TagLabel；Path B: asset tagged with glossary → derived TagLabel）
- 术语关系管理（parentTermId 自引用层级 + relatedTerms JSON broader/narrower/synonym）
- 自动传播引擎的 BizModel 实现（`NopMetaGlossaryTermBizModel.save`/`update` 中调用 `syncTagLabels`；`NopMetaTagLabelBizModel.save` 中调用 `propagateFromGlossaryTerm`）
- `NopMetaTag.fullyQualifiedName` 自动生成（根标签: `{classification.name}.{tag.name}`，子标签: `{parent.FQN}.{tag.name}`）
- `NopMetaTagLabel.glossaryTermId` FK 约束补充（to-one relation → `NopMetaGlossaryTerm`）
- `NopMetaSemanticType` 种子数据导入（9 个内置术语→`BuiltIn` Glossary + GlossaryTerm，触发于 `releaseModule`）
- 15 个集成测试覆盖 CRUD/FQN auto-gen/Propagation/Seed Data
- **依赖**：Phase 1
- **实现证据**：`nop-metadata.orm.xml` `<entities>` 段新增 2 个实体；codegen 生成 DAO/BizModel/Web；`NopMetaGlossaryTermBizModel.java`、`NopMetaTagLabelBizModel.java`、`NopMetaTagBizModel.java` 手写行为逻辑；`SeedGlossaryData.java` 手写 seed 逻辑；集成测试 `TestNopMetaGlossaryCrud`（4 tests）、`TestNopMetaTagBizModel`（4 tests）、`TestNopMetaGlossaryTermPropagation`（6 tests）、`TestSeedGlossaryData`（1 test）
...
#### Phase 4-A: 血缘驱动 TagLabel 自动传播

设计裁定见 §3.3.3。实现路径：
- `NopMetaTagLabelBizModel` 新增 `propagateTags(entityType, entityId)` action
- 传播引擎查询 `NopMetaLineageEdge` 表获取 `transformType=DIRECT` 的边
- 按 §3.3.3.1 ~ §3.3.3.5 的契约创建 `labelType=Propagated` 的 TagLabel
- 异步触发路径通过 `NopSysEvent` 发布事件（首版仅提供同步 action，异步为 follow-up）

#### Phase 4-B: AutoClassification 引擎（规则引擎 only）

> 裁定日期：2026-07-22

##### 首版 scope — 规则引擎 only

- **选型**：仅基于规则的分类建议（column name 模式匹配 + field type 启发式），**不引入 ML**。
- **拒绝方案**：ML-based 分类。理由：首版不确定训练数据质量和模型集成成本，模式匹配规则在初始阶段即可覆盖 80% 常见场景（如 `phone|mobile|tel|电话` → PII.Contact.Phone）。
- **拒绝方案**：全自动识别（trigger on schema change）。理由：自动识别产生大量噪声标签，应走用户显式触发 + G2 审批流确认。

##### 规则格式（`autoClassificationConfig` JSON schema）

`NopMetaClassification.autoClassificationConfig` 字段的 JSON schema：
...
```json
{
  ...
  "properties": {
    ...
    "conflictResolution": { "type": "string", "enum": ["skip", "overwrite", "manual"] },
    "minimumConfidence": { "type": "number", "minimum": 0, "maximum": 1 },
    "requireExplicitMatch": { "type": "boolean" },
    "rules": {
      "type": "array",
      "items": {
        "type": "object",
        "properties": {
          "pattern": { "type": "string", "description": "正则或通配模式" },
          "matchOn": { "type": "string", "enum": ["columnName", "fieldType", "columnNameOrType"], "default": "columnName" },
          "tagFQN": { "type": "string", "description": "匹配时建议的 Tag fullyQualifiedName" },
          "priority": { "type": "integer", "description": "优先级（数字越小优先级越高，0 为最高）" }
        },
        "required": ["pattern", "tagFQN"]
      }
    }
  }
}
...
```json
{
  ...
  "rules": [
    {"pattern": "phone|mobile|tel|电话", "tagFQN": "PII.Contact.Phone", "priority": 1},
    {"pattern": "email|e-mail|邮箱", "tagFQN": "PII.Contact.Email", "priority": 1},
    {"pattern": "id_card|身份证|ssn", "tagFQN": "PII.NationalID", "priority": 1},
    {"pattern": "name|姓名|user_name", "matchOn": "columnNameOrType", "tagFQN": "PII.PersonName", "priority": 2}
  ]
}
```
...
- **用户主动调用**：提供 `suggestTags(entityType, entityId)` mutation，用户按需触发生成建议标签。
- **拒绝自动触发**：不在表/字段同步后自动触发生成。理由：避免大量不准确的建议标签淹没资产视图。

##### 产出物规格

- 每条匹配规则生成一条 `TagLabel{source: Classification, labelType: Automated, state: Suggested}`。
- `reason` 字段记录匹配的 recognizer 信息（`"matched by rule: {rule.pattern} on {matchOn}"`）。
- 所有 `Automated` 标签进入 `Suggested` 状态，由 G2 审批流接管（submitForApproval）。
- 去重：已有 `(entityType, entityId, tagId, labelType)` 组合的不重复创建（labelType 包括 Manual/Propagated/Derived/Automated 均参与去重）。

#### Phase 4-C: GlossaryTerm 外部词汇表同步（裁定 — 推迟）

> 裁定日期：2026-07-22

- **裁定结论**：**首版推迟实现**。不提供 SKOS/RDF 导入导出 action。
- **理由**：
  1. `NopMetaGlossary.namespaces`、`NopMetaGlossaryTerm.conceptMappings`、`NopMetaGlossaryTerm.iri` 等 SKOS/RDF 字段已建模（§3.2.3、§3.2.4），可作为存储容器，无格式转换压力。
  2. 当前无用户或集成场景明确要求 SKOS/RDF 导入导出。`conceptMappings` 和 `iri` 字段可在首版中手工录入，无需批量同步。
  3. 同步逻辑（RDF 解析 / SKOS 往返转换 / URI 冲突检测）在不确定需求时过度设计。
- **注册为 `watch-only residual`**：见 `01-architecture-baseline.md` §八 Follow-up 登记表。
