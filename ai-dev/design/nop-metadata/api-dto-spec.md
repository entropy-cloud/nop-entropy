...
- DTO 放在 `nop-metadata-api/.../dto/`（共享模块，dao 层 I*Biz 接口和 service 层 BizModel 均可引用；全部 31 个 `@DataBean` 位于 `io.nop.metadata.api.dto` 包，R3.8 移除 AggregationRowDTO 后）。
- 共享 DTO（如 `ErrorDTO` / `KeyValueDTO`）也放在 `nop-metadata-api/.../dto/`（统一定位）。
- 字段命名与原 Map key 完全一致，便于迁移期前后兼容对照。
- 每个方法均以 DTO 返回（无 Map overload 保留——全部方法仅 DTO 返回；`@DataBean` 注解由 `io.nop.api.core.annotations.data.DataBean` 提供）。
...
- 每个 BizModel 方法均以 DTO 返回（无 Map overload 保留）。方法以 `@BizQuery` / `@BizMutation` 直接暴露给 GraphQL（xbiz 自动生成 schema 时 DTO 字段就是 GraphQL field，前端可用 selection 下推）。
