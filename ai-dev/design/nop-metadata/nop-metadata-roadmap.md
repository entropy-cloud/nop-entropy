...
- P4-dc-2. 跨库 JOIN 聚合（全端点组合：entity↔entity / external↔external / 混合）— 复用 executeJoin + 内存 GROUP BY: `planned`（plan 1500-2，design-first：§4.4.2 内存 GROUP BY 契约（精确-当-容纳/超限-失败）+ 合并行按端点命名空间取值；收口 0852-1/1200-1/0700-2 跨库 JOIN 聚合 deferred 项）
