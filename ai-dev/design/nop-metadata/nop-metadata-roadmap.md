...
> Last Updated: 2026-07-18 (queryAggregation having/orderBy 增强 completed via plan 2026-07-18-0900-2：queryAggregation 签名扩展 +having TreeBean +orderBy List<OrderFieldBean>（offset 之后）；FilterToSqlTranslator 新增 translate(filter, fieldResolver) 重载（既有 translate(TreeBean) 行为不变）；三条路径一致支持——entity/external-sql 单表 + JOIN 同库 3 条 SQL 生成 HAVING/ORDER BY 子句 + 跨库内存 MemoryFilterEvaluator/MemoryOrderByComparator；name 反查表从 loadMeasures/loadDimensions 原始实体列表构建，未知 name 显式失败（不静默跳过）；向后兼容 having=null+orderBy=null 零行为变化；§4.4.2 D11 落地；收口 0852-1/1200-1/1500-1/1500-2 Non-Blocking Follow-up「聚合 having/排序增强」项; 395 tests, 0 failures)
