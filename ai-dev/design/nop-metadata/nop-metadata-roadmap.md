...
> Last Updated: 2026-07-18 (entity 路径时间维度 granularity 分桶补齐 plan 2026-07-18-1100-2 落地：§4.4.2 D7 收口原 entity 路径 follow-up「granularity 暂不下沉到 SQL」，新增 D7.1 entity 路径 granularity 下沉机制裁定（候选 a 选定：bypass EQL 经 TableReferenceExecutor 平台 JDBC Connection 直查物理 SQL，复用 GranularityBucketing.translate 同 external/sql 路径；拒绝候选 b EQL 已知函数覆盖度不足 + 候选 c 内存分桶违反 D6 pushdown；澄清与 §4.4.1 D1.5 拒绝候选 B 区别——单表 entity 聚合物理表天然在平台库，混合端点拒绝理由不适用；D6 表格 entity 行补 bypass EQL 说明；MetaQueryContext 暴露 tableRefExecutor()；executeEntityAggregation 检测 temporal+granularity 时改走 bypass 路径，否则维持 orm().executeQuery EQL 路径（向后兼容）；5 新增 e2e 测试覆盖 entity 路径各 granularity 分桶 + entity vs external 一致性 + 失败路径；400 tests)
...
- Opt-3. expression 型 Measure 表达式语言设计与执行契约（design-first）: `planned`（plan 2026-07-18-1100-1，§4.4.2 D12 裁定：D12.1 表达式语言=方言原生 SQL 片段（拒绝 EQL/平台表达式引擎/混合方案及理由）；D12.2 三条路径执行契约（entity bypass EQL 经平台物理 JDBC Connection 入口 + external-sql withConnection + 跨库内存首版显式失败对齐 D10）；D12.3 安全模型（标识符白名单 + 值参数绑定 + 拒绝危险关键字/DDL/DML/副作用函数）；D12.4 失败路径 4+1 类 ErrorCode 候选；D12.5 save-time 校验裁定 + VARCHAR(1000) 容量约束；实现属 successor plan，5 处 ERR_AGGR_EXPRESSION_MEASURE 抛点维持不变）
