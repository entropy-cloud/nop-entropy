...
> Last Updated: 2026-07-18 (expression 型 Measure 表达式语言设计与执行契约 design-first plan 2026-07-18-1100-1 落地：§4.4.2 新增 D12（D12.1 表达式语言选型=方言原生 SQL 片段，含拒绝 EQL/平台表达式引擎/混合方案及理由；D12.2 三条路径执行契约：entity bypass EQL 经 `TableReferenceExecutor.java:73-83` 平台物理 JDBC Connection 入口 + external-sql withConnection+白名单+参数绑定 + 跨库内存首版显式失败对齐 D10；D12.3 安全模型：parse 阶段关键字/函数黑名单 + 标识符白名单 §2.7.1 D3 + 值参数绑定 FilterToSqlTranslator 模式；D12.4 失败路径 4 类 + 容量超限 ErrorCode 候选 aggr-expression-unparseable/-unsafe/-dialect-unsupported/-memory-not-computable/-too-long；D12.5 save-time 校验裁定+VARCHAR(1000) 容量约束；§4.4.2 D6 过渡说明更新指向 D12；§八 评估结论 新增 1 项 follow-up（expression 输出列血缘）；实现属 successor plan；5 处 ERR_AGGR_EXPRESSION_MEASURE 抛点维持不变至 successor 落地)
