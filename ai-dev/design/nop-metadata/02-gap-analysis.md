...
| Delta 版本管理 | ✅ isDelta + baseModelId 链 | 已设计 |
| 模块发现 | ✅ 配置式/自动扫描 | 已设计 |
...
**其他平台做法:**
- **DataHub**: DNF 策略组 (ConjunctivePrivilegeGroup + DisjunctivePrivilegeGroup)
- **OpenMetadata**: RBAC + 策略评估
- **Atlas**: Ranger 集成

**建议补充:**
```
MetaAccessPolicy                 — 访问策略（预留）
  ├── policyName
  ├── entityType                 — 适用的实体类型
  ├── condition                  — 条件表达式
  ├── privileges[]               — 允许的操作
...
利用 Nop 已有的权限框架 (`nop-auth`)，在 API 层集成。

---

### Gap 7: 数据源注册 (外部表) ⚠️

**现状:** Vision 中提到 Phase 2 支持外部数据源，但 01-architecture-baseline.md 中未定义数据源模型

**其他平台做法:**
- **DataHub**: DataPlatform + 数据源连接器
- **OpenMetadata**: DatabaseService → Database → DatabaseSchema → Table
- **Atlas**: Service → Database → Table 层级

**建议补充:**
```
MetaDataSource                    — 外部数据源
  ├── sourceName / displayName
  ├── sourceType                 — MYSQL | POSTGRESQL | CLICKHOUSE | HIVE
  ├── connectionConfig           — JSON 连接配置
  ├── syncStrategy               — FULL | INCREMENTAL
  ├── lastSyncAt
  └── status                     — ACTIVE | ERROR

MetaDataSourceTable              — 外部源同步的表
  ├── dataSourceId               → MetaDataSource
  ├── remoteTableName
  ├── syncedAt
  └── syncedSchema               — JSON schema 快照
```
...
**现状:** 设计中未定义 API 层（GraphQL / REST）

**其他平台做法:**
- **DataHub**: GraphQL (4100+ 行 GmsGraphQLEngine) + DataLoader 批量加载
- **OpenMetadata**: JAX-RS REST API + 异步端点
- **Marquez**: Dropwizard REST + GraphQL beta

**建议:**
- 利用 Nop 的 GraphQL 能力 (`nop-graphql`)，基于 MetaEntity 自动生成 CRUD API
- 参考 OpenMetadata 的 EntityRepository 模板方法模式
...
| **P2** | Gap 6: 访问控制 | Phase 2 集成 nop-auth |
| **P1** | Gap 7: 数据源注册 | Phase 2 核心功能 |
| **P1** | Gap 8: API 层 | Phase 1 必须实现，支撑前端交互 |
...
### 修改 2: 01-architecture-baseline.md 补充数据源模型
...
3. **Delta 版本管理**: 独特的 Delta + Full 双存储模式，支持模块间继承和版本对比（其他平台无此设计）
