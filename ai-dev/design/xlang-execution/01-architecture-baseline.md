...
graph LR
    ...
    XJ --> NJ["nop-javac<br/>(构建期 Java 源编译通路)"]
...
1. **资源产生时机**：构建期静态资源（`_vfs` 内可扫描的 xpl / xlib / expr / xbiz 等）vs 运行时动态源（字符串表达式、规则配置产物）。
...
4. **配置开关**：全局默认后端、按后端启用开关、强制解释器模式（诊断用）。

### 判定时机（三个）

| 时机 | 动作 | 归属 |
|---|---|---|
| 构建期 | codegen/xgen 任务扫描 `_vfs/**/*.xpl` / `*.xlib` / `*.expr` / `*.xbiz` → java 后端转译产出 `_gen/` 源码，随应用常规编译 | `nop-xlang-java` 构建任务（见 java 组架构 §六） |
| 模型加载期 | 编译单元经差量合并、Executable 树固化后，绑定可执行体：生成类优先 / 解释器兜底 | 模型加载层 + java 后端注册表 |
| 运行时求值期 | 动态脚本编译为 Executable 树后，按后端可用性裁决执行体 | 选择机制裁决入口 |
