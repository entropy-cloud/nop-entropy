...
> **W11-impl Part B 裁定标注（2026-08-17 回写）**：**引擎内部通道豁免授权记录检查**——`CredentialProviderImpl` 的 `engineGetDecryptedFields`/`engineUpdateTokenFields`/`engineUpdateInLock` 不查 `NopCredentialAuth`（判定矩阵仅串联于 `getCredential`/`getCredentialData` 明文出口）。理由：(a) 明文不外泄——`beginOAuthFlow` 只返回授权 URL（clientId 无 secret）、publicAccess 回调返回跳转页不含 token、`saveCredential` 路径有写分级门控；(b) 入口动作已被 W11 Part A 归属/管理员判定门控（写类矩阵 + W9 发起动作回补）；(c) 与矩阵第 5 行服务级信任边界一致（收紧针对"人"的冒用，不针对服务代码与已门控入口）。**两个用户上下文可达调用点显式登记为裁定覆盖对象**（live 可达性事实，非"GraphQL 不可达"）：`CredentialOAuthApiBizModel.beginOAuthFlow` → `OAuthFlowService`（engineGetDecryptedFields 经其可达，登录态用户上下文在线程上）、`NopCredentialBizModel.saveCredential`（oauth2 分组写经 engineUpdateInLock 可达）。`getCredential` 内惰性刷新发生在授权检查通过之后（锁下重读不重复检查）。测试固化：引擎通道在"授权记录存在 + 未命中用户上下文"下仍放行（`TestCredentialProviderRbacAuth.engineGetDecryptedFieldsExemptFromAuthCheck`/`engineUpdateChannelsExemptFromAuthCheck`）。
...
> **W11-impl Part B 裁定标注（2026-08-17 回写，管理面载体）**：**授权管理面载体 = 独立 `NopCredentialAuthBizModel`**（本节留白的 impl 裁定）——对齐 `NopCredentialUsageBizModel` 先例（实体自有查询面 + admin-only 双层：action-auth 收紧 admin + 运行时 `requireCredentialAdmin` 判定，无登录态同样拒绝），避免继续膨胀 787 行的 `NopCredentialBizModel`；grant/revoke 挂该 BizModel，查询动作（findPage/findList/findFirst/findCount/get/batchGet）同 admin-only 语义（非行过滤）。标准 `save`/`update`/`delete`/`batchDelete`/`updateByQuery`/`deleteByQuery`/`copyForNew` 禁用（抛 `UnsupportedOperationException`，Part A 六动作同口径；`delete` 亦禁用——revoke 为唯一删除通道、幂等契约所在）；bean 注册零手工动作（实体 BizModel 条目由 codegen `_service.beans.xml` 自动生成，动作在 retention 骨架上声明即被 GraphQL 发现）。roleId 不做存在性校验的语义已由测试固化（空 roleId 在 GraphQL 入口被 `field-empty-arg` 先序拒绝、BizModel 层 `role-id-required` 守卫直调路径，死 roleId 求交永不命中原样展示）。**级联清理实现裁定**：`cascade-delete` biz 级联 tag 的平台机制是经子 BizModel 的 `delete` 动作逐行调用（`CrudBizModel.deleteReferences → queueCascadeDelete`），与"子 delete 禁用"互斥——授权行的删除级联改由父侧 `NopCredentialBizModel` 删除回调内经 dao 显式物理清理（在 usage 引用计数拦截之后：删除被拦截时授权行与凭证同存属正常语义），`auths` 关系保留 `cascadeDelete="true"`（ORM flusher 对已加载集合的级联）但 tagSet 改 `not-pub`（不经 biz 级联、不暴露 GraphQL selection 面）。
