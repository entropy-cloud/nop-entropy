...
> Status: **superseded** — 本文档描述的 `flowVars` + `requiredVars` + `foreach` 独立步骤类型方案被 `flow-engine-design.md` v4 替代
> Last Reviewed: 2026-06-10
> Migrated from: `ai-dev/tools/opencode-goal-driver/DESIGN-context-passing-and-foreach.md`
> Parent: [flow-engine-design.md](./flow-engine-design.md)
> Superseded by: `flow-engine-design.md` §2.2 `extractVars` + §2.5 `subflow` (forEach/单次执行模式)
> Reviewers: (round 1 complete — see §8 for review log)
>
> **与 v4 的关键差异**：
> - v4 使用 `stepDef.extractVars` (regex 从 agent 输出提取变量) 替代本方案的 `<FLOW_VARS>` XML 块
> - v4 使用 `type: "subflow"` + `forEach` 字段替代本方案的 `type: "foreach"` 独立步骤类型
> - v4 的 subflow 通过 `delegates.loadSubFlow(name)` 动态加载，支持 forEach 和单次执行两种模式
> - 本方案的 `requiredVars` + session-continue repair 机制**未实现**，v4 未引入等价功能
