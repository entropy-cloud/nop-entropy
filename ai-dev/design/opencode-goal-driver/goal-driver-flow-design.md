...
**日期**：2026-06-16（step 名称简化：HEALTH_CHECK→CHECK, EXECUTE_ALL_ACTIVE_PLANS→PLANS, ROADMAP_CHECK→ROADMAP, PLAN_DRAFT→DRAFT, DEEP_AUDIT_LOOP→AUDIT）
...
2. **AI 修复前置**：`CHECK` 是 AI agent 步骤，可在编译失败时自动诊断修复，**支持无上下文重试**（每次 fresh session）
3. **PLANS 用 group + forEach**：`scan-active-plans` 脚本扫描所有活跃计划写入 `items`，`EXECUTE_EACH_PLAN` 用 `forEach` 对每个计划执行 `plan-execution` 子流程。不循环——主流程从 `ROADMAP` 或 `AUDIT` 返回后重新扫描
4. **深度审计流程**：`deep-audit-loop` 子流程三步骤（DEEP_AUDIT → ADVERSARIAL → DRAFT_PLANS），在子流程内完成审计和计划创建，不包含执行（执行由主流程的 `PLANS` 处理）
5. **Plan Draft 合并审计（v5）**：`DRAFT` 合并了拟制与独立子 agent 审计，单步输出 `created`+PLAN_FILE。审查循环（最多 2 轮，独立 session sub-agent）在步内完成，移除引擎级 PLAN_AUDIT 步骤及其 retry/onMaxRetries 转移
6. **深度审计出口**：`AUDIT` `clean` → `PLANS`（审计可能创建新计划，需要重新执行）
7. **子流程 marker 透传**：子流程返回内部最后一步的实际 marker，而非硬编码 `"complete"`/`"failed"`
8. **容错兜底路由到 PLANS**：所有子流程和 agent 步骤的意外失败都回到计划执行，不中断整体循环
...
v3 改用了堆栈嵌套：`PLANS` 是一个自包含的循环步骤，执行时遍历所有活跃计划逐个调用 `plan-execution` 子流程，执行完毕才返回。这类似于函数调用堆栈——"push 执行帧，pop 执行帧"。
...
```mermaid
flowchart TD
    HC["CHECK<br/>AI agent"] -->|pass| EAP["PLANS<br/>group"]
    HC -->|fail ×3| FAILED

    subgraph EAP_G["PLANS (group, maxRounds=1)"]
    ...
    EAP -->|done| RC["ROADMAP<br/>AI agent"]

    RC -->|pending| PD["DRAFT + self-audit<br/>AI agent"]
    RC -->|complete| DAL["AUDIT<br/>subflow: deep-audit-loop"]
...
**CHECK**（`ai-dev/tools/opencode-goal-driver/prompts/health-check.md`）
- AI agent 步骤，运行 `mvnw clean install`，失败时 AI 自行诊断并修复
- `fail` transition 配置了 `retry: CHECK, maxRetries: 3`，无 append buffer，因此每次重试是**空上下文 + 新 session**
- 仍失败时通过 `onMaxRetries` 结束

**PLANS**（`type: "group"`，`maxRounds: 1`）
...
- 不循环——主流程从 `ROADMAP` 创建的 plan 或 `AUDIT` 创建的计划返回后重新扫描
- 见 §四

**ROADMAP** → **DRAFT** → **PLANS**
- 标准 roadmap → 拟制+自审计 → 执行链路
- `DRAFT` 合并了拟制与审计（v5）：agent 先写计划，然后切换为 coordinator 模式，spawn 独立子 agent 对抗性审查（最多 2 轮），通过 pass criterion（zero Blocker/Major）后输出 `created`
...
**AUDIT**（子流程 `deep-audit-loop.json`）
- 见 §六

**退出机制**
- `PLANS` `done` → `ROADMAP` `complete` → `AUDIT` `clean` → `PLANS` → ...
- 引擎的 `maxCycleVisits`（默认 30）或 `maxTotalSteps`（默认 500）触发自然终止
- 当所有活跃计划执行完毕、roadmap 无待办、深度审计无新问题时，循环空转直到 `maxCycleVisits`
- `PLANS` 是 group（`maxRounds: 1`），没有内部循环——重新扫描靠主流程从 `ROADMAP`/`AUDIT` 返回

---

## 四、SCAN_PLANS + forEach 模式

`PLANS` 是一个 `type: "group"` 步骤（`maxRounds: 1`），内部包含两个子步骤：
...
4. 计划创建后子流程完成，返回 `clean`（主流程的 `PLANS` 会拾取新计划执行）
5. 不在子流程内循环——审计闭环由主流程的 `PLANS` 循环保证（执行完审计创建的计划后，再回到 AUDIT 重新审计）
...
| 步骤类型 | 返回 marker | 消费步骤 |
|---------|------------|---------|
| `PLANS` (group) | `"done"`（SCAN_PLANS 无活跃 plan 或执行完毕） | 主流程 → `ROADMAP` |
...
| `deep-audit-loop` (subflow) | `"clean"`（ADVERSARIAL 通过时） | `AUDIT` |
| `deep-audit-loop` (subflow) | `"failed"`（onError 时） | `AUDIT` |
...
| **子流程失败传导** | 子流程内部 `no_transition` → 子流程 `failed` → 父 transition `failed→PLANS` | 子流程内任何步骤的意外输出不会卡死；父流程路由到计划执行循环继续 |
| **onError 兜底** | 对 agent/subprocess 被杀（`ok:false`），指向 `PLANS` | 进程级故障也能恢复 |

### 子流程故障传播示例

```
DRAFT (deep-audit-loop 子流程内)
  ...
  → 父步骤 AUDIT: marker="failed"
  → transition: failed → PLANS
  → 流程继续（无活跃 plan，回到 ROADMAP）
```

### 子流程注意事项

- `PLANS` 是 `type: "group"`（内联步骤，无隔离 flowVars）；`EXECUTE_EACH_PLAN` 和 `AUDIT` 是 `type: "subflow"`
...
| `execute-all-active-plans` 子流程保持内部循环 | 子流程内 PLAN_ROUTER ↔ EXECUTE_PLAN 来回跳转，违背"堆栈嵌套"语义；改用 group + forEach 一次执行所有活跃计划，外部循环（ROADMAP/AUDIT）负责重新扫描 |
...
| 把 `plan-execution` 逻辑内联到主流程 | 两个地方（PLANS + deep-audit-loop）都用到，抽成子流程避免重复 |
