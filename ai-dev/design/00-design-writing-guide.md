...
### 图表选型：Mermaid vs 伪代码 vs 线框图

设计文档中需要图表时，按下表选择合适的表达方式。可以混合使用。

| 表达方式 | 适用场景 | 不适用场景 | 原因 |
|----------|---------|-----------|------|
| **Mermaid** | 拓扑结构（状态机、组件关系、数据流方向、模块依赖） | 算法逻辑、条件判断细节 | AI 读 Mermaid 原始文本而非渲染图，拓扑关系的文本表示 Mermaid 比 ASCII 更易维护 |
| **伪代码** | 算法逻辑（执行顺序、条件分支、变量赋值、错误处理链） | 组件关系、状态机拓扑 | 伪代码精确表达条件判断和执行顺序，Mermaid 的 edge label 是简写，丢失条件细节 |
| **线框图/ASCII art** | UI 界面布局（行列、分栏、嵌套区域的空间关系）；目标环境不支持 Mermaid 的场景 | 流程图、状态机、算法逻辑 | UI 布局是空间关系，Mermaid 无法表达"左栏 240px、右侧主内容区"，ASCII 线框图天然表达行列嵌套 |

**选型判断标准**：这段内容的关键信息是**谁连着谁**（→ Mermaid）还是**先做什么再做什么**（→ 伪代码）？

| 示例 | 关键信息 | 选型 |
|------|---------|------|
| 状态机：哪些状态之间有跳转 | 拓扑 | Mermaid `flowchart` 或 `stateDiagram-v2` |
| 组件依赖图：模块间的依赖方向 | 拓扑 | Mermaid `flowchart LR` |
| 数据流：数据从哪里来、到哪里去 | 拓扑 | Mermaid `flowchart` |
| 执行循环：先检查超时、再执行、再提取结果 | 算法 | 伪代码 |
| 错误处理链：catch → onError → retry → onMaxRetries | 算法 | 伪代码 |
| 数据转换规则：输入格式 → 中间结构 → 输出格式 | 算法 | 伪代码 |
| 模块分层架构 | 拓扑 | Mermaid `blockDiagram` 或 `flowchart` |
| API 调用时序 | 拓扑 + 时序 | Mermaid `sequenceDiagram` |
| UI 界面布局 | 空间关系 | ASCII 线框图 |

**混合使用建议**：同一个设计文档中，流程图用 Mermaid（§6），执行逻辑用伪代码（§4），各用最合适的表达方式。
