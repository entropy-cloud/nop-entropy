...
| G36 | state | 缺少 BroadcastState 类型 | Gap/P2 | 04-state: #1 | ✅ Closed (Stage 36, 2026-08-04) — **permanently excluded**（永久排除，不引入专用 BroadcastState 类型）。裁定：vision §七「去除：广播流」+ §四 Non-Goal「broadcast join」排除理由成立（复杂度极高、用例有限、CEP/lookup 替代）；无 demonstrated user need；operator state `BROADCAST` 重分布（`RedistributionMode.BROADCAST`，`MemoryOperatorStateBackend`，`TestE2EOperatorStateRedistribution`）已部分覆盖配置流分发用例。裁定证据见 plan `ai-dev/plans/nop-stream-production/2026-08-04-2200-2-final-gap-adjudication-g36-g66-g67.md`；vision §七裁决记录 #G36 |
