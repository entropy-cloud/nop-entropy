...
**定理 2（传输复合）**。若 `(G, T_G)` 与 `(H, T_H)` 是态射（A_X → A_Y → A_Z），则 `(H∘G, T_H∘T_G)` 是态射。恒等生成器以恒等传输为态射。故差量 actegory 与可传输生成器构成范畴 **ΔAct**。

证明（组合的等变性链，一步 T_G、一步 T_H）：

$$
(H\circ G)(s \oplus d) \;\approx\; H\big(G(s) \oplus T_G(d)\big) \;\approx\; H(G(s)) \oplus T_H(T_G(d))
$$

即 `T_{H∘G} = T_H ∘ T_G`；由引理 1 的忠实性约定，此传输唯一。∎
...
| Cordis effect | 仅可达差 | ✓（disposer） | 按坐标（其 Thm 40） | ✓ | DSH 运行时结构空间 |
