...
- 2026-08-28: check2 处置战役（plan346）**Phase 3 收口——全部 26 份报告 447/447 条处置完毕**。08-26 已完成的三单元回填勾选：nop-biz（12修+1暂缓，e3c2e4145a）、nop-graphql（15 全修，8a15ebccc7）、nop-auth（9修+6暂缓，72e7cc0ae3）。08-28 五单元并行子代理处置 + 主会话验收：nop-sys（10修+2复查非问题+1暂缓）、nop-wf（13修+2暂缓）、nop-batch（11修+9复查已修复）、nop-dyn（5修+11复查已修复）、nop-metadata（13 全修，owner doc 安全契约节补 4 条）。主会话 23 模块 reactor 独立复跑验证全绿，期间暴露并修复 1 处跨模块回归：nop-sys "死代码" claimNonBroadcastEvents/processClaimedNonBroadcastEvent 实被 nop-batch-sys 的 non-broadcast-consumer.batch.xml 以 XLang 调用（Java 向 grep 漏检 DSL 调用点，审计与处置复核均漏检），恢复方法并修正该条终态为复查非问题——方法论教训记 plan346 follow-ups。全系列终态分布：已修复 387 / 复查非问题 27 / 裁定暂缓 29 / 裁定不修复 5（449 条标注 = 447 发现 + 2 超条目处置）。
