...
第十二批批量修复（plan `ai-dev/plans/2026-08-01-0936-1-arm-p3-error-handling-agent-shell.md`，MA3.4 审计点名的 deferred successor 重开，同批 0936-2/0936-3 兄弟计划并行）已执行并收口。P2-MA3-1、P3-MA3-1 全部 `fixed`（裸 IAE → NopException + 模块 ErrorCode，消息逐字零漂移，仅异常类型/错误码变化）；0936-2（plan `ai-dev/plans/2026-08-01-0936-2-arm-p3-error-handling-core-coder.md`）同步收口，P3-MA3-2/P3-MA3-3 全部 `fixed`（core 7 处 + coder 4 处 → NopException + 模块 ErrorCode，英文描述，消息逐字零漂移，仅异常类型/错误码变化）：
...
**验证**：`./mvnw clean install -DskipTests -pl nop-ai -am -T 1C` BUILD SUCCESS；`./mvnw test -pl nop-ai -am -T 1C` BUILD SUCCESS（0 failures）；`scan-hollow-implementations.mjs --module nop-ai --severity high` exit 0；`check-doc-links.mjs --strict` exit 0（顺带修复 mission-driver-port-design.md 3 处预存 broken link）；closure audit 见 plan Closure 段。0936-2 侧验证：`./mvnw test -pl nop-ai/nop-ai-core -am -T 1C` + `./mvnw test -pl nop-ai/nop-ai-coder -am -T 1C` + 全量 nop-ai 回归均 BUILD SUCCESS（0 failures）；两模块 main grep 0 残留；closure audit 见 0936-2 plan Closure 段。
