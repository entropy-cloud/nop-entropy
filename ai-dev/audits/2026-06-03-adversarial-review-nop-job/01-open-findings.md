...
**Date**: 2026-06-03
**Scope**: nop-job module — 第 6 轮独立审查，聚焦先前 5 轮盲区：CronExpression 内部边界条件、Trigger 语义映射、JobPartitionResolver 多 coordinator 行为、RPC Invoker 超时和重试、Worker 执行错误路径、ORM 索引完整性、IoC bean 配置。
**Heuristics used**: 10x规模运维, 异常路径侦探, 模型攻击者, IoC 侦探, 未来破坏者

**Dedup**: 已逐一验证 AR-1~AR-35 当前代码状态。Round 5 的 11 项新发现中，9/11 已修复（AR-25,26,28,29,30,33,34,35），1/11 部分修复（AR-27 CronCalendar 无最大迭代保护），1/11 仍存在（AR-32 时钟不一致）。本报告仅包含全新发现。

### 先前发现验证摘要 (第 6 轮)

经代码验证，AR-25 至 AR-35 中 **9/11 已修复**：

| Prior ID | 当前状态 |
|----------|---------|
| AR-25 (P1) | **已修复**: `AnnualCalendar:28` excludeDays 初始化为 `Collections.emptyList()` |
| AR-26 (P2) | **已修复**: HolidayCalendar/AnnualCalendar 添加 `MAX_DAY_SCAN=366*5` |
| AR-27 (P2) | **部分修复**: CronCalendar skip-ahead 逻辑存在但仍无最大迭代保护 |
| AR-28 (P2) | **已修复**: `RpcBroadcastTaskBuilder:65-69` 过滤 `isHealthy() && isEnabled()` |
| AR-29 (P1) | **已修复**: `JobCompletionProcessorImpl:267-272,284` 添加 `allowResultCompletion` 门控 |
| AR-30 (P2) | **已修复**: `TriggerSpecHelper:41-42` getFireCount 映射到 `fireCount` 而非 `totalFireCount` |
| AR-31 (P2) | **部分修复**: `JobWorkerScannerImpl:253-254` updateTask 失败有 WARN 日志，但 silent early-return 路径仍无日志 |
| AR-32 (P3) | 仍存在：task builders 使用 `System.currentTimeMillis()` |
| AR-33 (P2) | **已修复**: `JobFireStoreImpl:202-226` cancelFire tasks 使用 `tryUpdateManyWithVersionCheck` |
| AR-34 (P2) | **已修复**: `JobScheduleStoreImpl` 所有主要路径使用 `tryUpdateManyWithVersionCheck` + 重试 |
| AR-35 (P2) | **已修复**: `JobScheduleStoreImpl:116-133` 添加 `hasWaitingFire` 幂等性检查 |

---

### [AR-36] `handleExecutionResult` 不检查 fire 状态 — 已取消的 fire 可被标记为 SUCCESS

- **文件**: `nop-job/nop-job-worker/src/main/java/io/nop/job/worker/engine/JobWorkerScannerImpl.java:202-211`
- **证据片段**:
  ```java
  // JobWorkerScannerImpl.java:202-211
  private void handleExecutionResult(String jobTaskId, JobFireResult result, Throwable err) {
      NopJobTask task = taskStore.loadTask(jobTaskId);
      Integer taskStatus = task.getTaskStatus();
      if (taskStatus != null
              && (taskStatus == TASK_STATUS_TIMEOUT
              || taskStatus == TASK_STATUS_CANCELED
              || taskStatus == TASK_STATUS_SUSPICIOUS)) {
          return;  // 只检查 task 状态，不检查 fire 状态
      }
      // ... 写入 SUCCESS 结果 ...
  ...
  ```
- **严重程度**: P1
- **现状**: Worker 的结果处理器仅检查 task 自身状态，不检查其父 fire 是否已被取消或超时。如果 cancelFire 在 task 更新前将 fire 标记为 CANCELED（但因为 task 版本冲突或 cancel loop 的 try-catch，task 仍为 RUNNING），worker 的 `handleExecutionResult` 会写入 SUCCESS 结果。Completion processor 随后看到 task SUCCESS，可能将已取消的 fire 重新标记为 SUCCESS。
- **风险**: 已取消的 fire 被错误标记为 SUCCESS，违反状态机语义。在快速取消场景（用户手动取消 + worker 几乎同时完成）下概率增加。
- **建议**: 在写入 task 结果前，检查 `fireStore.loadFire(task.getJobFireId()).getFireStatus()` 是否为终态。或者让 completion processor 在 `resolveFinalFireStatus` 中检查 fire 自身状态而非仅看 task 状态。
- **信心水平**: 很可能（需要 cancel + worker result 的精确时序窗口）
- **发现来源视角**: 异常路径侦探

---

### [AR-37] `RpcJobInvoker` RPC 调用传入 `null` 超时 — 无客户端超时保护

- **文件**: `nop-job/nop-job-service/src/main/java/io/nop/job/service/executor/RpcJobInvoker.java:61,89`
- **证据片段**:
  ```java
  // RpcJobInvoker.java:61 — 执行
  return rpcServiceInvoker.invokeAsync(serviceName, serviceMethod, request, null)
          .thenApply(this::toJobFireResult);

  // RpcJobInvoker.java:89 — 取消
  return rpcServiceInvoker.invokeAsync(serviceName, cancelMethod, request, null)
          .thenApply(ApiResponse::isOk);
  ```
- **严重程度**: P1
- **现状**: `invokeAsync` 的第四个参数是 `timeout`，两处调用均传 `null`。Schedule 的 `timeoutSeconds` 列从未传播到 RPC 调用。如果远端服务挂起，RPC 连接会无限等待（直到 TCP keepalive 超时，通常 2+ 分钟）。Worker 的 timeout checker 会标记 fire/task 为 TIMEOUT，但 RPC 连接和线程池资源仍在占用。
- **风险**: 挂起的 RPC 调用占用连接池条目和线程，在多个 schedule 同时挂起时耗尽资源。这是资源泄漏——coordinator 认为 fire 已超时，但 worker 端的 RPC 仍在等待。
- **建议**: 将 `schedule.getTimeoutSeconds()`（或 `fireStore.loadFire` 获取 fire 关联的 schedule 信息）传播为 RPC 超时参数。至少应使用全局默认超时（如 60 秒）而非无限等待。
- **信心水平**: 确定
- **发现来源视角**: 10x规模运维

---

### [AR-38] `nop_job_fire` 缺少 `(jobScheduleId, fireStatus)` 复合索引 — 活跃 fire 查询全表扫描

- **文件**: `nop-job/model/nop-job.orm.xml:305-318`, `nop-job/nop-job-dao/src/main/java/io/nop/job/dao/store/JobScheduleStoreImpl.java:412-419`
- **证据片段**:
  ```xml
  <!-- nop-job.orm.xml:305-318 — 当前索引 -->
  <index name="IX_NOP_JOB_FIRE_DISPATCH_SCAN" unique="false">
      <column name="fireStatus"/>
      <column name="partitionIndex"/>
      <column name="scheduledFireTime"/>
  </index>
  <index name="IX_NOP_JOB_FIRE_SCHEDULE" unique="false">
      <column name="jobScheduleId"/>
      <column name="scheduledFireTime"/>
  </index>
  ```
  ```java
  // JobScheduleStoreImpl.java:412-419 — findActiveFires / hasWaitingFire 查询
  query.addFilter(FilterBeans.eq(PROP_NAME_jobScheduleId, jobScheduleId));
  query.addFilter(FilterBeans.in(PROP_NAME_fireStatus,
          List.of(WAITING, DISPATCHING, RUNNING)));
  ```
- **严重程度**: P1
- **现状**: `findActiveFires` 和 `hasWaitingFire` 按 `(jobScheduleId, fireStatus)` 查询，但无对应复合索引。现有索引 `IX_NOP_JOB_FIRE_SCHEDULE` 覆盖 `(jobScheduleId, scheduledFireTime)` 但不含 `fireStatus`。对于有大量历史 fire 的 schedule（如运行数月的 cron job），此查询退化为对 schedule 所有 fire 的全扫描。
- **风险**: 10x 规模下（数万 schedule × 数月历史），planner 每周期的 `hasWaitingFire` + dispatcher 的 `findActiveFires` 查询性能严重退化。
- **建议**: 添加索引 `IX_NOP_JOB_FIRE_SCHEDULE_STATUS (jobScheduleId, fireStatus)`。
- **信心水平**: 确定
- **发现来源视角**: 10x规模运维

---

### [AR-39] Unique Key `UK_NOP_JOB_FIRE_SCHEDULE_TIME_SOURCE` 阻止同一时间不同 triggerSource 的 fire

- **文件**: `nop-job/model/nop-job.orm.xml:301-304`, `nop-job/nop-job-dao/src/main/java/io/nop/job/dao/store/JobScheduleStoreImpl.java:403-410`
- **证据片段**:
  ```xml
  <!-- nop-job.orm.xml:301-304 -->
  <unique-key name="UK_NOP_JOB_FIRE_SCHEDULE_TIME_SOURCE"
              columns="jobScheduleId,scheduledFireTime,triggerSource"/>
  ```
  ```java
  // JobScheduleStoreImpl.java:403-410 — hasWaitingFire 不检查 triggerSource
  private boolean hasWaitingFire(String scheduleId, Timestamp scheduledFireTime) {
      QueryBean query = new QueryBean();
      query.addFilter(FilterBeans.eq(PROP_NAME_jobScheduleId, scheduleId));
      query.addFilter(FilterBeans.eq(PROP_NAME_fireStatus, FIRE_STATUS_WAITING));
      query.addFilter(FilterBeans.eq(PROP_NAME_scheduledFireTime, scheduledFireTime));
      query.setLimit(1);
      return !fireDao().findAllByQuery(query).isEmpty();
  }
  ```
- **严重程度**: P2
- **现状**: AR-35 修复添加了 `hasWaitingFire` 幂等性检查，但该检查不区分 `triggerSource`。Unique key 包含 `triggerSource`，所以理论上允许同一 schedule + 同一时间不同 source 的 fire。但 `hasWaitingFire` 不检查 source——如果 SCHEDULE source 的 fire 处于 WAITING，MANUAL source 的同一时间 fire 不会被幂等性检查拦截（因为 hasWaitingFire 只看 WAITING 状态），但如果 MANUAL fire 先插入，SCHEDULE fire 的幂等性检查会误判为"已有 waiting fire"。反过来，如果 cron planner 在 `triggerNow` 之后执行，且 `scheduledFireTime` 碰巧等于 cron 计算出的时间，unique key 会阻止 SCHEDULE fire 的插入——抛出约束违反异常。
- **风险**: 手动触发 + cron 触发恰好在同一秒时，cron fire 插入失败并产生 DB 约束违反异常。Planner 的 try-catch 会记录错误并跳过这个 schedule 周期，不产生数据损坏但浪费一次调度。
- **建议**: `hasWaitingFire` 应同时检查 `triggerSource`。或者评估 unique key 是否应移除 `triggerSource` 列——如果同一时间的不同 source fire 在业务上不应同时存在。
- **信心水平**: 很可能
- **发现来源视角**: 组合爆炸测试

---

### [AR-40] `HandleMisfireTrigger` 对 `OnceTrigger` 无效 — Misfire 阈值被忽略

- **文件**: `nop-job/nop-job-core/src/main/java/io/nop/job/core/trigger/HandleMisfireTrigger.java:36-39`, `nop-job/nop-job-core/src/main/java/io/nop/job/core/trigger/OnceTrigger.java:31-32`
- **证据片段**:
  ```java
  // HandleMisfireTrigger.java:36-39
  long startTime = evalContext.getLastScheduledTime();
  if (startTime <= 0) { ... }
  if (startTime < afterTime - misfireThreshold)
      startTime = afterTime - misfireThreshold;  // 封顶到阈值窗口
  return trigger.nextScheduleTime(startTime, evalContext);

  // OnceTrigger.java:31-32
  public long nextScheduleTime(long afterTime, ITriggerEvalContext evalContext) {
      if (scheduleTime > 0) return scheduleTime;  // 忽略 afterTime!
  ```
- **严重程度**: P2
- **现状**: `HandleMisfireTrigger` 的 misfire 处理策略是：如果上次调度时间距现在超过阈值，将 `startTime` 封顶到 `afterTime - misfireThreshold`，然后传给内部 trigger。但 `OnceTrigger` 忽略 `afterTime` 参数，直接返回其固定的 `scheduleTime`。因此 misfire 封顶对 ONCE 触发器完全无效——一个配置在 3 天前的 ONCE 触发器，即使 `misfireThreshold=1h`，仍会在下一次扫描时立即触发。
- **风险**: ONCE 触发器的 misfire 保护形同虚设。如果应用停机数天后重启，所有到期的 ONCE 触发器会同时触发，不受 misfire 阈值限制。
- **建议**: `HandleMisfireTrigger` 应在调用内部 trigger 前检查返回值是否在可接受的时间窗口内。对于返回时间远早于 `afterTime - misfireThreshold` 的结果，应返回 -1（不再触发）。
- **信心水平**: 确定
- **发现来源视角**: 异常路径侦探

---

### [AR-41] `invoker.invokeAsync(ctx)` 返回 `null` 被静默当作 SUCCESS

- **文件**: `nop-job/nop-job-worker/src/main/java/io/nop/job/worker/engine/JobWorkerScannerImpl.java:191-194`, `nop-job/nop-job-worker/src/main/java/io/nop/job/worker/engine/DefaultJobExecutionContextBuilder.java:32-34`
- **证据片段**:
  ```java
  // JobWorkerScannerImpl.java:191-194
  var promise = invoker.invokeAsync(ctx);
  if (promise == null) {
      handleExecutionResult(runningTask.getJobTaskId(), null, null);
  }

  // DefaultJobExecutionContextBuilder.java:32-34
  if (result == null) {
      return new JobTaskExecutionUpdate(TASK_STATUS_SUCCESS, null, null, false);
  }
  ```
- **严重程度**: P2
- **现状**: 如果 `IJobInvoker` 实现的 `invokeAsync` 返回 `null`（而非抛异常），worker 将其当作 SUCCESS 处理。这意味着一个框架 bug 或错误实现可能导致 task 被标记为"执行成功"而实际未执行任何工作。
- **风险**: 第三方 invoker 实现如果意外返回 null，task 静默成功。对于关键业务任务（如支付对账、数据同步），这意味着丢失执行而无人察觉。
- **建议**: 将 `null` promise 当作错误而非成功：`handleExecutionResult(taskId, null, new NopException(ERR_JOB_INVOKER_RETURNED_NULL))`。
- **信心水平**: 确定
- **发现来源视角**: 模型攻击者（trust boundary）

---

### [AR-42] `JobPartitionResolver` 每次扫描都查询 naming service — 无缓存

- **文件**: `nop-job/nop-job-coordinator/src/main/java/io/nop/job/coordinator/engine/JobPartitionResolver.java:65`
- **证据片段**:
  ```java
  // JobPartitionResolver.java:65
  List<ServiceInstance> servers = namingService.getInstances(svcName);
  ```
- **严重程度**: P2
- **现状**: `resolvePartitions()` 在每次 planner/dispatcher/completion/timeout 扫描周期都被调用。每次调用都查询 naming service。如果 naming service 是远程服务（Consul、Nacos），每个扫描周期产生一次网络调用。在 30 秒扫描间隔、多 partition 的部署下，每分钟可能产生数十次 naming service 查询。
- **风险**: 不必要的 naming service 负载。在 naming service 响应慢或不可达时，所有扫描周期被阻塞。
- **建议**: 添加短期缓存（如 5-10 秒 TTL），避免每周期都查询。`isUnstable` 逻辑已提供稳定性保护，缓存不会影响正确性。
- **信心水平**: 确定
- **发现来源视角**: 10x规模运维

---

### [AR-43] `CronExpression.getTimeAfter()` 每次调用创建 `new GregorianCalendar()` — GC 压力

- **文件**: `nop-job/nop-job-core/src/main/java/io/nop/job/core/trigger/CronExpression.java:140-141`
- **证据片段**:
  ```java
  // CronExpression.java:140-141
  Calendar calendar = new GregorianCalendar();
  calendar.setTimeZone(this.timeZone);
  ```
- **严重程度**: P2
- **现状**: `GregorianCalendar` 是重量级对象（内部 int[] 数组 ~40 字段）。每次 `getTimeAfter()` 调用都创建新实例。对于秒级 cron（`*/1 * * * * ?`），每个扫描周期对每个 schedule 调用一次。1000 个 schedule × 每 5 秒扫描 = 每秒 200 次 Calendar 分配。
- **风险**: GC 压力随 schedule 数量线性增长。在 10k+ schedule 部署下，Young GC 频率显著增加。
- **建议**: 将 Calendar 实例存储为 ThreadLocal 或复用 `calendar.setTimeInMillis()` 重置状态。
- **信心水平**: 确定
- **发现来源视角**: 10x规模运维

---

### [AR-44] `JobScheduleStoreImpl` 内部 helper 方法仍使用 `updateEntityDirectly` — cancel/reset 路径无版本保护

- **文件**: `nop-job/nop-job-dao/src/main/java/io/nop/job/dao/store/JobScheduleStoreImpl.java:470-497`
- **证据片段**:
  ```java
  // JobScheduleStoreImpl.java — cancelFire helper（overlay 路径内部调用）
  private void cancelFire(NopJobFire fire, Timestamp time) {
      fire.setFireStatus(FIRE_STATUS_CANCELED);
      // ...
      fireDao().updateEntityDirectly(fire);  // 无版本检查
  }

  // cancelTasks helper
  private void cancelTasks(String fireId, Timestamp time) {
      // ...
      taskDao().updateEntityDirectly(task);  // 无版本检查
  }

  // resetFailedTasks helper
  private void resetFailedTasks(String fireId, Timestamp time) {
      // ...
      taskDao().updateEntityDirectly(task);  // 无版本检查
  }
  ```
- **严重程度**: P2
- **现状**: AR-34 修复了 `JobScheduleStoreImpl` 的主要 schedule 更新路径（`insertFireAndAdvanceSchedule`、`overlayFireAndAdvanceSchedule` 等），使它们使用 `tryUpdateManyWithVersionCheck`。但内部 helper 方法 `cancelFire`、`cancelTasks`、`resetFailedTasks`（overlay 和 recovery 路径使用）仍使用 `updateEntityDirectly`。这些方法更新 fire 和 task 的状态/错误码，与 AR-33 修复的外部 `cancelFire`（`JobFireStoreImpl`）不一致。
- **风险**: overlay 路径的内部 cancel 可能覆盖 timeout checker 并发标记的 TIMEOUT 状态为 CANCELED。与外部 cancelFire（已修复）的语义不一致。
- **建议**: 内部 helper 也改用 `tryUpdateManyWithVersionCheck`，或提取为共享方法。
- **信心水平**: 很可能
- **发现来源视角**: 事务边界追踪者

---

### [AR-45] `CronExpression.equals()` 忽略 `timeZone` — 不同时区的表达式比较为相等

- **文件**: `nop-job/nop-job-core/src/main/java/io/nop/job/core/trigger/CronExpression.java:447-458`
- **证据片段**:
  ```java
  // CronExpression.java — equals 方法比较 BitSets 但不比较 timeZone
  public boolean equals(Object o) {
      // ... 比较 months, daysOfMonth, daysOfWeek, hours, minutes, seconds ...
      // 不比较 this.timeZone
  }
  ```
- **严重程度**: P3
- **现状**: 两个从相同 cron 字符串但不同时区构造的 `CronExpression` 实例，`equals()` 返回 `true`，但 `getTimeAfter()` 返回不同结果。如果用作 `HashMap` key 或 `HashSet` 元素，会产生逻辑错误（相同的 key 但不同行为）。
- **风险**: 当前代码未观察到将 CronExpression 作为集合 key 使用。但如果 `TriggerBuilder` 缓存 trigger 实例（如使用 Map 去重），时区差异会被静默忽略。
- **建议**: 在 `equals()` 和 `hashCode()` 中加入 `timeZone` 比较。
- **信心水平**: 确定
- **发现来源视角**: 死代码清道夫

---

### [AR-46] `JobPartitionResolver` 首次调用信任 naming service — 启动时集群不稳

- **文件**: `nop-job/nop-job-coordinator/src/main/java/io/nop/job/coordinator/engine/JobPartitionResolver.java:90-96`
- **证据片段**:
  ```java
  // JobPartitionResolver.java:90-96
  private boolean isUnstable(List<ServiceInstance> current) {
      ...
      if (prev == null) {
          return false;  // 首次调用总是认为稳定
      }
  ```
- **严重程度**: P2
- **现状**: `isUnstable` 在首次调用时返回 `false`（总是稳定），直接返回分区结果。如果应用启动时 naming service 返回过时的成员数据（例如 Consul 的 stale read），分区基于错误的成员列表计算，工作被分配到已死亡的实例。
- **风险**: 集群中断期间启动 coordinator，首次扫描分配工作到死亡节点。在滚动更新场景下概率增加。
- **建议**: 首次调用时返回 `true`（不稳定），强制等待一个稳定窗口后再返回分区。
- **信心水平**: 很可能
- **发现来源视角**: 10x规模运维

---

### [AR-47] `RpcBroadcastTaskBuilder.emptyIfNull()` 是死代码

- **文件**: `nop-job/nop-job-coordinator/src/main/java/io/nop/job/coordinator/engine/RpcBroadcastTaskBuilder.java:101-103`
- **证据片段**:
  ```java
  // RpcBroadcastTaskBuilder.java:101-103
  private Map<String, Object> emptyIfNull(Map<String, Object> map) {
      return map == null ? Collections.emptyMap() : map;
  }
  ```
- **严重程度**: P3
- **现状**: 方法在文件中无任何调用。可能是重构遗留。
- **风险**: 无功能风险。维护时可能困惑。
- **建议**: 删除。
- **信心水平**: 确定
- **发现来源视角**: 死代码清道夫
...
**1. handleExecutionResult 不检查 fire 状态 (AR-36, P1)**

Worker 的结果处理器只看 task 状态不看 fire 状态。在 cancel + worker result 的竞态窗口下，已取消的 fire 可被重新标记为 SUCCESS。这是状态机的正确性违反——fire 一旦进入 CANCELED 终态，不应被任何路径复活。

**2. RPC 调用无超时 (AR-37, P1) + 缺失索引 (AR-38, P1)**

两个独立的 P1 问题：RPC 调用传入 null 超时导致资源泄漏，活跃 fire 查询缺少复合索引导致全表扫描。两者都在 10x 规模下显著影响可用性。

**3. OnceTrigger misfire 无效 (AR-40, P2) + PartitionResolver 首次信任 (AR-46, P2)**

两个独立的边界条件问题：ONCE 触发器不受 misfire 阈值保护，集群启动时首次分区计算信任可能过时的成员数据。

### Blind Spot Assessment

本轮仍可能遗漏：

1. **IJobInvoker.cancelAsync 返回值未检查**: `DefaultJobCancelHandler:36-43` 调用 `invoker.cancelAsync()` 但只处理异常，不处理返回 `false`（取消失败）。Cancel 失败被静默忽略。
2. **CronExpression 时区传播**: cron 表达式解析和执行时区是否与 schedule 的 `pauseCalendarSpec` 时区一致，未深入验证。
3. **批量操作级联**: 批量 archive/delete schedule 时 fires/tasks 的级联清理策略未检查。
4. **Multi-coordinator 分区重平衡**: coordinator 实例数变化时，正在执行的 fire 的 partition 分配是否正确更新。
5. **RpcJobInvoker 错误码分类**: 所有非 OK 响应都当作 ERROR，无重试语义（429/503 等瞬时错误不应永久失败）。
...
|---------|------|---------|
| P0      | 0    | —       |
| P1      | 4    | AR-36(fire 状态竞态), AR-37(RPC 无超时), AR-38(缺失索引) + AR-25/29 已修复 |
| P2      | 8    | AR-39(unique key 误拦截), AR-40(OnceTrigger misfire), AR-41(null promise), AR-42(naming 缓存), AR-43(GC 压力), AR-44(helper 无版本保护), AR-46(首次信任) |
| P3      | 4    | AR-32(时钟不一致), AR-45(equals 忽略时区), AR-47(死代码), AR-27 部分(CronCalendar 无 max iteration) |
...
|---------|------|---------|
| P0      | 0    | —       |
| P1      | 3    | fire 状态竞态, RPC 无超时, 缺失索引 |
| P2      | 7    | unique key 误拦截, OnceTrigger misfire, null promise, naming 缓存, GC 压力, helper 版本保护, 首次信任 |
| P3      | 3    | equals 忽略时区, 死代码, CronCalendar 遗留 |
