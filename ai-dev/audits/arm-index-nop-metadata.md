...
| `P2-MA2-01`（MA2.1 新增） | NopMetaTagLabel.tag/glossaryTerm 的 refPropName="tagLabels" 反向集合缺失（NopMetaTag/NopMetaGlossaryTerm 无 tagLabels to-many） | MR1 裁决：**修复**（plan-2026-08-04-1004-3，R1.2）——live 影响面 0 手写引用（仅 _gen 派生），缺口 = 显式声明语义（cascadeDelete 孤儿行防护 + displayName/i18n + ref-pub） |
| `P2-MA2-02`（MA2.1 新增） | NopMetaDataProduct.businessDomain 的 refPropName="dataProducts" 反向集合缺失（NopMetaBusinessDomain 无 dataProducts to-many） | MR1 裁决：**修复**（plan-2026-08-04-1004-3，R1.2）——live 影响面 0 手写引用；域→产品为治理核心导航路径 |
| `P2-MA2-03`（MA2.1 复核历史 07-20-1554#维度04-02 仍 open） | SQL 保留字 PRIMARY/CONSTRAINT 用作列 code（**实体归属修正：PRIMARY 在 NopMetaEntityField:747（primaryField 列）、CONSTRAINT 在 NopMetaEntityUniqueKey:920（constraintName 列）**；裁决依据以 Oracle DDL 未引号事实为准——Oracle `PRIMARY SMALLINT default 0` / `CONSTRAINT VARCHAR2(100)` 为语法错误，MySQL 反引号可解析） | MR1 裁决：**修复**（plan-2026-08-04-1004-3，R1.3）——改名 PRIMARY→IS_PRIMARY / CONSTRAINT→CONSTRAINT_NAME（name 不变，无契约影响） |
