...
> P1 未闭包数：**2**（2 项已裁定 watch-only residual（MISSING-AUTH/16-01）；MA1-001 fixed 于 MR1；**MR2 收口（plan-2026-08-04-1543-3 Phase 2+3）：MA3-001/002、MA4-401、MA4-601、20-01 全部 fixed（R2.1~R2.5，各带行为回归测试），P1 表 6 项 MR 归属项全部闭环**）。
>
...
| `P2-MA3-03`（MA3.4 新增） | upsertExternalTable schema 维度未进 DB UK——多 schema 同名表功能与模型冲突（RACE 复核新增，非并发也必然 UK 冲突） | MR2（需 ORM 模型变更，plan-first）——**plan-2026-08-04-1543-3 Phase 1 裁决：deferred**（ORM UK 变更 + 迁移/DDL 再生成；单 schema 部署无影响；Successor: MR3/DDL 管线，与 P2-MA6.6-001 同族） |
...
| `P2-MA4-501`（MA4.5 新增） | `*Service` 命名违规 2 处存活（NopMetaSearchService 审计后复发 + QualityAlertWorkflowService 漏网；02-01 残余确认） | MR2（命名批量修复，随 02-01）——**plan-2026-08-04-1543-3 Phase 1 裁决：deferred**（改名涉及 beans.xml bean id + 19 文件引用 + xwf c:script import 全量同步，纯命名规范项无功能影响；Successor: MR3 命名治理批次） |
| `P2-MA4-602`（MA4.6 新增） | helper 镜像测试跨 4 文件重复 30+ 方法（safeAlias/aggSqlOf 逐字复制） | MR2（收敛单一 helper 测试文件）——**plan-2026-08-04-1543-3 Phase 1 裁决：deferred**（纯测试重组；P1-MA4-601 修复后文件主体已改造，残余镜像无功能风险；Successor: MR3 测试治理批次） |
