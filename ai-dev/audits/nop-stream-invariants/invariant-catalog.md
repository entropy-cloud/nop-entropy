...
### 不变式 #6 — 输出契约族（side-output 转发契约）（定稿，Cycle 2 / I1 沉淀；HG-01 更新 2026-08-14）

- **陈述**：任何 `Output.collect(OutputTag, X)` 调用必须被转发到注册的 side-output 消费者，不得静默丢弃；无注册消费者 → fail-fast。跨 task 已知违约实例（`RecordWriterOutput` / `BroadcastingRecordWriterOutput` 空体 no-op）以过渡 pin 登记（关联 `HG-01`），禁止静默移除。**更新（HG-01，2026-08-14，plan `2026-08-14-0900-1`）**：跨 task 线协议支持已落地——`collect(OutputTag)` 构造 side-output 元素经 `RecordWriter.emitElement` 广播送达消费 task，消费端 `processInputGate` 路由到 `sideOutputConsumers`（按 tag id 遍历匹配），无消费者 fail-fast 移至消费端路由点（错误码复用 `ERR_STREAM_SIDE_OUTPUT_NO_CONSUMER`）；注册表分类迁移 `fail-fast` → `forward`（棘轮理由在案）。
- **覆盖失败族**：输出契约族（RL-7 修复族）+ 跨 task 契约缺口（P1，I6 裁定：interim fail-fast 预授权分派 Cycle 2 / I4；线协议支持 = `HG-01` 人工确认门 → 2026-08-14 人工批准并落地）
- **历史审计证据**：R15-AR-4（`2026-05-31-adversarial-review-nop-stream-r9/01-open-findings.md`，`ChainingOutput` 侧输出静默丢弃，pre-fix 行号 `ChainingOutput.java:84-86`）；I2 探查 PR-1 确认 live（I2-probing-report.md）；RL-7 修复 `b20fcd0e1`（`ChainingOutput.java:111` 转发 + 无消费者 fail-fast `ERR_STREAM_SIDE_OUTPUT_NO_CONSUMER`）；HG-01 线协议落地 `2026-08-14-0900-1`。
- **检测方法**：JUnit `TestOutputContractInvariant`（core，14 用例：参数化穷举 4 个 main `Output` 实现类三态行为（分类以 `output-contract-registry.json` 为准）+ 接线断言 + 反射实例化断言 + 入站路由用例）+ mjs `scan-output-contract`（类级枚举 V1 / 失效类 V2 / 行为漂移 V3 / 新增发射点 V4 / 失效发射点 V5）+ 发射点注册表 `output-contract-registry.json`（6 发射点）+ 既有 E2E `TestSideOutputChainingE2E`（runtime，6 用例，含跨 task 送达 + 无消费者 fail-fast + BRWO 扇出）。
- **live 修复状态**：in-task 路径 fixed（I4 `b20fcd0e1`）；跨 task = **forward 已落地**（HG-01，2026-08-14：线协议送达 + 消费端 fail-fast；注册表分类迁移 `fail-fast` → `forward`，过渡 pin 2 条 = 历史记录——`mjs-pins.json` 注记在案）。**状态 = 已沉淀**（门禁全绿：JUnit 门禁类 0 failures + mjs `all` exit 0 + E2E 全绿，2026-08-14 收口）。
