...
> **处置（fix-ai-check 分支，2026-08-22）**: 确认非误报，已修复。修复方式（per-subtask 隔离，参照 Flink 每 subtask 独立函数实例的做法）：nop-stream-core 新增 `StreamOperator.copyForSubtask(int subtaskIndex)`（默认委托无参版本）与 `OperatorChain.deepCopy(int)`，三个运行时调用点（GraphExecutionPlan、RemoteGraphExecutionPlanBuilder、SupervisionLoop.rebuildTask）均传入真实 taskIndex（region restart 后 subtask 身份稳定，保证幂等键跨重启一致）；`TwoPhaseCommitSinkFunction.copyForSubtask(int)` 基类默认 subtaskIndex>0 时 fail-fast 抛 UnsupportedOperationException（No-Silent-No-Op，防止未实现拷贝语义的子类被静默共享）；`StreamSinkOperator.copyForSubtask(int)` 对 2PC udf 产出独立副本。两个生产 sink 均实现独立副本（fresh currentBuffer + 空 pendingCommits）：FileTwoPhaseCommitSink 副本 subtask>0 时 temp/final 文件名与 manifest key 加 `.s{N}` 后缀（subtask 0 保持旧命名，存量单 subtask 部署兼容；FilePendingCommit 记录 owner subtaskIndex，commit 按 owner 计算路径，恢复重提交路径稳定），JdbcTwoPhaseCommitSink ledger 表加 `subtask_id` 列（复合主键 epoch_id+subtask_id，幂等守卫按 subtask 区分），同时 `JdbcTwoPhaseCommitSink.invoke` 的 `currentBuffer.add` 补 `synchronized(currentBuffer)` 与 saveState/rollback 对齐（报告中点名的锁不一致）。注意：存量旧 schema ledger 表（单列主键）需按新 DDL 重建，否则新 INSERT 会显式报错（fail-loud 而非静默丢数）。测试：`nop-stream-connector` `TestFileTwoPhaseCommitSink#testParallelSubtaskCopiesCommitIndependently`（修复前两个 deepCopy 副本共享同一实例，同 epoch 的 pendingCommits 互相覆盖且写同一 `.epoch-N.tmp` 互相截断，最终只有 1 个 subtask 的批次落盘）、`nop-stream-connector-jdbc` `TestJdbcTwoPhaseCommitSinkParallelIsolation#testParallelSubtaskCopiesCommitIndependently`（修复前同 epoch ledger 撞键，仅 1 个 subtask 的数据落库 target_data）、`nop-stream-core` `TestOperatorSubtaskIsolation#streamSinkCopyIsolatesTwoPhaseCommitUdfPerSubtask`（修复前算子副本共享同一 2PC udf 实例）。红→绿均验证；`./mvnw test` 结果：nop-stream-core 1499 通过/0 失败、nop-stream-connector 38/0、nop-stream-connector-jdbc 34/0、nop-stream-runtime 842/0（含 TestE2EJdbcTwoPhaseCommitSink 等 e2e）；不变式门禁 `check-nop-stream-invariants.mjs inventory/sync/scan-iterations` OK（gate-inventory.json 已登记新方法 copyForSubtask）。
...
> **处置（fix-ai-check 分支，2026-08-22）**: 确认非误报，已修复。三个 `run()` 入口统一重置生命周期标志：`MessageSourceFunction.run`（`running=true`、`failed=false`、`pendingError=null`，并每次重建 `shutdownLatch`——原实现仅判 null 创建，cancel 后 latch 恒为 0，即使补了 running 重置也会忙转/立即退出）；`DebeziumCdcSourceFunction.run`（`running=true`，`initCompletionLatch` 改为每次新建 latch，原实现仅重置 draining 且 latch 只在 null 时创建）；`BatchLoaderSourceFunction.run`（`running=true`）。runEntered CAS、subscription 重订阅等既有机制不变。测试：`nop-stream-connector` `TestMessageSourceFunctionRestart#testRunAfterCancelResumesDelivery`（修复前 cancel 后再次 run() 立即返回，重启后消息不再投递）；`nop-stream-connector-batch` `TestBatchLoaderSourceFunction#testRunAfterCancelResumesDataFlow`（修复前第二次 run() 0 条数据，expected [c, d] but was []）；`nop-stream-connector-debezium` `TestDebeziumCdcSourceFunction#testRunAfterCancelReEntersLoop`（修复前再次 run() 立即返回、CDC 引擎不重建，测试用 createMessageSource 注入无引擎 stub 保持 hermetic）。红→绿均验证；`./mvnw test` 结果：nop-stream-connector 38 通过/0 失败、nop-stream-connector-batch 36/0、nop-stream-connector-debezium 25（1 skip 为既有 @Disabled 的 TestDebeziumCdcSourceCompletion）。
