# [P0/fixed] TypeScript 破坏输入恢复不终止（merge-path 位置回退循环）
...
- 零宽循环类无现网成员（15 输入验证集全部毫秒级终止，guard 未触发），guard 转为 safety-net-only（`RecoveryTerminationTest` 守护）；多轮恢复树形状残差 5 例为 watch-only（与主节已裁定类相同）。
