...
详见 [DESIGN-group-step.md](./DESIGN-group-step.md)。
...
2. while (当前 step 不是终态)：
   ...
   b. 根据 step.type 执行：
       ...
       - group: 执行子步骤循环，返回 exit marker（见 DESIGN-group-step.md）
...
CLOSURE_VERIFY 是一个 group step（maxRounds=3），内部循环"脚本检查 → AI 审计 → 再检查"。详见 [DESIGN-group-step.md](./DESIGN-group-step.md)。
