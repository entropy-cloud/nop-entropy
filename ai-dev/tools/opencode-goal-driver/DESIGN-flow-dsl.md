...
> Status: **superseded** — 已由 `ai-dev/design/opencode-goal-driver/flow-engine-design.md` v4 整合并替代
...
| `group` | 子步骤分组，支持循环控制 | steps 映射 + maxRounds | 返回子步骤 exit 的 marker |

### 2.3 Marker & Transition

Agent step 的输出中包含 XML 标签 `<AI_STEP_RESULT>值</AI_STEP_RESULT>`，Engine 提取标签值作为 **marker**。

`resultTag` 默认为 `"AI_STEP_RESULT"`，通常不需要配置。仅在特殊场景（如单元测试中需要区分不同标签）才覆盖。

每个 Step 定义一个 `transitions` 映射：`marker → action`。

### 2.4 Group Step

Group Step 是一种**黑盒复合步骤**，将多个子步骤封装为一个整体。从外部看，它与普通步骤无异——通过 marker 与外部 transitions 交互。

详见 [group-step-design.md](../../../design/opencode-goal-driver/group-step-design.md)。

**核心语义：**
- 子步骤通过 `{ goto: "内部步骤名" }` 在 group 内跳转，或 `{ exit: "marker" }` 退出 group
- `{ goto: "_retry" }` 触发新一轮（round++），从第一个子步骤重新开始
- `maxRounds` 控制最大轮次，超出时返回 `onExhausted` marker（默认 `"fail"`）
- group 内部不知道外部步骤的存在
...
const flow = {
  ...
  steps: {
    ...
    CLOSURE_VERIFY: {
      type: "group",      // 见 DESIGN-group-step.md
      maxRounds: 3,
      steps: { /* SCRIPT_CHECK + AI_AUDIT */ },
      transitions: {
        pass: { goto: "BUILD_VERIFY" },
        fail: { retry: "EXECUTE", maxRetries: 5 },
      },
    },

    BUILD_VERIFY: {
      type: "agent",
      prompt: "验证模块 {module} 的构建是否通过...",
      transitions: {
        pass: { goto: "ROADMAP_CHECK" },      // dev loop: 回到路线图检查
        fail: { retry: "EXECUTE", maxRetries: 3 }, // 构建失败 → 重新执行修复
      },
      onError: { retry: "EXECUTE", maxRetries: 2 },
      onMaxRetries: { goto: "ROADMAP_CHECK" }, // 重试耗尽 → 不终止，回到路线图重新评估
    },
...
所有 prompt 和 command 中使用 `{variable}` 格式的模板变量，由 Engine 在运行时从 context 替换。

#### 格式规则

| 类型 | 格式 | 用途 | 替换者 |
|------|------|------|--------|
| 系统变量 | `{UPPER_SNAKE_CASE}` | 跨 step 共享的固定语义变量 | config / Engine 自动注入 |
| 临时变量 | `{camelCase}` | 单 step 内的局部变量 | config / 上一步输出 |

系统变量和临时变量的区分标准：

- **系统变量**：在 3 个以上 step 中使用，或由项目结构决定（如模块名、项目根目录）
- **临时变量**：仅在单个 step 使用，或由每次执行动态决定（如最新计划路径、上一步输出）
...
| 变量 | 来源 | 说明 |
|------|------|------|
| `{MODULE}` | config.moduleName | 目标模块路径（如 `nop-ai/nop-ai-agent`） |

> 注意：当前代码中实际使用的是 `{module}`（小写），属历史遗留。新增变量应使用 `{UPPER_SNAKE_CASE}`。已有小写变量暂不强制迁移，但新增时必须遵循大写规则。

**临时变量**：

| 变量 | 来源 | 说明 |
|------|------|------|
| `{module}` | config.moduleName | 当前模块名（历史遗留，等同 `{MODULE}`） |
| `{projectRoot}` | config.projectRoot | 项目根目录 |
| `{latest_plan}` | Engine 自动检测 | 最新活跃计划文件路径 |
| `{prev_output}` | 上一步输出 | append=true 时自动可用 |
| `{DATE}` | 运行时日期 | yyyy-MM-dd 格式 |

#### 禁止的格式

| 格式 | 原因 |
|------|------|
| `{{varName}}` | 与 Mustache/Handlebars 冲突，Nop 有自身模板引擎 |
| `<path>`, `<name>` | 与 XML 标签混淆（prompt 中常含 XML 示例） |
| `%VAR%` | Windows 环境变量风格 |

#### 引擎实现

变量替换由 `engine.js` 的 `_templateVar()` 方法执行：
...
2. while (当前 step 不是终态)：
   ...
   b. 根据 step.type 执行：
       ...
       - script: 调用 JS 函数，取返回值
        - group: 执行子步骤循环，返回 exit marker（见 [group-step-design.md](../../../design/opencode-goal-driver/group-step-design.md)）
...
**BUILD_VERIFY 重试耗尽时**：onMaxRetries `{ goto: "ROADMAP_CHECK" }`——不终止 flow，而是回到路线图检查重新评估。这避免了 EXECUTE agent 反复声称成功但构建一直不过时 flow 被迫终止的问题。ROADMAP_CHECK 会重新评估当前状态，可能创建新的修复计划。

```
BUILD_VERIFY(fail) × 3 → onMaxRetries → ROADMAP_CHECK → PLAN_DRAFT(修复构建的计划) → ...
```

### 7.4 场景四：Closure verify 循环

```
EXECUTE → CLOSURE_VERIFY (group)
            round 1: SCRIPT_CHECK(fail) → AI_AUDIT(incomplete) → exit "fail"
            → retry EXECUTE (with REMAINING items)
          → EXECUTE → CLOSURE_VERIFY
            round 1: SCRIPT_CHECK(fail) → AI_AUDIT(complete) → _retry
            round 2: SCRIPT_CHECK(pass) → exit "pass"
            → BUILD_VERIFY
```

CLOSURE_VERIFY 是一个 group step（maxRounds=3），内部循环"脚本检查 → AI 审计 → 再检查"。详见 [group-step-design.md](../../../design/opencode-goal-driver/group-step-design.md)。
...
| CLOSURE_VERIFY.SCRIPT_CHECK | fail | pass | pass |
| CLOSURE_VERIFY.AI_AUDIT | complete | - | - |
...
→ EXECUTE(success)
→ CLOSURE_VERIFY (round 1: SCRIPT_CHECK(fail) → AI_AUDIT(complete) → round 2: SCRIPT_CHECK(pass) → exit "pass")
→ BUILD_VERIFY(pass)
...
→ PLAN_AUDIT_EVIDENCE(pass)
→ EXECUTE(success)
→ CLOSURE_VERIFY (round 1: SCRIPT_CHECK(pass) → exit "pass")
→ BUILD_VERIFY(pass)
