...
这里有一个微妙的地方，差量是定义在支持差量运算的结构空间中的，因此一切皆差量是针对这个已经存在的空间而言的。构造这个空间其实是需要工作的，也是需要成本的，但是它并不是差量空间中的0。这就好比是应用程序是在操作系统之上运行，我们需要先构造应用程序得以实际运行的基础空间，但是我们一般并不会把操作系统本身看作是应用系统的Base部分。
