...
- Nop 证明了**结构空间的可逆可以做到什么程度**：不修改源码、不触碰运行轨迹，仅靠差量合并就能支撑粗粒度的产品定制与系统复用——因为结构空间天然是"静态的、可重放"的，字段级坐标 + 结合律 + 逆元让它拥有完整的差量代数。
- dsh 证明了**运行时结构空间的可逆可以做到什么程度**：通过为每个注册行为显式配备逆、由运行时跟踪组合，在"插件"这个粗粒度上实现卸载、热替换、自修改——代价是逆的正确性依赖契约而非验证，坐标粒度停留在插件级，且必须依赖系统边界来划分可逆/不可逆。

对 Nop 而言，dsh 论文最大的借鉴价值在于：**它把"运行时结构空间的部分可逆"这个方向形式化了**——给出了逆的代数（twisted composition）、逆的复合条件（pairwise independence）、以及最重要的概念工具"系统边界"。Nop 若通过 plugin 机制实现运行时可逆，面临的正是论文 [1, §6] 中讨论的问题：哪些状态在边界内（可逆）、哪些在边界外（只能补偿）、依赖如何声明与解析、卸载顺序如何保证——而 Nop 在结构空间已经建立的差量代数，恰好可以弥补 dsh 论文在配置层的形式化空白。
