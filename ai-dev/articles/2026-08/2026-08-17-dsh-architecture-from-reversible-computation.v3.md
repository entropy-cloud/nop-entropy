...
这份设计收获的反应，一半是兴奋，一半是困惑。知乎上的质疑相当集中；六篇社区深读文章里也有三篇各用整节篇幅回应同一类困惑——[18] 的"代价与挑战"、[19] 的"冷思考"、[21] 干脆把整篇文章组织成"五个工程难题"。把这些疑问归纳起来，是五条：
...
5. 这套机制的工程代价值得吗？副作用注册要包装追踪、属性访问要过 Proxy、动态加载叠加作用域链让调试变得困难——"为什么我的插件没加载？"（依赖不满足，fiber 停在 PENDING，没有报错[21]）；OSGi 的类加载器地狱与 Mach 的 IPC 开销就是前车之鉴[18][19][21]。
...
疑问②说得很对："一切皆插件"确实不是 DSH 的发明。但正因为"插件"这个词被用得太滥，才需要先把它区分清楚。按组合能力的强弱，世面上的"插件化"可以分为三重境界（社区文章 [19] 给出了类似的划分，这里按本文的视角重新表述）：
...
在实现层面（[1, §5.1.1]），dsh/Cordis 把这一切收敛为一个单一原语 `ctx.effect(callback)`：callback 做副作用、返回撤销函数，撤销函数被推进 disposables 列表。卸载就是一行代码——`for (const disposable of disposables.splice(0).reverse()) ...`（`dsh/vendor/cordis/src/fiber.ts`，已核实）——按注册的相反顺序逐个撤销，像退栈。
...
第二篇结束之前，值得停下来看看这套理论是怎么跨过"理想国与实现之间"的鸿沟的。软件史上"理论上完美、工程上失败"的插件化方案并不少：OSGi 模块化理论完备，败于类加载器地狱；微内核 Mach 的隔离优雅，败于 IPC 开销。开篇疑问⑤的代价之问，社区文章 [21] 把它展开为"五个工程难题"（副作用真的可逆吗、解药一定对吗、性能开销谁来付、调试怎么做、生态如何冷启动），又把 dsh 的应对归纳为三步；这三步恰好与本文 2.4 节逐层核实的事实一一对应：

| 工程三步法（[21] 的归纳） | dsh/Cordis 的落地（本文核实） |
|---|---|
...
三步法里还有一条值得单独指出：**系统边界不止是论文概念，它有物理执行者**。dsh 的沙箱本身就是插件（sandbox 服务接口 + sandbox-local 实现，Linux 上 bwrap 回退 Landlock、macOS Seatbelt、Windows 受限令牌+ACL，均已核实源码），Landlock 后端是一个 C11 原生插件，fail-closed——沙箱无法启用就拒绝运行而不是放行。OS 级强制访问控制把"边界内/边界外"从论文定义变成了内核保证。这与可逆计算划分 R-Core 与 I-Boundary、对跨界操作记录证据并补偿的主张完全同构：**不可逆性不被否认，而是被显式收编**。

社区文章 [21] 的三步法归纳与本文基于源码的核实结论相互独立又完全对齐，这本身就是一个信号：dsh 的工程选择不是偶然的技巧堆砌，而是"可逆并不要求全量"这一原则在约束条件下的必然形态。用 [21] 的话说：好的理论不会直接告诉你怎么做，但它会告诉你什么路是走不通的。
...
可逆计算理论强调：要在不修改主系统源码的情况下通过定制实现修改，必须建立**领域结构坐标系**——否则差量无法精确定位。2.4.6 节已经核实：dsh 的结构坐标是 entry id，粒度是整段 config 覆盖。这条推论把它推到极致：**没有字段级坐标系，就没有真正的差量定制**——并顺带处理一个社区误读。
...
有趣的是，社区已经自发地用"差值思维"解读 dsh：文章 [20]（一个独立作者的连载系列）明确把本文的理论（Y = X + ΔY、可逆计算、Nop 平台）与 dsh 并列对比，并断言"差值思维是软件构造的必然归宿"。直觉方向完全正确——这说明从差量视角看 dsh 是"自然"的。

但该文对 dsh 配置层的描述需要纠正：它把 dsh 的 patch 机制描述为 `$remove` / `$set` 指令构成的差量代数，能表达删除（负元素）、能像 Git 一样重排合并（Δ·Δ⁻¹=0）。这与源码不符（见上两节核实）：`applyEntryPatches` 只支持以 entry id 为坐标的整段字段覆盖与插入新行，没有深合并、没有负元素、没有结合律保证。"整段覆盖 + 后写的赢"离"差量代数"的距离，就像"离屏渲染"离"微积分"的距离。同一篇文章还把可逆计算理论的提出者署成了别人的名字——直觉传开了，源头却传错了；这处走样与 patch 机制的误读是同一种病：直觉先于事实。

这个误读本身就很有价值——它精确地演示了差量直觉与差量代数的落差：**看到"系统可分解为增量"是直觉，让增量满足坐标定位、负元素、逆运算、结合律才是数学**。dsh 有差量之实（运行时结构空间的带逆叠加），无差量之形（结构空间的整段覆盖）。把另一半补齐，正是可逆计算结构空间做法的用武之地——这就是总结论②的具体含义。
...
这些命题最后都指向同一件事：**约束的艺术**。可逆、scope、边界、观测等价，本质上都是在回答"在哪里限制、限制到什么粒度、越界之后怎么办"。用社区作者的一句话收束这一篇：你不能防御一个你无法限制的东西——架构，就是限制的艺术（[22]）。
...
3. **"副作用不可避免，怎么可能可逆"**——可逆不是逆向执行。它是：运行时结构空间的每个注册行为出生即携带逆，卸载按 LIFO 撤销；边界外显式不可逆，以保留/补偿管理；恢复的判据是观测等价而非字面还原（2.1、2.4 节）。
...
17. chino. *DeepSeek Harness 拆解：一套能拼装的 Agent 架构*. 微信公众号，2026. 本地副本：`ai-dev/references/dsh-community-articles/`
18. Kerry. *Cordis如何支撑DeepSeek Harness的插件化构建：基于源码与架构互证的深度分析*. 微信公众号，2026. 本地副本同 [17]
19. 数字双生子. *DeepSeek Harness 架构解析：Cordis 插件运行时的深度剖析*. 微信公众号，2026. 本地副本同 [17]
20. 数字双生子. *DeepSeek Harness 架构解析：以差值定义系统，用插件组装智慧*. 微信公众号，2026. 本地副本同 [17]
21. 数字双生子. *DeepSeek Harness 架构解析：从理论到实现的鸿沟，如何跨越？*. 微信公众号，2026. 本地副本同 [17]
22. 数字双生子. *DeepSeek Harness 架构解析：沙箱、Code Mode 与安全的未来*. 微信公众号，2026. 本地副本同 [17]
...
| 断言 | 源码位置 |
|---|---|
| dispose = `disposables.splice(0).reverse()` 逆序执行 | dsh/vendor/cordis/src/fiber.ts:431 |
