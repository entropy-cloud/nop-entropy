...
全文顺着"一切皆插件，插件皆可逆"这句宣言展开成三篇，正好逐一回应四条疑问：

- **一切皆插件 => 一切皆差量**：插件化有三重境界，"一切皆插件"成立的真正条件是插件成为差量（回应疑问①②）；
- **插件皆可逆 => Delta 可正可负**：可逆不是逆向执行，而是每个 effect 出生即携带逆；它买到的不是"撤销"这个动作，而是不停机演化整个系统的能力（回应疑问③④）；
- **Delta 概念的自然推论**：坐标系、负元素、Generator、结构空间与运行时空间的分界线——这也是可逆计算理论能给 DSH 补上的东西。

在与读者的深入讨论中，还有三个更具体的技术问题反复出现，它们是上面四条疑问的技术内核：
...
这四个问题的答案分布在第一篇的四个小节：问题 1 关于基本单元——什么样的插件才配作差量，由 1.1（插件的三重境界）与 1.4（差量的具体形态）回答；问题 2–4 关于运算本身——"+"号的精确数学，集中在 1.3 回答。在那之前，1.2 先把公式里的 Base 指认清楚。
...
现在可以回答开头的问题 2–4 了。先给结论：**dsh 的"+"在运行时结构空间是 effect 的 twisted composition ⊕，在 scope 链上是分层遮蔽（nearest shadowing farthest），在事件坐标上是注册序——三处都不是可交换的普通加法，而这恰恰是对的**。

**问题 2：加号如何精确定义？** Cordis 论文把 effect 建模为 Γ → Γ × (Γ → Γ)——一次变更同时产生新状态与逆函数，变更的复合采用扭曲复合 (f₁,g₁)∘(f₂,g₂)=(f₁∘f₂, g₂∘g₁)，逆以相反顺序累积（详见 2.3 节）。运行时结构就是这些带逆注册的叠加：RuntimeStructure = ⊕ᵢ Registerᵢ。这个 ⊕ 不是集合求并，也不是普通求和——每次 registration 都在某个运行时坐标（服务 key、事件名、realm）上叠加一个带逆的 delta。

**问题 3：顺序等价吗？** 不等价，而且 dsh 为不同坐标显式声明了不同的顺序语义（dispatch mode 五类，详见 2.4.8 节）。工具可见性就是一例，我核对了 dsh 的 tools 源码：每个 scope 维护自己的 ToolLayer，view() 解析时按"祖先层从远到近、同名近者遮蔽远者、本 scope 自己的注册最后压上"的顺序合成（`dsh/packages/core/tools/src/index.ts` 中 view() 的注释原话："farthest ancestor first, the exact scope last"、"a nearer scope's same-name entry shadows"）。同一个工具在不同会话里的可见性是运行时按 scope 链实时计算的。并发判定同样是逐次的：调度器在每次调用前重新询问工具的 isConcurrencySafe(args)，读文件与写不同路径可以并行，写同一路径互斥——顺序与冲突语义由坐标与参数共同决定，而非静态白名单。

**问题 4：没有 Base，Plugin1 + Plugin2 能定义吗？** 这里出现了 dsh 与 Nop 的一处深刻拓扑差异：**dsh 的插件之间不直接组合**。Plugin1 与 Plugin2 谁也没把谁当作操作数，它们都垂直地组合到运行时上下文（坐标系）上——星型拓扑，kernel 是事实上的幺元。而 Nop 的差量链是链型拓扑：Δ₃ ⊕ Δ₂ ⊕ Δ₁ ⊕ Base，任何一段差量链本身就是合法的差量（结合律保证），可以脱离具体 Base 先行组合、按需挂接。星型拓扑胜在运行时安全（每个插件只面对运行时，不需要理解其他插件），链型拓扑胜在定制深度（差量可以定制差量）。这个分野贯穿第三篇的全部推论。
...
1. **"一切皆插件是没有必要的统一"**——统一不在功能形态，而在代数结构。Agent Loop 与 Memory 作为功能毫无相似之处，作为"施加在运行时坐标系上的带逆差量"完全同构（1.1 节）。
...
工程代价之问（开销、调试、生态成熟度）不在上面的清单里，因为它不是概念疑虑而是经验命题——论文也只把 Koishi 案例作为存在性验证、把开销测量留作未来工作；本文 2.6 节对它做了正面分析（三个让步：降级承诺、收敛义务、工具化验证）。
