...
- [ ] W4-1 扫码回调端点 + accessCode 编排落 `nop-ai-gateway`：暴露为 `@BizModel` 的 `@BizMutation`/`@BizQuery` 方法（或注册 HTTP 路由，参照既有 `LoginApiBizModel` 暴露方式）；收到信道扫码回调 → 经 `IChannelBindProvider` + `IChannelBindService.findBinding` 查绑定得 platformUserId → **创建 session**（accessCode 消费链 `getUserContextAsync` 按 sessionId 查缓存，扫码登录无预存 session 必须先建——路径待 Phase 0 裁定）→ 构造 `IUserContext` → 调 `IAuthTokenProvider.generateAccessCode(ctx, ttl)` 签发一次性 code → 返回前端。**`nop-ai-gateway` 新增依赖 `nop-biz-auth-core`（取 `IAuthTokenProvider`/`IUserContextCache`，无环——后者不依赖 `nop-ai-gateway`）**。Exit criterion：端点真实可达（W6-2 端到端命中）。**Plan**：`2026-08-08-1837-5-nop-channel-scan-login.md`（draft，Phase 0 设计裁定 session 创建路径后转 active）。
- [ ] W4-2 验证既有 `ILoginSpi.getLoginResultAsync(AccessCodeRequest)` + `ILoginService.parseAccessCode`（→ `IAuthTokenProvider.parseAccessCode`）路径**无需改动**即可消费 W4-1 用 `generateAccessCode` 签发的 code；前端轮询/换取 `LoginResult`。新增信道扫码登录只实现 `IChannelBindProvider`，登录主流程既有契约不被破坏（用集成测试 + 文件 hash 对比证明 `ILoginService`/`ILoginSpi`/`IAuthTokenProvider` 契约未被修改；roadmap"零改动"完成定义经 Phase 0 诚实修正——session 创建路径可能需 additive SPI）。**Plan**：`2026-08-08-1837-5-nop-channel-scan-login.md`（draft）Phase 0-2。
