...
> Last updated: 2026-09-02 (item 13 done：复合场景单进程 LOCAL 落地——fraud-example 扩展为场景模块（S1 `fraud-s1-cdc.stream.xml` 26-transform 四 CEP 链 + S2 base/delta XDSL + 场景主代码 9 类 + 测试 28 用例）全绿；验收断言 A1-1..7/A2-1..7 映射表 100% 落库（A2-4 LOCAL P>1 显式路由 plan 3——引擎 2PC sink P>1 fail-fast 硬门禁）；就地修复 6 项场景发现引擎缺陷（JobGraph 虚拟链顺序/partitioner 丢失、ProcessOperator keyed backend、CEP 状态 JSON 持久化 P2-INV-6、byte[] marker 往返、2PC pendingCommits 键、终态 barrier 竞态——bug note 2026-09-02）；Gap A/B/C 100% 落地（死文件删除/UserTransactionHistory 复活/UnusualAmount 去 stub/S2-java 对照变体）；设计文档 A.0 落地裁定 7 项回写；`./mvnw test -pl nop-stream -am -T 1C` 全绿 + 四工具门禁 exit 0；next todo: item 14)
> Sources:
