...
> Last updated: 2026-09-02 (item 14 done + ★M3 解锁（13+14 done）：复合场景分布式落地——launch 基建扩展（ClusterPipelineFactory seam + RemotePipelineSpec 声明运输 + TM 侧 checkpoint 接线 + 分布式提交/abort 通道）被场景 gated 测试真实消费；分布式验证矩阵 C0—C3 100% 逐格（C2 = TM 2→3 跨集群恢复演练 + keyed-P>1 显式路由 CONN-01 successor/item 29；C3 = sink 内节流两场景）；7 项分布式缺陷就地修复（bug note 2026-09-02-distributed）；item 28 遗留核验显式记录；运行手册初稿 distributed-runbook.md；gated 13/13 + legacy 7/7 启用态绿、默认态全模块绿、四工具门禁 exit 0；M3 分布式场景基线成立)
> Sources:
