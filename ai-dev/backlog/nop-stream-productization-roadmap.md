...
> Last updated: 2026-09-03 (item 25 → `done`：checkpoint manifest 版本化与校验和落地（P-REQ-20 go 裁定 / D-DRIFT-2 收敛）——`EpochManifest` 补 `stateFormatVersion`/`checksum` 两字段（单一版本真值 = core 新类 `CheckpointFormatVersions`，runtime 别名引用），写入/校验集中 `CheckpointSerDe` 咽喉（serialize stamp + deserialize 三段 fail-fast：信封上界/双版本面一致性/checksum 复算，legacy 缺字段显式容忍），coordinator 零改动、接线由真实路径断言钉定，五类测试 14 + 双存储回归 2 + gated 多 JVM 1/0/0/0，plan `2026-09-03-1723-3` completed，closure audit APPROVED（session `ses_f98b9c5e8ffe7AzE79lS268S4g`），全量 3386/0/0/25 绿；items 26/27/28/29/31/32 保持 `todo`)
> Sources:
