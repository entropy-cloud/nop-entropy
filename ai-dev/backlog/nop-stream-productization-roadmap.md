...
> Last updated: 2026-09-01 (item 10 done：connectors 审计报告落库（resolved）+ 小缺陷 8 项修复（CN-1 high 恢复后光标回退 + 序列化器 fail-fast 等，14 新用例，fix-revert 验证）+ hollow 工具 P6b 误报修正；D-GAP 两重点产出（候选钩子清单供 item 20 + XDef 覆盖表）；无大缺陷无 Follow-up 追加；item 22 枚举事实补全（connector 16 文件）；next todo: item 11（plan 已就绪）)
> Sources:
...
- 22. [Follow-up，来源 item 7 plan `2026-09-01-0938-2`（同报告 §1.2 #6）；item 10 closure 事实补全] 测试代码通配符导入清理：core 169 / runtime 108 / cep 15 / flow 1 / **connector 16（connector 5 / batch 5 / jdbc 2 / debezium 4，item 10 审计 live 复算）** 个 test 文件（main 已全模块清零）；跨模块统一 sweep，避免 items 8—11 各自重复机械修改（item 11 closure 将补记 rocksdb 4 + fraud-example 5）: `todo`
