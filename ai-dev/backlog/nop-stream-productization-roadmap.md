...
> Last updated: 2026-09-03 (item 20 → `done`：提交前校验产品化落地（core 探测契约 `ConnectivityCheckable` + 驱动分派 + 凭据支持 `StreamCredentialSupport`（nop-credential-api 唯一新依赖）+ 五族探测实现 + flow validate 校验核心（层 1/2/3，错误含选项名，exit 0/1/2）+ runtime 入口族 `conf-validate`/`dry-run` 子命令 + CDC 凭据引用驻留/引擎侧瞬态解密（`nop-message-debezium` 零变更）+ 62 新测试含 S1 dry-run E2E），P-REQ-13/14 验收达成，closure audit APPROVED（session `ses_f9a677b85ffeJzf9OP3uOSpdFH`）；items 21/24/30 保持 `planned` 待执行；items 22/23/25—32 保持 `todo`)
> Sources:
