...
> Last updated: 2026-09-04 (items 28/31 → `done`：remote-deploy 数据面通道收敛与持续运行稳定性修复——D1 订阅范围三态（TM per-subtask 收敛 + remoteDeployMode 协调器零订阅 + 单 JVM 全订阅不回退）/ D2 队列满有界等待 + typed 溢出失败 / D3 stall 恢复预算分池 + 冷却窗 / W-8 JDBC retained manifests override + 接线验证；设计 = `dataplane-transport-design.md`；复验 = SOAK-3 36000 行 exactly-once（36× 旧阈值）+ CHAOS-1 kill ×2 fencing 严格递增 + BP-1 三档全推进 + C0—C3/legacy 不回退，runId 留档 `_tmp/mini-stream-cluster/`；plan `2026-09-03-1951-3` completed，closure audit APPROVED（session `ses_f9781e482ffe2cTz95Xlzo5zvZ`）；随复验发现并路由 items 33（manifest retention）/34（HA 接管 duplicate-key 中止，pre-existing 证据 = item 15 产物同签名）；items 29/32 保持 `todo`；前序 item 27 → `done` 记录保持)
> Sources:
