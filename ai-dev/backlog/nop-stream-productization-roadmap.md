...
- 15. 分布式稳定性与性能演练（长时 soak、backpressure 行为、chaos 矩阵 + 指标采集；允许与 Phase P 并行）: `planned`（plan `ai-dev/plans/nop-stream-productization/2026-09-02-2216-2-stability-performance-exercise.md` active，三轮独立审查共识；执行顺序建议在 item 16 之后——偏好非硬前置，先于 item 16 执行时按 Phase 1 代理观察降级裁定）
