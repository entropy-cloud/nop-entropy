...
> Last updated: 2026-09-03 (items 22/23/25 → `planned`：DRAFT_PLANS 轮起草三份执行 plan 并经独立子 agent 对抗性审查达成 consensus（item 22 plan 两轮、items 23/25 plan 三轮），plans `2026-09-03-1723-{1,2,3}`；审查关键修正 = item 22 静态通配为主体的事实基线 + 门禁接入时序（--module scope 过渡期不红默认面）、item 23 双门禁注册表（output-contract/wiring-registry）+ red-list 同步面与编译器不可见引用面（javadoc/内联/反射字符串）、item 25 checksum 计算位置锁作用域裁定（含 sync-fallback 残余）+ 小数文本往返钉定 + 双版本面不一致子裁定；items 26/27/28/29/31/32 保持 `todo`)
> Sources:
...
- 22. [Follow-up，来源 item 7 plan `2026-09-01-0938-2`（同报告 §1.2 #6）；item 10 closure 事实补全；item 11 closure 事实补全] 测试代码通配符导入清理：core 169 / runtime 108 / cep 15 / flow 1 / connector 16（connector 5 / batch 5 / jdbc 2 / debezium 4，item 10 审计 live 复算）/ **rocksdb 4 / fraud-example 4（item 11 审计复算 5，其 TestGeographicAnomalyPatternFix 重写后剩 4）** 个 test 文件（main 已全模块清零）；跨模块统一 sweep，避免 items 8—11 各自重复机械修改: `planned`（plan `ai-dev/plans/nop-stream-productization/2026-09-03-1723-1-test-wildcard-import-cleanup.md`，2026-09-03 draft review 达成 consensus；起草复核固化事实基线：317 文件全部含 `static Assertions.*` 形态、非静态类型通配仅 core 23/runtime 55/connector 2；plan 含防回潮门禁（invariants 工具新子命令 + committed fixture + 默认面时序裁定），并显式排序为 item 23 前置）
- 23. [Follow-up，来源 item 7 plan `2026-09-01-0938-2`（同报告 §1.2 #8）] nop-stream-core execution 根包重组：31 个根文件中 Task 执行族（Task/SubtaskTask/TaskExecutor/StreamTaskInvokable）下沉子包（06-30 审计建议的 execution.runtime 拆分），属跨模块 import 变更: `planned`（plan `ai-dev/plans/nop-stream-productization/2026-09-03-1723-2-core-execution-package-reorg.md`，2026-09-03 draft review 达成 consensus；审查补全两处关键面：①双门禁注册表（output-contract-registry fqcn/file + wiring-registry 8 条 file:line wiringPoint）+ red-list 行号同步为必做项；②编译器不可见引用面（javadoc {@link} 3 main 文件/测试内联 FQN ≥5 文件/TestOutputContractInvariant 反射字符串 10+）独立盘点；序列化影响（Task/StreamTaskInvokable Serializable）与子包名裁定入 Phase 1）
...
- 25. [Follow-up，来源 item 8 plan `2026-09-01-0938-3`（runtime 审计报告 §2.1 ①a）] checkpoint manifest 版本化与校验和落地（P-REQ-20 go 裁定 / D-DRIFT-2 收敛载体）：EpochManifest 补 `stateFormatVersion`（alias CheckpointSerDe 格式信封版本为单一版本真值）+ `checksum`（canonical 序列化去 checksum 字段后 SHA-256）+ 协调器写入 / restore 旧 manifest 兼容 + 测试；落地后同步 checkpoint-design.md §2.6 字段表（跨 core/runtime，item 8 已裁定方向）: `planned`（plan `ai-dev/plans/nop-stream-productization/2026-09-03-1723-3-checkpoint-manifest-versioning-checksum.md`，2026-09-03 draft review 达成 consensus；审查强化三处设计裁定面：①checksum 计算位置不预承诺、按锁作用域裁定（非增量 build 在 monitor 内 + sync-fallback 残余显式裁定）；②确定性基底层钉定扩展到小数/浮点文本往返（keyedStates 透传/accumulator 任意类型的三选一裁定）；③stateFormatVersion 读侧不匹配 + 双版本面（信封键 vs 字段）不一致子裁定，消除 detectFormatVersion 静默接受高版本现状）
