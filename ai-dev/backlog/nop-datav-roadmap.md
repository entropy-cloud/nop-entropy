...
- D2-4. 前端（flux dashboard-filter 约定与 nop-datav 参数模型对齐）: `done` ✅（plan `ai-dev/plans/nop-datav/2026-08-15-1134-2-flux-dashboard-filter-param-alignment.md` completed — D2-4 后端交付：对齐契约定稿 linkage-design.md §十一（11.1–11.7：产出形态=参数描述符数组（拒绝完整 form schema/并入 D1-4 布局导出物）；date-range 契约形态钉死 delimited 绝对串 delimiter=`,`/valueFormat=yyyy-MM-dd 且后端拥有双向转换（exporter 合并 initialValue + resolver 增设 delimited 接受形态，additive 向后兼容、扁平 key 仍 canonical）；relative 语义/datetime valueFormat 显式出契约并核实记录；widget 词表封闭映射 {dropdown,date-picker}→{select,input-text,input-number,input-date,date-range}，词表外/组合非法显式报错不静默回退；options 裁定定义产出不含、flux 页面 schema 自供；URL 同步扁平 key canonical + delimited 兼容）+ `exportDashboardFilter` action（@BizQuery + @Auth admin,user，权限点双侧登记）+ `DashboardFilterDefExporter` 纯函数 + 2 新错误码 + 27 新测试（12 exporter + 8 resolver delimited + 5 API + 2 E2E 闭环含接线验证：定义→模拟 flux 提交→校验→getDashboardData SQL 注入，不同筛选值产出不同行集），557/0/0 datav-service 全绿；独立 closure audit CLOSABLE 无 Blocker/Major；flux 侧筛选栏实际渲染/对接为外部后续（非本仓库 debt））
