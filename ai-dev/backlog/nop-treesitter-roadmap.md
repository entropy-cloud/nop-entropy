...
- 15. Recovery hardening: `done` (plan `2026-09-11-1500-1-recovery-hardening.md`, closure-derived; real root cause of the python splat divergences was GSS pop-path enumeration order — FIFO BFS vs C stack__iter scan — fixed in a477a4c887, plus stale-index slice handling in reduce() fixed in 055c8cbaa6; supersedes the earlier selectTree/addSlice narratives below; buildErrorComposite child-count crash fixed f87068bc3f; 15-input cross-grammar recovery verification set 10/15 C-identical with 5 watch-only multi-round shape residuals; zero-width loop class has no live member — guard reclassified safety-net-only; 15(b) per-version scanner state proven by PythonScannerStateIsolationTest)
  - Superseded diagnoses (kept for history): selectTree variant-pick hypothesis (disproven by instrumentation — only one version ever reached accept); addSlice/extractor SHIFT-missing hypotheses (blob verified correct; the real issue was slice ordering).
- 16. Python corpus validation to ≥ 95%: `done` (plan `2026-09-11-1600-1-python-corpus-readjudication.md`, closure-derived; 3 splat-class adjudications removed after item 15 fixes — those sections now match upstream byte-for-byte; **115/117 = 98.3%**, 2 remaining adjudications are multi-round recovery shapes)
- 17. Performance: GLR allocation reduction: `done` (plan `2026-09-11-1700-1-glr-allocation-reduction.md`, closure-derived; landed: Subtree record caching + cons-cell pop paths (16.5 ops/s), scanner-bytecode validation memoized per language (was ~96% of 433 MB/op — re-validated per token scan; 57a3ec5b03). Result: pure runtime **42.5 ops/s ≈ JNI 39.2 (parity)**, allocation 433→86 MB/op. JFR measurement overthrew the dead-branch attribution: the parse is near-linear post-item-15. Adjudicated to successor design: per-subtree refcount reclamation (reference-graph argument in perf-tuning.md) and cursor-materialization-free walk (≈86 MB/op residual, now dominant); GLR-private pooling measured <1% — not worth landing. **User-directed successor perf-closure plan completed 2026-09-13** (`2026-09-12-1000-1-perf-cursor-and-benchmarks.md`, 0f9e6200aa): cursor-materialization-free walk landed (86 MB → 153 KB/op), all four item-13 C-vs-Java benchmarks now ≤ 3x (1.20x/1.26x/1.56x/2.76x), JNI parity exceeded (pure 101.4 vs JNI 45.2 ops/s))
