...
- 15. Recovery hardening (2026-09-10 status update: **identified but fix requires GLR core redesign**)
  - **Root cause confirmed**: GLR condense `compareVersions` treats pattern route and expression route versions as equal (cost=0, prec=0, not in error) → `NONE` → tries merge → fails (different states) → versions coexist BUT during advance, pattern version reaches state 1483 where `.` triggers REDUCE chain → the chain produces `list_splat_pattern(attribute(b,c))` on version 1 but `selectTree` picks the wrong tree because `compareTrees` compares symbol IDs where `attribute` (204) < `list_splat_pattern` (184) is FALSE → 204 > 184 means expression tree has higher symbol → the expression tree is selected instead.
  - **Why the expression tree wins**: The expression route tree `(assignment (pattern_list (identifier) (attribute (list_splat (identifier)) (identifier))) (identifier))` has pattern_list → attribute → list_splat → identifier. The pattern route tree has pattern_list → list_splat_pattern → attribute. In `compareTrees`, both trees start with `assignment`, recurse into pattern_list, recurse into children. The first differing child: `list_splat_pattern` (184) vs `attribute` (204). Since 184 < 204, the pattern tree SHOULD win. But the actual trees might have different shapes than expected.
  - **Diagnostic evidence**: blob small state 616 has 7 groups correctly covering all symbols (verified by direct dump); state 232 has 22 groups including `*` (11), `list_splat_pattern` (184), `attribute` (204), etc.; JNI produces the correct tree with the same grammar.
  - **Fix direction**: Need instrumented version tracing to see exactly which versions exist, what trees they carry, and why `selectTree` picks the wrong one. This is GLR core debugging that needs a dedicated session.
  - **Related**: 3 sections in expressions.txt (Assignments, Print used as identifier, Subscript expressions), 1 in errors.txt, 1 in literals.txt = 5 total adjudicated.
: (a) GLR reduce-chain version management — `addSlice` head==stopNode check causes later REDUCE to overwrite earlier REDUCE's version when both pop from the same GSS path (precisely diagnosed: state 232 `*` SHIFT missing from blob due to addSlice reuse, verified with full parse table dump); (b) per-version external-scanner state deepening for recovery path; (c) zero-width token loop root cause fix: `todo` (see `ai-dev/bugs/2026-09-10-treesitter-ts-recovery-nontermination.md` appendices 1-5; zero-progress guard shipped as stopgap)
