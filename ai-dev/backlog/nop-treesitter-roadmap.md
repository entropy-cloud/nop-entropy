...
- 17. Performance: GLR allocation reduction: `done` (plan `2026-09-11-1700-1-glr-allocation-reduction.md`, closure-derived; landed: Subtree record caching + cons-cell pop paths (16.5 ops/s, −80% GC pressure), scanner-bytecode validation memoized per language (was ~96% of 433 MB/op — re-validated per token scan; 57a3ec5b03). Result: pure runtime **42.5 ops/s ≈ JNI 39.2 (parity)**, allocation 433→86 MB/op. JFR measurement overthrew the dead-branch attribution: the parse is near-linear post-item-15. Adjudicated to successor design: per-subtree refcount reclamation (reference-graph argument in perf-tuning.md) and cursor-materialization-free walk (≈86 MB/op residual, now dominant); GLR-private pooling measured <1% — not worth landing)
