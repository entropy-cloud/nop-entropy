...
- W4. ILlmDialect 双向转换补全（parseRequestBody 补全 Anthropic/Gemini/Ollama/Responses 五 dialect + buildResponse/buildStreamChunk 逐 dialect + javadoc/UOE 消息更新 + 双向转换测试）：`done`（plan `2026-08-15-0849-1`，2026-08-15 独立 closure audit closure-approve：12 新方法 + 20 参数化双向转换测试（roundtrip/E2E 三方向闭环）+ 12 converter 接线测试（frontendLlm 4 dialect）+ 文档同步）
- W5. 模型类路由组 + 动态选择策略（nop-ai-core：model-class 数据结构与解析 + 选择策略接口 + 默认策略（健康度+并发感知+声明序）+ 规则策略（XLang 可选）；配置形态按 Q8 在 plan 阶段裁决；**粒度约束：若预估超出单 plan 规模（数据结构/接口/默认策略 与 游走/并发记账/规则策略 为两组可拆工作），plan-first 先行拆分裁定，不硬撑单文件**）：`done`（plan `2026-08-15-0849-2`：新 `model-class.xdef` 配置面（Q8 裁定）+ `LlmConfigHelper` 解析 + `io.nop.ai.core.routing`（`ISelectionStrategy`/`DefaultSelectionStrategy`/`ModelClassRouter`/`ConcurrencyRegistry`）+ 全池饱和 fail-loud `ERR_AI_MODEL_CLASS_SATURATED` + 35 新用例 + 文档回填；**Q10 规则策略拆 successor（out-of-scope improvement，Successor Required: yes，见 Work Items Q10-successor 条目）**）
- W5b（Q10 successor）. 规则选择策略（XLang 规则 DSL 形态 + IoC bean 绑定方式；`ISelectionStrategy` 的另一种实现，替换默认策略的可插拔扩展）：`done`（plan `2026-08-15-1116-1`：`RuleBasedSelectionStrategy`（rule.xdef DSL + `IRuleManager` 消费 + 固定输入/输出契约：model/provider/candidates/health/attempted（不含 accountKey）+ selectedIndex 输出，computed 输入谓词模式——`<expr>` op XML 不可用执行期实证）+ nop-ai-core nop-rule-core 依赖 + nop-ai-gateway bean 注册（ioc:optional ref + ruleName/ruleVersion）+ 13 策略用例 + 2 IoC 用例 + 接线/端到端全链 + requirement §五 Q10/§3.3/§3.4/§4.1 回填）
- W6. 本地 IChatService 适配器 + 流式 failover（nop-ai-gateway：包装 ChatServiceImpl + accountKey/accountBaseUrl/model 下沉 + chunk 流层首段缓冲/重订阅）：`done`（plan `2026-08-15-1116-2`：`ChatServiceFailoverAdapter` + `FailoverStreamFlow`（`io.nop.ai.gateway.failover`）——非流式切换链（分类→记账→重选→预算→fail-loud）+ 熔断探活恢复 + acquire 后复查 + 全终止路径并发配对；流式首段缓冲（N=10/T=1000ms 可配置）+ 窗口内重订阅 + 窗口外断流 + per-attempt token + 取消顺序契约；breaker/registry 单例 bean + `nopChatServiceFailoverAdapter` 非默认 bean；32 新用例全绿）
- W7. 网关形态：拦截器 + converter 扩展（nop-ai-gateway：IGatewayInterceptor 实现 + AiDialectBackendMessageConverter 流式扩展（stream=true）/per-request 动态 dialect/目标 provider LlmModel config + 字节流层缓冲/重订阅，依赖 W1 spike 结论）：`done`（plan `2026-08-15-1116-3`：`AiGatewayFailoverInterceptor`（onRequest 流式转换/onStreamElement per-attempt 反向转换/onError 分类 rethrow 或降级终止/invoke 非流式编排）+ `GatewayStreamingRetryCallback`（M-3 窄接口 + B-14 base 替换语义 + apiKey 下沉）+ `GatewayStreamingLifecycleListener`（fetch +1/终止 -1）+ converter 流式/动态 dialect/真实 config（properties 通道 B-9）+ nop-gateway 最小通用改动（`BufferedStreamingPublisher` + `IStreamingRetryCallback`/`IStreamingLifecycleListener` + gateway.xdef bufferEnabled/bufferSize/bufferTimeMs codegen 再生成，零 nop-ai 依赖）+ beans 注册（挂载契约 M-7）；13 新测试全绿（流式全链端到端 B-10/B-12/B-14 + 路由级 F3 + onError 契约 B-17 + 预算/窗口外/饱和/直通）+ 既有 152 网关用例零回归）
- W8. 可观测性指标 + 回归测试收口 + docs-for-ai 同步（切换次数/熔断状态/冷却/饱和指标契约；全链测试；使用文档）：`done`（**OBS-01 ✅ done**（plan `2026-08-15-1615-1`，2026-08-15 独立 closure audit `ses_ffb504316ffeX7gS4Oa7gajPmu` closure-approve：§3.6 指标契约（11 指标行 + 并发近似语义 + 冷却期三事件 + null-object/fail-fast 边界）+ `IFailoverMetrics`/`FailoverMetricsImpl`（micrometer，nop-job 先例）+ `CircuitObservation` 编排层派生（core 原语零改动）+ 两形态全事件点接线 + `nopAiFailoverMetrics` bean + 8 focused 指标测试 + 160 用例两次全绿零回归）；**OBS-02/03/04 ✅ done**（plan `2026-08-15-1615-2`，2026-08-15 独立 closure audit `ses_ffafacdf6ffeu4MwcP8IttwqqA` closure-approve：全链回归矩阵 66 格 100% 覆盖（15 新测试补齐全部 gap，含 CACHE_STATE_LOST 可达性配置与 409 errorMapping）+ 2 个 in-scope 行为缺口修复（本地非流式 CACHE_STATE_LOST 原地重发缺 acquire 挂起缺陷；网关流式探活恢复契约缺失 onStreamComplete → recordSuccess）+ `docs-for-ai/03-modules/nop-ai-gateway.md` 使用文档独立页（INDEX/source-anchors/nop-ai.md 同步 + doc link 0 errors）+ 需求文档 §五 Q2/Q3/Q4/Q5/Q7 状态收口（未裁决项显式保持待人工确认）+ 175 用例连续 2 次全绿 + nop-gateway 64 用例回跑））
- ★ **Milestone: AI 网关透明账号切换落地**（W1-W8 全部 done）：`done`（2026-08-15 W8 closure audit 通过后 W1-W8（含 W5b/Q10-successor）全部 done，派生状态自动满足；2026-08-19 补记账同步）
