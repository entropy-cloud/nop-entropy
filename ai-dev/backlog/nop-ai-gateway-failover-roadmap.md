...
- W7. 网关形态：拦截器 + converter 扩展（nop-ai-gateway：IGatewayInterceptor 实现 + AiDialectBackendMessageConverter 流式扩展（stream=true）/per-request 动态 dialect/目标 provider LlmModel config + 字节流层缓冲/重订阅，依赖 W1 spike 结论）：`planned`（plan `2026-08-15-1116-3`，独立 draft review 三轮 consensus 通过）
