...
- W8. 可观测性指标 + 回归测试收口 + docs-for-ai 同步（切换次数/熔断状态/冷却/饱和指标契约；全链测试；使用文档）：`planned`（**OBS-01 ✅ done**（plan `2026-08-15-1615-1`，2026-08-15 独立 closure audit `ses_ffb504316ffeX7gS4Oa7gajPmu` closure-approve：§3.6 指标契约（11 指标行 + 并发近似语义 + 冷却期三事件 + null-object/fail-fast 边界）+ `IFailoverMetrics`/`FailoverMetricsImpl`（micrometer，nop-job 先例）+ `CircuitObservation` 编排层派生（core 原语零改动）+ 两形态全事件点接线 + `nopAiFailoverMetrics` bean + 8 focused 指标测试 + 160 用例两次全绿零回归）；**OBS-02/03/04 归 plan `2026-08-15-1615-2`**（全链回归 + docs-for-ai + 需求文档收口，依赖 OBS-01））
