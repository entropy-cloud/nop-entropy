...
- W15-impl. 邮件验证码 + 可信设备实现（复用既有 `nop-integration-api` `IEmailSender`（腾讯实现已在）+ `TencentEmailSender`；可信设备/记住此设备：设备指纹 + 会话策略）：`planned` — plan: `ai-dev/plans/2026-08-17-2212-2-mfa-email-code-trusted-device-w15.md`（2026-08-17 DRAFT 轮起草，独立 draft review 两轮共识后 active：EmailCodeStore 三实现（装配触点单文件点名含 nopActiveEmailCodeStore 工厂 bean）+ email 因子全链（enabled 门控三入口统一）+ 登记通道 email 解锁 + NopAuthMfaTrustedDevice + 指纹豁免（checkMfaRequired headers 穿线 + 副本"不加豁免分支"显式裁定 + OAuth 回归断言）+ 登记（verifySecondFactorAndComplete 穿线/禁放 completeMfaLogin（恢复码共用）/ATTR_MFA_ACCESS_CODE attr 回填先例）+ 撤销矩阵 + 并发裁定；3 Major + 7 Minor 首轮发现全处置，复审 READY。执行顺序接 W14 之后）— 依赖：W12-design
