...
> Status: active
> Last updated: 2026-08-17（A1-audit `done`——六维度对抗审计 + P1×2 修复 + 裁决零悬挂 + 8 项 deferred 终局裁定，独立 closure audit Can Close；successor C1-hardening 登记 `todo`；W13-impl / W16-design 保持 `planned` 待执行轮）
...
- W13-impl. 角色级 MFA 强制策略引擎实现（策略模型：存储/继承/评估，角色 → 强制因子映射；一期全局开关 + 用户级启用保留兼容）：`planned` — plan: `ai-dev/plans/2026-08-17-0447-2-mfa-role-level-policy-engine.md`（NopAuthRoleMfaPolicy 实体 + evaluator max/AND 合并 + checkMfaRequired 第三态 + mfaRestricted 先设后存与 Dao-cache 白名单双触点 + 受限会话拦截（executor/checker 双触点，不受 operation-mfa.enabled 门控）+ 登记通道 proof token 往返 + confirmMfa 防降级 + OAuth 一期绕过修复（SPI 落 nop-biz-auth-core 预裁定）+ 4 错误码；无策略行 = 一期行为零回归）— 依赖：W12-design
