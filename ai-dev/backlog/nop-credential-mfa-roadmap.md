...
> Status: active
> Last updated: 2026-08-17（DRAFT 轮：W14-impl/W15-impl/W16-impl 三 plan 起草并经独立 draft review 两轮共识后 `planned`（执行序 W14→W15→W16，plan 文件 `2026-08-17-2212-{1,2,3}-*.md`；W16 批次裁定首批 + 配套登记扩展批次新工作项 W16-impl-ext + A3 依赖注记）。前序：EXECUTE 轮 C1-hardening `done` 合并登记——C1a `done`（plan 2026-08-17-1345-1 四 Phase 全落地——25 finding ID + 观察 2 修复收口，独立 closure audit 通过）+ C1b `done`（plan 2026-08-17-1345-2 两 Phase 全落地——@MfaRequired 四动作标注（reencryptAll/delete/grant/revoke）+ 依赖边 nop-credential-service → nop-biz-auth-api 显式化（API-only）+ 容器级元数据断言/负例/零介入回归 + owner docs 章节改写，独立 closure audit 零技术阻塞，证据见两 plan Closure 段）；W16-design 早前 `done`——迁移二期设计文档落盘（03-integration-metadata-migration-design 八节），四主题独立 review 全 PASS；W13-impl 早前 `done`）
...
- W14-impl. WebAuthn/FIDO2 实现（MfaType 扩展位——live 为 `NopAuthMfaSetting.mfaType` VARCHAR 列 + `NopAuthConstants.MFA_TYPE_*` 字符串常量，非枚举，白名单校验点随 W12-design 清单更新；术语/枚举化裁决留 W12-design；外部 MFA 服务 Authy/Duo 评估后并入或显式 deferred）：`planned` — plan: `ai-dev/plans/2026-08-17-2212-1-mfa-webauthn-fido2-w14.md`（2026-08-17 DRAFT 轮起草，独立 draft review 两轮共识后 active：库选型 POC 五项核对（含测试侧 CBOR 构造能力，POC 失败升级不自研）+ NopAuthMfaCredential 实体 + 三 ceremony（challenge 创建三触点同步含 MfaLoginPolicyServiceImpl 副本——W13 同步不变式履行）+ confirmWebauthnRegistration 受限会话白名单增补（minMfaLevel=3 引导流）+ sendMfaCode else 兜底分派重构（无双 plan 常量冲突）+ credential 管理 + signCount 并发语义裁定；2 Major + 7 Minor 首轮发现全处置，复审 READY）— 依赖：W12-design
