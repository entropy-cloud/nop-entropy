...
- W16-impl. nop-integration / nop-metadata 深度迁移实现：`planned` — plan: `ai-dev/plans/2026-08-17-2212-3-credential-integration-metadata-migration-w16.md`（2026-08-17 DRAFT 轮起草，独立 draft review 两轮共识后 active：**批次裁定 = 首批**（共享解析支持 + SMS×2 + metadata 数据源 + 三类型实例 + runbook；扩展批次 Email×2/Feishu/OSS/SFTP 登记为 roadmap 显式新工作项 = Phase 1 Decision 执行项，A3 收口以其登记为前提）；**SPI 增量裁定**（draft review Blocker 解裁定）：`CredentialData` 可选 typeName 属性（错型校验精确实现）+ 迁移支持 SPI（按名查/consumerRef 查/创建凭证，实现落 nop-credential-service；`ICredentialProvider` 六方法冻结保持）+ registerUsage live 行为更正（catch-all WARN 防启动阻断）+ 反查主源 consumerRef 唯一约束（长名截断+哈希）+ delete/deleteByQuery 双覆写 + testConnect 窄域 try；2 Blocker + 4 Major + 4 Minor 首轮发现全处置，复审裁定修完即 READY。执行顺序接 W14/W15 之后（email 家族扩展批次验证面依赖 W15 消费链））— 依赖：W16-design
