...
- A2-followup-1. MFA 敏感数据治理族（A2 裁决 successor-A）：`todo` — 覆盖 D5-F1（4 MFA 表裸 CrudBizModel 通用 mutation 通道收紧）/ D6-1（TrustedDevice 豁免行伪造面）/ D3-F3（恢复码 CRUD 植入）/ D1-7（setting.phone 通用面脱敏）/ D1-1（TOTP confirm/unbind 无尝试上限）/ D3-F2（恢复码 used 并发双花）/ D1-3（verifyChannelProof 失败审计）/ W12 路由项 3（联系方式修改通用 CRUD 敏感化）。实施形态需平台级裁定（override 拒绝 / 敏感列不可写 / xmeta 限制）——plan-first（权限/认证模型面）。不阻塞 A3-audit（P2/P3 治理项）。— 来源：`ai-dev/audits/2026-08/2026-08-18-1244-deep-audit-nop-auth/adjudication.md`
- A2-followup-2. 操作级 MFA 补全 + webauthn 管理面裁定落地（A2 裁决 successor-B）：`todo` — 覆盖路由项 1 落地（removeWebauthnCredential 加 @MfaRequired + 容器级元数据断言同步）/ D2-F2（mfaVerifyAsync scene 校验，需设计 §3.5 再裁定）/ add-key-while-enabled 端点（多设备 UX 正规入口，替代已关闭的"解绑保留行累积"路径）/ D5-F3（@MfaRequired 误标构建期 fail-fast 扩展）。不阻塞 A3-audit。— 来源：同上
