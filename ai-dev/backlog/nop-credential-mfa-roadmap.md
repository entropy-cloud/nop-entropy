...
- W11-impl. 凭证归属统一（系统级/用户级）+ RBAC 细粒度授权实现（NopCredential 归属字段 scope/ownerId 落库；`ICredentialProvider` 消费侧归属+授权校验；BizModel 按归属过滤 CRUD；多系统共用密钥服务的归属语义）：`doing` — **Part A 已收口**（归属统一）：plan `ai-dev/plans/2026-08-14-2342-3-credential-scope-ownership.md`（2026-08-16 执行收口：scope/ownerId ORM 列（NULL 视同 system，DDL 无默认值裁定）+ provider per-method 归属矩阵（明文出口 owner 唯一/管理面 owner+admin，解密前 fail-closed 先序 delFlag）+ BizModel 两层防御（defaultPrepareQuery 结构性读过滤 + 写分级 + 单条越权归一"不存在"）+ 六旁路动作收口（update/batchDelete/updateByQuery/deleteByQuery/copyForNew 禁用、batchGet 过滤语义）+ usage 查询面/reencryptAll 限管理员（`nop.credential.admin-roles` 配置）+ action-auth delta（saveCredential/mutation 对登录用户开放）+ W9 beginOAuthFlow 归属校验回补 + Web 归属最小面；一期契约锚点零回归，证据见 plan Closure 段）。**Part B（RBAC 细粒度授权）待办**：`NopCredentialAuth` 实体 + §6.3 判定矩阵消费侧串联 + 授权管理面 + Web AMIS——依赖 Part A 的 scope 列（user 级不叠加角色授权的判定前提），由 successor plan 起草产出；**W11-impl 的 `done` 以 Part A/Part B 双 plan 全部 closure audit 通过为准，勿因 Part A 收口标 done** — 依赖：W9-design
