...
- C1-hardening. 凭证库二期审计加固（A1-audit successor 收口）：`todo` — 来源：`ai-dev/audits/2026-08/2026-08-17-0518-deep-audit-nop-credential/adjudication.md` §1.2/§三。范围：P2 五项（D6-02 NopAiModel delete 不 unregisterUsage 运维死锁 / D1-01 继承查询动作 data 置空 / D1-03=D4-07 写路径错误码归一 / D3-01 vault active-key-id 配置归一 / D4-02 action-auth 与 admin-roles 双源对齐）+ P3 打包（D1-02/04、D2-02~05、D3-02~05、D4-04/06、D5-03/05/06/07、D6-03/04、wiring 测试增强）+ 可选项（PKCE 纵深加固 / oauth2 testCredential 连通性设计 / state 批量清理 / KMS 关窗自动化与 reencryptAll 进度上报）+ **@MfaRequired 标注小 plan**（A1-audit 裁定：应标注 `reencryptAll`/`delete`/`NopCredentialAuth.grant/revoke`，`saveCredential`/`beginOAuthFlow` 不标注（缩窄裁定）；依赖边 `nop-credential-service → nop-biz-auth-api` 为 API-only 干净边，已裁定可加）。建议拆分 C1a（代码加固）/C1b（@MfaRequired 标注）两小 plan。— 依赖：A1-audit done
