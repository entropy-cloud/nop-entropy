...
- W16-impl-ext. 深度迁移扩展批次（Email×2/Feishu/OSS/SFTP 发送器 credentialId 接线 + feishu/oss credentialId 配置键 + 对应凭证类型实例）：`planned` — plan: `ai-dev/plans/2026-08-18-0904-2-credential-integration-migration-ext-w16-impl-ext.md`（2026-08-18 DRAFT 轮：五家族接线按设计 §4.1 结论 6 分层时序（Email×2/SFTP 逐次消费期、Feishu start 期解析副本（登记载体执行期定稿 + 硬约束：可达 + usage 行断言）、OSS 构造期）+ 五类型实例 + 五枚 consumerRef token + fail-closed 矩阵负例（JavaEmailSender withTransport 吞异常/SftpClient connect 包装两处落点风险已标注）+ Email MFA 发码链 E2E（nop-auth-service 显式列入测试门）+ §七#1/#6/#7 裁定收口；经两轮独立 fresh subagent 对抗 review（首轮 1 Blocker（E2E 模块缺验证门）+ 2 Major（Feishu 登记载体不可达/fail-closed 落点）+ 8 Minor 回修；次轮 1 Blocker（:nop-credential 聚合器空转门 → :nop-credential-service）回修后共识），执行序 N=2，与 A2-audit 无依赖无交集）— 依赖：W16-impl + W15-impl done（email 消费链成立后验证面完整）。〔W16-impl plan 批次裁定的配套登记（设计 §八 硬约束：不得静默丢失；A3 收口审计以其登记为前提——A3 依赖其登记而非其完成）〕
