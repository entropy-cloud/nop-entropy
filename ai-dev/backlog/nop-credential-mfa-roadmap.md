...
- W16-impl-ext. 深度迁移扩展批次（Email×2/Feishu/OSS/SFTP 发送器 credentialId 接线 + feishu/oss credentialId 配置键 + 对应凭证类型实例）：`todo` — 依赖：W16-impl + W15-impl done（email 消费链成立后验证面完整）。〔W16-impl plan 批次裁定的配套登记（设计 §八 硬约束：不得静默丢失；A3 收口审计以其登记为前提——A3 依赖其登记而非其完成）〕
