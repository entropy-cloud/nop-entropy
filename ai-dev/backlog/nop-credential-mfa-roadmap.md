...
- A2-followup-1. MFA 敏感数据治理族（A2 裁决 successor-A）：`planned` — plan: `ai-dev/plans/2026-08-18-1924-1-mfa-sensitive-data-governance-a2-followup-1.md`。覆盖 D5-F1（4 MFA 表裸 CrudBizModel 通用 mutation 通道收紧，含同族瞬态码表边界裁定）/ D6-1（TrustedDevice 豁免行伪造面）/ D3-F3（恢复码 CRUD 植入）/ D1-7（setting.phone 通用面脱敏——裁定结构性排除 not-pub）/ D1-1（TOTP confirm/unbind 无尝试上限）/ D3-F2（恢复码 used 并发双花条件写）/ D1-3（verifyChannelProof 失败审计）/ W12 路由项 3（联系方式修改通用 CRUD 敏感化）。两轮独立 draft review 共识。不阻塞 A3-audit（P2/P3 治理项）。— 来源：`ai-dev/audits/2026-08/2026-08-18-1244-deep-audit-nop-auth/adjudication.md`
