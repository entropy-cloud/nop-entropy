...
- 44. Region-based failover（G28 续，P2）: `planned`（plan `ai-dev/plans/nop-stream-production/2026-08-03-1403-1-region-based-failover.md`，completed — vision 决策请求 + go/no-go 裁定 **go confirmed（选项 B 流式 + 物化点，2026-08-03，mission-driver proxy 确认同 Stage 41 D7 渠道）— 5 successor plans TBD**（优先级：blocking edge → region 识别 → supervision loop → drain/reconnect → per-region counter，见 `failover-design.md` §9.5）；吸收 Stage 27 deferred G57/G28 续/per-region counter → 归属 successor plans）
...
- 47. Unaligned + rescale 交互: `planned`（plan `ai-dev/plans/nop-stream-production/2026-08-03-1403-2-unaligned-rescale-interaction.md` — 修复 rescale 时 channel state 静默丢弃 live defect（`buildRescaledTaskState` plain `TaskStateSnapshot` → `instanceof TaskEpochSnapshot` 守卫失败）；首版 fail-fast 拒绝（`ERR_STREAM_CHANNEL_STATE_RESCALE_UNSUPPORTED`）；D4 guard focused test；吸收 Stage 43/45 deferred）
...
> Status: `planned` — go confirmed（blocking edge vision 决策已批准，选项 B 流式 + 物化点，2026-08-03）；5 successor plans TBD
