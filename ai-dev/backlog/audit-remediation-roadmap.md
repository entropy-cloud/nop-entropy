...
> 最后更新：2026-08-01（v16 — 1306-1 承接 2130 唯一未落地 successor：6 个 nop-ai-coder `ai-gen.orm.xml` 引用裁定不改（管线生成物语义自洽）+ 归档文件 `ai-gen.orm.xml`/0 字节 `ai-gen.orm.xlsx` 移除 + @Disabled `AiGenCodeTaskManual.saveOrmModel()` 删除，P0-MA2-01 全链路收口）
...
| 第十一批（代码卫生，2026-08-01） | `ai-dev/plans/2026-08-01-0746-2-arm-p3-code-hygiene.md` | MA4.2-12（13 个测试文件 System.out 逐文件裁定：5 转 SLF4J LOG / 1 删除死代码 / 7 手动+示例保留，对照表落盘 daily log，grep 命中=保留集合）+ MA4.2-13（IFileOperator.GrepResult 3 getter 冗余 public 移除，toString 保留；SplitChunk/SplitOptions 裁定不处理——移除即降 package-private 破坏跨包 Jackson 序列化）+ MA4.1-04（DefaultAiChatService.parseToolCalls:562 unchecked cast 补局部 @SuppressWarnings）。纯卫生变更零行为变更，测试数量零减少 | ✅（closed 2026-08-01） |
| 第十二批（错误处理规范，2026-08-01） | `ai-dev/plans/2026-08-01-0936-1-arm-p3-error-handling-agent-shell.md` | P2-MA3-1（nop-ai-shell 12 处裸 IAE → NopException + NopAiShellErrors 5 新码，Redirect/GroupExpr/PipelineExpr/LogicalExpr/ShellCommandRegistry）+ P3-MA3-1（nop-ai-agent 62 处裸 IAE/33 文件 → NopAiAgentException + ERR_AI_AGENT_INVALID_ARG，1834-3 NopAiMavenErrors `{msg}` param 先例）。消息逐字零漂移、cause 保持、测试断言全同步 + 3 个 focused 值级测试（@Test 零减少）、main grep 0 残留、catch-IAE 调用面 5 处核验零交集 | ✅（closed 2026-08-01） |
| 第十二批（错误处理规范 2，2026-08-01） | `ai-dev/plans/2026-08-01-0936-2-arm-p3-error-handling-core-coder.md` | P3-MA3-2（nop-ai-core 7 处裸 IAE/6 文件 → NopException + NopAiCoreErrors 7 新码：FILE_INVALID_EDIT_TYPE/PROMPT_TEMPLATE_NULL/UNSUPPORTED_PARSE_FROM_RESPONSE/UNKNOWN_ROLE/VECTOR_LENGTH_MISMATCH/SESSION_ID_IS_EMPTY/SESSION_ID_INVALID）+ P3-MA3-3（nop-ai-coder 4 处裸 IAE/3 文件 → NopException + AiCoderErrors 3 新码：UNSUPPORTED_CONVERSION/METHOD_SIGNATURE_NOT_FOUND/UNBALANCED_BRACES，JavaMethodReplacer 2 处为 audit 复核段 new finding 显式纳入）。英文描述、消息逐字零漂移、cause 保持、测试断言全同步 + 4 个 focused 值级测试文件（+9 @Test 零减少）、两模块 main grep 0 残留、catch-IAE 调用面 0 命中 | ✅（closed 2026-08-01） |
| 第十二批（错误处理规范 3，2026-08-01） | `ai-dev/plans/2026-08-01-0936-3-arm-p3-error-handling-tools-toolkit.md` | P3-MA3-5（nop-ai-tools 7 处裸 IAE/4 文件 → NopException + NopAiCoreErrors 7 新码：INVALID_PROJECT_NAME/THOUGHT_EMPTY/INVALID_THOUGHT_NUMBER/INVALID_TOTAL_THOUGHTS/TOTAL_THOUGHTS_LESS_THAN_NUMBER/INVALID_STAGE/INVALID_MAX_RESULTS，`nop.err.ai.tools.*` 前缀）+ nop-ai-toolkit 同规范残余（LocalToolFileSystem 9 处裸 IAE → NopAiException，audit 未单独列项本计划显式纳入，DefaultToolExecutorProvider 先例 + error-handling.md 模式二裁定：跨模块消费面仅 getMessage()、无程序化错误码消费）。英文描述、消息逐字零漂移、cause 保持、`isPathAllowed` 安全判定逻辑零变更（MA6.2-003 修复面不受影响）、测试断言全同步 + 2 个 focused 值级测试文件（+16 @Test 零减少）、两模块 main grep 0 残留、catch-IAE 调用面 0 命中 | ✅（closed 2026-08-01） |
| 2130 successor（P0-MA2-01 承接收口，2026-08-01） | `ai-dev/plans/2026-08-01-1306-1-arm-p2-coder-ai-gen-model-refs.md` | 2130 plan Deferred 段唯一未承接的 Successor Required: yes 项：6 个 `nop-ai-coder` 任务/模板文件对 `ai-gen.orm.xml` / `ai-gen.action-auth.xml` 的引用**裁定不改**（全部解析到目标项目 `${outputDir}` 内管线生成物——`ai-coder.task-lib.xml` `saveOrmModel` 写 → `ai-api-design`/`ai-service-design`/`ai-menu-design` `LoadOrmModel` 读 → 模板 `x:extends`，与 `nop-ai/model/` 归档文件概念不同）；归档文件 `ai-gen.orm.xml`（15 实体，位于 `nop-ai/model/`，唯一消费方为 @Disabled `AiGenCodeTaskManual.saveOrmModel()`）与 0 字节 `ai-gen.orm.xlsx` **移除**，`saveOrmModel()` 方法删除 + 7 个未使用 import 清理（转换语义由 `AiConverterTest.testConvertOrm` 覆盖）；全程零行为变更，3602 tests 0 failures，P0-MA2-01 溯源链完整收口 | ✅（closed 2026-08-01） |

其余 MA1-MA3/MA5-MA6 各里程碑的 P2/P3 findings 已由 MR1/MR2/MR3 计划逐批裁定为 `out-of-scope improvement`（MV 矩阵 open=0，无已确认 live defect 残留），登记为 **watch-only residual**。第九批（2026-08-01）按严重度排序重开了其中结构性/测试质量类高价值项（P2-MA1-004/005、P3-MA1-013/014/015/016、P3-MA3-003、MA4.3-09/13、MA4.4-01/02/03、MA4.2-06），其余低价值 watch-only 项（如 P3-MA1-023~030、P3-MA1-039）仍不入 scope。第十批（2026-08-01）承接 MA4.2-05（引擎大文件拆分）——nop-ai 唯一剩余的超 3000 行生产文件治理项，design-first。第十一批（2026-08-01）两个 plan 串行承接公开 API 文档债务 + 代码卫生收口：0746-1（MA4.2-07/-10、MA4.5-008/-009：18 个 deprecated 类迁移指引 + 45 个接口方法 javadoc + 7 个接口类级 javadoc + IFileOperator deprecated 消息英文化）→ 0746-2（MA4.2-12/-13、MA4.1-04：13 个测试文件 System.out 逐文件裁定 + GrepResult 冗余 public 移除 + SplitChunk/SplitOptions 裁定不处理 + parseToolCalls unchecked cast 注解），两个计划共享 IFileOperator.java 编辑面串行执行，全部零行为变更。第十二批（2026-08-01）重开 MA3.4 审计点名的错误处理规范残余：0936-1（nop-ai-agent 62 处 + nop-ai-shell 12 处裸 IAE → NopException + 模块 ErrorCode，P2-MA3-1/P3-MA3-1 fixed）+ 0936-2（nop-ai-core 7 处 + nop-ai-coder 4 处裸 IAE → NopException + NopAiCoreErrors/AiCoderErrors 10 新码，P3-MA3-2/P3-MA3-3 fixed）+ 0936-3（nop-ai-tools 7 处裸 IAE → NopException + NopAiCoreErrors 7 新码，P3-MA3-5 fixed；nop-ai-toolkit LocalToolFileSystem 9 处裸 IAE → NopAiException，同规范残余收口），三个兄弟计划模块面互不重叠、全部 closed，MA3.4 错误处理规范审计点名的全部 P2/P3 项收口。随后 `ai-dev/plans/2026-08-01-1223-1-arm-error-handling-doc-drift-sync.md`（第十二批 doc drift 收口，纯文档计划，零代码变更）承接 0936-2 Non-Blocking Follow-up / Closure Follow-up 登记的 error-handling.md 消息语言例外条款 drift：例外条款从"仅 nop-ai-agent / nop-ai-shell"同步为与 live 代码一致的 6 模块清单（agent / shell / core / coder / maven / code-analyzer，tools 码在 NopAiCoreErrors 随 core 说明），分界语义裁定为**来源驱动**（批次内从裸 IAE/UOE/RTE 逐字转换的码 → 英文；既有业务错误码 → 中文，`ERR_AI_TOOLS_INVALID_THOUGHT` 与英文码同处 `nop.err.ai.tools.*` 命名空间仍为中文即证明），起源叙事覆盖 UOE（2248-2）与 IAE/RTE（0936-1/2/3、1834-3）两类转换来源，0936-2 → 1223-1 追溯链完整。**2130 遗留 successor 收口**：`2026-08-01-1306-1-arm-p2-coder-ai-gen-model-refs.md`（2026-08-01）承接 2130 plan Deferred 段登记的 nop-ai-coder 6 文件 `ai-gen.orm.xml` 引用裁定（第十二批后唯一未承接的 Successor Required: yes 项）——逐文件追踪解析链（`ai-coder.task-lib.xml` saveOrmModel 写 `model/` 下 `ai-gen.orm.xml` 到 `${outputDir}` → design 任务 `LoadOrmModel` 经 `ResourceHelper.resolveResourceInDir` 读回 → 生成项目模板 `x:extends` 命中），裁定**不改 6 文件**（管线生成物语义自洽，与 `nop-ai.orm.xml` 源模型概念不同）；归档文件 `ai-gen.orm.xml`（15 实体，位于 `nop-ai/model/`，唯一消费方为 @Disabled `AiGenCodeTaskManual.saveOrmModel()`）与 0 字节 `ai-gen.orm.xlsx` 移除，`saveOrmModel()` 方法删除（转换语义由 `AiConverterTest.testConvertOrm` 覆盖），P0-MA2-01 双 ORM 漂移 finding 全链路闭环。
