...
> 最后更新：2026-08-05（v19 — **MR6 R6.2 收口（plan-2026-08-05-2157-2）**：P2-11（webhook 重定向 fail-closed：`configureRedirectPolicy` setter + per-dispatchWebhook 门禁 + 3xx 显式拒绝，`ERR_CHECKPOINT_WEBHOOK_REDIRECT_NOT_ALLOWED`，SSRF 测试 19→22）+ P2-12（rawJdbcUrl 明文凭据参数三处移除 + `ARG_RAW_JDBC_URL` 常量删除，连接安全测试 25→27）→ fixed，R6.2 → done；`./mvnw test -pl nop-metadata -am -T 1C` 906/0 全绿；arm-index §P2 P2-11/P2-12 行终态更新。prior v18（MR6 R6.1 收口：P2-10/P2-13 fixed + 901/0 全绿）保持不变）
