...
> 最后更新：2026-08-06（v30 — **MR8 R8.3 收口（plan-2026-08-06-0914-3 全 3 Phase）**：AR-18 + AR-19 两项导入与内存过滤正确性缺陷全部落地——AR-18 buildJoinConditionsJson/buildIndexColumnsJson 手拼 JSON → JsonTool.stringify 结构化序列化（特殊字符不再产出非法 JSON）+ buildSql 反序列化逐元素 instanceof Map 校验（复用 ERR_FIELD_RESOLVE_EXTERNAL_BUILD_SQL_INVALID + 新 elementIndex 参数，不再裸 ClassCastException）；AR-19 MemoryFilterEvaluator 三值逻辑重写（LIKE 元字符转义 / null 比较 UNKNOWN / IN·BETWEEN·NOT 三值对齐 / 空 or 节点恒真）与 FilterToSqlTranslator 语义对齐；判别性测试 +19 red→green 实测，`./mvnw test -pl nop-metadata -am -T 1C` **1016/0 全绿**（997 基线 + 19）；check-plan-checklist/check-doc-links/scan-hollow 全 0；docs 复核 No owner-doc update required；独立子 agent closure audit READY_TO_CLOSE；R8.3 → done。prior v29（MR8 R8.2 收口）保持不变）
...
| MR8 | MR7 | AR-11~23 裁决记录（R8.0 done，提级 19 + 归类 3，0 悬置）+ R8.x 修复工作项（R8.1 done——AR-11~15 五项落地，986/0 全绿；R8.2 done——AR-16 + AR-23③⑤④ 四项诊断与日志修复，997/0 全绿；R8.3 done——AR-18 + AR-19 两项导入与内存过滤正确性修复，1016/0 全绿；R8.4 组行，Deps = R8.0 done） |
