...
> 最后更新：2026-08-06（v28 — **MR8 R8.1 收口（plan-2026-08-06-0914-1 全 4 Phase）**：AR-11~15 五项质量执行/评分正确性缺陷全部落地——AR-11 judgeRegex 签名集合收窄（MySQL/H2 规则级正则错误显式 ERROR）+ docs P2-08 例外收窄；AR-12 cpId==null 移入 try 内 + ERR_CHECKPOINT_MISSING_ID（MA7.5-01 全路径不外抛）；AR-13 `{sqlHash}` 占位 + 脱敏 + expectPassWhen 真实 ruleKey；AR-14 异常规则 ERROR 行补写 + DTO totalRuleCount/skipCount/ruleResults 对账；AR-15 负年龄 fail-loud；判别性测试 +16 red→green 实测，`./mvnw test -pl nop-metadata -am -T 1C` **986/0 全绿**（970 基线 + 16）；check-plan-checklist/check-doc-links/scan-hollow 全 0；独立子 agent closure audit READY_TO_CLOSE；R8.1 → done。prior v27（MR8 R8.0 裁决收口）保持不变）
...
| MR8 | MR7 | AR-11~23 裁决记录（R8.0 done，提级 19 + 归类 3，0 悬置）+ R8.x 修复工作项（R8.1 done——AR-11~15 五项落地，986/0 全绿；R8.2~R8.4 组行，Deps = R8.0 done） |
