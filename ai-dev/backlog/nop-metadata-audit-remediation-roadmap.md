...
> 最后更新：2026-08-06（v25 — **MR7 R7.2 收口（plan-2026-08-06-0553-2）**：AR-04/AR-05 custom_sql 沙箱 DML/TCL + H2 文件族加固——CUSTOM_SQL_FORBIDDEN_WORDS 补 18 词（DML 族 INSERT/UPDATE/DELETE/MERGE/REPLACE + TCL 族 COMMIT/ROLLBACK/SAVEPOINT/SET/TRANSACTION + RENAME/LOCK/UNLOCK 逐项对齐 ExpressionMeasureValidator.KEYWORD_BLACKLIST + H2 文件族 FILE_READ/FILE_WRITE/BACKUP/CSVWRITE/CSVREAD）+ CUSTOM_SQL_FORBIDDEN_SEQUENCES 补 LOAD XML；javadoc 两处修正（"全覆盖"过度声明 → 逐项清单、MERGE/REPLACE 未来时表述删除）；判别性测试 +6 red→green 落地（CTE-DML/FILE_READ/CSVREAD 经 judge() 公开入口接线验证，修复前 focused 19/5 实测全红）；独立 closure audit（fresh session）READY_TO_CLOSE 12 项全 PASS；`./mvnw test -pl nop-metadata -am -T 1C` **949/0 全绿**（基线 943 + 新增 6）+ check-plan-checklist/check-doc-links/scan-hollow 全 0；R7.2 → done，R7.3（plan-2026-08-06-0553-3）登记 planned 待执行。prior v24（MR7 R7.1 收口）保持不变）
