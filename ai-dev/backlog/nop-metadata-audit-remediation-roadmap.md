...
> 最后更新：2026-08-06（v21 — **MR6 R6.4 收口（plan-2026-08-06-0105-1）**：P2-06（checkTableExists 元数据面 SQLException 显式抛 `ERR_AGGR_TABLE_VISIBILITY_CHECK_FAILED` + cause，空结果仍 false 业务语义保持，`TestAggregationHelperTableVisibility` 5/5 含 execute() 端到端传播）+ P2-07（parseDeltaModel fail-fast 抛 `ERR_MODEL_DELTA_PARSE_FAILED` + cause，事务回滚实证无部分状态；fixture 规格执行注记——原 duplicate-key 窄窗口实测不成立，改为 partial-override 窄窗口）+ P2-09（提审失败 fail-loud 抛 `ERR_TAG_LABEL_SUBMIT_APPROVAL_FAILED` + cause，save 回滚标签不落库；测试环境注入真实用户后正路径为真实提审成功）→ fixed，R6.4 → done；`./mvnw test -pl nop-metadata -am -T 1C` 919/0 全绿（909 baseline + 新增 8 判别性测试 + web +2，无 flaky 降级）；arm-index §P2 P2-06/07/09 行终态更新。prior v20（MR6 R6.3 收口）保持不变）
