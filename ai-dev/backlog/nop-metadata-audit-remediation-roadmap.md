...
> 最后更新：2026-08-06（v22 — **MR6 R6.5 收口（plan-2026-08-06-0105-2）**：P2-01（NopMetaSearchProcessor fail-closed 分支保留原始 cause + LOG.error，TestNopMetaSearchProcessor 8→11 恒等/错误码/日志断言）+ P2-02（AutoClassificationProcessor 正则编译失败 LOG.warn（pattern + classificationId + ruleIndex + throwable）+ 单次调用内 (classificationId,pattern) 去重防刷屏，正路径测试从零搭建）+ P2-04（4 处 quality catch 补 LOG.warn：parseValidations 改签名加 checkpointId、MetaQualityScorer 补 LOG 字段、readAutoScoreConfig/readRegisteredCron 记 checkpointId，默认值语义保持）→ fixed，R6.5 → done；`./mvnw test -pl nop-metadata -am -T 1C` 925/0 全绿（R6.4 919 + 新增 6 判别性测试；全量 `-am` 偶遇预存在 nop-stream rocksdb 性能 flaky，单跑复绿 2/2，按 R6.3 降级口径记录）；arm-index §P2 P2-01/02/04 行终态更新。prior v21（MR6 R6.4 收口）保持不变）
