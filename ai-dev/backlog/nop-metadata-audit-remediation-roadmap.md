...
> 最后更新：2026-08-06（v29 — **MR8 R8.2 收口（plan-2026-08-06-0914-2 全 4 Phase）**：AR-16 + AR-23③⑤④ 四项诊断与日志缺陷全部落地——AR-16 三处 LOG.info 完整 SQL/custom_sql 字面量 → sqlHash（完整 SQL 降 DEBUG）；AR-23③ refreshBlocking 失败入 IndexResult（failed + errors + indexed 如实）；AR-23⑤ 扫描故障新错误码 ERR_EXTERNAL_TABLE_SCAN_FAILED（真实 productName）+ precision/scale int→Integer（NULL 不归 0）+ 死码 ERR_DIALECT_NOT_SUPPORTED 删除；AR-23④ searchMetadata limit<0 显式拒绝（ERR_SEARCH_LIMIT_INVALID）；判别性测试 +11 red→green 实测，`./mvnw test -pl nop-metadata -am -T 1C` **997/0 全绿**（986 基线 + 11）；check-plan-checklist/check-doc-links/scan-hollow 全 0；独立子 agent closure audit READY_TO_CLOSE；R8.2 → done。prior v28（MR8 R8.1 收口）保持不变）
...
| MR8 | MR7 | AR-11~23 裁决记录（R8.0 done，提级 19 + 归类 3，0 悬置）+ R8.x 修复工作项（R8.1 done——AR-11~15 五项落地，986/0 全绿；R8.2 done——AR-16 + AR-23③⑤④ 四项诊断与日志修复，997/0 全绿；R8.3~R8.4 组行，Deps = R8.0 done） |
