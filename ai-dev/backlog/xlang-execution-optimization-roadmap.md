...
- I9. 统一后端选择机制（注册 SPI + 决策树 + 配置开关 + 降级观测）：`planned`（plan：`ai-dev/plans/xlang-execution-optimization/2026-08-21-0240-2-i9-unified-backend-selection.md`，2026-08-21 起草并通过两轮独立子agent 审查至共识（round-1 Ready/0 Blocker + 1 Major advisory（场景矩阵模块可见性硬约束）修复；round-2 Ready/0 Blocker，5/5 advisory 确认解决）；执行顺序 `{N}`=2 在 I8 后——plan 内置 I8 硬门禁（未 completed → blocked 回引擎，不绕行）） — 依赖：I4, I8
