...
> Status: active
> Last updated: 2026-08-20（I3 执行完毕 + 独立 closure audit CAN CLOSE 标 done——A 五族 + 残余转译 + 共享 helper 增量 + corpus 覆盖 A 对拍全绿 + 覆盖矩阵机制落地；此前：I3/I6 plan 起草并通过两轮独立子agent 审查至共识；I5 执行完毕 + 独立 closure audit CAN CLOSE 标 done——truffle 后端骨架 + 帧/slot 映射 + 子集翻译 + 翻译缓存落地，对拍 truffle 列激活）
...
- I4. java 转译覆盖 B（函数闭包/控制流/输出节点生成族）+ 覆盖矩阵闭环：`todo` — 依赖：I3。**I3 Phase 1 移交口径（起草时直接消费，勿重新推导）**：B 族 pending 集 = `ExecNodeBaseline.B_FAMILY` 35 类（控制流 11 + 输出/节点生成 12 + 函数邻接 6 + `LocationFunction`/`ReturnScopeValuesExecutable`/`ExecutableFunctionEvalAction`——后三者 I4 闭环时最终裁定转译或维持 pending）；矩阵机制已由 I3 Phase 1 定稿（基线文件级扫描 + 支持集可编程枚举 + 逐类真实转译 + fail-fast 反证 + 红灯注入，断言测试落 java 侧消费 test-jar 共享基线），I4 闭环 = B 族逐类转译后 registeredTarget 扩至全量 + 矩阵全绿
...
- I6. truffle 翻译覆盖 A（数据与作用域族，类别同 I3）：`planned`（plan：`ai-dev/plans/xlang-execution-optimization/2026-08-20-0610-2-i6-truffle-translate-coverage-a.md`，2026-08-20 两轮独立子agent 审查至共识（round-2 Ready 0 Blocker）；承接 I5 移交两项（kind 全量覆盖率实测 + 设计 Q3 残余按名访问路径）；I3 为执行顺序前置非依赖，fallback = 增量对账 + 先落地方为事实源（I5 D3 先例）；树指纹载荷覆盖为 plan 内硬验收（防缓存串用）） — 依赖：I5
