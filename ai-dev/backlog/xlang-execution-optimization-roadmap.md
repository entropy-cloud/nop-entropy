...
- I11. 构建集成 + native image 兼容 + docs-for-ai 同步：`planned`（plan：`ai-dev/plans/xlang-execution-optimization/2026-08-21-0450-2-i11-build-integration-native-docs.md`，2026-08-21 起草并经两轮独立 fresh 子agent 审查至共识（round-1 Not Ready 4 Major → 修复（对账扩六项含 D1 指纹复用/D4 xlib 形态 + 漏跑判别子必答 + e2e 默认 fixture 全真链路禁自定义 ClassLoader + native 下界预置）→ round-2 Ready 0 Blocker/0 Major，6 Minor 当场吸收）；Phase 1 必答：扫描口径清点（含运行时消费维度）/任务落点（ServiceLoader 门控预注）/`_gen/` 布局（postcompile 时序）/xlib 生产形态/清单格式（多 jar 聚合）/漏跑判别子/truffle 排除/native 载体（下界）/e2e 载体（模块环约束）） — 依赖：I10
