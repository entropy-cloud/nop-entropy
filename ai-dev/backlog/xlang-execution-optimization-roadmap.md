...
> Status: active
> Last updated: 2026-08-21（I12 plan 起草并通过六轮独立子agent 审查至共识（round-6 Ready 0 Blocker/0 Major/2 Minor 当场吸收，31 项修复逐轮落地）→ 状态 `todo`→`planned`——阶段二唯一剩余条目；此前（I11 执行完毕（三 Phase）+ 独立 fresh closure audit CAN CLOSE 标 done——构建任务 `XlangJavaGenTask`（模块 pom exec-maven-plugin 增量 execution + 同前端取树（parseClean/RCM）+ 指纹先于转译 + write-if-changed 幂等 + check 哨兵 + 写盘前折叠 fail-fast 原子性 + reflect-config 合并产物）+ 双清单文本行协议分离产物（`META-INF/nop-xlang/`）与 `GeneratedManifestFiles` 多 jar 聚合装载 + initializer 供给闭环（I10 双缝填充，缺省空态行为不变）+ 漏跑可观测（require-manifest 判别子缺省 false；红 = markUnavailable + WARN + 指标）+ xlib 每标签生产形态（`translateTagUnit` + `bindTagFunction` setBody 换绑 + `TagGeneratedBinding`）+ native 兼容（closed-world 断言 + trace 亲跑 vfs-index/reflect 证据 + 真实 native-image 环境不可得显式裁定 + truffle 排除文档化约定）+ e2e fixture 模块 `nop-xlang-java-e2e` 全真链路参考实现 + docs-for-ai 两新章节/INDEX/GEN-010；roadmap 三项验收落地（e2e 对拍三层+身份断言 5/5 / 漏跑红绿 / 重生成幂等））；此前（I10 执行完毕（三 Phase）+ 独立 fresh closure audit CAN CLOSE 标 done——生产 binder（双清单内存契约 + classpath 常规加载 + Executable 树指纹 SHA-256 一致性校验）+ 模型加载期绑定（生成类优先/解释器兜底 + 已绑定/降级直通 + 绑定结果随 RCM 缓存）+ 降级观测分级（stale vs 租户稳态去重）+ roadmap 三项验收落地（corpus 48 静态单元生产路径对拍全绿）；此前（I10/I11 plan 起草并通过两轮独立子agent 审查至共识（round-2 均 Ready 0 Blocker/0 Major）→ 状态 `todo`→`planned`；此前（I9 执行完毕（三 Phase）+ 独立 fresh closure audit CAN CLOSE 标 done——后端注册 SPI + 统一决策树 + 配置开关 + 降级观测 + 动态编译出口清单化核验 + 路由场景矩阵五类场景全绿（经 I1 框架断言）；此前（I8 执行完毕（三 Phase）+ 独立 fresh closure audit CAN CLOSE 标 done——SHARED 形态切换 + 共享 Engine/Context 池租借协议 + 翻译缓存淘汰 + 翻译失败观测事件 + 并发对拍 75 用例全绿（roadmap 验收两项落地）；此前（I7 执行完毕（三 Phase）+ 独立 fresh closure audit CAN CLOSE 标 done——truffle 覆盖 B 全族 33 类 + 非根 CallFunc 形态翻译（支持集收敛全量 120）+ 两级内联缓存（DSL dispatch + XStaticMethodNode 回填 + 探针可观测）+ ExitMode 异常族 + 急切值拷贝闭包 + 输出换缓冲 + corpus 覆盖 B 对拍 20/20 + 矩阵闭环 126；此前（I4 执行完毕（三 Phase）+ 独立 fresh closure audit CAN CLOSE 标 done——java 覆盖 B 三族 33 类转译（含非根 CallFunc 局部函数调用形态 + `LocationFunction` 转译并入；两类改判排除，B 族 35→33 / EXCLUDED 16→18 / java 目标集 120）+ corpus 覆盖 B 对拍全绿（19 单元，含 `$out` 模板单元——I2 移交闭合）+ 覆盖矩阵闭环（java 侧全量 120 + pending 清零；truffle 侧锚点适配 87）；此前：I4/I7 plan 起草并通过三轮独立子agent 审查至共识（I4：round-3 确认 Blocker/Major 全修复，F1 roadmap 漂移当场修正；I7：round-3 无 Blocker/Major，roadmap 137 计数当场修正）；此前：I6 执行完毕 + 独立 closure audit CAN CLOSE 标 done——truffle 覆盖 A 五族 + 残余 59 类翻译 + Q3 残余路径 + kind 实测 + truffle 侧覆盖矩阵落地，corpus 覆盖 A 对拍全绿；此前：I3 执行完毕 + 独立 closure audit CAN CLOSE 标 done——A 五族 + 残余转译 + 共享 helper 增量 + corpus 覆盖 A 对拍全绿 + 覆盖矩阵机制落地；I3/I6 plan 起草并通过两轮独立子agent 审查至共识；I5 执行完毕 + 独立 closure audit CAN CLOSE 标 done——truffle 后端骨架 + 帧/slot 映射 + 子集翻译 + 翻译缓存落地，对拍 truffle 列激活）
...
- I7. truffle 翻译覆盖 B（函数闭包/控制流/输出族，类别同 I4）+ 两级内联缓存 + 覆盖矩阵闭环：`done`（plan：`ai-dev/plans/xlang-execution-optimization/2026-08-20-1150-2-i7-truffle-translate-coverage-b.md`，2026-08-21 执行完毕（三 Phase）并独立 fresh closure audit CAN CLOSE（task `ses_fdf979f12ffezAa9PEeQ7d8Kcy`，0 Blocker/0 Major/3 Minor 收口处置）；落地：**Phase 1** fallback 未触发（I4 completed）+ 四项设计定稿（ExitMode 异常族 Exception 子类载体 + catch 点全量放行盘点 / 两级缓存适用面逐形态枚举（XGlobalFunc/XObjMethod/根 CallFunc 不适用理由 + XStaticMethodNode 回填 L2 常量直持）/ 闭包 = 急切值拷贝（否决物化帧——BuildFuncRef 载体语义不可复现；MATERIALIZE 无用法来源；设计 §四张力记档）/ 输出换缓冲 = XLangContext swap/restore + I4 helper 回调 + ExitMode cell 协议）；**Phase 2** B 族 33 类 + 非根 CallFunc 形态全量翻译（支持集 120；函数体独立 RootNode + `mapFunction` 嵌套帧扫描；`XFunctionDispatchNode` 全模块首个 DSL 缓存（L1 CallTarget 身份 + limit=3 泛化 + 探针计数器）；TreeFingerprints 白名单扩至 120 含函数载荷全量混合；nop-xlang 主代码零变更——I4 helper 集合无缺口）；**Phase 3** `truffleRegisteredTarget()` 收敛全量 120（与 javaTargetSet 一致，新鲜度测试同步）；**corpus 覆盖 B truffle 列 vs 解释器列对拍全绿（`TestCorpusCoverageBTruffleColumn` 20/20，三层断言 + 身份断言 + 输出缓冲比对）——验收第一项**；**覆盖矩阵闭环（`TestTruffleCoverageMatrix` 126 用例：120 逐类真实翻译 + pending 清零 + 排除两类/LazyCompiled null 载荷反证 + 红灯注入）——验收第二项**；缓存观测语料落点 = TestTranslatorCoverageB 合成树探针断言（17/17：命中/超限泛化/值身份不误命中/两拷贝时序载体/控制流边界/Try 交互/换缓冲异常路径恢复/XStaticMethodNode 常量同一性）；三模块全绿 1254/0/0（514+263+477）；I8 移交显式记录（EXCLUSIVE 保持 + SHARED/池/并发对拍/缓存淘汰）） — 依赖：I6
...
- I8. truffle 多线程运行时（Context 池 + 共享 Engine + SHARED 形态）：`done`（plan：`ai-dev/plans/xlang-execution-optimization/2026-08-21-0240-1-i8-truffle-multithread-runtime.md`，2026-08-21 执行完毕（三 Phase）并独立 fresh closure audit CAN CLOSE（task `ses_fdf4c2c90ffeTAJ5NrAwayOC7M`，0 Blocker/0 Major/1 Minor=target 陈旧调试报告已清理）；落地：**SHARED 形态切换**（注解常量 + `initializeMultipleContexts` 覆写（SHARED 激活观测旗标），注册结构未重构——I5 D1 保持；EXCLUSIVE javadoc 全模块收口）+ **共享 Engine 单例 + Context 池租借协议**（`XLangTruffleEngine` 惰性单例 fail-fast + `XLangContextPool`：activate 统一状态机/租借注入复用 handoff 链路/enter-leave 可重入批求值/归还残留检测+防御清空+退役/池耗尽有界阻塞/打开预热接线自检/`getLanguage()` I9 消费通道；配置 `nop.xlang.truffle.context-pool.max-size` 缺省=availableProcessors）+ **翻译缓存淘汰**（accessOrder LRU + 翻译锁外并行 + 淘汰计数/DEBUG 日志，键语义不变；`translation-cache.max-entries` 缺省 1024）+ **单元级翻译失败观测事件**（`TranslationFailureEvent/Listener/Recorder`，fail-fast 重抛不变 + 内置记录器恒记可查询，消费方=I9 truffle 侧 SPI 适配器）+ 探针 AtomicLong 化（SHARED 并发无数据竞争）；**SHARED 并发对拍全绿——验收第一项**（`TestCorpusConcurrentTruffleColumn` 75 用例：74 corpus 单元 × 4 线程经池 × 同一/不同单元两形态，逐线程三层对拍 vs 解释器单线程基线 + 身份断言（实例或指纹双通道）+ 无跨 Context 串值断言 + 共享缓存实例同一性 + SHARED 旗标）；**池租借协议正/负测试——验收第二项**（`TestXLangContextPool` 9 正测试 + `TestPoolProtocolNegative` 5 红绿对照：注入残留→归还红灯+Context 退役）；`TestTranslationCacheEviction` 5 + `TestTranslationFailureEvents` 4 + `TestConcurrentEvictionAndFailures` 2；执行中四缺陷 scope 内修复（存量编译缺陷 TestTranslatorCoverageB 非法构造器 / Source 内容唯一性必须全局（SHARED 共享 parse 缓存）/ 池预热绕过状态机致 Context 全退役 / parse 期异常 handoff 透传）；三模块全绿 1354/0/0（514+263+**577**=477 基线+100 新增）；I9 移交显式记录（Execution Notes §14）） — 依赖：I7
...
- I9. 统一后端选择机制（注册 SPI + 决策树 + 配置开关 + 降级观测）：`done`（plan：`ai-dev/plans/xlang-execution-optimization/2026-08-21-0240-2-i9-unified-backend-selection.md`，2026-08-21 执行完毕（三 Phase）并独立 fresh closure audit CAN CLOSE（task `ses_fdf20458affehHTggZje34AKoH`，0 Blocker/0 Major/2 Minor 当场修复）；落地：**后端注册 SPI**（nop-xlang 新包 `io.nop.xlang.backend`：契约/注册表/观测/决策 8 主类；`@GlobalInstance` 显式注册表，register 幂等 + 重复/槽位冲突/非法契约 fail-fast，无 classpath 扫描；两后端经 ICoreInitializer+services 显式注册——Janino 先例；初始化失败→不可用条目不阻断启动（truffle Supplier 缝注入验证））+ **统一决策树**（判定输入四项操作化：清单成员资格 SPI 查询/注册表可用性/配置式部署形态标记（系统属性字符串探测，无 GraalVM import）/开关读取；单跳降级无跨跳（静态分支永不咨询动态后端）；第三分支 = truffle 适配器消费 I8 `TranslationFailureListener`（池打开注册 + sourceKey 关联，fallback 仅在事件命中时发生）→ 降级解释器 + 观测；统一裁决入口 = `XLang.execute` choke point（trampoline 根唯一，全部动态出口产物流经；注册表空 fast-path 行为与现状一致））+ **配置开关**（`nop.xlang.execution.{java-backend-enabled=true, truffle-backend-enabled=true, force-interpreter=false, deployment-form=auto}`，`XLangConfigs` 增量 varRef；force 模式静默短路；不设全局默认后端）+ **降级观测**（WARN 消息键 `nop.xlang.execution.backend-degraded` + counter `nop.xlang.execution.backend-degradation` tags backend/reason + 不可用条目查询；命名契约落 `docs-for-ai/02-core-guides/xlang-and-xpl-basics.md` 新增章节）+ **动态编译出口清单化核验**（12 出口逐项接入点/不适用裁定 + 依赖方向结构性断言（nop-xlang main 禁引后端/GraalVM 类型 = 出口无自带 if/else 的结构性保证））+ **路由场景矩阵全绿——roadmap 验收项**（五类场景经真实出口 + I1 框架断言工具：静态→java（身份=生成类入口 Method 确定性派生）/动态→truffle（身份=XLangRootNode sourceTree 同一，经池运行时）/java 降级两形态（开关关+不可用）/truffle 降级（Engine 失败注入）/单元级降级判 FAIL（生产第三分支红/绿对照 + harness FAIL-not-SKIP 直驱）；模块可见性硬约束按后端拆分三落点）+ 后端未注册回归护栏显式用例；I8 供给消费闭合（池运行时接入 + 观测事件第三分支接线）；I10 移交显式记录（`IEvalStaticBackend#isStaticCandidate` + `IEvalStaticBindingBinder` 契约 + `markUnavailable` 接入缝 + 合成形态↔生产对账点，plan Execution Notes §9）；三模块全绿 1398/0/0（543+271+584 = 基线 514/263/577 全保持 + 44 新增）） — 依赖：I4, I8
...
- I10. java 生成类加载集成（绑定决策树 + RCM 缓存）：`done`（plan：`ai-dev/plans/xlang-execution-optimization/2026-08-21-0450-1-i10-java-generated-class-loading.md`，2026-08-21 执行完毕（三 Phase）并独立 fresh closure audit CAN CLOSE（task `ses_fdec13e4effeOOx6yIsyfwyz0i`，0 Blocker/0 Major/4 Minor 均为收口时动作）；落地：**Phase 1 六决策 repo-observable**（D1 指纹 = nop-xlang-java 新实现 `ExecutableTreeFingerprints` hex SHA-256（持久化清单数据界面，共享纪律非共享实现；白名单逐类覆盖矩阵断言化——开发中实际拦截一处缺口红灯实证）+ D2 双清单内存契约 `GeneratedClassManifest`（path→className+treeFingerprint，注入时 fail-fast；供给缝 `setGeneratedClassManifest` 内部经 I9 setBinder 缝）+ D3 hook = `XLang.parseXpl` 单点 + `bindLoadedUnit`（§五伪代码逐分支；bound/degraded 双包装 + choke point 直通；绑定后模型即 ComponentCacheEntry.model 等效语义，nop-core 零改动；I9 §9(f) 逐字对账）+ D4 xpl 单根纳入/xlib 每标签条目形态定稿（键 `path#tag` I11 衔接 + 入清单前置约束）+ E1-E6 复核 + D5 分级（`generated-fingerprint-mismatch` stale / `tenant-divergent-tree` 稳态 once-per-path 去重、指标不衰减；观测记录方 = 生产 binder 自记防双记）+ D6 夹具物化（corpus 48 静态单元 + 租户单元 = 49 夹具类 Maven test 常规编译 + 反漂移断言 50 用例 + 再生成载体））+ **Phase 2 生产落地**（生产 binder `GeneratedClassBindingBinder`（清单/指纹/租户分级/Class.forName/入口约定）+ 加载期绑定接线（`EvalStaticBoundExecutable`/`EvalStaticDegradedExecutable` + router 直通分支——无重复指纹计算/无虚假观测（计数断言））+ 租户隔离真实构造测试（CFG_TENANT_RESOURCE_ENABLED + ITenantResourceProvider `/_tenant/B/` 覆盖 + runWithTenant + 专用 modelType TenantAware cache；A 命中/B 稳态降级/实例隔离））+ **Phase 3 roadmap 三项验收**（① 绑定路由对拍：corpus 全部 48 静态单元经生产绑定路径（合成双清单注入→parseXpl→绑定→invoke choke point 直通；非 JavaBackendColumn 直驱/非 nop-javac/无自定义 ClassLoader）48/48 PASS——I1 harness 三层断言（typedEquals/scope/RecordingEvalOutput 序列/异常语义）+ JavaBackendIdentityRule 身份断言；② 降级红/绿对照（指纹失配/类缺失 → 分级观测 WARN+counter + INTERPRETER 基线一致；绿 = 零观测）；③ 清单外动态路径零降级）+ docs-for-ai 同步（生成类加载小节 + reason 枚量 + 分级语义）+ I11 移交五项（Execution Notes §8）；三模块全绿 **551/0/0(2skip) + 466/0/0 + 584/0/0**（I9 基线 543/271/584 全保持 + 新增 8/195/0）；checkstyle -Pqa EXIT=0；依赖方向结构性断言全绿） — 依赖：I9
...
- I11. 构建集成 + native image 兼容 + docs-for-ai 同步：`done`（plan：`ai-dev/plans/xlang-execution-optimization/2026-08-21-0450-2-i11-build-integration-native-docs.md`，2026-08-21 执行完毕（三 Phase）并独立 fresh closure audit CAN CLOSE（task `ses_fddcb5215ffe2mmdvAYc87DrGv`，0 Blocker；2 收口动作当场执行：plan GEN-008→GEN-010 勘误（source-anchors 实际锚点，008/009 为既有占用）+ changeset 提交）；落地：**Phase 1 十一项裁定 repo-observable**（I10 移交六项对账 + 扫描口径 live 清点表（xpl/xlib 纳入；xgen/xrun 排除（仅构建期消费）；xtask 排除（仅测试资源）；其余 XDSL 非独立编译单元）+ 任务落点 nop-xlang-java main 独立 mainClass（nop-codegen 零改动零新依赖边）+ `_gen` 布局（GENERATED_PACKAGE + src/main/java 落盘提交 + postcompile 时序记录）+ 双清单行协议格式（排序/TAB/64-hex + 多 jar 聚合语义）+ 漏跑判别子（清单在场性 + require-manifest 显式声明，缺省 false 合法空态静默）+ xlib 每标签形态（键 `path#tag`，I10 D4 一致）+ native 载体三下界 + e2e fixture 模块载体 + docs 落点）+ **Phase 2 实现**（`XlangJavaGenTask` + `GeneratedManifestFiles` + `EvalBackendRouter.bindTagFunction`/`XplLibTagCompiler` 换绑缝 + `translateTagUnit`/`ARGS_PARAM`/`TagGeneratedBinding`/指纹根级函数分支 + initializer 供给闭环接线；41 新增用例）+ **Phase 3 验收**（① e2e 对拍：`TestEndToEndGeneratedBinding` 5/5——真实 `_vfs` 资源经任务（构建管线 generate-test-resources 真实运行）→转译→Maven 常规编译→classpath 双清单→initializer 自动装载→绑定→invoke，java 列 vs 解释器列三层断言（typedEquals/输出序列/异常语义+loc 回映射）+ 身份断言双面（artifact = 生成类入口 Method）；② 漏跑红/绿：require=true+清单缺席 → markUnavailable + WARN（消息键 `nop.xlang.execution.backend-degraded` + reason `codegen-pipeline-missed`）+ 指标 + 非成员动态路径；③ 幂等：check 模式对 committed 产物零漂移 + 二轮零写 + 漂移注入红绿）+ native（closed-world 结构断言 + trace 亲跑 `nop-vfs-index.txt` 19KB + reflect 合流观测 + 环境不可得裁定 + `nop-kernel-cli -Pnative`/`nop-quarkus-demo` 复跑入口 docs 记录 + truffle 排除 profile/exclusion 配方）+ docs-for-ai（`xlang-and-xpl-basics.md` 两新章节 + 配置表 + reason 枚量 + INDEX 路由行 + source-anchors GEN-010）+ I12 移交六项（Execution Notes §14）；验证：四模块 **551/0/0(2skip) + 493/0/0 + 584/0/0 + 5/0/0** 全绿（I10 基线 551/466/584 全保持，nop-xlang-java +27、e2e 新模块 +5）；checkstyle -Pqa EXIT=0；`check-doc-links.mjs --strict` 0 errors；hollow scan 三模块 0 findings；依赖方向保持（nop-codegen pom 零改动 + `TestEvalBackendDependencyDirection` 全绿）） — 依赖：I10
...
- I12. 性能基准 + 全量三后端对拍收口 + 独立 closure audit：`planned`（plan：`ai-dev/plans/xlang-execution-optimization/2026-08-21-1105-1-i12-benchmark-full-compare-closure.md`，2026-08-21 起草并通过六轮独立子agent 审查至共识（round-1 Not Ready 5M/5m → round-2 Not Ready 1B/6m → round-3 Not Ready 1M/3m → round-4 Not Ready 1M/3m → round-5 Not Ready 1M/2m → round-6 **Ready** 0 Blocker/0 Major/2 Minor 当场吸收；31 项修复逐轮独立 live 复核到位）→ 状态 `todo`→`planned`；关键裁定：corpus 48 单元生产形态物化落盘 e2e `_vfs`（gen task 重跑 + 反漂移字节一致 + 三项机械适配预列：计数 7→55 / 5 个非 html 单元 oracle 重声明 / loc 路径重映射）+ 三列同资源路径 html 统一模式驱动（解释器列干净解析）+ benchmark main 域禁 test-jar + 解释器列路由旁路与三列身份核验 + GraalVM ask-first 获取尝试（"未尝试"不构成缺席理由）+ Q1/Q4 量化落基准报告专章 + 设计文档冻结保持 + Q1 数值判据锚定 GraalVM 形态数据行） — 依赖：I11（传递覆盖 I1-I10 全链）
