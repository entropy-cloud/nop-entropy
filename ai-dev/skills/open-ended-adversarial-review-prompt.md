...
开放式审查的常见失败模式是退化为单工件/单维度的窄结构化审计。如果你发现自己只在核实**单个计划/单个工件**的内部声明是否正确（例如某计划声称的列名/字段/计数是否与源一致），请暂停并确认：
- 你做的不是窄结构化审查（应改用 `plan-reviewer-prompt` / `deep-audit-prompts` 对应维度）？
- 你的视野是否覆盖了跨计划的虚假关闭、owner-doc 整体差距、路由/技能的系统性误用、跨批次才可见的重复失败模式、保护区域渗漏、文档漂移？

开放式审查要求把视野拉到项目级：寻找从未写下的假设、虚假关闭或薄弱证明、仅偶然通过狭窄验证的脆弱代码路径、应该提升为可复用检查的重复失败模式。
