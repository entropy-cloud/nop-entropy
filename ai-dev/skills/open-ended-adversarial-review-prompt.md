...
- 不要把自己绑定到 `ai-dev/skills/deep-audit-prompts.md` 的 18 个维度上逐项扫描。
...
与 `deep-audit-prompts.md` 类似，本审查支持多轮迭代深挖：

1. **第 1 轮**：开放探索，记录所有发现。
2. **第 2-N 轮**：基于前一轮发现暴露的线索继续深挖，只输出新发现。
3. 当连续一轮无新发现时停止。

每轮发现的保存和追加规则与 `deep-audit-prompts.md` 的"保存与追加规则"一致。

结果保存到 `ai-dev/audits/{日期}-adversarial-review-{模块名}/` 目录下。

## 与 deep-audit-prompts.md 的关系

- 本提示词是**发现导向**的开放式审查，不绑定固定维度。
- `deep-audit-prompts.md` 是**维度导向**的系统性审核，按固定维度逐项检查。
- 两者互补，不互斥。
- 如果你发现的问题恰好落回某个 deep-audit 维度，可以在发现中标注对应维度编号，方便后续交叉引用。
