...
1. 选择与模块相关的维度（不需要全部 20 个维度）。对模块类型的选择指引：
   ...
   | 模块类型 | 重点维度 |
   |---------|---------|
   | 流处理引擎（如 nop-stream） | 01 依赖图、02 模块职责、09 错误处理、15 类型安全、16 测试覆盖、17 代码风格、21 单元测试有效性 |
   | 业务模块（如 nop-auth、nop-job） | 01-11 全部、16-21 全部 |
   | 基础设施模块（如 nop-core、nop-xlang） | 01 依赖图、02 模块职责、09 错误处理、15 类型安全、16 测试覆盖、19 命名一致性、21 单元测试有效性 |
...
**前置条件**：阶段 A 和 B 的审计/审查已产出 `ai-dev/audits/` 下的结果文档，其中有需要修复的问题。
