...
/**
 * {@link IRemoteTaskClient} 的 HTTP 实现：经平台 {@link IRpcServiceInvoker}
 * （ClusterRpcServiceInvoker 注册中心路由 / HttpRpcServiceInvoker urlMap 静态路由）
 * 调用 worker 的普通 BizModel 方法（默认 invokeJob / getJobStatus）。
 * <p>
 * 每次调用注入 {@code nop-svc-target-host = task.targetHost} header 精确路由；
 * 状态查询与取消的载荷键统一为 {@code data.instanceId}（= DB jobTaskId）。
 */
public class HttpRemoteTaskClient implements IRemoteTaskClient {
    static final Logger LOG = LoggerFactory.getLogger(HttpRemoteTaskClient.class);
