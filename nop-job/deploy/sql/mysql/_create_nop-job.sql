...
CREATE TABLE nop_job_instance(
  ...
  EXEC_FAIL_COUNT INTEGER NULL    COMMENT '失败次数',
  ERR_CODE VARCHAR(200) NULL    COMMENT '错误码',
  ERR_MSG VARCHAR(500) NULL    COMMENT '错误消息',
  RECOVER_MODE BOOLEAN NULL    COMMENT '是否恢复模式',
  LAST_EXEC_ID VARCHAR(32) NULL    COMMENT '上次执行ID',
  LAST_EXEC_END_TIME DATETIME(3) NULL    COMMENT '上次执行完成时间',
  CREATED_BY VARCHAR(50) NOT NULL    COMMENT '创建人',
  CREATE_TIME DATETIME(3) DEFAULT CURRENT_TIMESTAMP(3)  NOT NULL    COMMENT '创建时间',
  UPDATED_BY VARCHAR(50) NOT NULL    COMMENT '修改人',
  UPDATE_TIME DATETIME(3) DEFAULT CURRENT_TIMESTAMP(3)  NOT NULL    COMMENT '修改时间',
