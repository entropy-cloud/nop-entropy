...
/**
 * A hash map implementation of {@link MapOfLong} that uses open addressing for keys. To minimize the memory footprint,
 ...
 */
public class LongHashMap<V> implements MapOfLong<V> {
    ...
    public LongHashMap(int initialCapacity, float loadFactor) {
        if (loadFactor <= 0.0f || loadFactor > 1.0f) {
            ...
            // using a bigger loadFactor would core rehashing before the desired load is reached.
        ...
        int capacity = MathHelper.safeNextPowerOfTwo(initialCapacity);
        mask = capacity - 1;
    ...
    public void putAll(Map<? extends Long, ? extends V> sourceMap) {
        if (sourceMap instanceof LongHashMap) {
            // Optimization - iterate through the arrays.
            @SuppressWarnings("unchecked")
            LongHashMap<V> source = (LongHashMap<V>) sourceMap;
    ...
    private final class KeySet extends AbstractSet<Long> {
        ...
        public boolean remove(Object o) {
            return LongHashMap.this.remove(o) != null;
        }
