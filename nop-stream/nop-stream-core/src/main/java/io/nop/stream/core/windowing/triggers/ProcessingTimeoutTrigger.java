...
public class ProcessingTimeoutTrigger<T, W extends Window> extends Trigger<T, W> {
    ...
    private final ReducingStateDescriptor<Long> timeoutStateDesc = new ReducingStateDescriptor<>("timeout",
            Long.class, LastValue.<Long>type());
    ...
    public TriggerResult onElement(T element, long timestamp, W window, TriggerContext ctx)
        ...
        SimpleAccumulator<Long> timeoutState = ctx.getSimpleAccumulator(this.timeoutStateDesc);
        long nextFireTimestamp = ctx.getCurrentProcessingTime() + this.interval;
        Long timeoutTimestamp = timeoutState.get();
        ...
        if (timeoutTimestamp == null) {
            timeoutState.add(nextFireTimestamp);
    ...
    public void clear(W window, TriggerContext ctx) {
        SimpleAccumulator<Long> timeoutTimestampState = ctx.getSimpleAccumulator(this.timeoutStateDesc);
        Long timeoutTimestamp = timeoutTimestampState.get();
        if (timeoutTimestamp != null) {
            ctx.deleteProcessingTimeTimer(timeoutTimestamp);
