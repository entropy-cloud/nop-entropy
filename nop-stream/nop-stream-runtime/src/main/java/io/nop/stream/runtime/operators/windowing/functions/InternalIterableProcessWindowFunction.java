...
public class InternalIterableProcessWindowFunction<IN, OUT, KEY, W extends Window>
    ...
    @Override
    public void process(KEY key, W window, InternalWindowContext context, Iterable<IN> input, Collector<OUT> out)
            throws Exception {
        ProcessWindowFunction.Context pwfContext =
                new ProcessWindowContextAdapter<>(key, window, context);
        wrappedFunction.process(key, window, input, pwfContext, out);
    }
