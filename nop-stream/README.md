...
> **注:** LOCAL 模式经 `GraphExecutionPlan` + `TaskExecutor` 直接执行；DISTRIBUTED 模式经 `IStreamExecutionDispatcher` 调度（`EmbeddedDistributedExecutor` / `RpcDistributedExecutor`，含控制面 RPC、数据面后端与 remote-deploy 多 JVM 能力，Stage 39-42 已落地）。`RuntimeTopology` 类仍处于概念阶段（0 Java 引用），不代表分布式能力缺失。
