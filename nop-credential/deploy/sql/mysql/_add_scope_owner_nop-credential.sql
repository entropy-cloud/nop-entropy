...
ALTER TABLE nop_credential ADD SCOPE VARCHAR(20) NULL COMMENT '凭证归属：system|user；存量 NULL 视同 system；创建后不可变更';
ALTER TABLE nop_credential ADD OWNER_ID VARCHAR(50) NULL COMMENT 'user 级凭证归属用户 userId（system 级恒空）；创建后不可变更，换人=删旧建新';
