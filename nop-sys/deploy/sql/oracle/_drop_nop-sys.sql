
drop table nop_sys_sequence;
drop table nop_sys_dict;
drop table nop_sys_i18n;
drop table nop_sys_checker_record;
drop table nop_sys_code_rule;
drop table nop_sys_notice_template;
drop table nop_sys_user_variable;
drop table nop_sys_variable;
drop table nop_sys_ext_field;
drop table nop_sys_lock;
drop table nop_sys_cluster_leader;
drop table nop_sys_event;
drop table nop_sys_broadcast_event;
drop table nop_sys_broadcast_cursor;
drop table nop_sys_service_instance;
drop table nop_sys_change_log;
drop table nop_sys_tag;
drop table nop_sys_compact_ext_field;
drop table nop_sys_dict_option;
drop table nop_sys_obj_tag;

