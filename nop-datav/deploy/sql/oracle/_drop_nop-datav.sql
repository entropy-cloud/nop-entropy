
drop table nop_datav_dashboard;
drop table nop_datav_export_task;
drop table nop_datav_screen;
drop table nop_datav_chat_session;
drop table nop_datav_panel;
drop table nop_datav_tab;
drop table nop_datav_dataset_ref;
drop table nop_datav_snapshot;
drop table nop_datav_filter_state;
drop table nop_datav_share;
drop table nop_datav_report_task;
drop table nop_datav_screen_widget;
drop table nop_datav_screen_snapshot;
drop table nop_datav_chat_message;
drop table nop_datav_alert_rule;
drop table nop_datav_report_delivery;
drop table nop_datav_alert_state;

