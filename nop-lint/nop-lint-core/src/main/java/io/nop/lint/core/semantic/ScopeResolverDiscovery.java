...
/**
 * The ServiceLoader discovery of the run's {@link ScopeResolver} (roadmap
 * item 33, plan Decision 5): the first implementation on the classpath
 ...
 * on this path.
 */
public final class ScopeResolverDiscovery {

    private static final AtomicReference<ScopeResolver> CACHE = new AtomicReference<>();

    private ScopeResolverDiscovery() {
    }
     ...
     */
    public static ScopeResolver discover() {
        ScopeResolver cached = CACHE.get();
        if (cached != null) {
            return cached;
        }
        for (ScopeResolver resolver : ServiceLoader.load(ScopeResolver.class)) {
            CACHE.set(resolver);
            return resolver;
        }
        return null;
