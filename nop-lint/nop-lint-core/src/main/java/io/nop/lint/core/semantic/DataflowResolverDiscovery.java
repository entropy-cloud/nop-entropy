...
/**
 * The ServiceLoader discovery of the run's {@link DataflowResolver} (roadmap
 ...
 * on this path.
 */
public final class DataflowResolverDiscovery {

    private static final AtomicReference<DataflowResolver> CACHE = new AtomicReference<>();

    private DataflowResolverDiscovery() {
    }
     ...
     */
    public static DataflowResolver discover() {
        DataflowResolver cached = CACHE.get();
        if (cached != null) {
            return cached;
        }
        for (DataflowResolver resolver : ServiceLoader.load(DataflowResolver.class)) {
            CACHE.set(resolver);
            return resolver;
        }
        return null;
