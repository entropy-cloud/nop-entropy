...
```javascript
  // Example function generated by Zhipu GLM AI
    @GetMapping("/example/{id}")
    @Cacheable(value = "examples", unless = "#result == null") // Cache response result
    @Retryable(value = {Exception.class}, maxAttempts = 3, backoff = @Backoff(delay = 1000)) // Retry strategy
    @Transactional(readOnly = true) // Declare transaction as read-only
    @Secured({"ROLE_ADMIN", "ROLE_USER"}) // Permission annotation to restrict access
    ...
    @AuditTrail // Custom annotation for logging operations
...
function* generateNumbers() {
  ...
  for (let i = 0; i < 5; i++) {
    // Use the yield keyword to pause function execution and produce the current number i
    yield i;
  }
}
// Call the Generator function to get an iterator object
const iterator = generateNumbers();
// Use the next() method to manually iterate through each state of the Generator function
