# Docs-for-AI 文档分析与核对指南

## 文档概述

本指南提供完整的文档分析和核对流程，确保 `docs-for-ai` 目录下的所有文档与当前源码和 `docs` 目录下的官方文档保持一致。

### 两种使用场景

- **分析评估场景**：使用"分析框架"部分，对文档进行深度分析、质量评分、问题发现
- **实际执行场景**：使用"执行指南"部分，进行文档更新、保守修改、最小变更
...
- **自研 XML 解析器**：自行设计并实现 XML 解析器，不依赖第三方库
...
- **避免通用知识**：不重复AI已知的Java/Spring基础概念
- **聚焦项目特色**：重点描述项目的独特架构和设计模式
...
2. **框架差异识别与验证**：
   ...
   - 不要使用 Spring 的 @Service/@Controller 注解
   - 不要使用 Spring 的 @Transactional 详细配置
   - CRUD不要自行实现，使用CrudBizModel上的内置方法
   - 不要使用 Spring 的 AOP 模式
...
通过本指南的分析和核对，可以确保AI专用文档既精简又准确，帮助AI大模型高效理解项目特性并生成符合最佳实践的代码。
