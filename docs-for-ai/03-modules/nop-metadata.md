...
- **错误消息识别性参数一致性（INV-ERROR-PARAM，plan 2026-08-15-1913-3）**：throw 点所用 ErrorCode 描述声明的识别性占位符 `{xxx}` 必须有对应 `.param()` 键（字面量或 `NopMetadataErrors.ARG_X`）——缺键/键错配时运行时渲染字面 `{xxx}`，失败对象身份对用户丢失。null 防御/值语义不存在分支用无必需占位符的错误码（不削既有码占位符——其他 throw 点可能已传齐）；禁止传 null 凑键覆盖（渲染空串空壳）。CI 门禁：`ai-dev/tools/run-nop-metadata-invariants.sh` guard 6（零命中 hard-gate；变量形态错误码须 `// invariant-ok:` 人工归类标注，`{error}` 附注占位符豁免）
