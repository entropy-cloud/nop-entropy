...
| `FileSourceReader` | source reader | 同 `FileSource`（字节精确光标：`currentOffset` = 下一读取字节位，按 LF/CRLF/CR 精确计字节，跨平台一致） | 随 source | 恢复后 `openSplit` seek 到 checkpoint 字节位，`activeBytesConsumed` 以恢复基线播种——首个快照不回退光标（`TestFileSourceAuditFixes.snapshotAfterFirstPostRestorePollAdvancesFromRestoredBase`）。**恢复契约三收口（AR-15，plan 2026-09-04-1326-3）**：① seek 用 `FileChannel` 精确定位 + size 校验，cursor/endOffset 超文件长度（截断）→ typed `IOException` 而非静默错位读（`FileInputStream.skip` 可越 EOF 谎报已跳过）；② 光标推进与 `snapshotState` 同 monitor（无 JMM 撕裂读）；③ 读取以 `split.getEndOffset()` 封顶（split 枚举后文件增大不越界发射）——三路径由 `TestFileSourceReaderRecovery` 钉定 |
| `FileTwoPhaseCommitSink`（SPI `file` sink） | sink（文件） | **exactly-once（两阶段提交）**：`getSinkConsistency()=TWO_PHASE_COMMIT`；每 epoch 写 `.epoch-N.tmp` → `saveState` 先落盘 → checkpoint durable 后 `ATOMIC_MOVE` 到终文件 + 原子更新 `manifest.properties`；commit 幂等（manifest 键守卫）；崩溃在 rename 与 manifest 之间只修复 manifest | **支持并行**（per-subtask 隔离：`copyForSubtask(int)` 独立拷贝，subtask>0 的 per-epoch 文件与 manifest 键带 `.sK` 后缀，subtask 0 保持 legacy 无后缀名——并行 subtask 互不覆盖；manifest 读改写经输出目录锁（JVM 内 ReentrantLock + 跨 JVM FileLock）串行化 + per-subtask tmp 名（AR-14：并行 commit 交错无丢条目/伪失败，`TestFileTwoPhaseCommitSink.testParallelSubtaskCommitsInterleaveWithoutManifestLoss` 钉定）；LOCAL e2e 证明 `TestParallel2PcFileE2E`） | pending 事务（tempPath/recordCount/subtaskIndex）入 checkpoint（key `pending-commits`）；恢复时 durable-未提交 epoch 重提交、未 durable epoch abort；杀恢复 exactly-once 由 `TestFileTwoPhaseCommitSink.testFileSinkKillRecoverExactlyOnce` 钉定 |
...
| `BatchLoaderSourceFunction`（SPI `batch-loader` source） | source（有界，nop-batch 桥接） | **AT_LEAST_ONCE（恢复=客户端跳过续读，D1 裁定方案 a）**：适配 `IBatchLoaderProvider`；offset 为 next-index 约定（=已发射记录数，对齐 `CollectionReplayableSource`），checkpoint 保存该计数；恢复 `seek(offset)` 后 `run()` 在遍历内**客户端跳过前 offset 条已发记录**再续发——一次恢复=一次续读，无全量重放、offset 不谎报（跳过实现收敛在 `BatchLoaderSourceFunction` 内，不改 `IBatchLoaderProvider`/`IBatchLoader` 平台接口）。**确定性前提（必读）**：客户端跳过仅当 `IBatchLoader` 恢复后重新遍历的前 N 条与崩溃前一致（确定性遍历序，如带 ORDER BY 的查询）才正确；违时（loader 排空仍不足 offset）typed fail-fast（`ERR_STREAM_STATE_ERROR`，skip shortfall）而非静默错位。负 offset typed 拒绝 | 无并行限制 | 发射计数 offset 入 operator state（key `source-offset`）；恢复时 `restoreState` 解析并调 `seek(offset)`；seek-续读/负 offset 拒绝/跳空 typed 三路径由 `TestBatchLoaderSourceFunction.testSeekThenRunResumesFromOffsetWithoutFullReplay` / `testSeekNegativeOffsetRejectedTyped` / `testSkipShortfallFailsTypedInsteadOfSilentMisposition` 钉定，算子级 round-trip 由 `testStreamSourceOperatorCheckpointRestoreWithBatchLoader` 钉定 |
