...
> **如何选择**：单实例、作业定义可丢失（重启重建）→ 本地模式。需要持久化、跨实例、可视化管理（AMIS 页面）→ 分布式模式。分布式模式退化为单实例（只部署一个节点）也能正常工作。
...
```xml
<!-- 调度器（协调器 + 工作者，单实例/集群均需要） -->
<dependency>
    <groupId>io.github.entropy-cloud</groupId>
    <artifactId>nop-job-coordinator</artifactId>
    <version>2.0.0-SNAPSHOT</version>
</dependency>
<dependency>
    <groupId>io.github.entropy-cloud</groupId>
    <artifactId>nop-job-worker</artifactId>
    <version>2.0.0-SNAPSHOT</version>
</dependency>

<!-- 业务层（BizModel + GraphQL + AMIS 管理页面） -->
<dependency>
    <groupId>io.github.entropy-cloud</groupId>
...
`nop-job-dao`、`nop-job-core`、`nop-job-api` 作为传递依赖被引入。

### 2.2 自动启动机制

引入上述依赖并配置数据库后，无需手动启动：
...
NopIoC 在容器启动时自动调用 `LifeCycleSupport` bean 的 `start()`，因此**单实例部署也能开箱即用**。
...
| 本地模式测试 | `nop-job/nop-job-core/src/test/java/io/nop/job/core/TestLocalJobScheduler.java` |
