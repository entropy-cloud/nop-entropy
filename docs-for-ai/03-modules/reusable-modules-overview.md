...
| [nop-wf](nop-wf.md) | 工作流/BPM 引擎，复杂审批、会签、加签、委托 | Yes | Yes | nop-service-framework |
...
| 序列号/编号生成 | **nop-sys** | nop_sys_sequence，支持多种重置策略；业务单据编号走 CodeRule（详见 `nop-sys.md`） |
...
| 业务编码规则 | **nop-sys** | nop_sys_code_rule（模式 `D{@year}{@month}{@seq:5}`，ORM `tagSet="code"` 自动集成；详见 `nop-sys.md`） |
| 报表/导出 | **nop-report** | Excel 模板 → XLSX/PDF/DOCX |
| 规则/决策 | **nop-rule** | 决策树 + 决策矩阵，版本管理 |
| 异步任务/编排 | **nop-task** | 多步骤逻辑流，信号、重试、超时 |
| 审批工作流 | **nop-wf** | 会签/加签/委托/转办/驳回，独立引擎 |
