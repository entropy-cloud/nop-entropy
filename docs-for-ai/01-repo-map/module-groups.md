...
| 高性能文件搜索（JDK 22+ 门控） | `nop-rg/` | 纯 JVM 的 ripgrep 等价文件搜索工具（grep 内容搜索 + glob 文件名匹配）。子模块经 JDK ≥ 22 激活 profile 挂入 reactor（同 `nop-utils` `java21-modules` 门控模式，JDK < 22 构建自动跳过）：`nop-rg-bom`（模块版本管理）、`nop-rg-core`（`maven.compiler.release=22`：`search` BMH 标量搜索、`glob` rg/globset 语义匹配、`io` FFM MemorySegment+Arena 分块 I/O、`walk` 并行文件遍历（gitignore/隐藏过滤、符号链接目录不递归）、`coordinator` 搜索编排/策略选择/行提取）、`nop-rg-cli`（picocli 命令行入口，`--json` 兼容 rg --json、`--delegate-rg` 委托系统 rg、`--jfr` JFR 诊断录制）、`nop-rg-benchmark`（JMH 1.37 基准：标量搜索/glob/端到端/rg 对比；运行方式见模块 README）、`nop-rg-vector`（Vector API SIMD 搜索，optional 依赖；`--vector` 时经 ServiceLoader SPI 发现，孵化模块缺失自动降级标量，classpath 缺失显式报错；启用需 `--add-modules jdk.incubator.vector`）。默认尊重 .gitignore（注意：rg 仅在 git 仓库内应用 .gitignore，nop-rg 无条件应用）。 |
| AST 规则引擎（在建） | `nop-lint/` | YAML 驱动的 AST lint 系统，构建于 nop-treesitter 纯 Java 运行时之上：ast-grep 风格源码 pattern 匹配 + XLang xscript 动态检查 + Nop 平台规则库。子模块：`nop-lint-core`（pattern 编译器与匹配内核、规则 DSL 加载、执行档位）、`nop-lint-java`（Java 语言适配与 Java 规则）、`nop-lint-nop`（Nop 平台规则库）。`nop-lint-js`（TS/TSX）与生态模块（maven-plugin/graphql/cli）按 roadmap 后续 Wave 落地。设计权威与 work items 动态状态由平台自身的 AI 开发记录区维护（design/nop-lint/ 13 篇 + backlog 单文件 roadmap，此处按边界规则不直接链接）。 |
