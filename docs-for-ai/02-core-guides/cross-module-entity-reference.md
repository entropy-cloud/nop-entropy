...
> **与 `tagSet="disp"` 自动路径属性的对比**：Nop 平台还有另一种显示名解析方式——在被引用实体上加 `tagSet="disp"`，由代码生成器自动创建 `{relation}.{displayProp}` 路径属性（详见 `orm-model-design.md` 的"显示名解析"节）。**同模块和跨模块都支持**：跨模块时只要 `notGenCode="true"` 桩的显示列也有 `tagSet="disp"`，管线完全一致（代码生成 → xmeta `ext:joinRightDisplayProp` → `control.xlib:view-relation` 自动映射）。本机制（D）仅在需要纯外键（无 `<to-one>` 关系）时才有必要。对于已有 `<to-one>` 关系的场景，应优先使用 `tagSet="disp"` 自动解析。
...
| **C** | nop-app-mall 中扩展 `NopAuthUser` 的 Delta ORM 文件 |
...
│
├─ 只在列表显示外部对象的显示名
│   ├─ 已有 `<to-one>` 关系（同模块或跨模块均可）
│   │   └─ 【tagSet="disp" + 自动路径属性】被引用实体（或桩）的显示列加 tagSet="disp"，代码生成器自动创建 xmeta 路径属性 + ext:joinRightDisplayProp，control.xlib view-relation 自动映射 AMIS 列（详情见 orm-model-design.md "显示名解析"）
│   │
│   └─ 无 `<to-one>` 关系（纯外键列）
│       └─ 【机制 D】 纯外键 + 冗余显示名字段
│
...
当列表只需显示关联对象的名称（且已有 `<to-one>` 关系）时，**优先使用 `tagSet="disp"` 自动解析**。

| 维度 | `tagSet="disp"` + 自动管线 | 机制 D：xmeta + @BizLoader + view.xml |
|------|---------------------------|--------------------------------------|
| **同模块** | ✅ 全自动 | ✅ 可用但完全冗余 |
| **跨模块** | ✅ 桩列加 `tagSet="disp"` 后全自动 | ✅ 可用但完全冗余 |
| **List 性能** | `view-relation` 自动处理，GraphQL 请求 `{rel}.{disp}`，ORM 懒加载 | `@BizLoader` 强制批量加载 |
| **N+1 防护** | ORM 内置批量懒加载（首次触发的 session 级 batch），或手动 `batchLoadProps` | `@BizLoader` 强制批量 |
| **前端列名** | view.xml 保持原始 `partnerId` 不变，`control.xlib` 自动映射 | view.xml 需改为 `partnerName` |
| **Java 代码** | **零** | `@BizLoader` 方法 |
| **xmeta** | 代码生成器自动产生，手写层零改动 | 需手写 `<prop name="partnerName">` |
| **EQL 点导航** | ✅ 支持（`o.partner.name`） | ❌ 不支持 |
| **依赖** | `<to-one>` 关系声明（机制 B 已自带） | 无 ORM 关系要求 |
| **适用场景** | 已有 `<to-one>` 关系的所有 FK 显示（同模块和跨模块均适用） | 纯外键列（无 `<to-one>` 关系）的临时显示 |

**结论**：只要 ORM 文件中已声明 `<to-one>` 关系（无论本模块还是 `notGenCode="true"` 跨模块桩），就使用 `tagSet="disp"` 自动管线。机制 D 仅适用于无 `<to-one>` 关系的纯外键列。**之前的项目实践（266 实体全部用机制 D）是走弯了。**

## 8. 主数据域（master-data）的处理策略

对于 nop-app-erp，`master-data` 被 8 个业务域 ~120 处引用。**推荐使用 `tagSet="disp"` 自动解析**。各业务模块已在 ORM 中声明了 `notGenCode="true"` 外部实体桩 + `<to-one>` 关系（机制 B），只需在桩的显示列上加 `tagSet="disp"`，代码生成器自动处理显示名管线。无需任何 Java `@BizLoader` 或手写 xmeta 派生属性。

主数据分级处理建议：

| 引用场景 | 推荐机制 | 理由 |
|---------|---------|------|
| 列表显示物料/往来单位/仓库的名称（已有 `<to-one>` 关系） | **`tagSet="disp"`** | 全自动，零 Java，control.xlib 自动映射 |
| 详情页带出完整主数据对象（已有 `<to-one>` 关系） | **`tagSet="disp"`** + ORM 懒加载（`batchLoadProps` 优化） | 按需加载，零额外代码 |
| 财务凭证行按多维主数据（科目/伙伴/项目/仓库/物料）筛选 | **机制 B**（外部实体引用，已存在） | EQL 点导航 + GraphQL 展开 |
| 库存流水按物料/仓库/批次多维分析 | **机制 B** | 同上 |
| 业务表之间引用（如凭证反查源单，无 `<to-one>` 关系） | **机制 D**（弱指针字符串） | 业务单据有生命周期，不宜强关联 |
| 纯外键列（无 `<to-one>` 关系）显示名称 | **机制 D** | 无 ORM 关联可依赖 |
...
- `orm-model-design.md` — `<to-one>` / `<to-many>` 语法规范（本模块内），以及 `tagSet="disp"` 自动显示名解析
