...
## XMeta 中可配置的 `graphql:*` 属性完整参考

> **注意**：命名空间是 `graphql:` 不是 `graph:`。所有属性在 `obj-schema.xdef` 和 `schema.xdef` 中定义。

属性分两类：**XML 属性**（在 `<prop>` 标签上）和 **子元素**（在 `<prop>` 内部）。

### XML 属性

| 属性 | 类型 | 说明 |
|------|------|------|
| `graphql:queryMethod` | 枚举: `findCount`/`findFirst`/`findList`/`findPage`/`findConnection` | 指定关联属性的查询方式。不为空时，GraphQL 类型自动变为 `GraphQLConnectionInput`，支持对子表进行条件查询、分页、排序 |
| `graphql:connectionProp` | prop-name | 指定 ORM 关联属性名。`graphql:queryMethod` 不为空时，框架自动从 ORM 模型读取关联配置（JOIN 条件、目标实体），无需手写 filter |
| `graphql:authObjName` | string | 覆盖数据权限使用的 BizObjName。默认使用被查询的 BizObjName 确定数据权限。明确指定后使用对应的权限配置 |
| `graphql:maxFetchSize` | int | GraphQL 查询最大返回行数限制 |
| `graphql:type` | string | 显式指定 GraphQL 类型，覆盖默认类型推导。如 `String`、`[Map]` |
| `graphql:inputType` | class-name | 自定义 GraphQL 查询的输入类型 |
| `graphql:dictName` | string | 字典名称。与 `dictValueProp` 配合，实现字典编码到显示标签的自动转换 |
| `graphql:dictValueProp` | prop-path | 字典值属性。与 `dictName` 配合 |
| `graphql:datePattern` | string | 日期格式。支持标准日期格式，特殊值 `ms` 返回毫秒数 |
| `graphql:labelProp` | prop-path | 指定 label 字段名。用于关联字段的显示文本（如 `relatedRoleList_label`） |
| `graphql:jsonComponentProp` | string | JSON 组件属性名。UI 视图分析器据此将组件数据包含在 GraphQL 查询 selection 中 |
| `graphql:mapper` | string | GraphQL 映射器，用于自定义 GraphQL 类型转换 |
| `graphql:joinLeftProps` | word-set | GraphQL 左连接属性集合 |
| `graphql:joinRightProps` | word-set | GraphQL 右连接属性集合 |
| `graphql:disableLogicalDelete` | boolean | 是否禁用逻辑删除过滤（查询时包含已删除数据） |
| `graphql:selection` | field-selection | 默认字段选择（子元素形式见下方） |
| `graphql:prop` | string | 通用属性引用 |

### XML 子元素（在 `<prop>` 内）

| 元素 | 内容类型 | 说明 |
|------|---------|------|
| `<graphql:filter>` | `filter-bean` | 额外的过滤条件。与运行时 query 合并，用于给 relation/connection 查询补默认过滤 |
| `<graphql:orderBy>` | `/nop/schema/query/order-by.xdef` | 默认排序规则 |
| `<graphql:selection>` | `field-selection` | 默认字段选择 |
| `<graphql:inputType>` | `class-name` | 自定义输入类型 |
| `<graphql:transFilter>` | `xpl-fn:(filter,query,forEntity)=>any` | 过滤条件转换函数。可用 XPL 编写自定义过滤逻辑 |

### 常见用途详解

#### 1. 关联子表查询（`queryMethod` + `connectionProp` / `filter`）

对一对多关联字段，配置 `graphql:queryMethod` 后，该字段在 GraphQL 中变为带分页/排序/条件的可查询关联：
...
```xml
<!-- 使用 connectionProp 自动读取 ORM 关联配置，不需手写 filter -->
...
| 值 | GraphQL 返回类型 | 分页 | 场景 |
|----|-----------------|------|------|
| `findCount` | `Int` | - | 仅计数 |
| `findFirst` | 单对象 | - | 取首条 |
| `findList` | `[对象]` | - | 列表 |
| `findPage` | `PageBean<对象>` | 支持 | 分页查询 |
| `findConnection` | `Connection<对象>` | 支持（cursor） | 游标分页 |

#### 2. 自定义过滤转换（`transFilter`）

将前端传入的 filter 转为自定义 SQL 片段：
    ...
    <graphql:transFilter>
        <and xpl:outputMode="node">
            ...
            <filter:sql xpl:lib="/nop/core/xlib/filter.xlib">
                exists(select o2 from NopAuthResource o2
...
**说明**：
- `graphql:transFilter` 在 xdef 中定义为 `xpl-fn:(filter,query,forEntity)=>any`，其 XPL 默认 `outputMode` 为 `none`。设置 `xpl:outputMode="node"` 使其输出 XML 节点树，节点名直接映射为 filter 树节点名（`and`、`eq`、`sql` 等）
- 没有现成的 `<filter:and>` 标签库函数，因为 filter 树的节点名就是 XPL 标签名，直接用 `<and>`、`<eq>` 等原始标签即可
- `<filter:sql xpl:lib="/nop/core/xlib/filter.xlib">` 显式导入 `/nop/core/xlib/filter.xlib`，将 SQL 片段包装为 `<sql>` 节点（避免与 XPL 内建 `<sql>` 标签冲突）。**注意**：`xmlns:filter` 声明对 XPL 编译无作用，必须用 `xpl:lib` 显式导入
- `filter.getAttr('value')` 获取前端传入的 filter 参数值

#### 3. 字典字段 label（`dictName` + `dictValueProp`）

见前文"字典字段的自动 _label 字段"章节。编译期自动生成。

#### 4. JSON 组件属性（`jsonComponentProp`）

当字段内容为 JSON 且前端需要按组件结构渲染时：

```xml
<prop name="jobParams" graphql:jsonComponentProp="jobParamsComponent"/>
```
...
```
xmeta <prop graphql:*>
  → GraphQLObjMetaHelper (读取 queryMethod/type/authObjName)
  → OrmFetcherBuilder (构建 connection fetcher, 合并 filter/orderBy)
  → OrmEntityPropConnectionFetcher (运行时: filter合并 + orderBy + 分页)
  → DictLabelFetcherProvider (dictName → DictLabelFetcher)
  → XuiViewAnalyzer (jsonComponentProp/labelProp → selection)
```

实现锚点见 `04-reference/source-anchors.md` 的 `GQL-001` 和 `DQL-006`。
