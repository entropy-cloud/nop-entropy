...
| BizModel/`I*Biz` 自定义方法签名缺 `IServiceContext context` 末参 | 对齐基类 `ICrudBiz` 契约（其所有方法末参均为 `IServiceContext context`）。`context` 承载 `IUserContext`（用户身份/数据权限）、缓存与事务上下文；跨服务调用（A 域注入并调用 B 域 `I*Biz`）必须把 `context` 透传给下游，否则下游拿不到身份、静默跳过数据权限、丢失缓存。查询带 `FieldSelectionBean` 时，`context` 仍是最后参数 |
