...
快照测试有一个天然盲区：**重新录制时，如果业务逻辑已经发生回归（比如借贷不平、状态机跳转错误），录制会把错误结果直接写成"预期输出"，测试仍然变绿。** 快照只保证"输出不变量"，不保证"输出正确性"。

三层验证中的显式断言层就是用来锚定关键业务不变量的。以下是一组在编写测试时需要显式断言的**核心锚点模式**：

| 锚点类别 | 示例断言 | 保护什么 |
|---------|---------|---------|
| **借贷平衡** | `assertEquals(totalDebit, totalCredit)` | 过账后的凭证分录平衡 |
| **状态机终点** | `assertEquals("APPROVED", voucher.get("status"))` | 审批/过账/冲销等状态变迁正确 |
| **金额演算** | `assertEquals(qty * unitPrice, lineTotal)` | 单价 × 数量 = 行金额 |
| **结转一致性** | `assertNotNull(period.get("closeTime"))` | 期末结账后期间正确关闭 |
| **唯⼀约束** | `assertNotNull(savedEntity.getId())` | 保存后主键已分配 |
| **业务规则拒绝** | `assertNull(executeRpc(...).getData())` / `assertNotEquals(0, result.getStatus())` | 非法操作（如已关闭期间过账）被正确阻止 |
| **冲销闭环** | `assertEquals("REVERSED", original.get("reversalStatus"))` | 冲销后原凭证标记正确 |
| **跨表一致性** | `assertEquals(stockMoveQty, inventoryChangeQty)` | 库存移动与库存账本一致 |

#### 自检清单

编写或审查一个快照测试时，对照以下问题：

- [ ] 测试的业务路径是否有明确的状态终点？如果有，是否已用 `assertEquals` 断言该状态？
- [ ] 是否涉及金额计算？如果有，是否已断言借贷平衡、金额演算或合计一致性？
- [ ] 是否涉及业务规则拒绝场景（非法操作被拦截）？如果是，是否已断言错误码或返回 status ≠ 0？
- [ ] 是否涉及跨实体数据流转（如过账生成凭证、审批触发库存移动）？如果是，是否已断言关键目标实体的核心字段？
- [ ] 如果今天重新录制这个测试的快照，有什么业务规则被破坏也不会被察觉？是否已为该规则写显式断言？
