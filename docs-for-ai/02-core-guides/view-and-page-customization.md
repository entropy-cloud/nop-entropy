...
**必须在 `<cells>` 中为每个字段配置 `domain` 属性**，控件生成器才能匹配到正确的 AMIS 控件（`domain="string"` → `input-text`，`domain="int"` → `input-text` + `isInt` 校验等）。
...
<form id="queryForm" editMode="edit" title="类型层级查询">
    ...
    <cells>
        <cell id="indexId" domain="string"/>
        <cell id="qualifiedName" domain="string"/>
        <cell id="direction" domain="string"/>
        <cell id="maxDepth" domain="int"/>
    </cells>
</form>
```
