...
除了默认的 AMIS 渲染管线（`web.xlib` + `control.xlib`），Nop 平台还支持 **Flux 渲染管线**（`flux-web.xlib` + `flux-control.xlib`），用于输出 `nop-chaos-flux` 前端框架所需的 Flux JSON Schema。

### 使用方式

Flux 渲染管线消费**完全相同的** `view.xml` 模型，只需在 page.yaml 中切换 xpl:lib：
