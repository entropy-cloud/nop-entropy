...
**消息语言规则**：
- `ErrorCode.define(...)` 中的描述消息默认使用**中文**，框架会通过 i18n 机制翻译。
- **例外（`nop-ai/*` 模块族全量英文）**：`nop-ai` 模块族的 `ErrorCode.define(...)` 描述统一使用**英文**（含各 `*Errors` 容器与模块内联 `define(...)`），不再区分转换类/业务类来源。英文描述保证 `getMessage()` / `getDescription()` 语义跨模块稳定、不经 i18n 改写，且可被 AI 直接阅读。覆盖模块：nop-ai-core（`NopAiCoreErrors`，含 `nop.err.ai.tools.*` 码）、nop-ai-service（`NopAiErrors`）、nop-ai-agent（`NopAiAgentErrors`）、nop-ai-shell（`NopAiShellErrors`）、nop-ai-coder（`AiCoderErrors`）、nop-ai-maven（`NopAiMavenErrors`）、nop-ai-code-analyzer（`NopAiCodeAnalyzerErrors`）、nop-ai-dsl-orm（`GptOrmErrors`）、nop-ai-gateway（`NopAiGatewayErrors`）、nop-ai-mcp-server（`McpServerErrors`）、nop-ai-toolkit（`NopAiToolkitErrors`）。
- **边界演进（为什么是全模块族）**：该例外最初仅覆盖"由历史裸异常转换、逐字保留历史消息"的转换类错误码（来源驱动分界，起源：plan 2026-07-31-2248-2 scan-hollow 基线清零的 UOE 转换、2026-08-01-0936-1/2/3 与 2026-07-31-1834-3 的 IAE/RTE 转换），并以 `NopAiCoreErrors.ERR_AI_MODEL_CLASS_SATURATED`（plan 2026-08-15-0849-2）为"plan 裁定英文业务码"先例；2026-09-15 的 plan 2026-09-15-1029-1（`NopAiErrors` / `AiCoderErrors`）与 plan 2026-09-15-1231-2（`NopAiCoreErrors` 25 码 + `GptOrmErrors` 1 码）把 nop-ai 存量业务错误码一并英文化后，边界收敛为**模块族粒度**——`nop-ai/*` 全族错误码描述零中文残留（live 复核 `\p{Han}` 零命中）。**来源驱动分界在 nop-ai 内不再适用**（同命名空间内不再存在"转换码英文 / 业务码中文"并存，如 `nop.err.ai.tools.*` 内既有转换码 `invalid-project-name` 亦有业务码 `invalid-thought`，现全部英文）；其他模块的新增业务错误码仍按默认规则使用中文。
- **ID 命名空间跨模块为有意为之（W2 下沉修订，plan 2026-08-15-0604-2）**：`ERR_AI_AGENT_INVALID_ARG`（`invalid argument: {msg}`）随 LLM 可靠性子集下沉从 `NopAiAgentErrors` 并入 `NopAiCoreErrors`，**错误码 ID `nop.err.ai.agent.invalid-arg` 刻意保留**（持久化/日志 ID，重命名即行为变更）——ID 命名空间 `nop.err.ai.agent.*` 因此跨 nop-ai-core（该码）与 nop-ai-agent（其余 30 码）两模块，属有意为之。
...
```
NopException                — Java 业务代码默认使用
  └─ NopEvalException       — XPL/XLang 执行引擎内部使用
       └─ NopScriptError    — XLang 脚本中 throw 使用
```

| 类 | 使用场景 | 特点 |
|----|---------|------|
| `NopException` | Java BizModel / Service | 正常 Java 堆栈 |
| `NopEvalException` | XPL 执行引擎内部 | 覆写 `fillInStackTrace()`，默认**不记录 Java 堆栈**，用 `xplStack` 记录 XLang 表达式堆栈 |
| `NopScriptError` | XLang 脚本（task.xml `<source>` / xbiz `<source>` / `<c:script>`） | 继承 `NopEvalException`，同样不记录 Java 堆栈 |

### 何时用哪个

```java
// Java 代码（BizModel/Service/Processor）
throw new NopException("erp.purchase.order-not-found")
    .param("orderId", orderId);
```

```javascript
// XLang 脚本（task.xml source / xbiz source 的 CDATA 段）
throw new NopScriptError("erp.purchase.order-not-found")
    .param("orderId", orderId);
```

`<c:throw>` 标签内部自动创建 `NopEvalException`，无需手动选择。

### 调试开关

`NopEvalException` 及其子类（含 `NopScriptError`）默认不填充 Java 堆栈（已有 XPL 堆栈足够定位）。调试时如需 Java 堆栈：

```yaml
# application.yaml
nop:
  exceptions:
    eval-exception-fill-stacktrace: true
```

对应配置项 `CFG_EXCEPTION_FILL_STACKTRACE`（`ApiConfigs.java:36`，默认 `false`）。

XPL 堆栈深度由 `nop.exceptions.max-xpl-stack-size` 控制（默认 10）。
