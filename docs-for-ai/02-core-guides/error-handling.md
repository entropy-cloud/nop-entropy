...
- **例外归属以持有类模块为粒度（W2 下沉修订，plan 2026-08-15-0604-2）**：`ERR_AI_AGENT_INVALID_ARG`（`invalid argument: {msg}`）随 LLM 可靠性子集下沉从 `NopAiAgentErrors` 并入 `NopAiCoreErrors`，**错误码 ID `nop.err.ai.agent.invalid-arg` 刻意保留**（持久化/日志 ID，重命名即行为变更）——例外归属以持有类所在模块（nop-ai-core）为准，ID 命名空间 `nop.err.ai.agent.*` 因此跨 nop-ai-core（该码）与 nop-ai-agent（其余 30 码）两模块，属有意为之。
- **plan 裁定英文码（W5，plan 2026-08-15-0849-2）**：`NopAiCoreErrors.ERR_AI_MODEL_CLASS_SATURATED`（`nop.err.ai.model-class.saturated`，英文描述 "No candidate is currently available for model class: {modelClass}"）为**新增业务错误码但按 plan 裁定使用英文**——模型类路由（`io.nop.ai.core.routing`）为公共能力组件，饱和错误会被网关/适配器编排（W6/W7）与前端消费，英文描述保证跨模块语义稳定、不经 i18n 改写。此类"plan 裁定英文业务码"以本条目为先例（与既有"转换类错误码英文"例外并列，均以持有类模块为粒度）。
