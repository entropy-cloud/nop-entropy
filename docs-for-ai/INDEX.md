...
- `nop-runner/` 是 CLI / runner 入口（`nop-cli` uber-jar 在 `nop-runner/nop-cli/` 构建产物中，提供 `gen`/`convert` 子命令，获取与用法见 `03-runbooks/bootstrap-new-application.md`）；`nop-demo/` 和 `demo/` 是示例入口。
