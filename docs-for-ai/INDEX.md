...
| 理解 nop-auth（认证/权限/多租户） | `03-modules/nop-auth.md`（多租户核心机制见 `02-core-guides/tenant-model.md`，MFA 两阶段登录见同文件"多因子验证（MFA）"章节 + 操作级 MFA（敏感操作二次验证）见同文件"操作级 MFA"章节 + 角色级强制策略与受限会话见同文件"角色级强制策略与受限会话（W13）"章节 + `02-core-guides/auth-and-permissions.md`"两阶段登录（MFA）"章节） |
