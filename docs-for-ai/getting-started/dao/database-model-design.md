...
Nop平台使用Excel或XML定义数据库模型，自动生成数据库表结构、实体类、DAO接口等，支持多种数据库方言。

## 核心概念

### 1. 数据库模型
- 定义：描述数据库表结构和关系的模型
- 格式：Excel或XML，如`app.orm.xlsx`
- 内容：实体、字段、关系、索引等
...
| 工作表 | 用途 | 主要字段 |
|-------|------|---------|
| Entities | 实体定义 | 实体名、表名、主键、描述 |
| Fields | 字段定义 | 实体名、字段名、数据类型、长度、约束 |
| Relations | 关系定义 | 主实体、从实体、关系类型、外键字段 |
| Indexes | 索引定义 | 实体名、索引名、索引字段、索引类型 |
...
1. **使用Excel设计**：优先使用Excel设计模型，可视化编辑更方便
2. **遵循命名规范**：统一实体、表、字段命名
3. **合理设计关系**：避免过于复杂的关系结构
4. **考虑性能**：设计时考虑查询性能和数据量
5. **版本管理**：对模型文件进行版本控制
6. **测试验证**：生成SQL后进行验证，确保正确性
...
数据库模型设计是Nop平台开发的基础，通过Excel或XML定义模型，自动生成数据库表结构、实体类、DAO接口等。

**核心要点**：

1. **模型定义**: 使用Excel或XML定义实体、字段、关系
