...
| `SYS-001` | `nop-sys/model/nop-sys.orm.xml` + `nop-sys/nop-sys-dao/src/main/java/io/nop/sys/dao/message/SysDaoMessageService.java` + `nop-sys/nop-sys-dao/src/test/java/io/nop/sys/dao/message/TestSysDaoMessageService.java` | sys-event 当前可靠性基线：普通事件保留 `NopSysEvent` 作为 shared queue，按 stable `partitionIndex` + row lease + 分区头扫描串行处理；广播事件拆为 `NopSysBroadcastEvent` + `NopSysBroadcastCursor`，按 `subscriberId + topic` 的 cursor/lease 恢复； focused tests 覆盖发送分流、cursor 推进、失败阻塞与 `ConsumeLater` 重排队 |
