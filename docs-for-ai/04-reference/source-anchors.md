...
| `EXT-009` | `nop-frontend-support/nop-web/src/main/resources/_vfs/nop/web/xlib/flux-web.xlib` + `flux-web/` 目录 | Flux 页面生成库：28 个标签（含共享容器分派 `GenContainerModel`），替代 web.xlib 输出 Flux 页面 JSON；完整支持 `layout-syntax-reference.md` 中所有 layout DSL 语法（`*` 必填、`@` 只读、`!` 隐藏标签、跨列、分组、Tab、查询运算符）；`NormalizeAction` 实现 onClick 优先透传 + AMIS actionType 到 Flux ActionSchema 转换；页面级容器 `tabs`/`wizard`/`group` 输出 Flux `TabsSchema`/`WizardSchema`/`GridSchema`（`items`/`steps` 字段、`body` 数组直接书写、tab/step 内容优先级 `page` > `body` > `name`）；Flux 已确认不支持的 form/page 属性见 `docs-for-ai/02-core-guides/flux-rendering.md`。 |
