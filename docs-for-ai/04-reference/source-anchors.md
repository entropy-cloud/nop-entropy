...
| `SYS-001` | `nop-sys/model/nop-sys.orm.xml` + `nop-sys/nop-sys-dao/src/main/java/io/nop/sys/dao/message/SysDaoMessageService.java` + `nop-sys/nop-sys-dao/src/main/java/io/nop/sys/dao/message/BroadcastEventProcessor.java` + `nop-batch/nop-batch-sys/src/main/java/io/nop/batch/sys/SysEventBatchTrigger.java` + `nop-sys/nop-sys-dao/src/test/java/io/nop/sys/dao/message/TestSysDaoMessageService.java` | sys-event 当前可靠性基线：普通事件保留 `NopSysEvent` 作为 shared queue，`nop-sys-dao` 提供 stable `partitionIndex` + row lease + 重排队原语，`nop-batch-sys` 可作为 batch trigger 周期扫描执行；广播事件使用 `NopSysBroadcastEvent` append-only 事件流 + `BroadcastEventProcessor` 通过 `findNext` keyset pagination + 内存游标 + 时间窗口消费，不持久化消费进度，消费失败不阻塞后续事件； focused tests 覆盖发送分流、`findNext` 顺序消费、多订阅者 fan-out 与 `ConsumeLater` 重排队 |
