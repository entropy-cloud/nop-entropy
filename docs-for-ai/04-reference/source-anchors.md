...
| `EXT-008` | `nop-frontend-support/nop-web/src/main/resources/_vfs/nop/web/xlib/flux-control.xlib` | Flux 控件映射库：75 个 `{mode}-{type}` 标签，替代 control.xlib 输出 Flux JSON 控件 |
| `EXT-009` | `nop-frontend-support/nop-web/src/main/resources/_vfs/nop/web/xlib/flux-web.xlib` + `flux-web/` 目录 | Flux 页面生成库：37 个标签，替代 web.xlib 输出 Flux 页面 JSON；`NormalizeAction` 实现 onClick 优先透传 + AMIS actionType 到 Flux ActionSchema 转换 |
