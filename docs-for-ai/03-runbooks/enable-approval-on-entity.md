...
> **设计原理**：[`../../ai-dev/design/nop-wf/approvable-entity-design.md`](../../ai-dev/design/nop-wf/approvable-entity-design.md)（可审批实体集成设计）
...
```xml
<entity name="...LeaveRequest" tagSet="use-approval" ...>
    <columns>
        <column name="approveStatus" ext:dict="wf/approve-status" mandatory="true" .../>
        <!-- WORKFLOW 模式才需要 flowInstanceId -->
        <column name="flowInstanceId" type="String" precision="32" .../>
        <!-- 其他业务字段 -->
    </columns>
</entity>
```
...
> `approveStatus` 与 `flowInstanceId` 字段集权威在 `<domain>/model/*.orm.xml`。若实体未声明 `flowInstanceId`，codegen 会按 `use-approval` 补齐（仅 WORKFLOW 模式需要）。
...
- `IXxxBiz extends ICrudBiz<Xxx>, IApprovableBiz<Xxx>`
- BizModel 中 5 个标准 action 的通用实现（状态守卫 + wf 启动 + 回调钩子）
- `flowInstanceId` 列（若步骤 1 未显式声明）
...
标签内部 `inject('biz_LeaveRequest')` 调用继承自 `IApprovableBiz` 的 `approve`/`reject`。**审批状态由业务 action 改写，不由 wf 引擎直写**（不使用 `bizEntityStateProp`）。
...
WORKFLOW 模式:
  前端调 submitForApproval() ──▶ action source: UNSUBMITTED→SUBMITTED + 启动 wf(绑 flowInstanceId)
        │
...
- 设计原理：[`ai-dev/design/nop-wf/approvable-entity-design.md`](../../ai-dev/design/nop-wf/approvable-entity-design.md)
