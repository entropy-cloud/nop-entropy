...
- `StreamModelFingerprint`: **exists** with `builder()`, `build()`, `isCompatibleWith()`. Used as parameter in `PartitionedPlanGenerator.generate()` but **no validation call rejects incompatible requirements** — fingerprint is a passive data carrier
