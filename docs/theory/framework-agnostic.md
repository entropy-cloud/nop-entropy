...
此外，Nop平台通过NopIoC容器提供统一的依赖注入机制。依赖注入可以看作是对上下文环境对象的一种按需使用，从而避免一次性获取到所有上下文信息产生不必要的依赖。NopIoC使用`@Inject`等Java标准注解，借此实现框架无关的依赖注入功能。
...
NopIoC的设计回归到了声明式依赖注入容器的初心，在概念层面确保，使用NopIoC容器所管理的bean永远可以都可以交由其他IoC容器来进行配置
