...
比如说，NopGraphQL引擎可以同时对外提供GraphQL和REST两种形式的接口服务，其中REST方式下统一采用JSON格式进行请求数据和响应数据的编解码，而SpringMVC框架出于兼容性的考虑，不得不框架内包含多种编码策略，并在运行时执行很多多余的判断。
...
笔者本人所在的公司是银行核心系统的软件提供商，笔者对于高并发、高复杂度的toB软件开发颇有心得，也有很多国产化新创软件产品的开发经验，因此Nop平台对于高性能以及高复杂度的toB软件产品开发已经内置了很多支持，并在实践中经历了大量验证。
...
目前一般程序员对于抽象的理论知识的了解程度限制在大学本科水平，基本是停留在牛顿和莱不尼兹的时代，如果不努力提升自己对抽象知识的理解能力，那么即使是将互联网上所有的计算机相关的信息一网打尽，
