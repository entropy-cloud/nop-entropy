...
* 然后通过`sys/{objName}`来引用这个数据表来作为字典。例如 `sys/LitemallBrand`
...
* `seq`：利用`SequenceGenerator`来自动生成主键。缺省使用`nop_sys_sequence`表来记录`sequence`。
...
* `disp`：数据记录的显示名称。在字典表或者选择列表中会作为`label`被使用。
