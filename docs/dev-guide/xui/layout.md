...
4. 如果为分组指定Label，则使用FieldSet控件来显示分组，并通过符号`^`表示分组收缩，符号`>`表示分组展开

5. 在字段名前标记符号`!`，表示不显示Label。
...
例如对于字段gender，我们指定了它使用auth/gender这个字典，取值范围只能是字典范围内的值。对于字段email,它的数据域(domain)设置为email，要求录入格式必须是满足email格式规范。
