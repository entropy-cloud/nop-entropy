...
>
> 注：本文档为历史改进计划。当前实现状态请以 `nop-stream-core` 与
> `nop-stream-runtime` 中的代码为准。
...
| 能力 | 当前状态 | 目标状态 |
|------|---------|---------|
| CheckpointCoordinator | ✅ 已实现（需持续完善） | ✅ 完整实现 |
| CheckpointStorage | ✅ LocalFile/Jdbc 已实现 | ✅ 多种后端支持 |
| CheckpointBarrier | ✅ 已实现 | ✅ 完整实现 |
| PendingCheckpoint | ✅ 已实现 | ✅ 完整实现 |
| 两阶段提交 | ❌ 不存在 | ✅ 完整实现 |
| 故障恢复 | ✅ 基础恢复已实现 | ✅ 完整实现 |
...
public interface StreamOperator<OUT> extends CheckpointListener, KeyContext, Serializable {
    ...
    // ✅ 当前接口提供默认方法
    // default OperatorSnapshotResult snapshotState(long checkpointId)
    // default void initializeState(TaskStateSnapshot taskStateSnapshot)
}
```

**说明**：`snapshotState`/`initializeState` 已存在默认实现，
但具体算子仍需按需覆盖以实现业务状态快照。

### 3.2 缺失组件

| 组件 | 说明 | 优先级 |
|------|------|--------|
| CheckpointCoordinator | 并发/容错语义需继续强化 | P0 |
| CheckpointBarrier | 与执行图对齐策略需补齐 | P1 |
| PendingCheckpoint | 边界条件测试需补充 | P1 |
| CompletedCheckpoint | 结构已具备，需增强恢复覆盖 | P1 |
| CheckpointStorage | 已实现本地/JDBC，需扩展后端 | P1 |
