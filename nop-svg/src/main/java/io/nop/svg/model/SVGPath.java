 ...
 * (Bezier) curves and elliptical arc. This class delegates lines and curves to an enclosed <code>GeneralPath</code>.
 * Elliptical arc is implemented using an <code>Arc2D</code> in float precision.
 *
 * <p>
 ...
 * @version $Id: SVGPath.java 594018 2007-11-12 04:17:41Z cam $
 */
public class SVGPath implements Shape, Cloneable {
    ...
    public static SVGPath parse(String s) {
        if (StringHelper.isBlank(s))
            return null;
        SVGPathParser parser = new SVGPathParser();
        parser.parse(s);
        return parser.getSVGPath();
    }

    /**
     * Constructs a new <code>SVGPath</code>.
     */
    public SVGPath() {
    ...
    /**
     * Constructs a new <code>SVGPath</code> with the specified winding rule to control operations that require the
     * interior of the path to be defined.
     */
    public SVGPath(int rule) {
        path = new GeneralPath(rule);
    }

    /**
     * Constructs a new <code>SVGPath</code> object with the specified winding rule and the specified initial capacity
     * to store path coordinates.
     */
    public SVGPath(int rule, int initialCapacity) {
        path = new GeneralPath(rule, initialCapacity);
    }

    /**
     * Constructs a new <code>SVGPath</code> object from an arbitrary <code>Shape</code> object.
     */
    public SVGPath(Shape s) {
        this();
        append(s, false);
    }
     ...
     * @param angle        the angle from the x-axis of the current coordinate system to the x-axis of the ellipse in degrees.
     * @param largeArcFlag the large arc flag. If true the arc spanning less than or equal to 180 degrees is chosen, otherwise
     *                     the arc spanning greater than 180 degrees is chosen
     * @param sweepFlag    the sweep flag. If true the line joining center to arc sweeps through decreasing angles otherwise it
    ...
    public String toSVGString() {
        ...
        ISVGPathIterator iterator = getExtendedPathIterator();
        ...
        while (!iterator.isDone()) {
            ...
            switch (segType) {
                case PathIterator.SEG_MOVETO:
                    ...
                    break;
                case PathIterator.SEG_LINETO:
                    sb.append("L").append(coords[0]).append(',').append(coords[1]);
                    break;
                case PathIterator.SEG_QUADTO:
                    sb.append("Q").append(coords[0]).append(',').append(coords[1]).append(' ').append(coords[2]).append(',')
                            .append(coords[3]);
                    break;
                case PathIterator.SEG_CUBICTO:
                    sb.append("C").append(coords[0]).append(',').append(coords[1]).append(' ').append(coords[2]).append(',')
     ...
     * Delegates to the enclosed <code>GeneralPath</code>.
     */
    public ISVGPathIterator getExtendedPathIterator() {
        return new EPI();
    }

    class EPI implements ISVGPathIterator {
        int segNum = 0;
    ...
    public Object clone() {
        try {
            SVGPath result = (SVGPath) super.clone();
