...
import static io.nop.orm.OrmErrors.ERR_EQL_INVALID_DATETIME_TYPE;
import static io.nop.orm.OrmErrors.ERR_EQL_INVALID_INTERVAL_UNIT;
import static io.nop.orm.OrmErrors.ERR_EQL_INVALID_SQL_TYPE;
import static io.nop.orm.OrmErrors.ERR_EQL_PRECISION_NOT_POSITIVE_INT;
import static io.nop.orm.OrmErrors.ERR_EQL_SCALE_NOT_NON_NEGATIVE_INT;
...
public class EqlASTBuildVisitor extends _EqlASTBuildVisitor {
    ...
    public java.lang.String SqlTypeExpr_name(ParseTree node) {
        ...
        if (StdSqlType.fromStdName(name) == null)
            throw new OrmException(ERR_EQL_INVALID_SQL_TYPE).loc(ParseTreeHelper.loc(node)).param(ARG_SQL_TYPE, name)
