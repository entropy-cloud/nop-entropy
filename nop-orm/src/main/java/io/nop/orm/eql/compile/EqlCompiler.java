...
import io.nop.dao.dataset.impl.BaseDataFieldMeta;
import io.nop.dao.dataset.impl.BaseDataSetMeta;
...
import static io.nop.orm.OrmErrors.ERR_EQL_NOT_SUPPORT_MULTIPLE_STATEMENT;
...
public class EqlCompiler implements ISqlCompiler {
    ...
    public ICompiledSql compile(String name, String eql, ISqlCompileContext context) {
        ...
        if (program.getStatements().size() > 1)
            throw new OrmException(ERR_EQL_NOT_SUPPORT_MULTIPLE_STATEMENT).param(ARG_SQL, eql);

        SqlStatement stm = program.getStatements().get(0);
        boolean dump = stm.hasDecorator(OrmConstants.DECORATOR_DUMP);
        if (dump) {
            stm.dump("parsed_eql");
        }
