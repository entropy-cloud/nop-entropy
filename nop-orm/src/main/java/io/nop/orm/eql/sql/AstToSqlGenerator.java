...
import static io.nop.orm.OrmErrors.ARG_AST_NODE;
...
public class AstToSqlGenerator extends AstToEqlGenerator {
    ...
    protected String normalizeTableName(String tableName) {
        return GenSqlHelper.normalizeTableName(dialect, tableName);
    }
    ...
    void printCol(String owner, String colName) {
        ...
        if (colName.startsWith("@")) {
            ...
            sqlText = GenSqlHelper.replaceOwner(owner, sqlText);
            print(sqlText);
        } else {
            if (owner != null) {
                print(owner);
        ...
        if (tables.isEmpty()) {
            ...
            for (SqlSingleTableSource table : tables) {
                ...
                if (entityModel.isUseTenant()) {
                    ...
                    GenSqlHelper.appendCol(sb, dialect, alias, entityModel.getTenantColumn());
                    sb.append('=');
    ...
    private NopException newError(ErrorCode errorCode, EqlASTNode node) {
        return new OrmException(errorCode).param(ARG_AST_NODE, node);
    }
