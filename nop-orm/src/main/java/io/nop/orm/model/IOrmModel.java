...
import static io.nop.orm.OrmErrors.ARG_TABLE_NAME;
import static io.nop.orm.OrmErrors.ERR_ORM_UNKNOWN_COLLECTION_NAME;
import static io.nop.orm.OrmErrors.ERR_ORM_UNKNOWN_ENTITY_MODEL_FOR_TABLE;
import static io.nop.orm.OrmErrors.ERR_ORM_UNKNOWN_ENTITY_NAME;
...
public interface IOrmModel {
    ...
    default IEntityModel requireEntityModelByTableName(String tableName) {
        ...
        if (entityModel == null)
            throw new OrmException(ERR_ORM_UNKNOWN_ENTITY_MODEL_FOR_TABLE).param(ARG_TABLE_NAME, tableName);
        return entityModel;
    }
    ...
    default IEntityModel requireEntityModel(String entityName) {
        ...
        if (entityModel == null)
            throw new OrmException(ERR_ORM_UNKNOWN_ENTITY_NAME).param(ARG_ENTITY_NAME, entityName);
        return entityModel;
    }
    ...
    default IEntityRelationModel requireCollectionModel(String collectionName) {
        ...
        if (collectionModel == null)
            throw new OrmException(ERR_ORM_UNKNOWN_COLLECTION_NAME).param(ARG_COLLECTION_NAME, collectionName);
