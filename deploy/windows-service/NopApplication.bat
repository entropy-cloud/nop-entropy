...
powershell -Command "(Get-Content '%REALPATH%lib\NopServiceWrapperTemplate.xml') -replace 'JAVA_EXE', '%JAVA_EXE%' | Out-File -encoding ASCII '%REALPATH%lib\NopServiceWrapper.xml'"

rem call the NopServiceWrapper.exe passing all the parameters
"%REALPATH%lib\NopServiceWrapper.exe" %*
