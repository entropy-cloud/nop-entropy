
drop table  if exists  nop_dyn_app;
drop table  if exists  nop_dyn_module;
drop table  if exists  nop_dyn_entity;
drop table  if exists  nop_dyn_entity_relation;
drop table  if exists  nop_dyn_patch_file;
drop table  if exists  nop_dyn_module_dep;
drop table  if exists  nop_dyn_app_module;
drop table  if exists  nop_dyn_sql;
drop table  if exists  nop_dyn_file;
drop table  if exists  nop_dyn_page;
drop table  if exists  nop_dyn_entity_meta;
drop table  if exists  nop_dyn_domain;
drop table  if exists  nop_dyn_entity_relation_meta;
drop table  if exists  nop_dyn_function_meta;
drop table  if exists  nop_dyn_prop_meta;

