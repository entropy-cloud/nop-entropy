...
| `--vector` | Vector API（SIMD）字面量搜索。前置：classpath 含 nop-rg-vector 且 JVM 加 `--add-modules jdk.incubator.vector`；模块在但孵化模块缺失时降级标量并输出 stderr 提示，classpath 缺失时显式报错。`--regex` 优先于 `--vector`。**模式长度 < 16 字节时自动走标量 BMH**（长度阈值策略，plan 2267 R1：6B SIMD 实测仅为标量 72-80%，16B 起反超 2.5-3.5x） |
...
- 符号链接（plan 2273）：目录符号链接不递归遍历（目录环会无限递归，rg 默认也不跟随）；文件符号链接按普通文件读取其内容（rg 默认不搜索符号链接）；悬空链接跳过。
